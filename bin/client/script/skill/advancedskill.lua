...
--		},
	...
	tm_adskill ={ --ÌÆÃÅ½ø½×¼¼ÄÜ
		skill_desc=
			function(level)
				return "Khi sinh lùc thÊp h¬n 25% cã x¸c suÊt <color=orange>"..floor(Link(level,SKILLS.tm_adskill.autorescueskill[3]) - 15*18*256).."Tû lÖ %<color> béc ph¸t 200% Hãa gi¶i s¸t th­¬ng vµ 20% bá qua Phßng thñ vËt lý\n"..
		...
		anti_physicsres_p={{{1,20},{2,20}},{{1,18*1},{19,18*1},{20,18*2},{50,18*2}}},
		autorescueskill=
		{
			{
	...
	xy_adskill ={ --åÐÒ£½ø½×¼¼ÄÜ
		skill_desc=
			function(level)
				return "Tr¹ng th¸i <color=water>[PhÊt Y]<color>: Thi triÓn hé thuÉn cã thÓ hÊp thu s¸t th­¬ng b»ng <color=orange>"..floor(Link(level,SKILLS.xy_adskill.staticmagicshield_p[1])/100)..
				"<color> lÇn néi lùc tèi ®a\nHé thuÉn lµm míi <color=orange>mçi "..floor(Link(level,SKILLS.xy_adskill.staticmagicshield_p[2])/18+1)..
				" gi©y<color>, duy tr× <color=orange>"..floor(Link(level,SKILLS.xy_adskill.fastwalkrun_p[2])/18).." gi©y<color>\n\n"
			end,
		autoreplyskill=
		{
		...
		autocastskill = {
			{{1,2134*256 + 1},{20,2134*256 + 20}},--Ä¿±ê¼¼ÄÜid * 256 + µÈ¼¶
