...
function OnLeave(nPlayerIndex)
	...
	elseif (SubWorld == SubWorldID2Idx(337) or SubWorld == SubWorldID2Idx(338) or SubWorld == SubWorldID2Idx(339)) then
...
function tongkim_out(nPlayerIndex)
	...
	if (nPlayerDataIdx > 0) then
		print(format("[TK - OUT] PlayerIndex %d - DataIndex %d", PlayerIndex, nPlayerDataIdx))
...
function vuotai_out(nPlayerIndex)
	...
	NewWorld(THANHTHI_RETURN[nTask][1], THANHTHI_RETURN[nTask][2], THANHTHI_RETURN[nTask][3])
...
function phonglang_out(nPlayerIndex)
	...
	local w, x, y = Rev2Pos(GetCurRevID());
