...
FACTION_INFO = {
	...
	{ 3, "vuhontang", 2, 97,  "Hoan nghªnh b¹n gia nhËp Vò Hån ph¸i",                                                                                    "Gia nhËp Vò Hån ph¸i" },
	{ 4, "xiaoyao",   3, 105, "Hoan nghªnh b¹n gia nhËp Tiªu Dao ph¸i",                                                                                  "Gia nhËp Tiªu Dao ph¸i" },
}
...
function gianhapmonphai(newfac)
	...
	if check_yes(FACTION_INFO[nCurFac][1]) == 1 then
		...
		Msg2Faction("" ..
			GetName() ..
			" tõ h«m nay gia nhËp " ..
			GetFactionName() .. ", xin b¸i kiÕn c¸c vÞ huynh tû! Mong c¸c vÞ quan t©m chØ gi¸o!")
	end
end;
...
function xuatsu()
	...
	Msg2Faction("" ..
		GetFactionName() .. " " .. GetName() .. " thµnh tµi xuÊt s­, b¸i biÖt ch­ vÞ ®ång m«n phiªu b¹t giang hå ")
end;
...
function check_yes(series) -- °´¼¼ÄÜÈ·¶¨ÈÎÎñ±äÁ¿µÄ¼ì²é¡£¡£¡£ÉÐÎ´ÏàÓ¦¸üÐÂ
	if (GetSeries() ~= series) then
		Talk(1, "",
			"Ngò hµnh cña ng­¬i kh¸c ngò hµnh " ..
			sr2name(series) .. " víi bän ta!  §Õn ®©y lµm g×? H·y lªn diÔn ®µn häc hái ®i!")
		return 0;
	end
	return 1;
end;
...
SKILLNORMAL = {
	...
	[11] = {    --hoa son
		{ 1347, 0 }, ---B¹ch Hång Qu¸n NhËt
		{ 1372, 0 }, ---Thanh V©n Tèng S¶ng
		{ 1349, 0 }, ---KiÕm T«ng Tæng QuyÕt
		{ 1374, 0 }, ---Long NhiÔu Th©n
		{ 1350, 0 }, ---D­ìng Ng« KiÕm Ph¸p
		{ 1375, 0 }, ---H¶i N¹p B¸ch Xuyªn
		{ 1351, 0 }, ---Kim Nh¹n Hoµnh Kh«ng
		{ 1376, 0 }, ---Long HuyÒn KiÕm KhÝ
		{ 1354, 0 }, ---Hi Di KiÕm Ph¸p
		{ 1378, 0 }, ---KhÝ ChÊn S¬n Hµ
		{ 1355, 0 }, ---Thiªn Th©n §¶o HuyÒn
		{ 1379, 0 }, ---KhÝ Qu¸n Tr­êng Hång
		{ 1358, 0 }, ---HuyÔn Nh·n V©n Yªn
		{ 1380, 0 }, ---Ma V©n KiÕm KhÝ
		{ 1360, 0 }, ---Th­¬ng Tïng Nghªnh Kh¸ch
	},
	[12] = {    --vu hon
		{ 1733, 0 }, ---§o¹t cê
		{ 1734, 0 }, ---Tr­êng Anh Huy XÝch
		{ 1964, 0 }, ---Vò Hån ThuÉn Ph¸p
		{ 1975, 0 }, ---Vò Hån §ao Ph¸p
		{ 1976, 0 }, ---C«ng Trung BØnh TÝnh
		{ 1977, 0 }, ---H¸m S¬n KÝch
		{ 1965, 0 }, ---ThØnh Anh §Ò NhuÖ L÷
		{ 1979, 0 }, ---Né Ph¸ch Thiªn Nhai
		{ 1963, 0 }, ---TriÒu Thiªn KhuyÕt
		{ 1980, 0 }, ---C« T­¬ng B¸ch ChiÕn Hµm
		{ 1981, 0 }, ---C¬ Xan Lç Nhôc
		{ 1971, 0 }, ---TrÝ Dòng Siªu Lu©n
		{ 1982, 0 }, ---Vò Môc Di Th
	},
	[13] = {    --tieu dao
		{ 2114, 0 }, ---Ng« C©u S­¬ng TuyÕt
		{ 2136, 0 }, ---Tïy ý Khóc
		{ 2115, 0 }, ---Tiªu Dao KiÕm Ph¸p
		{ 2137, 0 }, ---Tiªu Dao CÇm Ph¸p
		{ 2116, 0 }, ---Di Cung Ho¸n Vò
		{ 2120, 0 }, ---û KiÕm PhÊt Thiªn
		{ 2140, 0 }, ---Qu¶ng L¨ng T¸n
		{ 2121, 0 }, ---Ng©n Yªn B¹ch M·
		{ 2118, 0 }, ---T¸p §¹p L­u Tinh
		{ 2138, 0 }, ---L¹c Nh¹n B×nh Sa
		{ 2122, 0 }, ---HiÖp Cèt Nhu T×nh
		{ 2123, 0 }, ---Thiªn Lý §éc Hµnh
	}
};
...
SKILL120AR =
{
	709, 708, 710, 711, 712, 713, 714, 715, 716, 717, 1365, 1984, 2127
};
...
SKILL180_ARRAY = {
	[1] = {
		{ "Kim Quyªn La H¸n", 1220, 1 }
	},
	[2] = {
		{ "Chinh ChiÕn B¸t Ph­¬ng", 1221, 1 }
	},
	[3] = {
		{ "HÊp Tinh trËn", 1223, 1 }
	},
	[4] = {
		{ "S©m La §éc H¶i", 1222, 1 }
	},
	[5] = {
		{ "Ba La T©m Kinh", 1224, 1 }
	},
	[6] = {
		{ "HuyÒn B¨ng V« Tøc", 1225, 1 }
	},
	[7] = {
		{ "¤ Y QuyÕt", 1227, 1 }
	},
	[8] = {
		{ "LiÖt Háa PhÉn Thiªn", 1226, 1 }
	},
	[9] = {
		{ "Ch©n Vò ThÊt TiÖt", 1228, 1 }
	},
	[10] = {
		{ "Thiªn C­¬ng KhÊt KÝnh", 1229, 1 }
	},
	[11] = {
		{ "H¹o Nhiªn Chi KhÝ", 1370, 1 }
	},
	[12] = {
		{ "TËn Trung B¸o Quèc", 1986, 1 }
	},
	[13] = {
		{ "Mai Danh Èn TÝnh", 2132, 1 }
	},
}
