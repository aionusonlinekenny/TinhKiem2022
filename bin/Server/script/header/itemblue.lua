function nhantrangbi()
    --dofile("script/header/itemblue.lua");
    trangbixanh()
end
...
tbDoXanh =
{
    ...
    [7] =
    {
        ...
        tbEquip =
        {
            ...
            { "LiÖt Thiªn TriÒn Thñ",   0, 0, 6 },
            { "Tinh chÕ ThuÉn §ao",   0, 0, 7 },
            { "Tinh chÕ §ao ThuÉn",   0, 0, 8 },
        }
    },
    [8] =
    {
...
function trangbixanh()
    ...
    CreateNewSayEx1("<npc> Xin mêi lùa chän trang bÞ:", tbOpt)
end
...
function laydoxanh3(nIndex, nType, nSeries)
    --g_AskClientNumberEx(1, 10, "Sè l­îng:", { laydoxanh4, { nIndex, nType, nSeries } })
    --local tbEquipSelect = tbDoXanh[nType]["tbEquip"][nIndex]
    local tbEquipSelect = tbDoXanh[nType]["tbEquip"][nIndex]
    for i = 1, 10 do
        AddItemEx(tbEquipSelect[2], tbEquipSelect[3], tbEquipSelect[4], 10, nSeries, 100, 10)
    end
end
