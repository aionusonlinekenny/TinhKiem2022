...
function DropRate(NpcIndex)
	...
	local MONEYdrop = (RANDOM(210, 340)) * MONEY;
	local nGenre    = RANDOM(1, getn(KindItem)); --KindFiveElement[1]
	local nLevel    = RANDOM(6, 8);      -- level trang bi
	...
	if (btype == 1) then --boss xanh
		...
		for i = 1, dropnum do
			...
			elseif (nSel > 5) then
				...
				nNumLine = RANDOM(2, 5); -- so dong cua trang bi xanh
				nDetail  = KindItem[nGenre][1];
				nParti   = RANDOM(KindItem[nGenre][2], KindItem[nGenre][3]);
	...
	elseif (nSel == 4) then
		DropMenpai(NpcIndex);
		-- elseif (nSel == 3) then
		-- nNumLine  = RANDOM(1,5); -- so dong cua trang bi xanh
		-- CalCulator_Drop(NpcIndex,nGenre,nDetail,nParti,nLevel,nSeries,nLuck,nNumLine);
	end
end
