...
function DropRate(NpcIndex)
	...
	local MONEY_1x = (RANDOM(30, 170)) * MONEY;
	local nGenre   = RANDOM(1, getn(KindItem)); --KindFiveElement[1]
	local nLevel   = RANDOM(1, 3);       -- level trang bi
	local nSeries  = GetNpcSer(NpcIndex);
	local nLuck    = GetLucky(0) + 40;   --may man cua nguoi choi
	...
	if (btype == 1) then --boss xanh
		...
		for i = 1, dropnum do
			...
			elseif (nSel > 5) then
				...
				nLuck    = GetLucky(0) + 40; -- xin hon quai thuong 1 diem may man;
				nNumLine = 0; -- so dong cua trang bi xanh
				nDetail  = KindItem[nGenre][1];
				nParti   = RANDOM(KindItem[nGenre][2], KindItem[nGenre][3]);
