...
MAX_QUEST_PERDAY = 50 --max nhiem vô mçi ngµy
...
--[7][8][9] loai ma thuat
function main()
	--dofile("script/global/npcchucnang/datau.lua");
	...
	if (bAccept == 0) then
		Say(
			format(
				"<npc>: VÞ hiÖp kh¸ch h«m nay ®· hoµn thµnh <color=yellow>%d<color> nhiÖm vô, cã thÓ thùc hiÖn l¹i <color=yellow>%d<color>, ng­¬i muèn lµm n÷a kh«ng?",
				nDone, MAX_QUEST_PERDAY - nDone), 2,
	...
	if (nTaskKind == 0) then                 --loai update
		...
		if (nKind == 6 or nKind == 7) then
			strPrivate = format("H·y ®Õn %s t×m gióp ta <color=yellow>%d<color> quyÓn %s",
				GetWorldName(GetNumber(3, GetTask(TASK_DIADOCHI), 1)), GetTask(TASK_DATAU_NEED), strName)
		elseif (nKind == 5) then
		...
		strMain = format(
			"§©y lµ nhiÖm vô thø <color=green>%d<color>, ®· liªn tiÕp hoµn thµnh <color=green>%d<color> nhiÖm vô. %s",
			nDone + 1, GetTask(TASK_DATAU_NUM), strPrivate);
	else                                 --loai giao item
		...
		elseif (x == 2 or x == 3) then
			...
			strPrivate = format("H·y t×m cho ta mãn nµy: <color=yellow>%s<color> %s Nhá nhÊt:%d Lín nhÊt:%d", strDetail,
				KIND_EQUIP_HAVEMAGIC[nNum][2], GetNumber(3, GetTask(TASK_DIADOCHI), 1),
				GetNumber(3, GetTask(TASK_DIADOCHI), 2));
		end
		strMain = format(
			"§©y lµ nhiÖm vô thø <color=green>%d<color>, ®· liªn tiÕp hoµn thµnh <color=green>%d<color> nhiÖm vô. %s",
			nDone + 1, GetTask(TASK_DATAU_NUM), strPrivate);
...
function xacnhanhuy2()
	...
	Msg2Player(
		"<color=yellow>Chuçi nhiÖm vô d· tÈu ®· xãa bá hoµn toµn. B©y giê sÏ ph¶i lµm l¹i tõ nhiÖm vô ®Çu tiªn!<color>")
end
...
function shortway()
	...
	Say(10193, 3,
		...
		"Sö dông 50 v¹n ng©n l­îng/huytien",
		"§ãng/no"
	)
end
...
function phanthuong()
	...
	StrTab[count] = "§ãng/no"
	local strMain = format(
		"Khi tæng sè nhiÖm vô hoµn thµnh ®¹t ®Õn møc chØ ®Þnh cã thÓ nhËn phÇn th­ëng ­u ®·i. HiÖn t¹i hoµn thµnh tÝch lòy lµ <color=green>%d<color> nhiÖm vô.",
		GetTask(TASK_DATAU_NUM));
...
function SelANum(sel)
	...
	if (nSel == 1) then
		...
		local szItemName = GetItemName(AddItemEx(0, 10, 29, 10, 0, 0, 0))
		AddCountNews(
			format(
				"Ng­êi ch¬i <color=red>%s<color> hoµn thµnh nhiÖm vô d· tÈu nªn nhËn ®­îc <color=yellow>%s<color>",
				GetName(), szItemName), 1)
	elseif (nSel == 2) then
		if (GetSex() == 0) then
			--AddItemID(AddItem(2, 0, 190, 0, 0, 0, 5));
			local szItemName = GetItemName(AddGoldItem(189))
			AddCountNews(
				format(
					"Ng­êi ch¬i <color=red>%s<color> hoµn thµnh nhiÖm vô d· tÈu nªn nhËn ®­îc <color=yellow>%s<color>",
					GetName(), szItemName), 1)
			...
			local szItemName = GetItemName(AddGoldItem(193))
			AddCountNews(
				format(
					"Ng­êi ch¬i <color=red>%s<color> hoµn thµnh nhiÖm vô d· tÈu nªn nhËn ®­îc <color=yellow>%s<color>",
					GetName(), szItemName), 1)
	...
	elseif (nSel == 3) then
		...
		local szItemName = GetItemName(AddGoldItem(161))
		AddCountNews(
			format(
				"Ng­êi ch¬i <color=red>%s<color> hoµn thµnh nhiÖm vô d· tÈu nªn nhËn ®­îc <color=yellow>%s<color>",
				GetName(), szItemName), 1)
	elseif (nSel == 4) then
		...
		local szItemName = GetItemName(AddGoldItem(160))
		AddCountNews(
			format(
				"Ng­êi ch¬i <color=red>%s<color> hoµn thµnh nhiÖm vô d· tÈu nªn nhËn ®­îc <color=yellow>%s<color>",
				GetName(), szItemName), 1)
	elseif (nSel == 5) then
		...
		local szItemName = GetItemName(AddGoldItem(166))
		AddCountNews(
			format(
				"Ng­êi ch¬i <color=red>%s<color> hoµn thµnh nhiÖm vô d· tÈu nªn nhËn ®­îc <color=yellow>%s<color>",
				GetName(), szItemName), 1)
	elseif (nSel == 6) then
		...
		local szItemName = GetItemName(AddGoldItem(165))
		AddCountNews(
			format(
				"Ng­êi ch¬i <color=red>%s<color> hoµn thµnh nhiÖm vô d· tÈu nªn nhËn ®­îc <color=yellow>%s<color>",
				GetName(), szItemName), 1)
	elseif (nSel == 7) then
		...
		local szItemName = GetItemName(AddGoldItem(167))
		AddCountNews(
			format(
				"Ng­êi ch¬i <color=red>%s<color> hoµn thµnh nhiÖm vô d· tÈu nªn nhËn ®­îc <color=yellow>%s<color>",
				GetName(), szItemName), 1)
	elseif (nSel == 8) then
		...
		local szItemName = GetItemName(AddGoldItem(164))
		AddCountNews(
			format(
				"Ng­êi ch¬i <color=red>%s<color> hoµn thµnh nhiÖm vô d· tÈu nªn nhËn ®­îc <color=yellow>%s<color>",
				GetName(), szItemName), 1)
	elseif (nSel == 9) then
		...
		local szItemName = GetItemName(AddGoldItem(RANDOM(162, 163)))
		AddCountNews(
			format("Ng­êi ch¬i <color=red>%s<color> hoµn thµnh nhiÖm vô d· tÈu nªn nhËn ®­îc <color=yellow>%s<color>",
				GetName(), szItemName), 1)
	end
end;
...
function thuong()
	...
	-- 	strRand)
	local indexRan = random(1, 8)
	OpenFinishDatauBox(indexRan);
end;
...
function kinhnghiem()
	...
	if (nCurExp ~= nil) then
		...
		if (nNextExp > 0) then
			...
			local nAddExp = RANDOM(EXP_REWARDS[nLevel][1] * nDiff, EXP_REWARDS[nLevel][2] * nDiff) * 5; --x5 exp th­ëng d· tÈu
			if (nAddExp > 0) then
				ClearTask()
				DropEventDT()
			end
...
function tien()
	...
	local nTotalMoney = floor(RANDOM(nLevel * (2000 + nDiff), nLevel * (2000 + nDiff) * nDiff * 2) * 1.2); -- x5 tiÒn th­ëng d· tÈu
