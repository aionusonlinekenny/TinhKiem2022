...
REPAIR_ITEM = "Söa trang bÞ tæn h¹i/repair_item";
...
function thuytinh()
	...
	Say("LINK:<npc>", 0,
		"Thñy tinh kh¶m n¹m liªn quan ®Õn sù c©n b»ng ngò hµnh, ph­¬ng ph¸p chuyÓn linh khÝ tuyÖt nhiªn kh«ng thÓ dïng søc ng­êi ®­îc, kh«ng h¼n nh­ ®¹i hiÖp hi väng thµnh vËt phÈm tuyÖt thÕ nh©n gian, mµ vò khÝ ®å phæ vµ vùc ngoµi kú tr©n chØ gióp chuyÓn ®æi ®¼ng cÊp cña trang bÞ. C¸c h¹ muèn kh¶m n¹m trang bÞ nµy kh«ng?",
		4,
...
function doihe(doxanhid, Rdetail, Rparti, Rlevel, Rseries)
	AddItemID(AddItem(0, 0, Rdetail, Rparti, Rlevel, Rseries, 80, 10, RANDOM(7, 10), RANDOM(6, 10), RANDOM(10),
		RANDOM(10), RANDOM(10)));
	RemoveItem(doxanhid, 1);
...
function crystalhelp()
	Talk(1, "",
		"Muèn kh¶m nh÷ng vËt nµy kh«ng ®¬n gi¶n chót nµo, tiÒn c«ng rÊt cao, lµm xong còng ch­a ch¾c lµ ®· võa ý, thÕ nµo cã nuèn kh¶m kh«ng? §ång ý? VËy kh¶m nh÷ng thø g×? ")
end
...
function helmhelp()
	Talk(1, "",
		"Nh÷ng thø cßn l¹i nµy sÏ gióp ng­¬i t¨ng ®­îc ®¼ng cÊp mò, nh­ng ng­¬i ph¶i thùc hiÖn ®óng theo c¸c b­íc tr×nh tù cña nã, nÕu kh«ng th× sè ®iÓm cña ng­¬i sÏ l¹i bÞ gi¶m xuèng ")
end
...
function repair_item()
	if (GetCash() < 2000000) then
		Talk(1, "", "Ng­¬i kh«ng mang ®ñ 200 v¹n l­îng råi, ®ñ tiÒn th× quay l¹i nhÐ!");
		return
	end
	GiveItemUI("Söa trang bÞ tæn h¹i", "§¹i hiÖp h·y ®Æt trang bÞ tæn h¹i vµo ®©y", "repair_item2", "onCancel")
end

function repair_item2()
	local nCount, nFIndex = 0, 0;
	for i = 0, 5 do
		for j = 0, 3 do
			nFIndex = FindItemEx(14, i, j);
			if (nFIndex > 0 and GetDurability(nFIndex) == 0) then --trang bi tæn h¹i, ®é bÒn = 0
				Pay(2000000)
				RepairBroken(nFIndex)
				nCount = nCount + 1
			end
		end
	end
	...
	if (nCount > 0) then
		Msg2Player("Phôc håi trang bÞ thµnh c«ng!")
	end
end
