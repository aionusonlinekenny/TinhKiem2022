...
function chr_buy()
	...
	if (Mth == 12 and (Dy >= 23 and Dy <= 26)) then
		Say(
		"LÔ gi¸ng sinh, ta cã hai b¶o vËt cho ng­¬i, ®ã lµ <color=red>¤ng giµ N«en<color> vµ <color=red>Thiªn sø gi¸ng sinh<color>. NÕu nh­ muèn mua 1 trong sè ®ã th× <color=red>399 v¹n l­îng<color>, Mua cïng lóc c¶ hai th× chØ cÇn <color=red>688 v¹n l­îng<color>. Cßn do dù g× n÷a ",
			4, "Mua mÆt n¹ ¤ng giµ N« en (399 v¹n) /mask", "Mua mÆt n¹ Thiªn sø gi¸ng sinh(399 v¹n) /mask",
			"Mua c¶ hai(688 v¹n) /mask_all", "§Ó ta suy nghÜ/no")
	else
		Talk(1, "",
			"Mçi n¨m, tõ ngµy <color=Yellow>23 ®Õn ngµy 26 th¸ng 12<color> ta b¸n c¸c lo¹i mÆt n¹ gi¸ng sinh, ng­¬i cã thÓ tïy ý ®Õn mua");
	end;
end
...
function mask_all()
	if (GetCash() >= 6880000) then
		...
		Say("Quªn mang tiÒn µ? Kh«ng sao, ta ë ®©y ®îi ng­¬i. Nhí quay l¹i nhanh mét chót", 1, "§­îc !/no")
	end
end
