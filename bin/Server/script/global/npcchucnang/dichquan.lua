...
function main(NpcIndex)
	Say(
		"<npc>: §· l©u ta kh«ng mµng ®Õn chuyÖn trªn giang hå, thêi cuéc lóc nµy qu¸ hçn lo¹n, ¾t h¼n triÒu ®×nh sÏ cã nhiÒu viÖc ph¶i lµm",
		3,
...
function nvtinsu()
	...
	if MapId ~= 11 and MapId ~= 162 then
		Talk(1, "",
			"Tham gia nhiÖm vô TÝn Sø h·y ®i t×m DÞch Quan <color=red>Thµnh §«<color>hoÆc <color=red>§¹i Lý<color>!")
		return
	end

	Say("LINK:<npc>", 0,
		"ViÖc qu©n c¬ cña TriÒu ®×nh gÇn ®©y lu«n bÞ giÆc Kim biÕt tr­íc, ta nghi ngê cã néi gi¸n. Nh­ng tr­íc khi mÊy tªn b¹i lo¹i nµy bÞ ph¸t hiÖn, ta muèn r»ng tin nµy kh¾p n¬i ®Òu biÕt, ng­¬i cã muèn gãp mét phÇn søc cho Quèc gia kh«ng?",
		7,
...
function giaonvtinsu()
	...
	elseif nTask ~= 0 then
		Say("LINK:<npc>", 0,
			"Xin h·y l­îng thø, ng­¬i vÉn ch­a hoµn thµnh nhiÖm vô hiÖn t¹i. L·o Xa Phu bªn c¹nh ta cã thÓ ®­a ng­¬i ®Õn b¶n ®å nhiÖm vô !",
			1, "KÕt thóc ®èi tho¹i/no")
	else
		Say("LINK:<npc>", 0,
			"Ch­a tiÕp nhËn nhiÖm vô nµo ng­¬i cßn l¹i tr¶ nhiÖm vô! Ng­êi ®©u? B»m c¸i tªn nµy lµm nh©n b¸nh bao cho ta!",
			1, "KÕt thóc ®èi tho¹i/no")
	end
end
...
function nhannvtinsu()
	...
	if (GetLevel() < 120) then
		Say("LINK:<npc>", 0,
			"Xin l­îng thø, ®¼ng cÊp cña ng­¬i hiÖn t¹i ch­a ®ñ 120. h·y tiÕp tôc t«i luyÖn råi ®Õn gÆp ta.", 1,
			"KÕt thóc ®èi tho¹i/no")
	elseif (nIsDoing ~= 0) then
		Say("LINK:<npc>", 0,
			"Xin lçi! NhiÖm vô TÝn sø cña b¹n ch­a hoµn thµnh, kh«ng thÓ tiÕp nhËn nhiÖm vô gièng nhau, xin hoµn thµnh nhiÖm vô nµy tr­íc, c¸m ¬n!",
			1, "KÕt thóc ®èi tho¹i/no")
	else
		local nTaskFlag = nvtrangthai()
		if nTaskFlag == 0 then
			Say("LINK:<npc>", 0, "Xin l­îng thø, H«m nay ng­¬i ®· vÊt v¶ råi. Ngµy mai h·y quay l¹i nhÐ.", 1,
				"KÕt thóc ®èi tho¹i/no")
			return
		elseif nTaskFlag == -1 then
			Say("LINK:<npc>", 0,
				"H«m nay ng­¬i ®· mÖt mái råi. NÕu ng­¬i cã <color=yellow>Thiªn Khè B¶o LÖnh<color>, ta sÏ ph¸ lÖ cho ng­¬i tham gia thªm mét lÇn n÷a.",
				1, "KÕt thóc ®èi tho¹i/no")
			return
		elseif nTaskFlag == 2 then
			Msg2Player(
			"Ng­¬i cø cÇm Thiªn Khè B¶o LÖnh tham gia nhiÖm vô, <color=yellow>nhiÖm vô hoµn thµnh<color> ta sÏ thu håi LÖnh Bµi")
		end
		if (MapId == 11) then
...
function luachonnv(i, j)
	if (i == THANHTHI_DICHUYEN[j][2]) then
		...
		Talk(1, "",
			"B¹n nhËn ®­îc tõ " ..
			THANHTHI_DICHUYEN[j][3] ..
			"®Õn" ..
			THANHTHI_DICHUYEN[j][4] ..
			"cña nhiÖm vô TÝn sø ®Æc biÖt," ..
			THANHTHI_DICHUYEN[j][3] .. "DÞch tr¹m DÞch quan cung tèng" .. name .. "§¹i nh©n!")
		return 10
	end
end
...
function nhannv()
	...
	for i = 1, getn(THANH_DICHTRAM) do
		if (THANH_DICHTRAM[i][1] == w) then
			...
			Msg2Player("NhËn nhiÖm vô giao hµm th­ tõ " ..
			GetWorldName(w) .. " ®Õn " .. GetWorldName(THANH_DICHTRAM[i][2][nRand]) .. ".");
			AddNote(13, 1,
				"NhËn nhiÖm vô giao hµm th­ tõ " ..
				GetWorldName(w) .. " ®Õn " .. GetWorldName(THANH_DICHTRAM[i][2][nRand]) .. ".");
			Talk(1, "", 10097);
		end
	end
