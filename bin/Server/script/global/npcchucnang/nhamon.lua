...
function nvheorung()
	...
	if (GetNumber(1, GetTask(TASK_DUNGCHUNG1), 8) == 1) then
		if (GetNumber(2, GetTask(TASK_DUNGCHUNG2), 2) >= 10) then
			...
			Earn(500);
			Talk(1, "",
				"§· giÕt chÕt 10 con heo rõng? Tèt qu¸! Ta thay mÆt tr¨m hä Thµnh §« ®a t¹ ng­¬i vµ c¸c dòng sÜ ®· ra tay. §©y lµ 500 l­îng th­ëng!");
	...
	elseif (nCurTime > 0 and nCurTime - nTaskTime < 3600) then
		Talk(1, "",
			"GÇn Thµnh §« cã rÊt nhiÒu nói, th­êng cã heo rõng xuÊt hiÖn, lÇn nµy may nhê cã ng­¬i míi cã thÕ ®¶m b¶o thu ho¹ch n¨m nay.");
		return 1;
	end
	return 0;
end;
