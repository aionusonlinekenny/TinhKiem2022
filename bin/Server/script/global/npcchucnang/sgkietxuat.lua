...
function giaodich(sel)
	if (sel == 0) then
		Talk(1, "",
			format("HiÖn t¹i %s cã ®iÓm vinh dù lµ <color=yellow>%d<color>", GetSex() == 0 and "®¹i hiÖp" or "n÷ hiÖp",
				GetHonor()));
		Sale(92, 5)
	else
		Talk(1, "",
			format("HiÖn t¹i %s cã ®iÓm uy danh lµ <color=yellow>%d<color>", GetSex() == 0 and "®¹i hiÖp" or "n÷ hiÖp",
				GetRespect()));
		Sale(95, 6)
	end;
end;
