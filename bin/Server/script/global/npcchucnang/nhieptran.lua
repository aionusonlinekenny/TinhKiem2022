...
function main(NpcIndex)
	--dofile("script/global/npcchucnang/nhieptran.lua")
	...
	if (bHave > 0) then
		Say("LINK:<npc>", 0,
			format(
				"NÕu ai cã ®ñ 160 s¸t thñ gi¶n trong tay th× giÊc méng trë thµnh ®Ö nhÊt s¸t thñ ®· xem nh­ hoµn thµnh. Mçi ngµy chØ cÇn hoµn thµnh <color=yellow>%d nhiÖm vô<color> lµ ®­îc. <enter>H·y tiÕp nhËn nhiÖm vô ?th¸ch thøc thêi gian? ®Ó thÓ nghiÖm søc m¹nh ®ång ®éivµ kh¶ n¨ng s¸t thñ cña m×nh.",
				MAX_PERDAY), 5,
			FIGHT_TIME,
			"Di chuyÓn ®Õn täa ®é Boss/move_to_boss",
...
function khieuchien()
	...
	for i = 0, 7 do
		...
		if (nIndex > 0) then
			...
			local nSTG = GetItemCount(0, 6, 399);
			if (nSTG < 1) then
				PlayerIndex = nCaptain;
				Talk(1, "", 10245)
				return
			...
			if (nTaskValue > FTIME_MAXTIMES) then
				PlayerIndex = nCaptain;
				Talk(1, "", 10245)
				return
			end
	...
	if (count > 0) then
		for i = 1, count do
			...
			SetCurCamp(1)
			if (GetItemCount(0, 6, 399) > 0) then
				DelItem(1, 0, 6, 399)
...
function nhannv()
	if (GetCash() < 50000) then
		Talk(1, "", "Ng­¬i cÇn cã <color=yellow>5 v¹n ng©n l­îng<color> ®Ó nhËn nhiÖm vô!")
		return
	end
	...
	local str = format("%s ®ang ë %s %d/%d. Bµn tay h¾n nhuém ®Çy m¸u t­¬i. Ta kh«ng tin lµ ng­¬i giÕt ®­îc h¾n.",
		BOSS_SATTHU[nNum][1], GetWorldName(BOSS_SATTHU[nNum][2]), floor(BOSS_SATTHU[nNum][3] / 256),
		floor(BOSS_SATTHU[nNum][4] / 512));
	Talk(1, "", str);
	str = format("B¹n nhËn nhiÖm vô h¹ s¸t <color=Yellow>%s<color> %s <color=Red>%d,%d<color>.", BOSS_SATTHU[nNum][1],
		GetWorldName(BOSS_SATTHU[nNum][2]), floor(BOSS_SATTHU[nNum][3] / 256), floor(BOSS_SATTHU[nNum][4] / 512));
	AddNote(14, 1, str);
end;

function htsatthugian()
	GiveItemUI("Giao diÖn hîp thµnh s¸t thñ gi¶n",
		"5 s¸t thñ lÖnh cïng ®¼ng cËp sÏ hîp thµnh mét s¸t thñ gi¶n cÊp t­¬ng øng, thuéc tÝnh cña s¸t thñ gi¶n ®­îc t¹o thµnh cã liªn quan ®Õn thuéc tÝnh cña 5 s¸t thñ lÖnh",
		"ht");
end
...
function move_to_boss()
	if (GetCash() < 100000) then
		Talk(1, "", "Ng­¬i cÇn cã <color=yellow>10 v¹n ng©n l­îng<color> ®Ó di chuyÓn!");
		return
	end
	...
	if (NewWorld(BOSS_SATTHU[nNum][2], floor(BOSS_SATTHU[nNum][3] / 32), floor(BOSS_SATTHU[nNum][4] / 32)) == 1) then
		Pay(100000);
		SetFightState(1);
	end;
end
