...
function Revive(NpcIndex)
	...
	if (Series == 0) then           --KIM
		SetNpcSkill(NpcIndex, 419, 30, 1);
		SetNpcSkill(NpcIndex, 419, 30, 2);
		SetNpcSkill(NpcIndex, 419, 30, 3);
		SetNpcSkill(NpcIndex, 424, 30, 4);
		nSTVL = 25;
	elseif (Series == 1) then --MOC
		SetNpcSkill(NpcIndex, 420, 30, 1);
		SetNpcSkill(NpcIndex, 420, 30, 2);
		SetNpcSkill(NpcIndex, 420, 30, 3);
		SetNpcSkill(NpcIndex, 425, 30, 4);
		nDoc = 8;       --doc sat
	elseif (Series == 2) then --THUY
		SetNpcSkill(NpcIndex, 421, 30, 1);
		SetNpcSkill(NpcIndex, 421, 30, 2);
		SetNpcSkill(NpcIndex, 421, 30, 3);
		SetNpcSkill(NpcIndex, 426, 30, 4);
		nBang = 25;     --neu quai he thuy thi cho bang sat
	elseif (Series == 3) then --HOA
		SetNpcSkill(NpcIndex, 422, 30, 1);
		SetNpcSkill(NpcIndex, 422, 30, 2);
		SetNpcSkill(NpcIndex, 422, 30, 3);
		SetNpcSkill(NpcIndex, 427, 30, 4);
	...
	elseif (Series == 4) then --THO
		SetNpcSkill(NpcIndex, 423, 30, 1);
		SetNpcSkill(NpcIndex, 423, 30, 2);
		SetNpcSkill(NpcIndex, 423, 30, 3);
		SetNpcSkill(NpcIndex, 428, 30, 4);
		nLoi = 25;
	else --KHONG CO HE, truong hop nay la add sai hay sao do
		SetNpcSkill(NpcIndex, 418, 30, 1);
		SetNpcSkill(NpcIndex, 418, 30, 2);
		SetNpcSkill(NpcIndex, 418, 30, 3);
		SetNpcSkill(NpcIndex, 418, 30, 4);
		nSTVL = 25;
	end
	...
	if (bBlueBoss == 1) then --thiet lap thuoc tinh cho boss xanh
		...
		SetNpcLife(NpcIndex, NPCLIFE);                                --set mau' cho boss xanh
		SetNpcExp(NpcIndex, NPCEXP * EXP_RATE * 4);                 --kinh nghiem
		SetNpcReplenish(NpcIndex, 1);                               --phuc hoi sinh luc
		SetNpcHitRecover(NpcIndex, 22);                             --cao nhat la 100(ko giat) --boss xanh 60 la vua
