...
function GetNpcLevelData(Series, Level, StyleName, ParamStr)
	if (StyleName == "AIMode") then --主动攻击AI模式
		return 3
	end
	...
	if (StyleName == "Skill2" or StyleName == "Skill3" or StyleName == "Skill1") then --近程五行攻击
		if (Series == 0) then
			return 419
	...
	if (StyleName == "Skill4") then
		if (Level < 60) then              --小于60级的动物，只会近程物理攻击
			return 418                    --无五行相克物理攻击
		end
		return 416                        --断魂刺
	end
