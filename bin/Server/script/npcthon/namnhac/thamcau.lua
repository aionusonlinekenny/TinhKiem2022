...
function main(NpcIndex)
	if (GetTongFigure() == 3) then
		if (GetNumber(1, GetTongParam(), 9) < 1) then
			Talk(4, "sel_color", "VÞ <sex> nµy xin dõng b­íc ®·. ", "Huynh ®µi, cã chuyÖn g× cÇn chØ gi¸o? ",
				"T¹i h¹ ThÈm C©u, hiÖn cã 1 con thiªn lÝ m· muèn b¸n, xem vÞ huynh ®µi nµy nhÊt ®Þnh lµ mét ®¹i t«ng s­ cña chÝnh ph¸i. Ngùa tèt cña t¹i h¹ thËt kh«ng xÊu hæ khi phôc vô anh hïng. ",
				"§Ó ta xem thö ngùa quý cña huynh...")
		else
			Talk(1, "giaodich",
				"Ng­¬i nhÊt ®Þnh ph¶i ®èi ®·i tèt víi ngùa, h»ng ngµy ph¶i t¾m cho nã, b¸nh ®Ëu ph¶i gi· bét cho nhuyÔn... ")
		end;
	else
		giaodich();
	end;
...
function buy_color(sel)
	local nSel = sel + 1;
	Say(
		"Kh«ng sai, <sex> qu¶ nhiªn biÕt nh×n! T¹i h¹ ®©y còng kh«ng muèn ph¶i b¸n nã, nh­ng hiÖn giê t¹i h¹ cÇn 300 v¹n l­îng b¹c! Hy väng <sex> ®èi ®·i tèt víi nã!",
		2,
