...
function main(nNpcIndex)
	...
	if (player_Faction == "") then
		Talk(1, "enroll_select",
			"Bæn bang ®· cã danh x­ng Thiªn h¹ ®Ö nhÊt bang, danh chÊn giang hå.Nh©n tµi ngäa Hæ tµng Long, thêi nµo còng cã ")
	...
	elseif (player_Faction == "cuiyan") then
		if (GetTask(TSK_TOUSHI_FLAG) == 1) then
			...
			Talk(1, "",
				"Tõ l©u nghe tiÕng c¸c c« n­¬ng Thóy Yªn ®Ñp nh­ tiªn n÷, vŞ c« n­¬ng nµy cho ¨n mµy ta İt tiÒn uèng r­îu ®­îc ch¨ng?")
		end
	...
	elseif (player_Faction == "tangmen") then
		if (GetTask(TSK_TOUSHI_FLAG) == 1) then
			...
			Talk(1, "",
				"¸m khİ §­êng m«n khiÕn nhiÒu kÎ võa nghe danh ®· biÕn s¾c. §ao ph¸p cña quı m«n còng lîi h¹i kh«ng kĞm!")
		end
	...
	elseif (player_Faction == "tianwang") then
		if (GetTask(TSK_TOUSHI_FLAG) == 1) then
			...
			Talk(1, "",
				"Bæn bang vµ quı bang hîp thµnh Thiªn h¹ l­ìng ®¹i ph¸i, chóng ta nªn gióp ®ì nhau nhiÒu h¬n ®Ó thèng lÜnh vâ l©m! Ha! Ha! Ha!")
		end
	elseif (player_Faction == "shaolin") then
		if (GetTask(TSK_TOUSHI_FLAG) == 1) then
			...
			Talk(1, "",
				"Quı ph¸i v× sù tån vong cña Vâ l©m Trung nguyªn, d¸m ®­¬ng ®Çu víi Kim quèc hïng m¹nh! KÎ ¨n mµy nµy mu«n phÇn béi phôc")
		end
	elseif (player_Faction == "wudang") then
		if (GetTask(TSK_TOUSHI_FLAG) == 1) then
			...
			Talk(1, "",
				"Vâ §ang hiÖp kh¸ch lõng danh thiªn h¹, xøng lµ Danh m«n chİnh ph¸i! Xin cho kÎ ¨n mµy nµy göi lêi hái th¨m ®Õn §¹o NhÊt Ch©n Nh©n vµ c¸c vŞ §¹o tr­ëng!")
		end
	...
	elseif (player_Faction == "tianren") then
		if (GetTask(TSK_TOUSHI_FLAG) == 1) then
			...
			Talk(1, "",
				"MÆc dï quı ph¸i lµ chñ, nh­ng C¸i bang vµ Thiªn NhÉn x­a nay vèn kh«ng ®éi trêi chung. Chóng ta kh«ng cÇn ph¶i nhiÒu lêi. HÑn gÆp nhau trªn sa tr­êng!")
		end
	end
end
...
function enroll_select()
	if (GetSeries() == 3) and (GetCamp() == 0) then
		if (GetLevel() >= 10) then
			Say("Cã høng thó gia nhËp C¸i bang kh«ng? Mäi ng­êi cã c¬m cïng ¨n, cã r­îu cïng uèng!", 2,
				"Gia NhËp C¸i bang/go", "§Ó ta suy nghÜ kü l¹i xem/thing")
		else
