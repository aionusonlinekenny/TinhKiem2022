...
function main(nNpcIndex)
	...
	if (player_Faction == "") then
		Talk(1, "enroll_select",
			"C«n L«n ph¸i ë T©y Vùc xa x«i, rÊt İt giao h¶o víi Trung Nguyªn. Tr¶i qua mÊy ®êi ch­ëng m«n ®· dÇn trë thµnh mét trong NhÊt ®¹i m«n ph¸i. ")
	elseif (player_Faction == "kunlun") then
		if (GetNumber(1, GetTask(TASK_DUNGCHUNG2), 9) == 0) then
			if GetLevel() >= 60 then
				Say(
					"<npc>: Nghe nãi sau khi ng­¬i xuèng nói ®· lËp chót c«ng danh, cã nhí ®Õn s­ ®Ö s­ muéi chóng ta kh«ng?",
	...
	elseif (player_Faction == "emei") then
		if (GetTask(TSK_TOUSHI_FLAG) == 1) then
			...
			Talk(1, "",
				"N÷ l­u th× nªn häc c¸ch ch¨m sãc chång con! Vung g­¬m móa kiÕm ch¼ng ra thÓ thèng g×!  §óng lµ thêi thÕ ®¶o lo¹n! ")
		end
	...
	elseif (player_Faction == "tianwang") then
		if (GetTask(TSK_TOUSHI_FLAG) == 1) then
			...
			Talk(1, "",
				"Thiªn v­¬ng bang tuy hiÖn nay kh¸ m¹nh, nh­ng do mét n÷ l­u l·nh ®¹o!  Råi còng cã ngµy lµm tiªu tan c¬ nghiÖp")
		end
	elseif (player_Faction == "shaolin") then
		if (GetTask(TSK_TOUSHI_FLAG) == 1) then
			...
			Talk(1, "",
				"Ai còng b¶o ThiÕu l©m lµ vâ häc chİnh t«ng nh­ng s¸nh sao ®­îc víi Thiªn s­ ®¹o thuËt cña C«n L«n ta?")
		end
	...
	if (GetSeries() == 4) and (GetCamp() == 0) then
		if (GetLevel() >= 10) then
			Say(
				...
				2,
				"Gia nhËp C«n L«n/go", "§Ó ta suy nghÜ kü l¹i xem/thing")
		else
