...
function main(nNpcIndex)
	...
	if (player_Faction == "") then
		Talk(1, "enroll_select",
			"ThiÕu l©m ph¸i lu«n ®­îc xem lµ Vâ l©m ®Ö nhÊt m«n ph¸i, mÖnh danh 'Th¸i s¬n b¾c ®Èu' cña Trung nguyªn vâ l©m")
	elseif (player_Faction == "shaolin") then
		if (GetNumber(1, GetTask(TASK_DUNGCHUNG2), 9) == 0) then
			if GetLevel() >= 60 then
				Say("<npc>: S­ ®Ö! S­ phô l¹i ph¸i ®Ö xuèng nói ­?", 5,
					"ChuyÓn ph¸i/doiphai", "XuÊt s­ xuèng nói/xuatsu", "T×m hiÓu khu vùc luyÖn c«ng/map_help",
					"T×m hiÓu vâ nghÖ bæn m«n/skill_help",
					"Xin thay ta vÊn an s­ phô /no")
			else
				Say("<npc>: S­ ®Ö! S­ phô l¹i ph¸i ®Ö xuèng nói ­?", 3, "T×m hiÓu khu vùc luyÖn c«ng/map_help",
					"T×m hiÓu vâ nghÖ bæn m«n/skill_help", "Xin thay ta vÊn an s­ phô /no")
			end
		else
			Say("<npc>: S­ ®Ö! S­ phô l¹i ph¸i ®Ö xuèng nói ­?", 4, "Trïng ph¶n m«n ph¸i/trungphansumon",
	...
	elseif (player_Faction == "emei") then
		if (GetTask(TSK_TOUSHI_FLAG) == 1) then
			...
			Talk(1, "",
				"Thanh HiÓu S­ Th¸i cña quı ph¸i tinh th«ng phËt häc, vâ nghÖ cao c­êng, tiÓu t¨ng ng­ìng mé ®· l©u!")
		end
	...
	elseif (player_Faction == "wudu") then
		if (GetTask(TSK_TOUSHI_FLAG) == 1) then
			...
			Talk(1, "",
				"§éc M«n cña quı ph¸i thiªn h¹ ®Òu o¸n ghĞt! BÇn t¨ng m¹o muéi khuyªn c¸c ng­¬i nªn tu söa PhËt tËp! Quay ®Çu lµ bê!")
		end
	...
	elseif (player_Faction == "tianren") then
		if (GetTask(TSK_TOUSHI_FLAG) == 1) then
			...
			Talk(1, "",
				"Quı gi¸o tuy ®­îc Kim quèc b¶o hé, nh­ng giang hå cã quy t¾c cña giang hå. Kh«ng ph¶i muèn lµm g× th× lµm.")
		end
	elseif (player_Faction == "gaibang") then
		if (GetTask(TSK_TOUSHI_FLAG) == 1) then
			...
			Talk(1, "",
				"D­íi sù l·nh ®¹o cña quı bang, d©n chóng ®· cã nh÷ng ngµy yªn b×nh h¹nh phóc! V¹n lÇn béi phôc! Béi phôc!")
		end
	end
end
...
function enroll_select()
	if(GetSex() == 1)then
		Talk(1, "", "Thİ chñ, ®õng lµm bÇn t¨ng thÌm g¸i chø!")
		return
	end
	if (GetSeries() == 0) and (GetCamp() == 0) then
		if (GetLevel() >= 10) then
			Say("ThiÕu L©m kh«ng hæ danh lµ danh m«n chİnh ph¸i! Huynh ®Ö cã muèn gia nhËp kh«ng?", 2,
				"Gia nhËp ThiÕu L©m ph¸i/go", "§Ó ta suy nghÜ kü l¹i xem/thing")
		else
