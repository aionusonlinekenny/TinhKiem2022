...
SKILLS = {
	...
	jinyan_hengkong = { --½ðÑãºá¿Õ
		...
		skill_desc =
			function(level)
				...
				return format(
					"Gì ®å trªn ng­êi <color=orange>%d<color> trong ph¹m vi mµn h×nh<color=blue> KiÕm Hoa Ho¸n Tinh<color>, <color=blue> B¨ng T­íc ViÖt Chi<color>, <color=blue> U Hån PhÖ ¶nh<color>,\n",
					nMag)
			end,
	},
	...
	tianshen_daoxuan = { --ÌìÉðµ¹Ðü
		...
		skill_desc =
			function(level)
				...
				return format(
					"Gì ®å trªn ng­êi <color=orange>%d<color> trong ph¹m vi mµn h×nh<color=blue> Bçng Huýnh L­îc §Þa<color>, <color=blue>Giang H¶i Né Lan<color>,\n",
					nMag)
			end,
	},
	pogun_poqiang = { --ÆÆ³¤±øÊ½
	...
	huanyan_yunyan = { --»ÃÑÛÔÆäÎ
		...
		lifemax_p = { { { 1, 10 }, { 25, 35 }, { 36, 55 }, { 37, 55 } }, { { 1, 60 * 18 }, { 2, 180 * 18 } } },
		...
		fasthitrecover_v = { { { 1, 1 }, { 10, 5 }, { 40, 20 }, { 41, 20 } }, { { 1, -1 }, { 2, -1 } } },
		anti_do_hurt_p = { { { 1, 0 }, { 29, 0 }, { 30, 5 }, { 40, 10 }, { 41, 10 } }, { { 1, -1 }, { 2, -1 } } },
		walkrunshadow = {
			[1] = { { 1, 1 }, { 20, 1 } },
			[2] = { { 1, 60 * 18 }, { 20, 180 * 18 } }
		},
		fastwalkrun_p = { { { 1, 9 }, { 30, 60 } }, { { 1, 60 * 18 }, { 2, 180 * 18 } } }
	},
	cangsong_yingke = { --²ÔËÉÓ­¿Í
		...
		skill_desc =
			function(level)
				...
				return format(
					"Gì ®å trªn ng­êi <color=orange>%d<color> trong ph¹m vi mµn h×nh<color=blue> V« ¶nh Xuyªn<color>, <color=blue> ThiÕt Liªn Tø S¸t<color>, <color=blue> Cµn Kh«n NhÊt TrÞch<color>,\n",
					nMag)
			end,
	},
	...
	duoming_start = { --Ö÷¶¯¸¨Öú¶áÃüÁ¬»·ÈýÏÉ½£
		autoreplyskill = { { { 1, 1363 * 256 + 1 }, { 20, 1363 * 256 + 20 }, { 21, 1363 * 256 + 21 } }, { { 1, 10 * 60 * 18 }, { 20, 10 * 60 * 18 } }, { { 1, 15 * 18 * 256 + 1 }, { 20, 15 * 18 * 256 + 30 }, { 31, 15 * 18 * 256 + 31 }, { 32, 15 * 18 * 256 + 32 } } },
	...
	zixia_jianqi = { --×ÏÏ¼½£Æø
		...
		skill_desc =
			function(level)
				...
				return format(
					" Sinh mÖnh thÊp h¬n 25%% trong chíp m¾t cã <color=orange>%d%%<color> tû lÖ tiÕp xóc mçi nöa gi©y håi phôc %d ®iÓm, kÐo dµi trong %d gi©y.\n",
					nMag, nPois, nTime)
			end,
	},
	...
	jiujian_start = { --Ö÷¶¯¸¨Öú¾Å½£ºÏÒ»
		autoattackskill = { { { 1, 1368 * 256 + 1 }, { 20, 1368 * 256 + 20 }, { 21, 1368 * 256 + 21 } }, { { 1, 10 * 60 * 18 }, { 20, 10 * 60 * 18 } }, { { 1, 7 * 18 * 256 + 1 }, { 20, 7 * 18 * 256 + 20 }, { 30, 7 * 18 * 256 + 30 }, { 31, 7 * 18 * 256 + 31 } } },
		manareplenish_v = { { { 1, -10000 }, { 20, -10000 } }, { { 1, 10 * 60 * 18 }, { 2, 10 * 60 * 18 } } },
		manareplenish_p = { { { 1, -200 }, { 20, -200 } }, { { 1, 10 * 60 * 18 }, { 2, 10 * 60 * 18 } } },
		...
		skill_desc =
			function(level)
				...
				return format(
					"Khi nÐ tr¸nh ®èi ph­¬ng, <color=orange>%d%%<color> tû lÖ phãng thÝch <color=blue>KiÕm ph¸p - §éc C« Cöu KiÕm<color>, \nThêi gian håi chiªu <color=orange>%d<color> gi©y.\n",
					nMag, nPois)
			end,
		skill_eventskilllevel = { { { 1, 1 }, { 20, 20 } } },
		skill_showevent = { { { 1, 1 }, { 10, 1 }, { 20, 1 } } },
	...
	wuji_jianqi1 = { --ÎÞ¼«½£Æø1
		...
		skill_desc =
			function(level)
				...
				return format(
					"KhÝ tr­êng bïng næ t¹o thµnh <color=orange>%d<color> ®iÓm s¸t th­¬ng thuéc tÝnh b¨ng, ®ång thêi lµm kÎ ®Þch ®øng yªn trong <color=orange>%.2f<color> gi©y,\n",
					nMag, nPois)
			end,
	...
	shenguang_xuanrao1 = { --Éñ¹âÐýÈÆ
		...
		skill_desc =
			function(level)
				...
				return format(
					"Bïng næ xung quanh <color=orange>%d<color> trong ph¹n vi phe ta <color=blue> Ma V©n KiÕm KhÝ <color>, g©y ra s¸t th­¬ng cao vµ ph¸ hiÖu qu¶ ®ãng b¨ng.\n",
					nMag)
			end,
		skill_skillexp_v = { { { 1, 300 },
			{ 2,  600 },
