...
function main(nItemIdx)
	...
	if (nLfTime > 0 and nLfTime <= 30) then
	...
	if (SERVER_MODE == 0) then
		nNewIdx = AddStackItem(50, 1, 2, 0, 5, 0, 0, 0) -- ngu hoa ngoc lo hoan
		LockItem(nNewIdx, -2)
	else
		nNewIdx = LockItem(AddStackItem(50, 1, 2, 0, 5, 0, 0, 0), -2) -- that khieu bo tam dan
	end;
	--end
	Msg2Player("B¹n nhËn ®­îc 50 " .. GetNameItem(nNewIdx) .. ", khÊu trõ " .. nPayMoney .. " l­îng");
	SetParamItem(nItemIdx, GetCurServerSec() + 30);
end
