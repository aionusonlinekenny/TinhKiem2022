...
function main(nItemIdx)
	...
	if (n_fac < 0) then
		Talk(1, "",
			"Gièng nh­ quyÓn s¸ch nµy miªu t¶ chİnh lµ c¸c m«n ph¸i cao cÊp vâ c«ng , ng­¬i kh«ng biÕt bän hä huyÒn c¬ ");
		return 1;
	end
	...
	for i = 1, getn(tb_150skill[n_fac]) do
		...
		if (skill ~= -1 and skill ~= 20) then
			tinsert(tb_Desc,
				format("Th¨ng cÊp " .. "%s/upgrade_skilllevel(%d)", GetSkillName(tb_150skill[n_fac][i]),
				tb_150skill[n_fac][i]));
		end
	end
