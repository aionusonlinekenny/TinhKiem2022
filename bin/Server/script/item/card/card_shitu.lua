...
function main1()
	if (GetTranslife() > 0 and GetLevel() < 80) then
		Say("LÜnh héi B¾c §Èu Tr­êng Sinh ThuËt - T©m Ph¸p Thiªn xong, ë cÊp 80 vÒ tr­íc kh«ng thÓ sö dông S­ §å ThiÕp.",
			0);
		return
	end;
	...
	local str =
	"S­ ®å thiÕp dïng ®Ó g¾n kÕt quan hÖ s­ ®å, h¬n n÷a cã thÓ sö dông chøc n¨ng s­ ®å t­¬ng quan. NÕu b¹n ch­a cã quan hÖ s­ ®å, h·y lËp ®éi nhãm víi ng­êi ch¬i kh¸c råi míi sö dông S­ ®å thiÕp!"
	Say(str, getn(tb), tb)
	return
end
