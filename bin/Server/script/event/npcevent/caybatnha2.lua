...
function main(id)
	...
	Say("§· ®Õn giai ®o¹n ch¨m sãc <color=green>thø 2<color>. H·y t­íi n­íc cho c©y!",2,
	"T­íi n­íc (cÇn 1 thïng n­íc)/tuoinuoc",
	"Tho¸t/no")
	else
	Talk(1,"","H·y chê ®îi! Sau "..(floor((2160-nPassTime)/18)+1).." gi©y n÷a míi cã thÓ t­íi n­íc <color=green>lÇn 2")
	end
	else	--cuop
		...
		DelNpc(id);
		local newid = AddNpc(1530,1,SubWorldID2Idx(w),x,y,0);
...
function tuoinuoc()
	...
	local nPIndex = GetTeamMem(nTeamId, i );
		if(nPIndex > 0 and nPIndex ~= tempid) then
			...
			if(nNpcIdx > 0) then
			PlayerIndex = nPIndex;
			AddOwnExp(floor(BATNHA_EXP2/2));
			Msg2Player("B¹n nhËn ®­îc "..floor(BATNHA_EXP2/2).." ®iÓm kinh nghiÖm tõ c©y B¸t Nh·.")
			end
		end
	end
end;
