...
function Timeout(nNpcIdx)
	if (GetNpcParam(nNpcIdx, 1) == 1) then
		for i = 1, getn(HUYHOANG_THAP) do
			local nNewNpcIdx = AddNpcNew(1111, 1, HUYHOANG_THAP[i][1], HUYHOANG_THAP[i][2], HUYHOANG_THAP[i][3],
				QUAHH_FILE, 6)
	...
	elseif (GetNpcParam(nNpcIdx, 1) == 2) then
		for i = 1, getn(HUYHOANG_TRUNG) do
			local nNewNpcIdx = AddNpcNew(1111, 1, HUYHOANG_TRUNG[i][1], HUYHOANG_TRUNG[i][2], HUYHOANG_TRUNG[i][3],
				QUAHH_FILE,
				6)
	...
	elseif (GetNpcParam(nNpcIdx, 1) == 3) then
		for i = 1, getn(HUYHOANG_CAO) do
			local nNewNpcIdx = AddNpcNew(1111, 1, HUYHOANG_CAO[i][1], HUYHOANG_CAO[i][2], HUYHOANG_CAO[i][3], QUAHH_FILE,
				6)
			SetNpcParam(nNewNpcIdx, 1, 3);
			SetNpcTimeout(nNewNpcIdx, TIME_SWITCH);
		end
	elseif (GetNpcParam(nNpcIdx, 1) == 4) then
		local nNewNpcIdx = AddNpcNew(1118, 1, 336, 46332, 95226, QUAHH_FILE, 6);
