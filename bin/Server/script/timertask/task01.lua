	...
	elseif (SubWorld == SubWorldID2Idx(396)) then
		...
		for i = 1, getn(MAP_PHONGLANGDO) do
			if (SubWorld == SubWorldID2Idx(MAP_PHONGLANGDO[i])) then
				ontime_phonglangdo(nIndex, i, MAP_PHONGLANGDO[i])
				print("Start PLD [%d]", MAP_PHONGLANGDO[i])
				--return
			end
		end
		...
		for i = 1, getn(MAP_LIENDAU) do
			if (SubWorld == SubWorldID2Idx(MAP_LIENDAU[i])) then
				...
				print("Start Lien dau [%d]", MAP_LIENDAU[i])
				--return
			end
		end
	end
...
function ontime_phonglangdo(nIndex, nGlbMission, nMap)
	StopMissionTimer(nIndex, 1);            --tat hen gio so 1
	SetGlbMission(DATA_MAPPLD + nGlbMission, 0)
	if (GetMSPlayerCount(nIndex, 0) == 0) then --map nay ko ai tham gia->close
		CloseMission(nIndex);
		return
	end
	...
	StartMissionTimer(nIndex, 2, 9 * 1080) --thoi gian boss ra
	StartMissionTimer(nIndex, 3, 30 * 1080) --thoi gian qua ben kia
	--bat dau add npc
	release_npcphonglang(nMap)
end;
