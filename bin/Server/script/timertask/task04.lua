...
function ReportMemberState(times)
	...
	if (RestMin > 0) and (RestSec == 0) then
		str1 = "Th­îng l«i ®µi, thêi gian b¾t ®Çu thi ®Êu cßn: <color=yellow>" ..
		RestMin .. "<color> phót xin h·y chuÈn bÞ."
		Msg2MSAll(BW_MISSIONID, str1);
		if ((GO_TIME - times) == 3) then
			str1 = GetMissionS(CITYID) ..
			"Th­îng l«i ®µi, thêi gian b¾t ®Çu thi ®Êu chØ cßn<color=yellow>1<color> phót xin h·y chuÈn bÞ s½n sµng."
		end
...
function bw_noticecaptainkey(str1)
	...
	for i = 1, 2 do
		...
		if (masteridx > 0) then
			...
			str = "C¸c tuyÓn thñ l«i ®µi <color=yellow> " ..
			GetMissionS(CITYID) ..
			"<color> <color=yellow> " ..
			nCount ..
			"<color> thi ®Êu víi <color=yellow> " ..
			nCount .. "<color>, vµo l«i ®µi sè <color=yellow>[" .. nKey .. "]<color>. Xin h·y dÉn ®éi vµo l«i ®µi.";
			Msg2Player(str);
		end
	end;
...
function bw_begin_compete()
	...
	if (nMemNum1 <= 0 and nMemNum2 <= 0) then
		str1 = GetMissionS(CITYID) ..
		" L«i ®µi tranh tµi ®· ®Õn giê, ®éi<color=yellow>" ..
		CAPTAINNAME[1] ..
		"<color> cïng ®éi <color=yellow>" .. CAPTAINNAME[2] .. "<color> ®· kh«ng vµo l«i ®µi ®óng giê, hñy bá thi ®Êu";
	...
	if (nMemNum1 <= 0) then
		...
		"L«i ®µi tranh tµi ®· ®Õn giê, ®éi <color=yellow>" ..
		CAPTAINNAME[1] ..
		"<color> vµo l«i ®µi kh«ng ®óng giê, ®éi <color=yellow>" .. CAPTAINNAME[2] .. "<color> ®­îc xem lµ chiÕn th¾ng!"
		Msg2MSAll(BW_MISSIONID, str1);
		GameOver();
		StopMissionTimer(BW_MISSIONID, BW_SMALLTIME_ID);
	...
	if (nMemNum2 <= 0) then
		...
		" L«i ®µi tranh tµi ®· ®Õn giê, ®éi <color=yellow>" ..
		CAPTAINNAME[2] ..
		"<color> vµo l«i ®µi kh«ng ®óng giê, ®éi <color=yellow>" .. CAPTAINNAME[1] .. "<color> ®­îc xem lµ chiÕn th¾ng!";
		Msg2MSAll(BW_MISSIONID, str1);
		GameOver();
		StopMissionTimer(BW_MISSIONID, BW_SMALLTIME_ID);
	...
	str = GetMissionS(CITYID) ..
	" L«i ®µi <color=yellow>" ..
	CAPTAINNAME[1] ..
	"<color> cïng <color=yellow>" .. CAPTAINNAME[2] .. "<color> §· vµo l«i ®µi, tranh tµi chÝnh thøc b¾t ®©u!";
...
function ReportBattle(times)
	...
	elseif (RestSec == 0) then
		...
		Msg2MSAll(BW_MISSIONID,
			"Thêi gian l«i ®µi: Thêi gian cßn <color=yellow>" ..
			RestMin .. "<color> phót <color=yellow>" .. RestSec .. "<color> gi©y.");
	end;
end;
...
function bw_checkwinner()
	...
	if (nMemNum1 <= 0) and (nMemNum2 <= 0) then
		str1 = GetMissionS(CITYID) ..
		" L«i ®µi Hai bªn ®ång thêi rêi l«i ®µi, ®éi <color=yellow>" ..
		CAPTAINNAME[1] .. "<color> cïng <color=yellow>" .. CAPTAINNAME[2] .. "<color> ngang tµi ngang søc";
		bw_all_gone(str1)
		return
	end;
	...
	if (nMemNum1 <= 0) then
		str1 = GetMissionS(CITYID) ..
		" L«i ®µi ®éi <color=yellow>" ..
		CAPTAINNAME[2] ..
		"<color> ®¸nh b¹i ®éi <color=yellow>" .. CAPTAINNAME[1] .. "<color>, giµnh ®­îc th¾ng lîi cuèi cïng!"
		bw_all_gone(str1)
		return
	end;
	...
	if (nMemNum2 <= 0) then
		...
		" L«i ®µi ®éi <color=yellow>" ..
		CAPTAINNAME[1] ..
		"<color> ®¸nh b¹i ®éi <color=yellow>" .. CAPTAINNAME[2] .. "<color>, giµnh ®­îc th¾ng lîi cuèi cïng!"
		bw_all_gone(str1)
		return
	end;
end;
...
function bw_lose_time()
	...
	if (nMemNum1 <= 0) and (nMemNum2 <= 0) then
		str1 = GetMissionS(CITYID) ..
		"Hai bªn ®ång thêi rêi l«i ®µi, ®éi <color=yellow>" ..
		CAPTAINNAME[1] .. "<color> cïng <color=yellow>" .. CAPTAINNAME[2] .. "<color> ngang tµi ngang søc";
	end;
	...
	elseif (nMemNum2 > nMemNum1) then
		...
		" L«i ®µi ®éi <color=yellow>" ..
		CAPTAINNAME[2] ..
		"<color> cßn nhiÒu ng­êi h¬n ®éi <color=yellow>" .. CAPTAINNAME[1] .. "<color>, giµnh ®­îc th¾ng lîi cuèi cïng!"
