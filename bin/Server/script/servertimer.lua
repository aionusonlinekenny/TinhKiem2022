...
function main()
	...
	active_tongkim()
	--end;
	--print("======[Online= " .. GetPlayerCount() .. "], [Time= " .. GetLocalDate("%H:%M / %d-%m-%y") .. "]======")
end;
...
function addbossdaihk()
	if (Hr == 8 and Mn == 0) then
		releasebossdai()
		return
	end
...
function active_vuotai()
	--Check thoi gian
	--print("======[Check Vuot Ai], [Time= " .. GetLocalDate("%H:%M / %d-%m-%y") .. "]======")
	for i = 1, getn(VUOTAI_TIMER) do
		if (Hr == VUOTAI_TIMER[i][1]) then
			if (Mn == VUOTAI_TIMER[i][2]) then
				...
				for i = 1, getn(MAP_VUOTAI) do
					...
					if (nSubWorldId >= 0) then
						...
						print("======[Active Vuot Ai], [Time= " .. GetLocalDate("%H:%M / %d-%m-%y") .. "]======")
					end
				end
			end
...
function active_phonglangdo()
	--Check thoi gian
	--print("======[Check PLD], [Time= " .. GetLocalDate("%H:%M / %d-%m-%y") .. "]======")
	for i = 1, getn(PHONGLANGDO_TIMER) do
		if (Hr == PHONGLANGDO_TIMER[i][1] and Mn == PHONGLANGDO_TIMER[i][2]) then
			...
			for i = 1, getn(MAP_PHONGLANGDO) do
				nSubWorldId = SubWorldID2Idx(MAP_PHONGLANGDO[i])
				if (nSubWorldId >= 0) then
					SubWorld = nSubWorldId;
					OpenMission(1)
					StartMissionTimer(1, 1, 10 * 1080) --so phut de chinh thuc bat dau
					print("======[Active Phong Lang Do], [Time= " .. GetLocalDate("%H:%M / %d-%m-%y") .. "]======")
				end
			end
		end
	end
...
function active_somayman()
	if (Hr == 16 and Mn == 45) then
		...
		Msg2SubWorld(
			"HiÖn t¹i ®· cã kÕt qu¶ 'con sè may m¾n'. Quı ®ång ®¹o xem t¹i <color=yellow>ThÇn Tµi<color> (BiÖn Kinh 220/190).");
	end;
end;
	...
	if (nSubWorldId >= 0) then
		for i = 1, getn(TONGKIM_TIMER) do
			if (Hr == TONGKIM_TIMER[i][1] and Mn == TONGKIM_TIMER[i][2]) then
				SubWorld = nSubWorldId;
				local nKind = 1;--RANDOM(1, 2)
				OpenMission(1, TONGKIM_KIND[nKind], 2, 0, 1, 3)
				SetMission(0, nKind)
				AddCountNews(
					"TrËn ®¸nh Tèng Kim h×nh thøc " ..
					GetMissionName(1) ..
					" ®ang hÕt søc c¨ng th¼ng, tr­íc m¾t ®ang ë giai ®o¹n b¸o danh, nh÷ng ai muèn tham chiÕn xin h·y nhanh chãng ®Õn T­¬ng D­¬ng hoÆc Chu Tiªn trÊn ®¨ng kı b¸o danh; HoÆc sö dông Tèng Kim chiªu th­ ®Ó nhanh chãng ®Õn ®iÓm b¸o danh. §iÒu kiÖn tham gia: ®¼ng cÊp tõ 40. phİ b¸o danh 3000 l­îng",
					3);
				AddCountNews2(10222, 3)
				StartMissionTimer(1, 1, 2 * 1080) --so phut de chinh thuc bat dau
				if (GetMissionV(0) == 1) then
					StartMissionTimer(1, 2, 25 * 1080) --thoi gian nguyen soai ra
				end;

				StartMissionTimer(1, 3, 35 * 1080) --tong thoi gian ca tran
				print("======[Active Tong Kim], [Time= " .. GetLocalDate("%H:%M / %d-%m-%y") .. "]======")
			end
		end
		...
		if (RestTK > 0) then
			...
			if (Minute == 7 or Minute == 3) then
				AddCountNews(format(
					"Tèng Kim ®¹i chiÕn ph­¬ng thøc %s ®ang trong thêi gian b¸o danh, mäi ng­êi h·y mau ®Õn T­¬ng D­¬ng hoÆc Chu Tiªn trÊn ®Ó b¸o danh, hoÆc cã thÓ dïng Tèng Kim chiªu th­ ®Ó trùc tiÕp ®Õn ®iÓm b¸o danh. §iÒu kiÖn tham gia: ®¼ng cÊp tõ 40, phİ b¸o danh 3000 l­îng. Sau khi khai chiÕn, nÕu tæng sè ng­êi ch­a ®ñ vÉn cã thÓ tiÕp tôc b¸o danh!",
					GetMissionName(1)));
