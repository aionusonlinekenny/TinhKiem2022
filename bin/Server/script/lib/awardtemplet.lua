...
function tbAwardTemplet:GiveByRandom(tbItem, nAwardCount, tbLogTitle)
	if tbItem == nil then
		return 0
	end
	...
	if type(tbItem[1]) == "table" then -- 如果是多个物品
		if tbItem[1].nRate then     --按概率给某一个
			for i = 1, nAwardCount do
				self:GiveByRandom(tbItem, 1, tbLogTitle)
			end
			return 1
		else --按顺序给全部
			for i = 1, getn(tbItem) do
...
function tbAwardTemplet:GiveAwardByList(tbItem, nAwardCount, szLogTitle)
