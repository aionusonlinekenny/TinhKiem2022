...
function main(NpcIndex)
	...
	--dofile("script/feature/tongkim/tkmoquan.lua");
	local nSubWorldId = SubWorldID2Idx(380)
	if nSubWorldId < 0 then --chua mo map, ngung ham`
		return
	end

	if (GetSubWorldConnectionByIP(nSubWorldId, GetIP2()) >= MAX_PER_IP) then
		Talk(1, "", format("Mçi IP chØ ®­îc phÐp tham gia %d nh©n vËt!", MAX_PER_IP))
		return
	else
		...
		if (IsMission(1) == 0) then
			Say(
				10635,
				1, --dai chien chua bat dau, ngung ham`
				END_TALK
			)
			return
		end
		...
		if (bTong > 0) then --------------------------------------------------ben Tong
			...
			print(format("[TK - IN]PlayerIndex %d - DataIndex %d", PlayerIndex, nPlayerDataIdx))

			if (nPlayerDataIdx > 0) then --neu da gia nhap roi
				nGroup = GetMSIdxGroup(1, nPlayerDataIdx)
				print("Da gia nhap, OK")
				...
				Say(
					"Quèc gia h­ng vong, thÊt phu h÷u tr¸ch! B©y giê lµ lóc ta vµ c¸c ng­¬i b¸o ¬n ®Êt n­íc!\n\n" ..
						"HiÖn t¹i qu©n sè 2 bªn lµ: " .. nTongMem .. " --- " .. nKimMem .. "",
					2,
			...
			--if (GetMSRestTime(1, 5) <= 0) then
			-- if ((Hr == 11 or Hr == 13 or Hr == 15 or Hr == 17 or Hr == 19 or Hr == 21 or Hr == 23) and Mn > 15) then
			-- 	Say(
			-- 		10641,
			-- 		1, --ngung gia nhap
			-- 		END_TALK
			...
			if ((nTongMem > nKimMem + BALANCE_MEM) or nTongMem > 50) then --so luong da du~
				Talk(1, "", 10648)
				return
			end
			...
			if (nPlayerDataIdx > 0) then --neu da gia nhap roi
				...
				Say(
					"ThÒ chÕt còng trung thµnh víi §¹i Kim! §i nµo! Hìi c¸c dòng sÜ Kim quèc!\n\n" ..
						"HiÖn t¹i qu©n sè 2 bªn lµ: " .. nTongMem .. " --- " .. nKimMem .. "",
					2,
			...
			nTongMem = GetMSPlayerCount(1, 0)
			nKimMem = GetMSPlayerCount(1, 1)
			if ((nKimMem > nTongMem + BALANCE_MEM) or nKimMem > 50) then --so luong da du~
				Talk(1, "", 10639)
				return
			end
			local nTaskValue = GetTask(TASK_DUNGCHUNG3)
	...
	if (GetSubWorldConnectionByIP(nSubWorldId, GetIP2()) >= MAX_PER_IP) then
		...
		if (GetCash() < NEEDMONEY or GetLevel() < NEEDLEVEL) then
			Talk(
				...
				"",
				format(
					"B¹n kh«ng ®ñ <color=yellow>%d<color> cÊp hoÆc kh«ng ®ñ <color=yellow>%d<color> l­îng.",
					NEEDLEVEL,
					NEEDMONEY
				)
			)
