...
function main(nIndex)
	...
	if (SubWorld < 0) then
		Talk(1, "",
			"Ai còng t­ëng nghÒ B¶o tiªu lµ sung s­íng! ThËt sù mçi lÇn ra ®i ®Òu kh«ng d¸m høa hÑn tr­íc ngµy vÒ!")
		return
	end
...
function vantieu()
	...
	if (nTask == 0) then
		Say(
			"GÇn ®©y c«ng viÖc nhiÒu, ®¹o tÆc hoµnh hµnh kh¾p n¬i, mµ nh©n lùc l¹i thiÕu. VÞ ®©y cã muèn gióp ta mét chuyÕn kh«ng? SÏ cã lao phÝ xøng ®¸ng cho ng­¬i!",
			2,
...
function batdau()
	...
	local nRand = RANDOM(2)
	local nName = GetName()
	local nId = AddNpcNew(TIEUXA_TEMPLET[nRand][1], 1, w, x * 32, y * 32, "\\script\\feature\\van_tieu\\tieuxa.lua",
		GetCurCamp(), nil, nil, 5, 0, 500000)
	...
	Msg2Player("H·y mau hé tèng tiªu xa ®Õn Long M«n tiªu s­ ë Thanh Thµnh S¬n <color=Red>(243/219)<color>")
	n = n + 1
...
function cuahang()
	Say(
		"Ng­¬i hiÖn cã: <color=red>" ..
		GetItemCount(0, 6, 4774) .. "<color> Hé Tiªu lÖnh. H·y lùa chän vËt phÈm cÇn thiÕt cho tiªu xa:", 5,
...
function bicuop()
	...
	DelItem(1, 0, 6, 4771)
	Talk(1, "",
		"Ng­¬i bÞ c­íp tiªu råi sao? Còng may ng­¬i ®· ®o¹t l¹i Tiªu Kú danh dù cña Thµnh §« tiªu côc. VÊt v¶ cho ng­¬i råi! §©y lµ mét nöa phÝ lao vô")
end
...
function huybo()
	...
	SetTask(TASK_NPCVANTIEU, 0)
	Talk(1, "",
		"C«ng viÖc ¸p tiªu kh«ng ph¶i lµ ®¬n gi¶n. Ng­¬i ch©n ­ít ch©n r¸o b­íc vµo nªn còng khã tr¸ch! Sau nµy h·y cè g¾ng lªn!")
...
function timhieu()
	Talk(2, "",
		"Con ®­êng hiÓm trë nhÊt lµ b¨ng qua Thanh Thµnh S¬n. N¬i ®ã ®Çy rÉy thó d÷ vµ ®¹o tÆc hoµnh hµnh. Ng­¬i ph¶i hé tiªu an toµn ®Õn n¬i giao cho tiªu s­ trung chuyÓn ®óng thêi gian!",
		"Hµng hãa cÇn vËn chuyÓn cã 3 chñng lo¹i: vµng, b¹c, ®ång. Tiªu cµng gi¸ trÞ th× bän ®¹o tÆc cµng ®Ó m¾t tíi nªn cµng khã kh¨n nh­ng lao phÝ ng­¬i nhËn ®­îc còng t­¬ng xøng!")
end
