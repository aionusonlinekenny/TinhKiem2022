...
function OnDeath(nDesNpcIndex)
	if (SubWorld == SubWorldID2Idx(380)) then
		...
		--check cuu sat cung phe
		local nSrcCamp, nDstCamp, nOldPlayerIdx;
		nOldPlayerIdx = PlayerIndex
		nSrcCamp = GetCurCamp()
		PlayerIndex = nDesPlayerIdx;
		nDstCamp = GetCurCamp()
		PlayerIndex = nOldPlayerIdx

		if (nSrcCamp == nDstCamp) then
			return
		end
		...
		if (nPlayerDataIdx > 0 and nDesPlayerData > 0 and nOnline == 1 and nDesOnline == 1) then --co ton tai. va ca 2 dang online trong chien truong
			...
			SetPMParam(1, nDesPlayerData, 6,
				GetPMParam(1, nDesPlayerData, 6) + TICHLUY_GIETNGUOI + TICHLUY_GIETNGUOI * nDieRank);              --tich luy player,
			...
			local nTotalAccum = GetPMParam(1, nDesPlayerData, 2) + TICHLUY_GIETNGUOI + TICHLUY_GIETNGUOI * nDieRank +
			TICHLUY_LIENTRAM + CurNewLT;                                                                                       --tong so tich luy
			...
			if (GetCurCamp() == 1) then
				...
				tbRANKNAME[nRank] ..
				szDesName ..
				" h¹ träng th­¬ng ng­êi phe Kim " ..
				tbRANKNAME[nDieRank] .. szName .. ", tæng PK lµ " .. GetPMParam(1, nDesPlayerData, 5);
			else
				szBeKill = "Phe Kim " ..
				tbRANKNAME[nRank] ..
				szDesName ..
				" h¹ träng th­¬ng ng­êi phe Tèng " ..
				tbRANKNAME[nDieRank] .. szName .. ", tæng PK lµ " .. GetPMParam(1, nDesPlayerData, 5);
			end
			Msg2MSAll(1, szBeKill, -2); --thong bao toan nguoi choi
			Msg2Player("<color=Pink>Chóc mõng! B¹n h¹ gôc " ..
			tbRANKNAME[nDieRank] .. szName .. ", tæng PK lµ " .. GetPMParam(1, nDesPlayerData, 5) .. "<color>");
		end
	end
end;
