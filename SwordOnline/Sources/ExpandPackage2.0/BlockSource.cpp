...
{	
 ...
 //  		
	...
	void BlockSource::CloneBlockSourceToMe(BlockSource& Source)
	{
		iostream& WritePoint = m_BlockPoint.GetStreamPointToData();
		...
		// Sử dụng std::unique_ptr thay thế cho auto_ptr
		std::unique_ptr<char[]> DataBlock(new char[m_BlockLength]);

		SourcePoint.read(DataBlock.get(), m_BlockLength);

		if (SourcePoint.bad())
		{
			throw std::exception("read file error");
		}
		if (SourcePoint.eof())
		{
			throw std::exception("File is End when Read File BlockData");
		}

		WritePoint.write(DataBlock.get(), m_BlockLength);

		if (WritePoint.bad())
		{
			throw std::exception("write file error");
		}

		// Phần comment về istream_iterator và copy đã được loại bỏ
	}
