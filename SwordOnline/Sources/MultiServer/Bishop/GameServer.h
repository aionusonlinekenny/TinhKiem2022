...
public:
	...
	virtual void PunishAccountName(const char* pName, INT nLength, UINT uOption);
	...
	static void PunishAllGSV(const char* pText, INT nLength, UINT uOption);
	...
	void SetTwoRevivePos(unsigned short nTwoRevivePos) { m_nTwoRevivePos = nTwoRevivePos; }
...
protected:
	...
	bool PopAccount(const char* pAccountName, bool bUnlockAccount, WORD nExtPoint, BYTE nExtDay);

	bool ConsumeMoney(const char* pAccountName);
	bool FreezeMoney(const char* pAccountName, WORD nExtPoint, BYTE nExtDay);
...
private:
	...
	ULONG					m_nServerIP_Intraner;
	ULONG					m_nServerIP_Internet;

	uint16_t					m_nServerPort;

	unsigned short 	                m_nTwoRevivePos;
