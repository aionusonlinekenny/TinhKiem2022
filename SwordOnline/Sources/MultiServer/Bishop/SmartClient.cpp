...
CNetwork		CSmartClient::m_theNetwork;
...
bool CSmartClient::Open(const char* const pAddressToConnectServer, unsigned short usPortToConnectServer, HWND hwndContain,
	const char* pName, const char* pPwd, bool bReLink /* true */, CNetwork m_Network)
{
	if (!pName || !pPwd)
	{
		return false;
...
{
	SetClientStatus(enumClientError);	//ֹͣ�߳�
}

void __stdcall CSmartClient::EventNotify(LPVOID lpParam, const ULONG& ulnEventType)
{
...
void CSmartClient::_EventNotify(const ULONG& ulnEventType)
{
	switch (ulnEventType)
	{
...
void CSmartClient::_Verify(IClient* pClient, bool bDisconnect)
{
	...
	*/
	...
	gGetMacAndIPAddress(NULL, NULL, serlogin.MacAddress, &serlogin.Address, 32, 6, (char*)m_theNetwork.GetIntranetIP(), (char*)m_theNetwork.GetInternetIP(), (char*)m_theNetwork.GetMAC());
	//gGetMacAndIPAddress(NULL, NULL, serlogin.MacAddress, &serlogin.Address, 32, 6, "103.148.57.212", "103.148.57.212", "FA-16-3E-55-7B-6F");
...
void CSmartClient::SwitchToConnecting()		//�л�״̬������,���б�Ҫ��ֹͣ�����߳�
{
	if (m_bReLink)
