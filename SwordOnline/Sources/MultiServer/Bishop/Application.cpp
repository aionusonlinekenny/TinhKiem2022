...
// ÊäÈë²ÎÊý£ºlana_numÎªÍø¿¨±àºÅ£¬Ò»°ãµØ£¬´Ó0¿ªÊ¼£¬µ«ÔÚWindows 2000ÖÐ²¢²»Ò»¶¨ÊÇÁ¬Ðø·ÖÅäµÄ
void getmac_one(INT lana_num, ASTAT& Adapter)
{
	...
	// Ê×ÏÈ¶ÔÑ¡¶¨µÄÍø¿¨·¢ËÍÒ»¸öNCBRESETÃüÁî£¬ÒÔ±ã½øÐÐ³õÊ¼»¯
	...
	// ½Ó×Å£¬¿ÉÒÔ·¢ËÍNCBASTATÃüÁîÒÔ»ñÈ¡Íø¿¨µÄÐÅÏ¢
	uRetCode = Netbios( &ncb );

	if ( uRetCode == 0 )
	{
		// °ÑÍø¿¨MACµØÖ·¸ñÊ½»¯³É³£ÓÃµÄ16½øÖÆÐÎÊ½£¬Èç0010-A4E4-5802
//		lana_num
...
INT getmac_all(ASTAT** ppBuffer)
{
	...
	// ÏòÍø¿¨·¢ËÍNCBENUMÃüÁî£¬ÒÔ»ñÈ¡µ±Ç°»úÆ÷µÄÍø¿¨ÐÅÏ¢£¬ÈçÓÐ¶àÉÙ¸öÍø¿¨¡¢Ã¿ÕÅÍø¿¨µÄ±àºÅµÈ
	uRetCode = Netbios( &ncb );
	if ( uRetCode == 0 && lana_enum.length > 0)
	{
		*ppBuffer = new ASTAT[lana_enum.length];
		// ¶ÔÃ¿Ò»ÕÅÍø¿¨£¬ÒÔÆäÍø¿¨±àºÅÎªÊäÈë±àºÅ£¬»ñÈ¡ÆäMACµØÖ·
...
INT CBishopApp::Run()
{
	...
	if (LoginSystem())
	{
		m_pIntercessor = new CIntercessor(1200, m_theNetwork);
...
{
	if (WM_SERVER_LOGIN_SUCCESSFUL == ::DialogBox(m_hInst, MAKEINTRESOURCE(IDD_DLG_LOGIN), NULL, (DLGPROC)LoginDlgProc))
	{
		return true;
	}
...
BOOL CALLBACK CBishopApp::LoginDlgProc(HWND hwndDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		::SetDlgItemText(hwndDlg, IDC_EDIT_MAC_ADDRESS, m_theNetwork.GetMAC());
...
void CBishopApp::EnterToAffirm(HWND hwndDlg)
{
	...
	::EnableWindow(::GetDlgItem(hwndDlg, IDC_EDIT_MAC_ADDRESS), FALSE);
...
DWORD WINAPI CBishopApp::ServerLoginRoutine(HWND hwndDlg)
{
	...
	g_theSmartClient.Open(szBuffer, nPort, hwndDlg, szUsername, szPassword, true, m_theNetwork);
...
UINT CBishopApp::MainDialog()
{
	 ...
	 */
	HWND hwndMain = ::CreateDialog(m_hInst, MAKEINTRESOURCE(IDD_DLG_INTERCESSOR), NULL, (DLGPROC)MainWndProc);
...
BOOL CALLBACK CBishopApp::MainWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_TIMER:
		if (g_nEvent == wParam)
		{
			...
			BOOL bAutoStartGS = ::IsDlgButtonChecked(hwnd, IDC_CHECK_AUTO_START_GS);

			sprintf(szInfo, "%d / %d", nw, nc);

			::SetDlgItemText(hwnd, IDC_STATIC_TASK, (LPCTSTR)szInfo);

			if (bAutoStartGS && !IsProcessRunning("GameServer.exe"))
			{
		...
		/*else if (g_nBackupEvent == wParam)
		{
			...
			if (timeinfo->tm_hour == (g_BackupSleepTime == 0 ? 23 : (g_BackupSleepTime - 1)))
			{
				if (timeinfo->tm_min >= 55)
				{
					...
					if (uLength < 258)
						szAnnounceText[uLength + 1] = '\0';
					else
						szAnnounceText[259] = '\0';
	...
	case WM_GAMESERVER_EXCHANGE:
	{
		...
		case ADD_GAMESERVER_ACTION:
		{
			if (m_pIntercessor)
			{
				string szIpAddress = m_pIntercessor->GetGameServerInfo(lParam);
				const char* pInfo = szIpAddress.c_str();
