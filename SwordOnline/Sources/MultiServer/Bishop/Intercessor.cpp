...
CIntercessor::CIntercessor(ULONG lnMaxPlayerCount, CNetwork& theNetworkConfig)
	: m_hwndViewer(NULL)
...
void CIntercessor::_AccountEventNotify( const ULONG &ulnEventType )
{
	switch( ulnEventType )
	{
...
*/
void __stdcall CIntercessor::DBRoleEventNotify(LPVOID lpParam, const ULONG& ulnEventType)
{
	CIntercessor* pPlayerManager = reinterpret_cast<CIntercessor*>(lpParam);
...
void CIntercessor::_DBRoleEventNotify(const ULONG& ulnEventType)
{
	switch (ulnEventType)
	{
	case enumServerConnectCreate:
...
void __stdcall CIntercessor::PlayerEventNotify(LPVOID lpParam, const ULONG& ulnID, const ULONG& ulnEventType)
{
	CIntercessor* pPlayerManager = reinterpret_cast<CIntercessor*>(lpParam);
...
void CIntercessor::_PlayerEventNotify(const ULONG& ulnID, const ULONG& ulnEventType)
{
...
bool CIntercessor::StartupNetwork()
{
	...
	{
		pServerFactory->SetEnvironment(m_lnMaxPlayerCount, 10, 1000, 1024 * 16);	//8
...
INT	CIntercessor::Helper()
{
	...
	while (!m_hQuitEvent.Wait(0))
	{
		...
		while (pData && dataLength)
		{
			...
			{
				uID = *(const ULONG*)((const char*)pData + 2);
			}
...
const char* CIntercessor::GetGameServerInfo(const ULONG& ulnID)
{
	if (m_pGameSvrServer)
	{
