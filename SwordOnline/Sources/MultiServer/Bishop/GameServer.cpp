...
CBuffer::Allocator						CGameServer::m_theGlobalAllocator(1024 * 128, 1200); //1024 * 96, 200
 ...
 * class CGameServer
 */
CGameServer::CGameServer(IServer* pGameSvrServer, IClient* pAccountClient, UINT nIdentityID /* = -1 */)
...
{
	{
		...
		for (it = m_theAccountInThisServer.begin(); it != m_theAccountInThisServer.end(); it++)
		{
			...
			FreezeMoney(sAccountName.c_str(), 0, 0);
		}
	}
...
void __stdcall CGameServer::GameSvrEventNotify(LPVOID lpParam, const ULONG& ulnID, const ULONG& ulnEventType)
{
...
bool CGameServer::_UpdateMapID(const void* pData, size_t datalength)
{
	...
	int16_t* pMapID = (int16_t*)pUMI->szMapID;
...
#ifdef	CONSOLE_DEBUG
	cprintf("Registered %d MapID\n", m_theMapIDConvert.size());
#endif
...
{
#ifdef	CONSOLE_DEBUG
	cprintf("CGameServer::_NotifyPlayerLogin\n");
#endif
	...
	if (pPlayer)
	{
		...
		nMinLen = nMinLen > sizeof(npl.szAccountName) ? sizeof(npl.szAccountName) : nMinLen;
		if (nMinLen < 31) {
			memcpy((char*)npl.szAccountName, (const char*)pPPL->szAccountName, nMinLen);
			npl.szAccountName[nMinLen] = '\0';
		}
...
{
#ifdef	CONSOLE_DEBUG
	cprintf("CGameServer::_RegisterAccount\n");
#endif
...
#ifdef	CONSOLE_DEBUG
	cprintf("CGameServer::_NotifyEnterGame\n");
#endif
...
bool CGameServer::_NotifyLeaveGame(const void* pData, size_t datalength)
{
...
#ifdef	CONSOLE_DEBUG
	cprintf("CGameServer::_NotifyLeaveGame\n");
#endif
	...
	return PopAccount((const char*)pAccountName, (pLG->cCmdType != HOLDACC_LEAVEGAME), pLG->nExtPoint, pLG->nExtDay);
}
...
bool CGameServer::PopAccount(const char* pAccountName, bool bUnlockAccount, WORD nExtPoint, BYTE nExtDay)
{
	ASSERT(pAccountName);
	...
	if (HaveAccountInGameServer(pAccountName))
	{
		if (bUnlockAccount)
		{
			FreezeMoney(pAccountName, nExtPoint, nExtDay);
		}

		bool bOk = DetachAccountFromGameServer(pAccountName);
...
{
#ifdef	CONSOLE_DEBUG
	cprintf("CGameServer::AttatchAccountToGameServer\n");
#endif
...
bool CGameServer::HaveAccountInGameServer(const char* pAccountName)
{
	...
	if (_NAME_LEN > strlen(pAccountName))
	{
		...
		{
#ifdef	CONSOLE_DEBUG
			cprintf("CGameServer::HaveAccountInGameServer \n");
#endif
			return true;
		}
	}
...
#ifdef	CONSOLE_DEBUG
			cprintf("CGameServer::DetachAccountFromGameServer \n");
#endif
			return true;
		}
	}
...
bool CGameServer::FreezeMoney(const char* pAccountName, WORD nExtPoint, BYTE nExtDay)
{
	if (!pAccountName || !pAccountName[0])
	{
		return false;
...
{
#ifdef	CONSOLE_DEBUG
	cprintf("CGameServer::_PlayerLogicLogout\n");
#endif
...
{
#ifdef	CONSOLE_DEBUG
	cprintf("CGameServer::_SyncRoleInfo\n");
#endif
...
bool CGameServer::RegisterServer(UINT nID, IGServer* pGServer)
{
	...
	else
	{
		...
		std::pair< stdMapIDConvert::iterator, bool > result = m_theMapIDConvert.insert(stdMapIDConvert::value_type(nID, sl));
