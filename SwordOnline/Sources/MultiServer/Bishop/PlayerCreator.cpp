...
const TRoleData* CPlayerCreator::GetRoleData(UINT &uDataLength, LPROLEPARAM lpParam)
{
...
bool CPlayerCreator::GetRoleDataFromIni(BYTE* pData, const char* szFileName)
{
	...
	pRoleData->BaseInfo.RankEx.Release();
	pRoleData->BaseInfo.ilockpkstate = 0;
	...
	pRoleData->BaseInfo.itonglevel = 0;
	pRoleData->BaseInfo.itongmemnum = 0;
	pRoleData->BaseInfo.itongeff = 0;
...
//  Skill
	...
	pRoleData->dwSSkillOffset		= (BYTE*)pSkillData - pData;
...
//Item
	...
	for ( i = 0 ; i < nItemCount; i ++)
	{
		...
		pItemData->idurability		= -2;
		pItemData->iidentify		= 0;
		pItemData->iexpire			= 0;
		pItemData->istacknum		= 1;
		pItemData->ilockstate		= 0;
		pItemData->ilocktime		= 0;
		pItemData->iparamv			= -1;
		pItemData->ifortune			= 0;
		pItemData->iowner			= 0;
		pItemData->itradeprice		= 0;
		pItemData->ilocksell		= FALSE;
		pItemData->ilocktrade		= FALSE;
		pItemData->ilockdrop		= FALSE;
		pItemData->imantle			= 0;
		pItemData->iflash			= 0;
		pItemData->irow				= -1;
		pItemData->iugrlv			= 0;
		pItemData->iphysicval		= 0;
		pItemData->imagicval		= 0;
