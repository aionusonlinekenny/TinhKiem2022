...
public:
	...
	explicit CIntercessor(ULONG lnMaxPlayerCount, CNetwork& theNetworkConfig);
	...
	const char* GetGameServerInfo(const ULONG& ulnID);
...
protected:
	 ...
	 * Helper function
	 */
	void _PlayerEventNotify(const ULONG& ulnID,
		const ULONG& ulnEventType);

	//	void _AccountEventNotify( const ULONG &ulnEventType );
	void _DBRoleEventNotify(const ULONG& ulnEventType);
...
private:
	 ...
	 * Player information
	 */
	const ULONG m_lnMaxPlayerCount;
