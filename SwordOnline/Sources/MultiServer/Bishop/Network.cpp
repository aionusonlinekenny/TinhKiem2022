...
CNetwork::CNetwork()
	: m_pAccSvrClient(NULL), m_nAccSvrPort(0), m_nRoleSvrPort(0), m_nClientOpenPort(0), m_nGameSvrOpenPort(0)
{
}
...
bool CNetwork::Create()
{
	...
	DWORD dwLen = theConfigFile.ReadString("RoleServer", "RoleSvrIP", const_cast<char *>(m_sRoleSvrIP.c_str()), MAX_STRING_LEN, "");
	m_sRoleSvrIP.resize(dwLen + 1 /* '\0' */);

	m_nRoleSvrPort = theConfigFile.ReadInteger("RoleServer", "RoleSvrPort", 0);

	m_sAccSvrIP.resize(MAX_STRING_LEN);

	dwLen = theConfigFile.ReadString("AccServer", "AccSvrIP", const_cast<char *>(m_sAccSvrIP.c_str()), MAX_STRING_LEN, "");
	m_sAccSvrIP.resize(dwLen + 1 /* '\0' */);

	m_nAccSvrPort = theConfigFile.ReadInteger("AccServer", "AccSvrPort", 0);

	m_nClientOpenPort = theConfigFile.ReadInteger("Gateway", "ClientOpenPort", 0);
	m_nGameSvrOpenPort = theConfigFile.ReadInteger("Gateway", "GameSvrOpenPort", 0);

	m_sIntranetIP.resize(MAX_STRING_LEN);
	dwLen = theConfigFile.ReadString("Network", "IntranetIP", const_cast<char *>(m_sIntranetIP.c_str()), MAX_STRING_LEN, "103.148.57.212");
	m_sIntranetIP.resize(dwLen + 1);

	m_sInternetIP.resize(MAX_STRING_LEN);
	dwLen = theConfigFile.ReadString("Network", "InternetIP", const_cast<char *>(m_sInternetIP.c_str()), MAX_STRING_LEN, "103.148.57.212");
	m_sInternetIP.resize(dwLen + 1);

	m_sMac.resize(MAX_STRING_LEN);
	dwLen = theConfigFile.ReadString("Network", "MAC", const_cast<char *>(m_sMac.c_str()), MAX_STRING_LEN, "FA-16-3E-55-7B-6F");
	m_sMac.resize(dwLen + 1);
...
void CNetwork::Destroy()
{
	...
	*/
	...
	theConfigFile.WriteString("AccServer", "AccSvrIP", m_sAccSvrIP.c_str());
	theConfigFile.WriteInteger("AccServer", "AccSvrPort", m_nAccSvrPort);

	theConfigFile.WriteString("RoleServer", "RoleSvrIP", m_sRoleSvrIP.c_str());
	theConfigFile.WriteInteger("RoleServer", "RoleSvrPort", m_nRoleSvrPort);

	theConfigFile.WriteInteger("Gateway", "ClientOpenPort", m_nClientOpenPort);
	theConfigFile.WriteInteger("Gateway", "GameSvrOpenPort", m_nGameSvrOpenPort);

	theConfigFile.WriteString("Network", "IntranetIP", m_sIntranetIP.c_str());
	theConfigFile.WriteString("Network", "InternetIP", m_sInternetIP.c_str());
	theConfigFile.WriteString("Network", "MAC", m_sMac.c_str());
}
...
/*void __stdcall CNetwork::AccSvrEventNotify( LPVOID lpParam, const ULONG &ulnEventType )
{
