...
namespace OnlineGameLib {
	namespace Win32 {
			...
			DWORD lastError)
		{
			...
			Output(_T("CSocketServer::OnConnectionError() - : ") + sErrorInfo);
			Message(sErrorInfo.c_str());
		...
		void CSocketServer::Socket::Shutdown(int32_t how /* = SD_BOTH */)
		{
		...
		bool CSocketServer::Socket::IsConnected(int32_t how /*= SD_BOTH*/) const
		{
			if (how == SD_RECEIVE)
			{
		...
		int32_t CSocketServer::WorkerThread::Run()
		{
			try
			{
				while (true)
				{
					...
					CIOBuffer* pBuffer = nullptr;

					bool weClosedSocket = false;

					DWORD dwResult = m_iocp.GetStatus(reinterpret_cast<PDWORD_PTR>(&pSocket), &dwIoSize, reinterpret_cast<OVERLAPPED**>(&pBuffer));

					if (S_OK != dwResult)
					{
						if (ERROR_NETNAME_DELETED == dwResult)
						{
							// Lỗi kết nối bị xóa, đóng kết nối
							weClosedSocket = true;
						}
						else if (WSA_OPERATION_ABORTED != dwResult)
						{
							// Lỗi hệ thống khác, ném ngoại lệ
							throw CWin32Exception(_T("CIOCompletionPort::GetStatus() - GetQueuedCompletionStatus"), dwResult);
						}
					...
					if (!pSocket)
					{
						// Đã gửi completion key 0 để yêu cầu dừng luồng
						break;
					}
			...
			{
				// Xử lý ngoại lệ Win32
				m_server.OnError(_T("CSocketServer::WorkerThread::Run() - Win32 Exception: ") +
					e.GetWhere() +
					_T(" - ") +
					e.GetMessage());
			}
			...
			{
				// Chuyển đổi từ std::exception sang std::wstring
				std::string strWhat = e.what();
				std::wstring wstrWhat(strWhat.begin(), strWhat.end());

				// Xử lý ngoại lệ chuẩn C++
				std::wcout << L"CSocketServer::WorkerThread::Run() - Standard Exception: " << wstrWhat << std::endl;
				// hoặc
				// m_server.OnError(_T("CSocketServer::WorkerThread::Run() - Standard Exception: ") + wstrWhat);
			}
			catch (...)
			{
