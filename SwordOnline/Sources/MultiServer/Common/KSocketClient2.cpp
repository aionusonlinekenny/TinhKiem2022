...
KSocketClient2::KSocketClient2()
{
	...
	int32_t result = WSAStartup(SOCKET_VERSION_REQUIRED, &WSAData);

	if (result != 0) {
		// Xử lý lỗi khi khởi tạo Winsock
		// Ví dụ: In thông báo lỗi và thoát khỏi ứng dụng
		fprintf(stderr, "Không thể khởi tạo Winsock. Mã lỗi: %d\n", result);
	}
...
HRESULT STDMETHODCALLTYPE KSocketClient2::ConnectTo(const char* const& pAddressToConnectServer, uint16_t usPortToConnectServer)
{
	HRESULT hrResult = E_FAIL;
	int32_t nRetCode = false;

	struct sockaddr_in ServerAddr = { 0 };
	int32_t nServerAddrSize = sizeof(ServerAddr);
...
)
{
	...
	{
		unsigned char* pbyNextInBuffer = (m_InBuffer + (uint32_t)(*(WORD*)m_InBuffer));
		int32_t nCopySize = m_pbyInBufferEnd - pbyNextInBuffer;
	...
	while (true)
	{
		...
		if (!
			(
				...
				(m_pbyInBufferEnd < (m_InBuffer + (uint32_t)(*(WORD*)m_InBuffer)))
				)
			)
		{
