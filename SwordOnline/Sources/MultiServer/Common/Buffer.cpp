	...
	namespace Win32 {
		...
		void CBuffer::AddData(const char* const pData, size_t dataLength)
		{
			if (dataLength > m_size - m_used)
			{
				...
				Message("CBuffer::AddData - Not enough space in buffer!");
		...
		void CPackager::AddData(BYTE cID, const char* const pData, size_t dataLength, ULONG lnUserData /* = 0 */)
		{
		...
		void CPackager::AddData(BYTE cID, const BYTE* const pData, size_t dataLength, ULONG lnUserData /* = 0 */)
		{
		...
		CBuffer* CPackager::GetHeadPack(BYTE cID, size_t packLength /* = 512 */)
		{
			...
			if (m_theSend.end() != (it = m_theSend.find(cID)))
			{
				 ...
				 */
				ULONG lnUserData = pBuffer->GetUserData();

				pNewBufer->AddData((const char*)(&lnUserData), sizeof(ULONG));
		...
		CBuffer* CPackager::PackUp(const void* pData, size_t dataLength)
		{
			...
			const size_t nDataBegin = 1 + 1 + sizeof(ULONG);
			...
			if (_Tail(cFlag))
			{
				ULONG lnUserData = *(const ULONG*)((const char*)pData + 2);
