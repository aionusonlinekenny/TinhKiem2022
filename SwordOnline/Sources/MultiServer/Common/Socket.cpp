...
namespace OnlineGameLib {
	namespace Win32 {
		...
		void CSocket::Connect(const SOCKADDR_IN& address)
		{
			...
			{
				throw Exception(_T("CSocket::Connect() - SOCKET_ERROR") + std::to_string(::WSAGetLastError()));// ::WSAGetLastError());
			}
		}
		...
		void CSocket::Connect(const struct sockaddr& address, size_t addressLength)
		{
			...
			{
				throw Exception(_T("CSocket::Connect() - SOCKET_ERROR"), SOCKET_ERROR);// ::WSAGetLastError());
			}
		}
		...
		CSocket::InternetAddress::InternetAddress(ULONG address, uint16_t port)
		{
			/*
		...
		CSocket::Exception::Exception(const _tstring& where, DWORD error) : CWin32Exception(where, error)
		{
			throw Exception(_T(where.c_str()), error);
		}
		CSocket::Exception::Exception(const _tstring& where) : CWin32Exception(where)
		{
