...
namespace OnlineGameLib {
	namespace Win32 {
		...
		CSocketClient::CSocketClient(const _tstring& addressToConnectServer, unsigned short portToConnectServer, size_t maxFreeBuffers, size_t bufferSize /* = 1024 */) : CIOBuffer::Allocator(bufferSize, maxFreeBuffers), m_address(addressToConnectServer), m_port(portToConnectServer), m_connectSocket(INVALID_SOCKET)
		{
			m_theSendAccountBegin.wLen = 0;
			m_theSendAccountBegin.AccountBegin = {};
			m_uClientKey = 0;
			m_uKeyMode = 0;
			m_uServerKey = 0;
		}

		CSocketClient::CSocketClient(size_t maxFreeBuffers, size_t bufferSize /* = 1024 */) : CIOBuffer::Allocator(bufferSize, maxFreeBuffers), m_port(0), m_connectSocket(INVALID_SOCKET)
		{
			m_theSendAccountBegin.wLen = 0;
			m_theSendAccountBegin.AccountBegin = {};
			m_uClientKey = 0;
		...
		void CSocketClient::Connect(const _tstring& addressToConnectServer, unsigned short portToConnectServer)
		{
		...
		SOCKET CSocketClient::CreateConnectionSocket(const OnlineGameLib::Win32::_tstring& addressToConnectServer, unsigned short port)
		{
		...
		bool CSocketClient::WaitAndVerifyCipher()
		{
			...
			while (true)
			{
				...
				int32_t res = select(0, &fdRead, NULL, NULL, pstTime);
				if (res > 0)
				{
