...
namespace OnlineGameLib {
	namespace Win32 {
		...
		_tstring GetLastErrorMessage(DWORD last_error)
		{
			...
			DWORD result = FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, nullptr, last_error, 0, errmsg, 511, nullptr);

			if (result == 0) {
				DWORD errorMessageID = GetLastError();
				// Xử lý lỗi, ví dụ: in ra thông điệp lỗi hoặc thực hiện một hành động phù hợp
				_tstring errorMessage = _T("Error retrieving error message. Error code: ") + std::to_string(errorMessageID);
				std::cerr << errorMessage << std::endl; // In ra thông điệp lỗi
				return _T("Error retrieving error message");
			}
		...
		_tstring GetCurrentDirectory()
		{
			...
			if (0 == ::GetCurrentDirectory(size, buffer.data()))
			{
				throw CException(_T("GetCurrentDirectory()"), _T("Failed to get current directory"));
			}

			return _tstring(buffer.data());
		}
...
#pragma comment(lib, "Version.lib")
		...
		_tstring GetFileVersion()
		{
			...
			if (verSize != 0)
			{
				 ...
				 * How many language in this product?
				 */
				if (::GetFileVersionInfo(pModuleFileName, 0, verSize, buffer.data()))
				{
					if (::VerQueryValue(buffer.data(),
						const_cast<LPSTR>(_T("\\VarFileInfo\\Translation")),
						(void**)(&lpTranslate),
						&nTranslateSize))
					{
						if ((nTranslateSize / sizeof(LCP)) > 0)
						{
							...
							/*
							 * get version information from the first language.
							 */
							wsprintf(szSubBlock,
								TEXT("\\StringFileInfo\\%04x%04x\\FileVersion"),
								lpTranslate[0].wLanguage,
							...
							std::vector<BYTE> versionBuffer(verSize);

							if (::GetFileVersionInfo(pModuleFileName, 0, verSize, versionBuffer.data()))
							{
								LPTSTR pVersion = 0;
								UINT verLen = 0;

								if (::VerQueryValue(versionBuffer.data(),
									szSubBlock,
									(void**)(&pVersion),
									&verLen))
								{
