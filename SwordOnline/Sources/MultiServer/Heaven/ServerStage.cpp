				...
				*/
...

				{
					...
					if (used > 0)
					{
						...
						KSG_EncodeBuf(used, (unsigned char*)(pBuffer->GetBuffer() + s_len_protocol), &pCN->uServerKey);
...
STDMETHODIMP_(const char*) CIOCPServer::GetClientInfo(const unsigned long& ulnClientID)
{
	if (ulnClientID < m_nPlayerMaxCount)
	{
		LPCLIENT_NODE pCN = NULL;
		...
		if (pCN && pCN->pSocket)
		{
			const CPerConnectionData* pData = reinterpret_cast<const CPerConnectionData*>(pCN->pSocket->COpaqueUserData::GetUserPtr());

			if (pData)
			{
				//return pData->GetConnectionDetails().c_str();
				const std::string& connectionDetails = pData->GetConnectionDetails();
				char* info = new char[connectionDetails.size() + 1];
				std::strcpy(info, connectionDetails.c_str());
				return info;
			}
		}
	}
...
void CIOCPServer::OnConnectionClosed(Socket* pSocket)
{
	if (pSocket)
	{
		 ...
		 */
		CPerConnectionData* pData = reinterpret_cast<CPerConnectionData*>(pSocket->COpaqueUserData::GetUserPtr());
...
void CIOCPServer::ReadCompleted(Socket* pSocket, OnlineGameLib::Win32::CIOBuffer* pBuffer)
{
	...
	try
	{
		const CPerConnectionData* pData = reinterpret_cast<const CPerConnectionData*>(pSocket->COpaqueUserData::GetUserPtr());
...
void CIOCPServer::ProcessCommand(Socket* pSocket, const CIOBuffer* pBuffer)
{
	...
	const CPerConnectionData* pPCD = reinterpret_cast<const CPerConnectionData*>(pSocket->COpaqueUserData::GetUserPtr());
...
bool CIOCPServer::_HelperAddClient()
{
	...
	if (pSocket)
	{
		...
		pSocket->COpaqueUserData::SetUserPtr(pData);
...
bool CIOCPServer::_HelperDelClient()
{
	...
	if (pSocket)
	{
		pSocket->COpaqueUserData::SetUserData(NULL);
	}
	...
	if ((DWORD)(-1) != dwIndex)
	{
		...
		*/
		...
		Trace(pData->GetConnectionDetails().c_str(), GetTimeStamp() + _T(" Del a client [ID:") + ToString(dwIndex) + _T("] - Current total is ") + ToString(m_usedClientNode.size()));

		Output(GetTimeStamp() + _T("Del a client [ID:") + ToString(dwIndex) + _T("] - Current total is ") + ToString(m_usedClientNode.size()));
	}
