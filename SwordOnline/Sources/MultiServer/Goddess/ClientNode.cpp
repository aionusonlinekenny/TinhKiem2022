	...
	, m_pServer(pServer)
{
	...
	m_theProcessArray[c2s_rolecheckname_result] = &CClientNode::_CheckRoleName;
	m_theProcessArray[c2s_rolechangename_result] = &CClientNode::_ChangeRoleName;
}
...
uint32_t __stdcall CClientNode::ThreadFunction(void* pV)
{
...
void CClientNode::_QueryRoleList(const void* pData, size_t dataLength)
{
	...
	int32_t nRoleListCount = pPlayerList->pDataBuffer[0];
	int32_t nLen = pPlayerList->nDataLen;
	ULONG ulIdentity = pPlayerList->ulIdentity;
	...
	int32_t nCount = GetRoleListOfAccount(szAccountName, &DBI[0], nRoleListCount);
...
void CClientNode::_SaveRoleInfo(const void* pData, size_t dataLength)
{
	...
	{
		int32_t len = strlen(pRole->BaseInfo.szName);
		ASSERT(len > 0);
		len = len > 31 ? 31 : len;
...
void CClientNode::_CheckRoleName(const void* pData, UINT dataLength)
{
	ASSERT(m_pServer && pData && dataLength);

	TCheckNameData* pRoleInfo = (TCheckNameData*)pData;

	char szRoleName[_NAME_LENGTH];

	int nDataLen = strlen(pRoleInfo->nRoleName);//pRoleInfo->nDataLen;
	nDataLen = (nDataLen > _NAME_LENGTH) ? _NAME_LENGTH : nDataLen;

	int result = -1;

	if (nDataLen > 0 && nDataLen < _NAME_LENGTH)
	{
		memcpy(szRoleName, &pRoleInfo->nRoleName, nDataLen - 1);
		szRoleName[nDataLen] = '\0';

		if (CheckRoleName(szRoleName))
		{
			result = 1;
		}
		else
	...
	{
		TCheckNameData Info;
		Info.nProtoId = s2c_rolecheckname_result;
		Info.ulIdentity = pRoleInfo->ulIdentity;
		Info.nDataLen = result;
		sprintf(Info.nAccName, pRoleInfo->nAccName);
		sprintf(Info.nRoleName, pRoleInfo->nRoleName);
		m_pServer->SendData(m_nIndentity, (const void*)&Info, sizeof(Info));
	}

}

void CClientNode::_ChangeRoleName(const void* pData, UINT dataLength)
{
	ASSERT(m_pServer && pData && dataLength);

	TChangeNameData* pRoleInfo = (TChangeNameData*)pData;
	char szRoleName[_NAME_LENGTH];
	char szTarRoleName[_NAME_LENGTH];

	int nDataLen = strlen(pRoleInfo->nTarRoleName);//pRoleInfo->nDataLen;
	nDataLen = (nDataLen >= _NAME_LENGTH) ? (_NAME_LENGTH - 1) : nDataLen;

	if (nDataLen > 0)
	{
		memcpy(szTarRoleName, &pRoleInfo->nTarRoleName, nDataLen);
		szTarRoleName[nDataLen] = '\0';

		if (!CheckRoleName(szTarRoleName))
		{
			int nDataLen1 = strlen(pRoleInfo->nSrcRoleName);//pRoleInfo->nDataLen;
			nDataLen1 = (nDataLen1 >= _NAME_LENGTH) ? (_NAME_LENGTH - 1) : nDataLen1;

			memcpy(szRoleName, &pRoleInfo->nSrcRoleName, nDataLen1);
			szRoleName[nDataLen1] = '\0';

			ChangeRoleName(szRoleName, szTarRoleName);
		}
	}
}
...
void CClientNode::_DelRole(const void* pData, size_t dataLength)
{
	...
	int32_t nDataLen = pRoleInfo->nDataLen;
	nDataLen = (nDataLen > _NAME_LENGTH) ? _NAME_LENGTH : nDataLen;
	...
	if (nDataLen > 0)
	{
		...
		if (nDataLen < _NAME_LENGTH)
			szRoleName[nDataLen] = '\0';
		else
			szRoleName[_NAME_LENGTH - 1] = '\0';
...
void CClientNode::_RelayExtend(const void* pData, size_t dataLength)
{
	...
	case MAKEWORD(pf_gamemaster, gm_c2s_getrole):
	{
		...
		case gm_role_entergame_position:
		{
			...
			char* pReturnBuffer = (char*)_malloca(1 + sizeof(GM_GET_ROLE_DATA_SYNC) + sizeof(GM_ROLE_DATA_SUB_ENTER_POS));
...
void CClientNode::_GetRoleInfo(const void* pData, size_t dataLength)
{
	...
	int32_t nUsedLength = sizeof(TProcessData);
...
{//发送游戏统计数据(By Fellow,2003.7.22)
	...
	int32_t nDataLen = sizeof(TGAME_STAT_DATA);
