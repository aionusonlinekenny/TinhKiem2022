...
typedef int32_t(*GetIndexFunc)(DB*, const DBT*, const DBT*, DBT*);
...
class ZDBTable {
	...
	int32_t index_number = 0;											//二级索引数目
	char table_name[MAX_TABLE_NAME] = {};
protected:
	char env_path[MAX_TABLE_NAME] = {};
	DB_ENV* dbenv;												//数据库环境
	ZCursor* _search(bool bKey, const char* key_ptr, int32_t key_size, int32_t index);		//搜索指定记录
...
public:
	...
	int32_t addIndex(GetIndexFunc func, bool isUnique = false);		//增加索引项
	bool open();												//打开数据表
	void close();												//关闭数据表
	bool commit();												//基于事务的提交，目前使用自动提交，不使用

	//基本记录操作
	bool add(const char* key_ptr, int32_t key_size, const char* data_ptr, int32_t data_size);
	bool remove(const char* key_ptr, int32_t key_size, int32_t index = -1);
	...
	ZCursor* search(const char* key_ptr, int32_t key_size, int32_t index = -1) {
		return _search(false, key_ptr, key_size, index);		//搜索指定记录
	}
	bool next(ZCursor* cursor) {											//下一个记录
		return _next(false, cursor);
	}
	ZCursor* search_key(const char* key_ptr, int32_t key_size, int32_t index = -1) {	//搜索指定记录，返回主键值
