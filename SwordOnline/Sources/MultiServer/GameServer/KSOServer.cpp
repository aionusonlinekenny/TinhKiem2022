...
const INT KSwordOnLineSever::m_snMaxBuffer = 256;	//1024
const INT KSwordOnLineSever::m_snBufferSize = 1024 * 64; //16 //* 50
	...
	const ULONG& ulnEventType)
{
	...
	case enumClientConnectCreate:
		if (!g_SOServer.GetRunningDataBase())
		{
			for (;;)
			{
				if (g_SOServer.GetRunningDataBase())
		...
		if (!g_SOServer.GetRunningGateway())
		{
			for (;;)
			{
				if (g_SOServer.GetRunningGateway())
	...
	const ULONG& ulnEventType)
{
	...
	case enumServerConnectClose:
		...
		g_SOServer.SetRunningGateway(FALSE);
		break;
	}
}
	...
	const ULONG& ulnEventType)
{
	switch (ulnEventType)
	{
	case enumServerConnectCreate: // ���ӽ���ʱ��Ĵ���
		break;
	case enumServerConnectClose: // ���Ӷο�ʱ��Ĵ���
	...
	const ULONG& ulnEventType)
{
	...
	case enumServerConnectClose:
		...
		//g_SOServer.SetRunningStatus(FALSE);
		g_SOServer.SetRunningDataBase(FALSE);
		break;
	}
}
...
KSwordOnLineSever::KSwordOnLineSever()
{
	...
	m_nPrecision = 0;
	m_nGameLoop = 0;
	m_szGameServerIP[0] = 0;
	m_szIntranetIP[0] = 0;
	m_szInternetIP[0] = 0;
	m_szMAC[0] = 0;
	m_bRunningDataBase = FALSE;
	m_bRunningGateway = FALSE;
}
...
BOOL KSwordOnLineSever::Init()
{
	...
	m_bIsRunning = TRUE;
	m_bRunningDataBase = TRUE;
	m_bRunningGateway = TRUE;
	g_SetRootPath(NULL);
	system("color 0E");
	g_SetFilePath("\\");
	...
	iniFile.GetString("GameServer", "Ip", "222.189.237.112", m_szGameServerIP, sizeof(m_szGameServerIP));
	...
	iniFile.GetString("Transfer", "Ip", "192.168.22.105", m_szTransferIP, sizeof(m_szTransferIP));
	...
	iniFile.GetString("Network", "IntranetIP", "192.168.22.105", m_szIntranetIP, sizeof(m_szIntranetIP));
	iniFile.GetString("Network", "InternetIP", "192.168.22.105", m_szInternetIP, sizeof(m_szInternetIP));
	iniFile.GetString("Network", "MAC", "192.168.22.105", m_szMAC, sizeof(m_szMAC));
#ifdef WIN32
	...
	//if (!m_pGameStatus)
	if (m_pGameStatus.empty())
	{
		//m_pGameStatus = new GameStatus[m_nMaxPlayer];
		// 
		// Khởi tạo vector với m_nMaxPlayer phần tử
		m_pGameStatus.reserve(m_nMaxPlayer);
		for (INT i = 0; i < m_nMaxPlayer; ++i) {
			m_pGameStatus.push_back(std::make_unique<GameStatus>());
		}
	}
	...
	if (!GetLocalIpAddress(&m_dwIntranetIp, &m_dwInternetIp, 32, 6))
	{
	...
	m_pCoreServerShell->OperationRequest(SSOI_LAUNCH, (UINT)pCloneServer, 0);
	...
	{
		pClientFactory->SetEnvironment(1024 * 128 * 4); //128
	...
	m_pCoreServerShell->OperationRequest(SSOI_DATA_INIT, (UINT)pDataServer, 0);  //·¢ËÍ¿ªÊ¼ÓÎÏ·ÃüÁî
	...
	if (pClientFactroyFun && SUCCEEDED(pClientFactroyFun(IID_IClientFactory, reinterpret_cast<void**>(&pClientFactory))))
	{
		pClientFactory->SetEnvironment(1024 * 512 * 2);	//512
	...
	{
		cout << "Connect Transfer failed!" << endl;
		if (m_pTransferClient)
		{
	...
	if (pClientFactroyFun && SUCCEEDED(pClientFactroyFun(IID_IClientFactory, reinterpret_cast<void**>(&pClientFactory))))
	{
		pClientFactory->SetEnvironment(1024 * 1024 * 4); //4
	...
	IClient* pTongClient = m_pTongClient;
	//Sleep(5000);
	//------------------------ Connect tong end ----------------------------
	...
	if (pClientFactroyFun && SUCCEEDED(pClientFactroyFun(IID_IClientFactory, reinterpret_cast<void**>(&pClientFactory))))
	{
		pClientFactory->SetEnvironment(1024 * 1024 * 2); //1024
	...
	m_pCoreServerShell->OperationRequest(SSOI_TONG, (UINT)pTongClient, 0);
...
void KSwordOnLineSever::Release()
{
	...
	if (m_pServer)
	{
		...
		{
			printf("Save File Data All Script !");
			m_pCoreServerShell->OperationRequest(SSOI_SHUTDOWN, 0, 0);
		}
...
#ifdef _STANDALONE
	//	delete net_buffer;
	delete m_pTransferClient;
	m_pTransferClient = NULL;
	delete m_pDatabaseClient;
	m_pDatabaseClient = NULL;
	...
	if (m_pCoreServerShell && m_bIsRunning)
	{
		if (!m_bRunningDataBase)
		{
			for (;;)
			{
				...
				cout << "Repair connect database !!!" << endl;
			...
			for (i = 0; i < m_nMaxPlayer; i++)
			{
				...
				reg.bLock = true;
				m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NAME, (UINT)reg.Name, nIndex);
				if (m_pDatabaseClient)
		...
		if (!m_bRunningGateway)
		{
			printf("TU DONG KET NOI\n");

			for (INT igt = 0; igt < 1; igt++)
			{
				if (m_pGatewayClient)
				{
					m_pGatewayClient->Shutdown();
				...
				cout << "Repair connect Gateway !!!" << endl;
		...
		//{
		//	//printf("Game Auto Save Processing!\n");
		//	AutoSave();
		//}
		if (m_nGameLoop * 1000 <= m_Timer.GetElapse() * GAME_FPS)
		{
			...
			if (m_nGameLoop % (GAME_FPS * 60) == 0) // һ����һ��
			{
				KCoreConnectInfo Info;
				INT nCurrentPlayerCount = m_pCoreServerShell->GetConnectInfo(&Info);
				DWORD dwElapse = m_Timer.GetElapse();
				printf("==[Time: %s][Loop= %06d][OnlineTime= %02d:%02d:%02d][Online= %d]==\n", NowToString().c_str(), m_nGameLoop, dwElapse / (1000 * 3600), (dwElapse / (1000 * 60)) % 60, (dwElapse / 1000) % 60, Info.nNumPlayer);
				//SavePlayerData();
			}
#ifdef _STANDALONE
			if (m_nGameLoop % (GAME_FPS * 60) == 0) // һ����һ��
...
{
#define def_SAVE_TIME GAME_FPS * 60 * 5
	INT lnID;
	for (lnID = 0; lnID < m_nMaxPlayer; lnID++)
	{
		if (m_nGameLoop - m_pGameStatus[lnID]->nLastSave >= def_SAVE_TIME)
		{
			...
			m_pGameStatus[lnID]->nLastSave = m_nGameLoop;
		}
	}
...
void KSwordOnLineSever::ChatMessageProcess(const char* pChar, size_t nSize)
{
	...
	case chat_everyone:
	{
		...
		m_pCoreServerShell->OperationRequest(SSOI_BROADCASTING, (UINT)(pCeo + 1), pCeo->wChatLength);
	}
	break;
	default:
		break;
...
void KSwordOnLineSever::TongMessageProcess(const char* pChar, size_t nSize)
{
	...
	else if (pHeader->ProtocolFamily == pf_tong)
	{
		...
		case enumS2C_TONG_CREATE_SUCCESS:
		{
			...
			if (m_pCoreServerShell->OperationRequest(SSOI_TONG_CREATE, (UINT)&sSuccess, 0))
			{
			}
			else
			{
		...
		case enumS2C_TONG_ADD_MEMBER_SUCCESS:
		{
			...
			if (m_pCoreServerShell->OperationRequest(SSOI_TONG_ADD, (UINT)&sAdd, 0))
			{
			}
			else
			{
		...
		case enumS2C_TONG_BE_INSTATED:
		{
			...
			m_pCoreServerShell->GetGameData(SGDI_TONG_BE_INSTATED, (UINT)&sInstated, 0);
		}
		break;
		...
		case enumS2C_TONG_BE_KICKED:
		{
			...
			m_pCoreServerShell->GetGameData(SGDI_TONG_BE_KICKED, (UINT)&sKicked, 0);
		}
		break;
		...
		case enumS2C_TONG_LEAVE:
		{
			...
			m_pCoreServerShell->GetGameData(SGDI_TONG_LEAVE, (UINT)&sLeave, 0);
		}
		break;
		...
		case enumS2C_TONG_CHECK_CHANGE_MASTER_POWER:
		{
			...
			INT nRet = 0;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_GET_MASTER_POWER, (UINT)&sCheck, 0);
		...
		case enumS2C_TONG_CHANGE_AS:
		{
			...
			m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGE_AS, (UINT)&sChange, 0);
		}
		break;
		...
		case enumS2C_TONG_CHANGE_MASTER:
		{
			...
			m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGE_MASTER, (UINT)&sChange, 0);
		}
		break;
		case enumS2C_TONG_MONEY_SAVE:
		{
			...
			if (pChange.m_nPlayerIdx <= 0)
				break;
			m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGE_MONEY, (UINT)&pChange, 0);
		}
		break;
		...
		case enumS2C_TONG_LOGIN_DATA:
		{
			...
			m_pCoreServerShell->GetGameData(SGDI_TONG_LOGIN, (UINT)&sLogin, 0);
		}
		break;
		...
		case enumS2C_TONG_CHECK_CHANGE_AGNAME_POWER:
		{
			...
			INT nRet = 0;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_GET_AGNAME_POWER, (UINT)&sCheck, 0);
		...
		case enumS2C_TONG_BE_CHANGED_AGNAME:
		{
			...
			m_pCoreServerShell->GetGameData(SGDI_TONG_BE_CHANGED_AGNAME, (UINT)&sSync, 0);
		}
		break;
		case enumS2C_TONG_BE_CHANGED_CAMP:
		{
			...
			m_pCoreServerShell->GetGameData(SGDI_TONG_BE_CHANGED_CAMP, (UINT)&sSync, 0);
		}
		break;
		break;
		case enumS2C_TONG_BE_CHANGED_LEVEL:
		{
			...
			m_pCoreServerShell->GetGameData(SGDI_TONG_BE_CHANGED_LEVEL, (UINT)&sSync, 0);
		}
		break;
		case enumS2C_TONG_BE_CHANGED_MONEY:
		{
			...
			m_pCoreServerShell->GetGameData(SGDI_TONG_BE_CHANGED_MONEY, (UINT)&sSync, 0);
		}
		break;
		case enumS2C_TONG_BE_CHANGED_EFF:
		{
			...
			m_pCoreServerShell->GetGameData(SGDI_TONG_BE_CHANGED_TONG_EFF, (UINT)&sSync, 0);
		}
		break;
		case enumS2C_TONG_BE_CHANGED_RECRUIT:
		{
			...
			m_pCoreServerShell->GetGameData(SGDI_TONG_BE_CHANGED_RECRUIT, (UINT)&sSync, 0);
		}
		break;
		case enumS2C_TONG_BE_CHANGED_TONGPARAM:
		{
			...
			m_pCoreServerShell->GetGameData(SGDI_TONG_BE_CHANGED_TONGPARAM, (UINT)&sSync, 0);
		}
		break;
		case enumS2C_TONG_BE_CHANGED_JIYU:
		{
			...
			m_pCoreServerShell->GetGameData(SGDI_TONG_BE_CHANGED_JIYU, (UINT)&sSync, 0);
		}
		break;
		case enumS2C_TONG_BE_CHANGED_MEMBEREFF:
		{
			...
			if (sSync.m_nPlayerIdx <= 0)
				break;
			m_pCoreServerShell->GetGameData(SGDI_TONG_BE_CHANGED_EFF, (UINT)&sSync, 0);
		}
		break;
		default:
			break;
...
void KSwordOnLineSever::ChatGroupMan(const void* pData, size_t dataLength)
{
	...
	else
	{
		...
		for (INT i = 0; i < playercount; i++)
		{
			// TODO: ����Ҫ�Ӽ���NameID��lnID��һ����
			if (pPlayers[i] >= 0)
				m_pServer->PackDataToClient((ULONG)pPlayers[i], pExHdr, pckgsize);
		}
	}
}
...
void KSwordOnLineSever::DatabaseMessageProcess(const char* pData, size_t dataLength)
{
	...
	case s2c_rolecheckname_result:
	{
		...
		{
			if (m_pCoreServerShell->SetChangeRoleStatus(pnPD->ulIdentity, pnPD->nDataLen, pnPD->nRoleName))
			{
				printf("[CHANGE ROLENAME SUCCESS]:Tai khoan:%s Nhan vat:%s!\n", pnPD->nAccName, pnPD->nRoleName);
				SavePlayerData(pnPD->ulIdentity, TRUE);
			}
		}
	}
	break;
		...
		// m_bSaveFlag = TRUE;
	{
		...
		if (pPD->szName[0])
		{
			m_pCoreServerShell->RenameRoleRequest(nIndex, pPD->bLeave);
		}
		printf("Da luu thong tin nhan vat(%d)!\n", nIndex);
	}
	break;
	default:
...
void KSwordOnLineSever::TransferSmallPackProcess(const void* pData, size_t dataLength, KTransferUnit* pUnit)
{
	...
	case c2c_permitplayerexchangein:
	{
		...
		if (pPermit->bPermit)
		{
			...
			char szName[32] = {};
			m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ACCOUNT, (UINT)szName, nIndex);
			sLeaveGame.nExtPoint = m_pCoreServerShell->GetGameData(SGDI_CHARACTER_EXTPOINTCHANGED, nIndex, 0);
			sLeaveGame.nExtDay = m_pCoreServerShell->GetGameData(SGDI_CHARACTER_EXTDAY, nIndex, 0);
...
void KSwordOnLineSever::PlayerMessageProcess(const ULONG lnID, const char* pData, size_t dataLength)
{
	// #ifndef _STANDALONE
	_ASSERT(pData && dataLength);
	...
	try
	{
		...
		if (protocoltype >= _c2s_begin_relay && protocoltype <= _c2s_end_relay)
		{
			...
			{
			case c2s_extend:
				pClient = m_pTransferClient;
				break;
			case c2s_extendchat:
				pClient = m_pChatClient;
				break;
			case c2s_extendfriend:
				pClient = m_pTongClient;
				break;
			default:
				return;
			};
			...
			if (pClient && nIndex && dataLength > sizeof(tagExtendProtoHeader))
			{
				...
				if (protocoltype == c2s_extendchat)
				{
					...
					if (pEh->ProtocolType == chat_channelchat && pEh->channelid != 0) // ��GMƵ��Ҫ���˺͸�Ǯ
					{
		...
		case enumPlayerSyncEnd:
			if (ProcessSyncReplyProtocol(lnID, pData, dataLength))
			{
				...
				m_pGameStatus[lnID]->nGameStatus = enumPlayerPlaying;
				m_pGameStatus[lnID]->nLastSave = m_nGameLoop + (rand() % 291) + 10;
				PingClient(lnID);

				tagEnterGame eg;
				eg.cProtocol = c2s_entergame;

				m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ACCOUNT, (UINT)eg.szAccountName, nIndex);
				...
				m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NAME, (UINT)eg2.szCharacterName, nIndex);
				eg2.lnID = lnID;
				if (m_pTransferClient)
				...
				if (nPlayerIdx > 0 && nPlayerIdx < MAX_PLAYER)
				{
					...
					char szName[32];
					szName[0] = 0;
					m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NAME, (UINT)szName, nPlayerIdx);
		...
		case enumPlayerBegin:
		{
			...
			if (nIndex)
			{
				for (int i = 0; i < m_nMaxPlayer; i++)
				{
					if (i != lnID && m_pGameStatus[lnID]->nPlayerIndex == nIndex)
					{
						g_SOServer.SetNetStatus(i, enumNetUnconnect);
						break;
					}
				}
				...
				if (SendGameDataToClient(lnID, nIndex))
				{
					...
					m_pGameStatus[lnID]->bIsOffline = FALSE;
				}
				else
				{
	...
	if (pBuffer)
	{
		...
		case s2c_syncgamesvr_roleinfo_cipher:
		{
			...
			if (pGI->datalength > 0)
			{
				...
				if (Info.nNumPlayer < m_nMaxPlayer && bCRCCheck) // ȥ��ע��CRC�����û����޷�������Ϸ��
				{
...
void KSwordOnLineSever::GatewaySmallPackProcess(const void* pData, size_t dataLength)
{
	...
	try
	{
		...
		{
			tagUpdateMapID* pUMI;
			int16_t sMapID[200]; //1000

			INT nMapCount = m_pCoreServerShell->GetGameData(SGDI_LOADEDMAP_ID, (UINT)&sMapID, sizeof(sMapID));
			pUMI = (tagUpdateMapID*)new int16_t[nMapCount * sizeof(int16_t) + sizeof(tagUpdateMapID)];
			memset(pUMI, 0, nMapCount * sizeof(int16_t) + sizeof(tagUpdateMapID));

			pUMI->cProtocol = c2s_updatemapinfo;
			pUMI->cMapCount = nMapCount;
			memcpy(pUMI->szMapID, sMapID, nMapCount * sizeof(int16_t));

			/*printf("-----Send \ndatalen = %u, data = ", nMapCount * sizeof(char) + sizeof( tagUpdateMapID ));
			for (INT i = 0; i < 32; i++)
				printf("[%02X] ", ((BYTE *)pUMI)[i]);
			printf("\n");*/
			m_pGatewayClient->SendPackToServer((const void*)pUMI, nMapCount * sizeof(int16_t) + sizeof(tagUpdateMapID));
		...
		case s2c_gateway_broadcast:
			...
			break;
		case s2c_punish:
			GatewayPunishProcess((const char*)pData, dataLength);
			break;
		}
	}
	catch (...)
...
void KSwordOnLineSever::GatewayBoardCastProcess(const char* pData, size_t dataLength)
{
	...
	//printf("BroadCast type = %d, %08x, %08x, %d\n", pGatewayBroadCast->uCmdType, (UINT)pGatewayBroadCast->szData, pGatewayBroadCast->szData, strlen(pGatewayBroadCast->szData));
	switch (pGatewayBroadCast->uCmdType)
	{
	case AP_WARNING_ALL_PLAYER_QUIT:
	{
		m_pCoreServerShell->OperationRequest(SSOI_BROADCASTING, (UINT)pGatewayBroadCast->szData, strlen(pGatewayBroadCast->szData));
...
void KSwordOnLineSever::GatewayPunishProcess(const char* pData, size_t dataLength)
{
	// #ifndef _STANDALONE
	if (dataLength != sizeof(tagGatewayBroadCast))
		return;
	...
	switch (pGatewayBroadCast->uCmdType)
	{
	case AP_LOCK_ACCOUNTNAME:
		if (m_pCoreServerShell->OperationRequest(SSOI_LOCK_ACCOUNTNAME, (UINT)pGatewayBroadCast->szData, strlen(pGatewayBroadCast->szData)))
			printf("Lock Account: %s", pGatewayBroadCast->szData);
		break;
	case AP_UNLOCK_ACCOUNTNAME:
		if (m_pCoreServerShell->OperationRequest(SSOI_UNLOCK_ACCOUNTNAME, (UINT)pGatewayBroadCast->szData, strlen(pGatewayBroadCast->szData)))
			printf("Unlock Account: %s", pGatewayBroadCast->szData);
		break;
	case AP_LOCK_CHAT:
		if (m_pCoreServerShell->OperationRequest(SSOI_LOCK_CHAT, (UINT)pGatewayBroadCast->szData, strlen(pGatewayBroadCast->szData)))
			printf("Lock Speech: %s", pGatewayBroadCast->szData);
		break;
	case AP_UNLOCK_CHAT:
		if (m_pCoreServerShell->OperationRequest(SSOI_UNLOCK_CHAT, (UINT)pGatewayBroadCast->szData, strlen(pGatewayBroadCast->szData)))
			printf("Unlock Speech: %s", pGatewayBroadCast->szData);
		break;
	default:
		break;
	}
...
void KSwordOnLineSever::MainLoop()
{
	...
	//m_pServer->PreparePackSink();	//�պ�����û�е��õı�Ҫ
	m_pCoreServerShell->Breathe();

	// ���������ݿ��ѯ�������

#define	defMAX_STAT_QUERY_TIME		5 * 60 * 60 * 20		// lay du lieu xep hang ne
	...
	// �������Ƿ�ܳ�ʱ��û�з�PINGֵ�ˣ�1min��
	UINT lnID = m_nGameLoop % m_nMaxPlayer;
	UINT nIndex = m_pGameStatus[lnID]->nPlayerIndex;
	if (nIndex > 0 && nIndex <= m_nMaxPlayer && m_pGameStatus[lnID]->nGameStatus == enumPlayerPlaying)
	{
#define defMAX_PING_TIMEOUT 60 * 18 // 60 sec
#define defMAX_PING_INTERVAL 5 * 60 * 18 // 5 mins

		if (m_pCoreServerShell->GetCharacterLixian(nIndex) == LIXIAN_IDLE)
...
void KSwordOnLineSever::PingClient(const ULONG lnID)
{
...
void KSwordOnLineSever::ProcessPingReply(const ULONG lnID, const char* pData, size_t dataLength)
{
	_ASSERT(lnID < m_nMaxPlayer);
	_ASSERT(m_pGameStatus[lnID]->nPlayerIndex > 0 && m_pGameStatus[lnID]->nPlayerIndex <= m_nMaxPlayer);
...
void KSwordOnLineSever::SavePlayerData()
{
	...
	for (i = 0; i < m_nMaxPlayer; i++)
	{
		UINT nIndex = m_pGameStatus[i]->nPlayerIndex;

		if (GetNetStatus(i) == enumNetUnconnect && m_pCoreServerShell->GetCharacterLixian(nIndex) == LIXIAN_IDLE)
			continue;
		...
		{
			if (SavePlayerData(nIndex, false))
				//dungnd try to fix save data
				m_pCoreServerShell->SetSaveStatus(nIndex, SAVE_IDLE);
		}
	}
}

BOOL KSwordOnLineSever::SavePlayerData(UINT nIndex, bool bUnLock)
{
	if (!m_pDatabaseClient)
		return FALSE;
	...
	memset(sProcessData.szName, 0, sizeof(sProcessData.szName));
	m_pCoreServerShell->GetLastName(nIndex, sProcessData.szName);
	...
	if (pData && pData->dwDataLen)
	{
		...
		/*char szName[32];
		m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NAME, (UINT)szName, nIndex);*/

		//printf("Da luu thong tin nhan vat [%d] - [%s]\n", nIndex, szName);
		return TRUE;
	}
	char szName[32];
	m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NAME, (UINT)szName, nIndex);
	printf("Save Player [%d]-%s data failed\n", nIndex, szName);
	...
	for (i = 0; i < m_nMaxPlayer; i++)
	{
		...
		if (m_pCoreServerShell->IsPlayerExchangingServer(nIndex))
		{
			if (!m_pTransferClient)
			{
				...
				//m_pGameStatus[i]->bIsOffline = FALSE;
				continue;
			}
...
void KSwordOnLineSever::PlayerLogoutGateway()
{
	...
	for (INT nIndex = 1; nIndex < MAX_PLAYER; nIndex++)
	{
		if (m_pCoreServerShell->IsPlayerLoginTimeOut(nIndex))
		{
			...
			//g_DebugLog("[IsPlayerLoginTimeOut] Account [%s] Logout: ExtPoint[%d] - ExtDay[%d]\n", sLeaveGame.szAccountName, sLeaveGame.nExtPoint, sLeaveGame.nExtDay);
			m_pGatewayClient->SendPackToServer(&sLeaveGame, sizeof(tagLeaveGame));
		...
		{
			if (m_pCoreServerShell->GetCharacterLixian(nIndex) == LIXIAN_IDLE || m_pCoreServerShell->GetCharacterLixian(nIndex) == LIXIAN_REQUEST)
				SavePlayerData(nIndex, true);
			//			if (!SavePlayerData(nIndex))
			//				continue;
			if (m_pCoreServerShell->GetCharacterLixian(nIndex) == LIXIAN_IDLE || m_pCoreServerShell->GetCharacterLixian(nIndex) == LIXIAN_REQUEST)
			{
				if (m_pCoreServerShell->GetCharacterLixian(nIndex) == LIXIAN_REQUEST)
					m_pCoreServerShell->SetCharacterLixian(nIndex, LIXIAN_DOING);
				...
				//g_DebugLog("[IsCharacterQuiting] Account [%s] Logout: ExtPoint[%d] - ExtDay[%d]\n", sLeaveGame.szAccountName, sLeaveGame.nExtPoint, sLeaveGame.nExtDay);
				...
				m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NAME, (UINT)szName, nIndex);
				if (m_pCoreServerShell->GetCharacterLixian(nIndex) == LIXIAN_DOING)
					printf("Nguoi choi uy thac: [%d]%s..%s\n", nIndex, sLeaveGame.szAccountName, szName);
				else
					printf("Thoat game: [%d]%s..%s\n", nIndex, sLeaveGame.szAccountName, szName);
			}
...
INT KSwordOnLineSever::GetNetStatus(const ULONG lnID)
{
...
void KSwordOnLineSever::SetNetStatus(const ULONG lnID, NetStatus nStatus)
{
	if (lnID >= m_nMaxPlayer)
		return;
	...
	{
		if (m_pGameStatus[lnID]->nGameStatus == enumPlayerPlaying || (m_pGameStatus[lnID]->nGameStatus == enumPlayerExchangingServer && m_pGameStatus[lnID]->nExchangeStatus != enumExchangeCleaning)) // �������ʱ�Լ�����
		{
			INT nIndex = m_pGameStatus[lnID]->nPlayerIndex;
		...
		//if (m_pCoreServerShell->GetCharacterLixian(m_pGameStatus[lnID]->nPlayerIndex) == LIXIAN_IDLE)
		//{
			m_pGameStatus[lnID]->nGameStatus = enumPlayerBegin;
			m_pGameStatus[lnID]->nPlayerIndex = 0;
			m_pGameStatus[lnID]->nExchangeStatus = enumExchangeBegin;
...
INT KSwordOnLineSever::ProcessLoginProtocol(const ULONG lnID, const char* pData, size_t dataLength)
{
...
BOOL KSwordOnLineSever::SendGameDataToClient(const ULONG lnID, const INT nPlayerIndex)
{
	BOOL bRet = FALSE;
	// #ifndef _STANDALONE
	_ASSERT(m_pServer);
...
BOOL KSwordOnLineSever::ProcessSyncReplyProtocol(const ULONG lnID, const char* pData, size_t dataLength)
{
	const char* pBuffer = pData;
...
BOOL KSwordOnLineSever::ExitPlayer(INT lnID)
{
#ifndef __linux // SwitchToThread();
	Sleep(4000);
#else
...
void KSwordOnLineSever::SetRunningDataBase(BOOL bStatus)
{
	m_bRunningDataBase = bStatus;
}

BOOL KSwordOnLineSever::GetRunningDataBase()
{
	return m_bRunningDataBase;
}

void KSwordOnLineSever::SetRunningGateway(BOOL bStatus)
{
	m_bRunningGateway = bStatus;
}

BOOL KSwordOnLineSever::GetRunningGateway()
{
	return m_bRunningGateway;
}
...
BOOL KSwordOnLineSever::GetLocalIpAddress(DWORD* pIntranetAddr, DWORD* pInternetAddr, DWORD nMask, DWORD nMacLenght)
{
#ifndef _STANDALONE
	return gGetMacAndIPAddress(NULL, pIntranetAddr, NULL, pInternetAddr, nMask, nMacLenght, m_szIntranetIP, m_szInternetIP, m_szMAC);
#else
	char szIp[16];
...
void KSwordOnLineSever::ProcessPlayerTongMsg(const ULONG nPlayerIdx, const char* pData, size_t dataLength)
{
	if (nPlayerIdx <= 0 || nPlayerIdx >= MAX_PLAYER)
		return;
	if (!pData)
	...
	case enumTONG_COMMAND_ID_APPLY_CREATE:
	{
		...
		memcpy(sApply.m_szTongName, pApply->m_szName, sizeof(sApply.m_szTongName));

		// ��������������
		INT nRet = 0xff;
		if (m_pCoreServerShell)
			nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_APPLY_CREATE, (UINT)&sApply, 0);
		// ���������������
		if (nRet == 0)
		{
			...
			m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NAME, (UINT)szPlayerName, nPlayerIdx);
	...
	case enumTONG_COMMAND_ID_APPLY_ADD:
	{
		...
		if (m_pCoreServerShell)
			m_pCoreServerShell->GetGameData(SGDI_TONG_APPLY_ADD, (UINT)&sAdd, 0);
	}
	break;
	...
	case enumTONG_COMMAND_ID_ACCEPT_ADD:
	{
		...
		if (pAccept->m_btFlag == 0)
		{
			...
			m_pCoreServerShell->OperationRequest(SSOI_TONG_REFUSE_ADD, (UINT)&sRefuse, 0);
			break;
		}
		else
		{
			...
			INT nRet = FALSE;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_CHECK_ADD_CONDITION, (UINT)szTongName, (UINT)&sAdd);
			// �� relay �������Ӱ���
			if (nRet)
			{
				...
				szPlayerName[0] = 0;
				m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NAME, (UINT)szPlayerName, sAdd.m_nTargetIdx);
	...
	case enumTONG_COMMAND_ID_ACCEPT_REPLY_ADD:
	{
		...
		else
		{
			...
			INT nRet = FALSE;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_CHECK_ADD_CONDITION_REPLY, (UINT)szTongName, (UINT)&sAdd);
			// �� relay �������Ӱ���
			if (nRet)
			{
				char szPlayerName[32] = {};
				...
				szPlayerName[0] = 0;
				m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NAME, (UINT)szPlayerName, sAdd.m_nSelfIdx);
	...
	case enumTONG_COMMAND_ID_APPLY_INFO:
	{
		...
		case enumTONG_APPLY_INFO_ID_SELF:
			...
			sGet.m_szName[0] = 0;
			if (m_pCoreServerShell)
				m_pCoreServerShell->GetGameData(SGDI_TONG_GET_INFO, (UINT)&sGet, 0);
			break;
		...
		case enumTONG_APPLY_INFO_ID_MANAGER:
		{
			...
			sGet.m_szName[0] = 0;
			if (m_pCoreServerShell)
				nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_GET_INFO, (UINT)&sGet, 0);
			if (nRet == 0)
				break;
		...
		case enumTONG_APPLY_INFO_ID_TONG_HEAD:
		{
			...
			if (m_pCoreServerShell)
				dwTongNameID = m_pCoreServerShell->GetGameData(SGDI_TONG_GET_INFO, (UINT)&sGet, 0);
			if (dwTongNameID == 0)
				break;
	...
	case enumTONG_COMMAND_ID_APPLY_INSTATE:
	{
		...
		INT nRet = 0;
		if (m_pCoreServerShell)
			nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_INSTATE_POWER, (UINT)pApply, nPlayerIdx);
		if (nRet == 0)
			break;
	...
	case enumTONG_COMMAND_ID_APPLY_KICK:
	{
		...
		INT nRet = 0;
		if (m_pCoreServerShell)
			nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_KICK_POWER, (UINT)pKick, nPlayerIdx);
		if (nRet == 0)
			break;
	...
	case enumTONG_COMMAND_ID_APPLY_LEAVE:
	{
		...
		INT nRet = 0;
		if (m_pCoreServerShell)
			nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_LEAVE_POWER, (UINT)pLeave, nPlayerIdx);
		if (nRet == 0)
			break;
	...
	case enumTONG_COMMAND_ID_APPLY_CHANGE_MASTER:
	{
		...
		INT nRet = 0;
		if (m_pCoreServerShell)
			nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGE_MASTER_POWER, (UINT)pChange, nPlayerIdx);
		if (nRet == 0)
			break;
	...
	case enumTONG_COMMAND_ID_APPLY_GET:
	{
		...
		INT nRet = 0;
		if (m_pCoreServerShell)
			nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_MONEY_POWER, (UINT)pChange, nPlayerIdx);
		if (nRet == 0)
			break;
	...
	case enumTONG_COMMAND_ID_APPLY_CHANGE_AGNAME:
	{
		...
		INT nRet = 0;
		if (m_pCoreServerShell)
			nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGE_AGNAME_POWER, (UINT)pChange, nPlayerIdx);
		if (nRet == 0)
			break;
	...
	case enumTONG_COMMAND_ID_APPLY_CHANGE_SEX_AGNAME:
	{
		...
		INT nRet = 0;
		if (m_pCoreServerShell)
			nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGE_SEX_AGNAME_POWER, (UINT)pChange, nPlayerIdx);
		if (nRet == 0)
			break;
	...
	case enumTONG_COMMAND_ID_APPLY_CHANGE_CAMP:
	{
		...
		INT nRet = 0xff;
		if (m_pCoreServerShell)
			nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_APPLY_CHANGE_CAMP, (UINT)&pChange, 0);
	...
	case enumTONG_COMMAND_ID_APPLY_CHANGE_JIYU:
	{
		...
		INT nRet = 0;
		if (m_pCoreServerShell)
			nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGE_INFO_TONG, (UINT)pInfo, nPlayerIdx);
		if (nRet == 0)
			break;
	...
	case enumTONG_COMMAND_ID_APPLY_CHANGE_RECRUIT:
	{
		...
		INT nRet = 0;
		if (m_pCoreServerShell)
			nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGE_RECRUIT, (UINT)pInfo, nPlayerIdx);
		if (nRet == 0)
			break;
...
BOOL KSwordOnLineSever::CheckPlayerID(ULONG netidx, DWORD nameid)
{
...
string NowToString()
{
	...
	std::strftime(str, sizeof(str), "%d-%m %X", &buf);

	string time(str);
	time = std::regex_replace(time, std::regex("\n"), "");
	return time;
}

INT KSwordOnLineSever::NumberConnection() {
	INT nCount = 0;

	for (INT i = 0; i < m_nMaxPlayer; i++)
	{
		if (m_pGameStatus[i]->nPlayerIndex > 0)
			nCount++;
	}
