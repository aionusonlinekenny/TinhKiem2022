...
void BroadGlobal(const void* pData, UINT size, DWORD nFromIP, DWORD nFromRelayID)
{
	...
	UINT pckgsize = sizeof(RELAY_DATA) + data.routeDateLength;
...
INT LuaGlobalExecute(Lua_State * L)
{
	...
	if (szMsg && szMsg[0])
	{
		...
		UINT pckgsize = sizeof(GM_EXECUTE_COMMAND) + exe.wLength;
		void* pRelayData = _alloca(pckgsize);
...
INT LuaMsg2IP(Lua_State * L)
{
	...
	if (szMsg && szMsg[0])
	{
		...
		UINT pckgsize = sizeof(ChatMsgEx) + ChatMsgEx.SentenceLength;
		void* pRelayData = _alloca(pckgsize);
