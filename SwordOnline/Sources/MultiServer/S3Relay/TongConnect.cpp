...
INT	g_nTongPSSize[defTONG_PROTOCOL_SERVER_NUM] =
{
	...
	sizeof(STONG_MONEY_COMMAND),				// enumC2S_TONG_MONEY_SAVE
	sizeof(STONG_MONEY_COMMAND),				// enumC2S_TONG_MONEY_GET

	sizeof(STONG_CHANGE_AGNAME_COMMAND),			// enumC2S_TONG_CHANGE_AGNAME
	sizeof(STONG_ACCEPT_AGNAME_COMMAND),			// enumC2S_TONG_ACCEPT_AGNAME
	sizeof(STONG_ACCEPT_SEX_AGNAME_COMMAND),		// enumC2S_TONG_ACCEPT_SEX_AGNAME
	sizeof(STONG_CHANGE_CAMP_COMMAND),			// enumC2S_TONG_CHANGE_CAMP
	sizeof(STONG_CHANGE_TONG_INFO_COMMAND),		// enumC2S_CHANGE_TONG_LEVEL
	sizeof(STONG_CHANGE_TONG_INFO_COMMAND),		// enumC2S_CHANGE_TONG_MONEY
	sizeof(STONG_CHANGE_TONG_INFO_COMMAND),		// enumC2S_CHANGE_TONG_EFF
	sizeof(STONG_CHANGE_TONG_INFO_COMMAND),		// enumC2S_CHANGE_TONG_RECRUIT
	sizeof(STONG_CHANGE_TONG_INFO_COMMAND),		// enumC2S_TONG_CHANGE_JIYU
	sizeof(STONG_CHANGE_TONG_INFO_COMMAND),		// enumC2S_TONG_CHANGE_TONGPARAM
	sizeof(STONG_CHANGE_TONG_MEMBEREFF_COMMAND),// enumC2S_CHANGE_TONG_MEMBEREFF
	sizeof(STONG_MESSAGE_INFO_COMMAND),			// enumC2S_MESSAGE_TO_TONG
	sizeof(STONG_MESSAGE_INFO_COMMAND),			// enumC2S_MESSAGE_TO_FACTION
	sizeof(STONG_MESSAGE_INFO_COMMAND),			// enumC2S_MESSAGE_TO_CHATROOM
};
...
CTongConnect::CTongConnect(CTongServer* pTongServer, ULONG id)
...
void CTongConnect::RecvPackage(const void* pData, UINT size)
{
...
BOOL CTongConnect::PassToSomeone(DWORD ip, ULONG id, DWORD nameid, const void* pData, UINT size)
{
	...
	UINT pckgsize = sizeof(EXTEND_PASSTOSOMEONE) + size;
	EXTEND_PASSTOSOMEONE* pEps = (EXTEND_PASSTOSOMEONE*)_malloca(pckgsize);
	if (pEps == NULL)
		return FALSE;
...
//--------------------------------------------------------------------
void CTongConnect::Proc0_Tong(const void* pData, UINT size)
{
	if (!pData)
		return;
	...
	case enumC2S_TONG_CREATE:
	{
		...
		INT		nSex = pTongCreate->m_btSex;
		INT		nJoinTm = pTongCreate->m_nMasterJoinTm;
		BYTE	btLevel = pTongCreate->m_btLevel;
	...
	case enumC2S_TONG_ADD_MEMBER:
	{
		...
		if (nRet >= 0)
		{
			...
			sSync.m_btCamp = (BYTE)g_cTongSet.GetTongCamp(nRet);
			sSync.m_dwMoney = g_cTongSet.GetTongMoney(nRet);
			sSync.m_btLevel = (BYTE)g_cTongSet.GetTongLevel(nRet);
			sSync.m_dwTotalEff = g_cTongSet.GetTongEff(nRet);
			sSync.m_bRecruit = g_cTongSet.GetTongRecruit(nRet);
			sSync.m_nMemberNum = g_cTongSet.GetTongMemberNum(nRet);
			sSync.m_nTongParam = g_cTongSet.GetTongParam(nRet);
			sSync.m_nTongJiyuParam = g_cTongSet.GetTongJiyuParam(nRet);
			g_cTongSet.GetMasterName(nRet, sSync.m_szMasterName);
			g_cTongSet.GetMemberAgname(nRet, sSync.m_szAgname, pAdd->m_nSex);
	...
	{
		STONG_MONEY_COMMAND* pMoney = (STONG_MONEY_COMMAND*)pData;
		STONG_MONEY_SYNC	sMoney;
		g_cTongSet.ChangeMoney(pMoney, &sMoney);
		if (sMoney.ProtocolID != 0)
			this->SendPackage((const void*)&sMoney, sizeof(sMoney));
	}
	break;
	...
	break;
	case enumC2S_TONG_CHANGE_AGNAME:
	{
		STONG_CHANGE_AGNAME_COMMAND* pChange = (STONG_CHANGE_AGNAME_COMMAND*)pData;
		char	szName[32];
		STONG_ONE_LEADER_INFO	sInfo;
		memcpy(szName, pChange->m_szName, sizeof(pChange->m_szName));
		szName[31] = 0;
		...
		char	szAgname[32];
		memcpy(szAgname, pChange->m_szAgname, sizeof(pChange->m_szAgname));
		szAgname[31] = 0;
		...
		{ {
				...
				if (g_HostServer.FindPlayerByRole(NULL, std::_tstring(szName), &conndup, NULL, &nameid, &param))
				{
					...
					if (tongconndup.IsValid())
					{
						STONG_CHECK_GET_AGNAME_POWER_SYNC	sCheck;
						sCheck.ProtocolFamily = pf_tong;
						sCheck.ProtocolID = enumS2C_TONG_CHECK_CHANGE_AGNAME_POWER;
						sCheck.m_dwTongNameID = pChange->m_dwTongNameID;
						sCheck.m_btFigure = pChange->m_btFigure;
						sCheck.m_btPos = pChange->m_btPos;
						sCheck.m_dwParam = param;
						memcpy(sCheck.m_szName, szName, sizeof(szName));
						memcpy(sCheck.m_szAgname, szAgname, sizeof(szAgname));
						tongconndup.SendPackage((const void*)&sCheck, sizeof(sCheck));
					}
					break;
				}
	...
	case enumC2S_TONG_ACCEPT_AGNAME:
	{
		STONG_ACCEPT_AGNAME_COMMAND* pAccept = (STONG_ACCEPT_AGNAME_COMMAND*)pData;
		g_cTongSet.AcceptAgname(pAccept);
	}
	break;
	case enumC2S_TONG_ACCEPT_SEX_AGNAME:
	{
		STONG_ACCEPT_SEX_AGNAME_COMMAND* pAccept = (STONG_ACCEPT_SEX_AGNAME_COMMAND*)pData;
		g_cTongSet.AcceptSexAgname(pAccept);
	}
	break;
	case enumC2S_TONG_CHANGE_CAMP:
	{
		STONG_CHANGE_CAMP_COMMAND* pInfo = (STONG_CHANGE_CAMP_COMMAND*)pData;
		STONG_BE_CHANGED_CAMP_SYNC	sInfo;
		g_cTongSet.ChangeCamp(pInfo, &sInfo);
		if (sInfo.ProtocolID != 0)
			this->SendPackage((const void*)&sInfo, sizeof(sInfo));
	}
	break;
	case enumC2S_CHANGE_TONG_LEVEL:
	{
		STONG_CHANGE_TONG_INFO_COMMAND* pInfo = (STONG_CHANGE_TONG_INFO_COMMAND*)pData;

		STONG_CHANGE_TONG_INFO_SYNC	sInfo;
		g_cTongSet.ChangeTongLevel(pInfo, &sInfo);
		if (sInfo.ProtocolID != 0)
			this->SendPackage((const void*)&sInfo, sizeof(sInfo));
	}
	break;
	case enumC2S_CHANGE_TONG_MONEY:
	{
		STONG_CHANGE_TONG_INFO_COMMAND* pInfo = (STONG_CHANGE_TONG_INFO_COMMAND*)pData;

		STONG_CHANGE_TONG_INFO_SYNC	sInfo;
		g_cTongSet.ChangeTongMoney(pInfo, &sInfo);
		if (sInfo.ProtocolID != 0)
			this->SendPackage((const void*)&sInfo, sizeof(sInfo));
	}
	break;
	case enumC2S_CHANGE_TONG_EFF:
	{
		STONG_CHANGE_TONG_INFO_COMMAND* pInfo = (STONG_CHANGE_TONG_INFO_COMMAND*)pData;

		STONG_CHANGE_TONG_INFO_SYNC	sInfo;
		g_cTongSet.ChangeTongEff(pInfo, &sInfo);
		if (sInfo.ProtocolID != 0)
			this->SendPackage((const void*)&sInfo, sizeof(sInfo));
	}
	break;
	case enumC2S_CHANGE_TONG_RECRUIT:
	{
		STONG_CHANGE_TONG_INFO_COMMAND* pInfo = (STONG_CHANGE_TONG_INFO_COMMAND*)pData;

		STONG_CHANGE_TONG_INFO_SYNC	sInfo;
		g_cTongSet.ChangeTongRecruit(pInfo, &sInfo);
		if (sInfo.ProtocolID != 0)
			this->SendPackage((const void*)&sInfo, sizeof(sInfo));
	}
	break;
	case enumC2S_TONG_CHANGE_TONGPARAM:
	{
		STONG_CHANGE_TONG_INFO_COMMAND* pInfo = (STONG_CHANGE_TONG_INFO_COMMAND*)pData;

		STONG_CHANGE_TONG_INFO_SYNC	sInfo;
		g_cTongSet.ChangeTongParam(pInfo, &sInfo);
		if (sInfo.ProtocolID != 0)
			this->SendPackage((const void*)&sInfo, sizeof(sInfo));
	}
	break;
	case enumC2S_TONG_CHANGE_JIYU:
	{
		STONG_CHANGE_TONG_INFO_COMMAND* pInfo = (STONG_CHANGE_TONG_INFO_COMMAND*)pData;

		STONG_CHANGE_TONG_INFO_SYNC	sInfo;
		g_cTongSet.ChangeJiyu(pInfo, &sInfo);
		if (sInfo.ProtocolID != 0)
			this->SendPackage((const void*)&sInfo, sizeof(sInfo));
	}
	break;
	...
	{
		STONG_CHANGE_TONG_MEMBEREFF_COMMAND* pInfo = (STONG_CHANGE_TONG_MEMBEREFF_COMMAND*)pData;
		STONG_CHANGE_TONG_MEMBEREFF_SYNC sInfo;
		g_cTongSet.ChangeMemberEff(pInfo, &sInfo);
		if (sInfo.ProtocolID != 0)
			this->SendPackage((const void*)&sInfo, sizeof(sInfo));
	}
	break;
	case enumC2S_MESSAGE_TO_TONG:
	{
		STONG_MESSAGE_INFO_COMMAND* pInfo = (STONG_MESSAGE_INFO_COMMAND*)pData;

		g_cTongSet.MessageToTong(pInfo);
	}
	break;
	case enumC2S_MESSAGE_TO_FACTION:
	{
		STONG_MESSAGE_INFO_COMMAND* pInfo = (STONG_MESSAGE_INFO_COMMAND*)pData;

		g_cTongSet.MessageToFaction(pInfo);
	}
	break;
	case enumC2S_MESSAGE_TO_CHATROOM:
	{
		STONG_MESSAGE_INFO_COMMAND* pInfo = (STONG_MESSAGE_INFO_COMMAND*)pData;

		g_cTongSet.MessageToChatRoom(pInfo);
	}
	break;
	default:
		break;
...
void CTongConnect::Proc0_Friend(const void* pData, UINT size)
{
	EXTEND_HEADER* pHeader = (EXTEND_HEADER*)pData;
...
void CTongConnect::Proc1_Friend_AskAddFriend(const void* pData, UINT size)
{
...
void CTongConnect::Proc1_Friend_RepAddFriend(const void* pData, UINT size)
{
...
void CTongConnect::Proc1_Friend_GroupFriend(const void* pData, UINT size)
{
...
void CTongConnect::Proc1_Friend_EraseFriend(const void* pData, UINT size)
{
	...
	if (!g_FriendMgr.PlayerIsFriend(srcrole, pEf->friendrole))
		return;
	...
	{ {
			...
			DWORD dstnameid = 0;
			ULONG dstparam = 0;
			if (g_HostServer.FindPlayerByRole(NULL, pEf->friendrole, &dsthostconndup, NULL, &dstnameid, &dstparam))
			{
				REP_ERASEFRIEND_SYNC RefSync;
				RefSync.ProtocolFamily = pf_friend;
				RefSync.ProtocolID = friend_c2s_erasefriend;
				RefSync.pckgid = pEf->pckgid;
				strcpy(RefSync.srcrole, srcrole.c_str());

				PassToSomeone(dsthostconndup.GetIP(), dstparam, dstnameid, &RefSync, sizeof(REP_ERASEFRIEND_SYNC));
			}
			else
			{
				g_FriendMgr.PlayerDelFriend(std::_tstring(pEf->friendrole), srcrole);
			}
		}}
}

void CTongConnect::Proc1_Friend_AskSyncFriendList(const void* pData, UINT size)
{
...
void CTongConnect::Proc1_Friend_Associate(const void* pData, UINT size)
{
	...
	if (g_FriendMgr.PlayerIsFriend(role1, role2))
		return;
	...
	UINT basesize = sizeof(FRIEND_SYNCASSOCIATE) + group.size() + 1;
	UINT role1size = role1.size() + 1;
	UINT role2size = role2.size() + 1;
	UINT maxsize = basesize + (pFa->bidir ? max(role1size, role2size) : role2size) + 1;

	FRIEND_SYNCASSOCIATE* pFsa = (FRIEND_SYNCASSOCIATE*)_malloca(maxsize);
	...
	{ {
			...
			{
				UINT pckgsize = basesize + role2size + 1;
	...
	if (pFa->bidir)
	{
		...
		{
			UINT pckgsize2 = basesize + role1size + 1;
...
void CTongConnect::Proc1_Friend_AssociateBevy(const void* pData, UINT size)
{
	...
	{ {
			UINT basesize = sizeof(FRIEND_SYNCASSOCIATE) + group.size() + 1;
			UINT maxsize = basesize + rolessize + 1;
