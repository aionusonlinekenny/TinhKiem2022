...
void CRootClient::RecvPackage(const void* pData, UINT size)
{
...
void CRootClient::Proc0_Normal(const void* pData, UINT size)
{
	EXTEND_HEADER* pHeader = (EXTEND_HEADER*)pData;
...
void CRootClient::Proc0_Relay(const void* pData, UINT size)
{
	EXTEND_HEADER* pHeader = (EXTEND_HEADER*)pData;
...
// [wxb 2003-7-19]
void CRootClient::Proc2_Relay_Data_Here(const void* pData, UINT size)
{
	...
	case (MAKEWORD(pf_gamemaster, gm_c2s_execute)):
	{
		...
		szScriptAction[min(MAX_PATH - 1, pGMData->wLength)] = 0;
	...
	case (MAKEWORD(pf_gamemaster, gm_c2s_getrolelist)):
		_ASSERT(pRelayData->routeDateLength == sizeof(GM_GET_ROLE_LIST_CMD));
		{
			...
			TProcessData* pNewData = (TProcessData*)_malloca(nNewDataLen);
			if (pNewData != NULL)
			{
	...
	{ {
			...
			UINT pckgsize = sizeof(CHAT_EVERYONE) + pGMData->wSentenceLen;

			CHAT_EVERYONE* pCeo = (CHAT_EVERYONE*)_malloca(pckgsize);
			if (pCeo != NULL)
			{
	...
	{
		char* pDBAskBuffer = (char*)_malloca(pRelayData->routeDateLength + 1);
		if (pDBAskBuffer + 1 > 0)
		{
...
void CRootClient::Proc1_Relay_Data(const void* pData, UINT size)
{
	RELAY_DATA* pRelayData = (RELAY_DATA*)pData;
...
void CRootClient::Proc1_Relay_AskWay(const void* pData, UINT size)
{
...
void CRootClient::Proc1_Relay_LoseWay(const void* pData, UINT size)
{
...
void CRootClient::Proc2_Relay_AskWay_AccountRoleID(const void* pData, UINT size, BOOL acc)
{
	RELAY_ASKWAY_DATA* pAskWayData = (RELAY_ASKWAY_DATA*)pData;
...
void CRootClient::Proc0_PlayerCommunity(const void* pData, UINT size)
{
	EXTEND_HEADER* pHeader = (EXTEND_HEADER*)pData;
