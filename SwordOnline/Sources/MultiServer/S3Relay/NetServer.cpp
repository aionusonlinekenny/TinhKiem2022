...
BOOL CNetServer::Startup (UINT nPlayerMaxCount, UINT nPrecision, UINT maxFreeBuffers_Cache, UINT bufferSize_Cache,
						  ULONG ulnAddressToListenOn, unsigned short usnPortToListenOn)
{
	if (m_pServer)
		return FALSE;
...
BOOL CNetServer::Disconnect(ULONG id)
{
	if (!m_pServer)
		return FALSE;
...
void CNetServer::_NotifyClientConnectCreate(ULONG ulnID)
{
	AUTOLOCKWRITE(m_lockAccess);
...
void CNetServer::_NotifyClientConnectClose(ULONG ulnID)
{
	AUTOLOCKWRITE(m_lockAccess);
...
UINT CNetServer::GetConnectCount() 
{
...
BOOL CNetServer::IsConnectReady(ULONG id)
{
	AUTOLOCKREAD(m_lockAccess);
...
CNetConnectDup CNetServer::FindNetConnect(ULONG id)
{
	AUTOLOCKREAD(m_lockAccess);
...
BOOL CNetServer::BroadPackage(const void* pData, UINT size)
{
