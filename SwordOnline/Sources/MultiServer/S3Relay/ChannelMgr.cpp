...
static const UINT MAX_SENTLEN = 256;
...
CChannelMgr::CChannelMgr()
	...
	m_chChannTEAM(0), m_chChannFAC(0), m_chChannTONG(0), m_chChannMSGR(0), m_chChannCR(0), m_chChannSCRN(0),
	m_minTeamID(0), m_maxTeamID(-1), m_minFacID(0), m_maxFacID(-1), m_minTongID(0), m_maxTongID(-1), m_minMsgrID(0), m_minCrID(0), m_maxMsgrID(-1),
	m_defCost(0),
	m_costTeam(0), m_costFac(0), m_costTong(0), m_costScrn(0), m_costBc(0), m_costMsgr(0), m_costCr(0)
{
...
BOOL CChannelMgr::Initialize()
{
	...
	m_chChannCR = gGetPrivateProfileCharEx(sec_cr, key_escSpec, file_channcfg, defCHANN_CR);
	...
	m_minCrID = gGetPrivateProfileIntEx(sec_cr, key_minID, file_channcfg, 0);
	m_maxCrID = gGetPrivateProfileIntEx(sec_cr, key_maxID, file_channcfg, -1);
	...
	m_costCr = (BYTE)gGetPrivateProfileIntEx(sec_cr, key_cost, file_channcfg, (INT)(UINT)m_defCost);
...
BOOL CChannelMgr::IsSubscribed(DWORD ip, ULONG param, DWORD channid)
{
	DUMMY_AUTOLOCKREAD(m_lockChannel);
...
BOOL CChannelMgr::GmQueryChannelID(const std::_tstring& channel, BOOL force)
{
	...
	{
		{
			UINT pckgsize = sizeof(RELAY_ASKWAY_DATA) + sizeof(PLAYERCOMM_NOTIFYCHANNELID);
...
BOOL CChannelMgr::IsUsed(DWORD ip, ULONG param, DWORD channid)
{
	DUMMY_AUTOLOCKREAD(m_lockChannel);
...
BOOL CChannelMgr::PassToSpecMan(DWORD ToIP, ULONG ToRelayID, DWORD ToNameID, const void* pData, UINT size)
{
	CNetConnectDup conndup = g_ChatServer.FindChatConnectByIP(ToIP);
	if (!conndup.IsValid())
		return FALSE;

	UINT pckgsize = sizeof(CHAT_SPECMAN) + size;
...
BOOL CChannelMgr::PassToGroupMan(DWORD ToIP, BOOL verify, const MANLIST& mans, const void* pData, UINT size)
{
	if (mans.empty())
		return TRUE;

	const UINT playerlen = verify ? sizeof(tagPlusSrcInfo) : sizeof(WORD);
	UINT basesize = sizeof(CHAT_GROUPMAN) + size;
...
BOOL CChannelMgr::PassToGM(const void* pData, UINT size)
{
	UINT pckgsize = sizeof(RELAY_ASKWAY_DATA) + size;

	RELAY_ASKWAY_DATA* pAskWayData = (RELAY_ASKWAY_DATA*)_malloca(pckgsize);
	pAskWayData->ProtocolFamily = pf_relay;
	pAskWayData->ProtocolID = relay_c2c_askwaydata;
	pAskWayData->nFromIP = 0;
...
BOOL CChannelMgr::IsValidCrID(DWORD idCr)
{
	if (m_minCrID > m_maxCrID)
		return TRUE;

	return idCr >= m_minCrID && idCr <= m_maxCrID;
}
...
BOOL CChannelMgr::SomeoneChat(DWORD srcIP, const tagPlusSrcInfo& SrcInfo, const CHAT_SOMEONECHAT_CMD* pSomeoneChatCmd)
{
	...
	{
		UINT syncsize = sizeof(CHAT_SOMEONECHAT_SYNC) + pSomeoneChatCmd->sentlen;
		CHAT_SOMEONECHAT_SYNC* pCscSync = (CHAT_SOMEONECHAT_SYNC*)_malloca(syncsize);
		if (pCscSync != NULL)
		{
			...
			strcpy(pCscSync->item, pSomeoneChatCmd->item);
	...
	{
		{
			UINT _exsize = sizeof(DWORD) + _NAME_LEN + sizeof(BYTE) + pSomeoneChatCmd->sentlen;	////X
			UINT feedbacksize = sizeof(CHAT_FEEDBACK) + _exsize;
			CHAT_FEEDBACK* pCfb = (CHAT_FEEDBACK*)_malloca(feedbacksize);
			if (pCfb != NULL)
			{
				pCfb->ProtocolType = chat_feedback;
				pCfb->packageID = pSomeoneChatCmd->packageID;
				pCfb->code = sent ? codeSucc : codeFail;
				strcpy(pCfb->item, pSomeoneChatCmd->item);
				pCfb->someflag = 0;
...
BOOL CChannelMgr::ChannelChat(DWORD srcIP, const tagPlusSrcInfo& SrcInfo, const CHAT_CHANNELCHAT_CMD* pChannelChatCmd)
{
	...
	if (rChannInfo.god)
	{
		...
		{
			UINT pckgsize = sizeof(CHAT_MSG_EX) + pChannelChatCmd->sentlen;
			CHAT_MSG_EX* pChatMsgEx = (CHAT_MSG_EX*)malloc(pckgsize);
			if (pChatMsgEx != NULL)
			{
	...
	else
	{
		...
		{//relegate to gamesvr
			...
			UINT syncsize = sizeof(CHAT_CHANNELCHAT_SYNC) + pChannelChatCmd->sentlen;
			UINT pckgsize = sizeof(CHAT_RELEGATE) + syncsize;

			CHAT_RELEGATE* pCr = (CHAT_RELEGATE*)malloc(pckgsize);
			if (pCr != NULL)
			{
				...
				strcpy(pCccSync->item, pChannelChatCmd->item);
				pCccSync->someflag = pChannelChatCmd->someflag;
				...
				else if (rChannInfo.channname[1] == m_chChannCR)
				{
					if (theID == -1 || channIP == 0)
						return FALSE;
				...
				else if (rChannInfo.channname[1] == m_chChannSCRN || rChannInfo.channname[1] == m_chChannBC)
				{
					if (channIP == 0)
						return FALSE;
		...
		{//broad on channel
			...
			UINT syncsize = sizeof(CHAT_CHANNELCHAT_SYNC) + pChannelChatCmd->sentlen;
			CHAT_CHANNELCHAT_SYNC* pCccSync = (CHAT_CHANNELCHAT_SYNC*)malloc(syncsize);
			if (pCccSync != NULL)
			{
				pCccSync->ProtocolType = chat_channelchat;
				pCccSync->wSize = syncsize - 1;
	...
	if (rChannInfo.channname[1] == m_chChannSCRN)
	{
		UINT _exsize = sizeof(DWORD) + sizeof(BYTE) + pChannelChatCmd->sentlen;	////X
		UINT feedbacksize = sizeof(CHAT_FEEDBACK) + _exsize;
		CHAT_FEEDBACK* pCfb = (CHAT_FEEDBACK*)malloc(feedbacksize);
		if (pCfb != NULL)
		{
			pCfb->ProtocolType = chat_feedback;
...
BOOL CChannelMgr::QueryChannelID(const std::_tstring& channel, DWORD ip, ULONG param, DWORD nameid)
{
	assert(FALSE);
	return FALSE;
...
BOOL CChannelMgr::Subscribe(DWORD ip, ULONG param, DWORD channelid)
{
	assert(FALSE);
	return FALSE;
...
BOOL CChannelMgr::Unsubscribe(DWORD ip, ULONG param, DWORD channelid)
{
	assert(FALSE);
	return FALSE;
...
BOOL CChannelMgr::FreeChannID(DWORD channid, DWORD ip, ULONG param)
{
	assert(FALSE);
	return FALSE;
...
BOOL CChannelMgr::ClearPlayer(DWORD ip, ULONG param)
{
	assert(FALSE);
	return FALSE;
...
BOOL CChannelMgr::Block_QueryChannelID(const std::_tstring& channel, DWORD ip, ULONG param, DWORD nameid)
{
...

	{
		{
			//notify channelid
			UINT methodsize = _NAME_LEN + sizeof(DWORD) * 2;
			UINT pckgsize = sizeof(RELAY_ASKWAY_DATA) + methodsize + sizeof(PLAYERCOMM_NOTIFYCHANNELID);
...
BOOL CChannelMgr::Block_Subscribe(DWORD ip, ULONG param, DWORD channelid)
{
	if (channelid == -1)
		return FALSE;
...
BOOL CChannelMgr::Block_Unsubscribe(DWORD ip, ULONG param, DWORD channelid)
{
	if (channelid == m_channidGM)
		return FALSE;
...
BOOL CChannelMgr::Block_FreeChannID(DWORD channid, DWORD ip, ULONG param)
{
	if (channid == -1)
		return FALSE;
...
BOOL CChannelMgr::Block_ClearPlayer(DWORD ip, ULONG param)
{
	DUMMY_AUTOLOCKWRITE(m_lockChannel);
...
BOOL CChannelMgr::B_QueryChannelID(const std::_tstring& channel, DWORD ip, ULONG param, DWORD nameid)
{
	return Block_QueryChannelID(channel, ip, param, nameid);
}

BOOL CChannelMgr::B_Subscribe(DWORD ip, ULONG param, DWORD channelid)
{
	return Block_Subscribe(ip, param, channelid);
}

BOOL CChannelMgr::B_Unsubscribe(DWORD ip, ULONG param, DWORD channelid)
{
	return Block_Unsubscribe(ip, param, channelid);
}

BOOL CChannelMgr::B_FreeChannID(DWORD channid, DWORD ip, ULONG param)
{
	return Block_FreeChannID(channid, ip, param);
}

BOOL CChannelMgr::B_ClearPlayer(DWORD ip, ULONG param)
{
	return Block_ClearPlayer(ip, param);
}


UINT CChannelMgr::GetChannelCount()
{
	DUMMY_AUTOLOCKWRITE(m_lockChannel);
...
BOOL CChannelMgr::SayOnSomeone(DWORD ip, ULONG param, DWORD nameid, const std::_tstring& name, const std::_tstring& sent)
{
	CNetConnectDup hostconndup;
	if (!g_HostServer.FindPlayerByIpParam(NULL, ip, param, &hostconndup, NULL, NULL, NULL))
		return FALSE;

	UINT syncsize = sizeof(CHAT_SOMEONECHAT_SYNC) + sent.size();
	CHAT_SOMEONECHAT_SYNC* pCscSync = (CHAT_SOMEONECHAT_SYNC*)_malloca(syncsize);
	pCscSync->ProtocolType = chat_someonechat;
	pCscSync->wSize = syncsize - 1;
...
BOOL CChannelMgr::SayOnChannel(DWORD channid, BOOL filter, const std::_tstring& ids, const std::_tstring& name, const std::_tstring& sent)
{
	...
	if (rChannInfo.god)
	{
		...
		UINT pckgsize = sizeof(CHAT_MSG_EX) + sent.size();
		CHAT_MSG_EX* pChatMsgEx = (CHAT_MSG_EX*)_malloca(pckgsize);
		pChatMsgEx->ProtocolFamily = pf_playercommunity;
		pChatMsgEx->ProtocolID = playercomm_channelchat;
	...
	else
	{
		...
		{//relegate to gamesvr
			...
			UINT syncsize = sizeof(CHAT_CHANNELCHAT_SYNC) + sent.size();
			UINT pckgsize = sizeof(CHAT_RELEGATE) + syncsize;

			CHAT_RELEGATE* pCr = (CHAT_RELEGATE*)_malloca(pckgsize);
			pCr->ProtocolType = chat_relegate;
			pCr->wSize = pckgsize - 1;
			pCr->nFromIP = 0;
			pCr->nFromRelayID = -1;
		...
		{//broad on channel
			...
			UINT syncsize = sizeof(CHAT_CHANNELCHAT_SYNC) + sent.size();
			CHAT_CHANNELCHAT_SYNC* pCccSync = (CHAT_CHANNELCHAT_SYNC*)_malloca(syncsize);
			pCccSync->ProtocolType = chat_channelchat;
			pCccSync->wSize = syncsize - 1;
			pCccSync->packageID = -1;
			strcpy(pCccSync->someone, name.c_str());
