...
CNetConnect* CHostServer::CreateConnect(CNetServer* pNetServer, ULONG id)
{
...
BOOL CHostServer::FindPlayerByAcc(CHostConnect* pConn, const std::_tstring& acc, CNetConnectDup* pConnDup, std::_tstring* pRole, DWORD* pNameID, ULONG* pParam, std::_tstring* pRealAcc)
{
	AUTOLOCKREAD(m_lockIpMap);
...
BOOL CHostServer::FindPlayerByRole(CHostConnect* pConn, const std::_tstring& role, CNetConnectDup* pConnDup, std::_tstring* pAcc, DWORD* pNameID, ULONG* pParam, std::_tstring* pRealRole)
{
	AUTOLOCKREAD(m_lockIpMap);
...
BOOL CHostServer::FindPlayerByIpParam(CHostConnect* pConn, DWORD ip, ULONG param, CNetConnectDup* pConnDup, std::_tstring* pAcc, std::_tstring* pRole, DWORD* pNameID)
{
	AUTOLOCKREAD(m_lockIpMap);
...
/*
BOOL CHostServer::BroadOnPlayer(BOOL byAcc, DWORD fromIP, ULONG fromRelayID, const void* pData, UINT size)
{
	AUTOLOCKREAD(m_lockIpMap);
...
UINT CHostServer::GetPlayerCount()
{
	AUTOLOCKREAD(m_lockIpMap);
...
BOOL CHostServer::TracePlayer()
{
	...
	for (IP2CONNECTMAP::iterator it = m_mapIp2Connect.begin(); it != m_mapIp2Connect.end(); it++)
	{
		...
		if (pConn)
		{
			...
			UINT count = pConn->GetPlayerCount();
			total += count;
