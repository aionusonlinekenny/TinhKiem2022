...
public:
	BOOL Startup(UINT maxFreeBuffers, UINT bufferSize, const char * pAddressToConnectServer, unsigned short usPortToConnectServer, BOOL block = TRUE);
	BOOL Shutdown();
...
public:
	BOOL SendPackage(const void* pData, UINT size);
...
public:
	...
	DWORD GetSvrIP() const {return m_SvrIP;}
	BOOL SendPackage(const void* pData, UINT size) const;
