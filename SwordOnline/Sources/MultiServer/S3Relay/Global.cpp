...
BOOL gInitialize()
{
	...
	const UINT playercntRelay = gGetPrivateProfileIntEx(sec_relay, key_playercnt, file_relaycfg, 0);
	const UINT precisionRelay = gGetPrivateProfileIntEx(sec_relay, key_precision, file_relaycfg, 0);
	const UINT freebufferRelay = gGetPrivateProfileIntEx(sec_relay, key_freebuffer, file_relaycfg, 0);
	const UINT buffersizeRelay = gGetPrivateProfileIntEx(sec_relay, key_buffersize, file_relaycfg, 0);
	...
	const UINT freebufferRoot = gGetPrivateProfileIntEx(sec_root, key_freebuffer, file_relaycfg, 0);
	const UINT buffersizeRoot = gGetPrivateProfileIntEx(sec_root, key_buffersize, file_relaycfg, 0);
	...
	const UINT freebufferDB = gGetPrivateProfileIntEx(sec_dbrole, key_freebuffer, file_relaycfg, 0);
	const UINT buffersizeDB = gGetPrivateProfileIntEx(sec_dbrole, key_buffersize, file_relaycfg, 0);
	...
	const UINT freebufferGateway = gGetPrivateProfileIntEx(sec_gateway, key_freebuffer, file_relaycfg, 0);
	const UINT buffersizeGateway = gGetPrivateProfileIntEx(sec_gateway, key_buffersize, file_relaycfg, 0);
	...
	const UINT playercntHost = gGetPrivateProfileIntEx(sec_host, key_playercnt, file_relaycfg, 0);
	const UINT precisionHost = gGetPrivateProfileIntEx(sec_host, key_precision, file_relaycfg, 0);
	const UINT freebufferHost = gGetPrivateProfileIntEx(sec_host, key_freebuffer, file_relaycfg, 0);
	const UINT buffersizeHost = gGetPrivateProfileIntEx(sec_host, key_buffersize, file_relaycfg, 0);
	...
	const UINT playercntTong = gGetPrivateProfileIntEx(sec_tong, key_playercnt, file_relaycfg, 0);
	const UINT precisionTong = gGetPrivateProfileIntEx(sec_tong, key_precision, file_relaycfg, 0);
	const UINT freebufferTong = gGetPrivateProfileIntEx(sec_tong, key_freebuffer, file_relaycfg, 0);
	const UINT buffersizeTong = gGetPrivateProfileIntEx(sec_tong, key_buffersize, file_relaycfg, 0);
	...
	const UINT playercntChat = gGetPrivateProfileIntEx(sec_chat, key_playercnt, file_relaycfg, 0);
	const UINT precisionChat = gGetPrivateProfileIntEx(sec_chat, key_precision, file_relaycfg, 0);
	const UINT freebufferChat = gGetPrivateProfileIntEx(sec_chat, key_freebuffer, file_relaycfg, 0);
	const UINT buffersizeChat = gGetPrivateProfileIntEx(sec_chat, key_buffersize, file_relaycfg, 0);
	...
	if (g_cTongDB.Open())
	{
		...
		{
			rTRACE("tongset init ok ! Number Tong: %d", g_cTongDB.GetTongCount());
		}
		else
...
const UCHAR* gGetHostMac(INT adapt, UINT* pSize)
{
	static struct _MACSTORE
	{
		...
		_MACSTORE()
		{
			...
			gGetMacAndIPAddress(macLocal, NULL, macGlobal, NULL, _localtag, MAC_SIZE, (char*)intranet_ip.c_str(), (char*)internet_ip.c_str(), (char*)mac.c_str());
		}
	} _macstore;
...
DWORD gGetHostIP(INT adapt)
{
	static struct _IPSTORE
	{
		...
		_IPSTORE() : ipLocal(0), ipGlobal(0)
		{
			...
			gGetMacAndIPAddress(NULL, &ipLocal, NULL, &ipGlobal, _localtag, 0, (char*)intranet_ip.c_str(), (char*)internet_ip.c_str(), (char*)mac.c_str());
		}
	} _ipstore;
...
{
	const UINT freebuffer = gGetPrivateProfileIntEx(sec_relay, key_freebuffer, file_relaycfg, 0);
	const UINT buffersize = gGetPrivateProfileIntEx(sec_relay, key_buffersize, file_relaycfg, 0);
	...
	const RELAY_ASKWAY_DATA* pAskWay, UINT sizeAskWay, RELAY_ASKWAY_DATA* pClntWay, UINT sizeClntWay)
{
	...
	UINT sizeNeed = sizeAskWay + sizeof(DWORD) * 2;
	...
	const void* pLosePckg, UINT sizeLosePckg, void* pLoseway, UINT sizeLoseway)
{
	...
	UINT sizeNeed = sizeLosePckg + sizeof(RELAY_DATA);
...
BOOL gIsLegalString(LPCTSTR sz, UINT lenlolmt, UINT lenhilmt)
{
	if (sz == NULL || lenhilmt < lenlolmt)
		return FALSE;
	for (UINT pl = 0; pl < lenlolmt; pl++)
		if (sz[pl] == 0)
			return FALSE;
	for (UINT ph = lenlolmt; ph < lenhilmt; ph++)
		if (sz[ph] == 0)
			return TRUE;
	return TRUE;
}
