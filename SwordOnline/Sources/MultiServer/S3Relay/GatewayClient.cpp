...
void CGatewayClient::RecvPackage(const void* pData, UINT size)
{
...
void CGatewayClient::OnServerEventClose()
{
	...
	for (ID2ASKWAYMAPMAP::iterator it = m_mapAskWayMaps.begin(); it != m_mapAskWayMaps.end(); it++)
	{
		...
		UINT sizePckg = sizeof(RELAY_ASKWAY_DATA) + rAskWayMap.pAskWayMap->wMethodDataLength + rAskWayMap.pAskWayMap->routeDateLength;

		UINT sizeLoseData = sizeof(RELAY_DATA) + sizePckg;
...
void CGatewayClient::Proc0_NotifySvrIP(const void* pData, UINT size)
{
...
BOOL CGatewayClient::PassAskWayMap(DWORD fromIP, const RELAY_ASKWAY_DATA* pAskWayMap)
{
	...
	UINT size = sizeof(RELAY_ASKWAY_DATA) + pAskWayMap->wMethodDataLength + pAskWayMap->routeDateLength;
