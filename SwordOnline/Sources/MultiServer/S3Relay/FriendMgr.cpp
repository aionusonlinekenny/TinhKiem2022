...
BOOL CFriendMgr::Initialize()
{
	...
	m_StorePlayerPerUdtDB = (UINT)gGetPrivateProfileIntEx(sec_database, key_storeplayerperudtdb, file_friendcfg, 0);
	m_LoadPlayerPerUdtDB = (UINT)gGetPrivateProfileIntEx(sec_database, key_loadplayerperudtdb, file_friendcfg, 0);
...
BOOL CFriendMgr::SomeoneLogin(const std::string& role)
{
	...
	if (!mapIp2Players.empty())
	{
		...
		UINT datasize2 = sizeof(FRIEND_STATE) + role.size() + 2;
		UINT basesize2 = sizeof(EXTEND_PASSTOBEVY) + datasize2;
...
BOOL CFriendMgr::SomeoneLogout(const std::string& role)
{
	...
	UINT datasize2 = sizeof(FRIEND_STATE) + role.size() + 2;
	UINT pckgsize2 = sizeof(EXTEND_PASSTOSOMEONE) + datasize2;
...
BOOL CFriendMgr::SomeoneSyncFriends(const std::string& role, ULONG param, DWORD nameid, const CNetConnectDup& conndup, BOOL full, BYTE pckgid)
{
	...
	if (!mapGrp2Friends.empty())
	{
		...
		UINT basesize = sizeof(EXTEND_PASSTOSOMEONE) + sizeof(REP_SYNCFRIENDLIST);
		...
		for (GRP2FRIENDSMAP::iterator itGrp = mapGrp2Friends.begin(); itGrp != mapGrp2Friends.end(); itGrp++)
		{
			...
			for (FRIENDLIST::iterator itFriend = rFriends.begin(); itFriend != rFriends.end(); itFriend++)
			{
				...
				//					spec	state	name						0
				UINT appendsize = 1	+	1	+	theFriend.second.size()	+	1;
...
BOOL CFriendMgr::DB_StoreSomeone(const std::_tstring& someone, const MEM_FRIENDRECORDLIST& memFriends)
{
	...
	UINT cntFriends = memFriends.size();
	...
	UINT sizeMax = sizeof(DB_FRIENDRECORD) + (sizeof(_BASIC_STR) + _NAME_LEN) * cntFriends;
	...
	UINT sizeUsed = (BYTE*)pCursor - (BYTE*)pdbFriendList;
	assert(sizeUsed > 0);
...
BOOL CFriendMgr::BkgrndUpdateDB()
{
	...
	UINT playercount = m_mapPlayers.size();
	...
	if (m_cursorBkgrndUdtDB <= playercount/2)
	{
		...
		for (UINT i = 0; i < m_cursorBkgrndUdtDB; i++)
			++ itBase;
	}
	else
	{
		itBase = m_mapPlayers.end();
		for (UINT i = playercount; i > m_cursorBkgrndUdtDB; i--)
			-- itBase;
	}
	...
	do
	{
		...
		if (m_StorePlayerPerUdtDB != UINT(-1)
		...
		else
		{
			...
			if (m_LoadPlayerPerUdtDB != UINT(-1)
...
UINT CFriendMgr::GetPlayerCount()
{
	AUTOLOCKREAD(m_lockFriend);
...
BOOL CFriendMgr::PlayerAssociateBevy(const _BEVY& bevy, const std::_tstring& group)
{
	...
	const UINT count = vecPlayer.size();
	const UINT last = count - 1;

	for (UINT idxSrc = 0; idxSrc < last; idxSrc++)
	{
		...
		for (UINT idxDst = idxSrc + 1; idxDst < count; idxDst++)
		{
