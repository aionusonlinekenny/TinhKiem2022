...
ZDBTable::ZDBTable(const char* path, const char* name) {
...
#else
#endif
	strcat_s(env_path, "\\");  //VS 2010 不需要加双斜杠
	strcat_s(env_path, path);
	...
	if (!db_env_create(&dbenv, 0)) {
		...
		if (!dbenv->open(dbenv, env_path, DB_CREATE | DB_INIT_LOG | DB_INIT_MPOOL | DB_INIT_TXN | DB_RECOVER, 0)) { //| DB_INIT_LOCK
			ret = dbenv->set_flags(dbenv, DB_AUTO_COMMIT | DB_CDB_ALLDB | DB_TXN_NOSYNC, 1); //DB_TXN_NOSYNC
			index_number = 0;
			...
			for (index = 0; index < index_number; index++) {
				...
				if (!db_create(&index_db[index], dbenv, 0)) {
					if (!is_index_unique[index]) {
						if (index_db[index]->set_flags(index_db[index], DB_DUP | DB_DUPSORT))
							break;
					}
					if (index_db[index]->open(index_db[index], NULL, index_table_name, NULL, DB_BTREE, DB_CREATE | DB_AUTO_COMMIT, 0664))
						break;
...
void ZDBTable::close() {
	...
	if (primary_db != NULL)
		primary_db->close(primary_db, 0);
	for (INT index = 0; index < index_number; index++)
		index_db[index]->close(index_db[index], 0);
}
...
bool ZDBTable::GetRecordEx(char* aBuffer, INT& size, char* aKeyBuffer, INT& keysize, CursorPointer cpMode, INT index)
{//取得按游标某一个数据(新版函数)
	if (!dbcp)
	{//如果数据库指针没有初始化，先初始化dbcp
		if (index_db[index]->cursor(index_db[index], NULL, &dbcp, 0))
