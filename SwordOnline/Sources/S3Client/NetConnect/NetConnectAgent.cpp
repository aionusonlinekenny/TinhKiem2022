...
void __stdcall ClientCallBack(LPVOID lpParam, const ULONG& ulnEventType);
...
//	功能：建立连接
//--------------------------------------------------------------------------
INT	KNetConnectAgent::ClientConnectByNumericIp(const unsigned char* pIpAddress, uint16_t nPort)
{
	DisconnectClient();
...
INT KNetConnectAgent::ConnectToGameSvr(const unsigned char* pIpAddress, uint16_t uPort, GUID* pGuid)
{
	g_DebugLog("KNetConnectAgent::ConnectToGameSvr");
	//Disconnect Bishop
	DisconnectGameSvr();
	if (pGuid == NULL)
		return false;
			...
			while (pMsg < (PROTOCOL_MSG_TYPE*)(pBuffer + nSize))
			{
				...
				else if (g_pCoreShell)
				{
					...
					if (g_pCoreShell->GetProtocolSize(Msg) > 0)
						...
						pMsg = (PROTOCOL_MSG_TYPE*)(((char*)pMsg) + PROTOCOL_MSG_SIZE + (*(uint16_t*)(((char*)pMsg) + PROTOCOL_MSG_SIZE)));
				}
...
void KNetConnectAgent::UpdateClientRequestTime(bool bCancel, UINT uTimeLimit)
{
...
{
	g_DebugLog("KNetConnectAgent::ProcessSwitchGameSvrMsg");
