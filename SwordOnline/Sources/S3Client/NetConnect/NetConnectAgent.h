...
public:
	...
	//建立连接
	INT		ClientConnectByNumericIp(const unsigned char* pIpAddress, uint16_t pszPort);
	//关闭连接
	void	DisconnectClient();

	INT		ConnectToGameSvr(const unsigned char* pIpAddress, uint16_t uPort, GUID* pGuid);
	...
	void	UpdateClientRequestTime(bool bCancel, UINT uTimeLimit = DEF_TIMEOUT_LIMIT);
...
#define	MAX_MSG_COUNT	1 << (PROTOCOL_MSG_SIZE * 8)
	...
	UINT			m_uClientRequestTime;		//发出情求的时间
	UINT			m_uClientTimeoutLimit;
};
