...
INT	UiInit()
{
	...
	Player_SectSort::RegisterSelfClass();
...
void UiPaint(INT nGameLoop)
{
	...
	sprintf(Info, "FRAME=%d|PING=%d", s_nFrameRate, dwPing);
	g_pRepresentShell->OutputText(14, Info, -1, 10, 20, 0xFFcc00, 0);
...
//	功能：处理输入
//--------------------------------------------------------------------------
void	UiProcessInput(UINT uMsg, UINT uParam, INT nParam)
{
	if (uMsg == WM_COPYDATA)
	{
		if (nParam)
...
void UiGetScreenSize(INT& nWidth, INT& nHeight)
{
