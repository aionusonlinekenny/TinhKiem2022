...
void KUiBase::SetUserAccount(char* pszId, char* pszRole)
{
	...
	{
		m_UserAccountId[0] = 0;
		UINT	uID1 = g_FileName2Id(pszId);
		UINT	uID2 = ~g_FileName2Id(pszRole);
...
void KUiBase::LoadPrivateConfig()
{
	...
	if (pConfigFile)
	{
		//----获取窗口方案设定----
		char	SchemeName[32];
		pConfigFile->GetString("Main", "Scheme", "M芻 nh", SchemeName, 32);
...
INT KUiBase::GetSchemePath(const char* pScheme)
{
	...
	for (INT i = 0; i < nCount; i++)
	{
		...
		if (pIni->GetString(THEME_SECTION, Key, "", Buff, 32))
		{
			if (strcmp(Buff, pScheme) == 0)
			{
				...
				{
					if (nResolutionW == 1024 && nResolutionH == 768)
						sprintf(m_CurSchemePath, "%s\\%s_L", UI_INFO_FILE_PATH, Buff);
					else
	...
	if (pIni->GetString(THEME_SECTION, "0", "", m_CurScheme, 32) && pIni->GetString(THEME_SECTION, "0_Path", "", Buff, 32) && Buff[0])
	{
		if (nResolutionW == 1024 && nResolutionH == 768)
			sprintf(m_CurSchemePath, "%s\\%s_L", UI_INFO_FILE_PATH, Buff);
		else
