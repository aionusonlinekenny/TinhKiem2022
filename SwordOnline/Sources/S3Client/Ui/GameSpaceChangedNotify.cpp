...
#include "UiCase/UiDaTau.h"
#include "UiCase/UiProgressBarLoading.h"

bool UiCloseWndsInGame(bool bAll);

extern iCoreShell* g_pCoreShell;

void GameWorldTips(UINT uParam, INT nParam);

//--------------------------------------------------------------------------
//	¹¦ÄÜ£º½ÓÊÜÓÎÏ·ÊÀ½çÊý¾Ý¸Ä±äÍ¨ÖªµÄº¯Êý
//--------------------------------------------------------------------------
void CoreDataChangedCallback(UINT uDataId, UINT uParam, INT nParam)
{
	...
	case GDCNI_EXIT_GAME:
		...
		break;
	case GDCNI_PLAYER_OFFLINE:
		if (g_pCoreShell)
		{
			g_pCoreShell->OperationRequest(GOI_PLAYER_OFFLINE, 0, 0);
		}
...
void KClientCallback::CoreDataChanged(UINT uDataId, UINT uParam, INT nParam)
{
...
void KClientCallback::ChannelMessageArrival(DWORD nChannelID, char* szSendName, const char* pMsgBuff, uint16_t nMsgLength, const char* pItem, BYTE btSomeFlag, bool bSucc)
{
	...
	if (KUiMsgCentrePad::GetChannelSubscribe(nIndex) && KUiMsgCentrePad::IsChannelType(nIndex, KUiMsgCentrePad::ch_Screen))
	{
...
void KClientCallback::MSNMessageArrival(char* szSourceName, char* szSendName, const char* pMsgBuff, uint16_t nMsgLength, const char* pItem, bool bSucc)
{
	if (nMsgLength >= MAX_MESSAGE_LENGTH)
		return;
