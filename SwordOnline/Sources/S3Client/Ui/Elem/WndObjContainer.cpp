...
//--------------------------------------------------------------------------
void KWndObjectBox::SetObjectGenre(UINT uGenre)
{
...
//	¹¦ÄÜ£ºÉèÖÃÈÝÄÉµÄ¶ÔÏó
//--------------------------------------------------------------------------
void KWndObjectBox::HoldObject(UINT uGenre, UINT uId, INT nDataW, INT nDataH)
{
...
void KWndObjectBox::PaintWindow()
{
	...
	if (m_Object.uGenre != CGOG_NOTHING && g_pRepresentShell && g_pCoreShell)
	{
		...
		else if (m_Style & OBJCONT_S_HAVEOBJBGCOLOR)
		{
			...
			ITEM_IN_ENVIRO_PROP eProp = (ITEM_IN_ENVIRO_PROP)g_pCoreShell->GetGameData(GDI_ITEM_IN_ENVIRO_PROP, (UINT)&Obj, 0);
			if (Obj.Obj.uGenre == CGOG_ITEM || Obj.Obj.uGenre == CGOG_NPCSELLITEM || Obj.Obj.uGenre == CGOG_PLAYERSELLITEM || Obj.Obj.uGenre == CGOG_PLAYERVIEWITEM)
			{
				...
				{
					//g_DebugLog("KWndObjectBox::PaintWindow() - %d", Obj.Obj.uId);
...
//	¹¦ÄÜ£º´°¿Úº¯Êý
//--------------------------------------------------------------------------
INT KWndObjectBox::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WM_LBUTTONDOWN:
		...
		if (m_pParentWnd)
		{
			if (m_Object.uGenre != CGOG_NOTHING)
			{
				...
				m_pParentWnd->WndProc(WND_N_RIGHT_CLICK_ITEM,
					(UINT)&m_Object, (INT)(KWndWindow*)this);
			}
			else if (m_Style & OBJCONT_S_ENABLE_CLICK_EMPTY)
			{
				m_pParentWnd->WndProc(WND_N_LEFT_CLICK_ITEM,
...
INT KWndObjectBox::DropObject(bool bTestOnly)
{
	...
	else
	{
		...
		Pick.h = 0;
		Pick.v = 0;
		m_pParentWnd->WndProc(WND_N_ITEM_PICKDROP, (UINT)&Pick, (INT)&Drop);
	}
	return true;
}
...
void KWndObjectMatrix::PaintWindow()
{
	...
	// Lock mutex trước khi thực hiện truy cập vào dữ liệu chia sẻ
	...
	for (INT i = 0; i < m_nNumObjects; i++)
	{
		...
		else if (m_Style & OBJCONT_S_HAVEOBJBGCOLOR)
		{
			...
			else
			{
				...
				{
					//Shadow.Color.Color_dw = l_BgColors[5];
					if (Obj.nContainer == UOC_NPC_SHOP)
						g_DebugLog("KWndObjectMatrix::PaintWindow(): Container[%d] - Id[%d] - Genre[%d] - eProp[%d]", Obj.nContainer, Obj.Obj.uId, g_pCoreShell->GetGenreItem(Obj.Obj.uId, Obj.Obj.uGenre), eProp);
...
//	¹¦ÄÜ£º´°¿Úº¯Êý
//--------------------------------------------------------------------------
INT KWndObjectMatrix::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WM_LBUTTONDOWN:
...
INT KWndObjectMatrix::PickUpObjectAt(INT x, INT y)
{
	...
	if (nPicked >= 0)
	{
		...
		m_pParentWnd->WndProc(WND_N_ITEM_PICKDROP, (UINT)&Pick, NULL);
		return true;
	}
	return false;
}
