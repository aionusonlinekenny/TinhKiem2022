...
UINT KWndButton::ms_uBtnTipTextColor = 0xffffffff;	//按钮提示名称文字的颜色
INT			 KWndButton::ms_nDisableBtnTip   = false;		//是否静止按钮的提示文字

//设置按钮提示名称文字的颜色
void KWndButton::SetAllButtonTipTextColor(UINT uColor)
{
...
//	功能：窗口函数（处理消息）
//--------------------------------------------------------------------------
INT KWndButton::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	if (IsDisable())
	...
	case WM_MOUSEMOVE:
		...
		{
			if (m_pParentWnd)
				m_pParentWnd->WndProc(WND_N_BUTTON_OVER, (UINT)(KWndWindow*)this, 0);
	...
	case WM_MOUSEHOVER:
		...
		{
			m_pParentWnd->WndProc(WND_N_BUTTON_HOLD, (UINT)(KWndWindow*)this, 0);
		}
		else if (ms_nDisableBtnTip == false && m_szTip[0])
		{
			char szTip[256];
	...
	else
	{
		...
		if (m_pParentWnd)
			m_pParentWnd->WndProc(WND_N_BUTTON_CLICK,
				(UINT)(KWndWindow*)this, (m_Flag & WNDBTN_F_CHECKED));
	}
}
...
void KWndButton::OnRButtonDown()
{
	OnButtonClick();
	if (m_pParentWnd)
		m_pParentWnd->WndProc(WND_N_BUTTON_LCLICK,
			(UINT)(KWndWindow*)this, (m_Flag & WNDBTN_F_CHECKED));
}
//--------------------------------------------------------------------------
//	功能：响应鼠标左键在此放开
...
void KWndButton::OnLBtnUp()
{
	...
	if (m_pPressedDownBtn == this && (m_Flag & WNDBTN_F_DOWN))
	{
		...
		OnButtonClick();
		if (m_pParentWnd)
			m_pParentWnd->WndProc(WND_N_BUTTON_CLICK, (UINT)(KWndWindow*)this, (m_Flag & WNDBTN_F_CHECKED));
	}
	m_pPressedDownBtn = NULL;
}
...
void KWndImageTextButton::Set5IntText(INT nNumber)
{
	m_Text.Set5IntText(nNumber);
}

void KWndImageTextButton::Set6IntText(INT nNumber, char Separator)
{
...
void KWndImageTextButton::SetImage(int16_t nType, const char* pszImgName, bool bAdjustWndSize)
{
	if (bPart)
		m_ImagePart.SetImage(nType, pszImgName, bAdjustWndSize);
	else
		m_Image.SetImage(nType, pszImgName, bAdjustWndSize);
}