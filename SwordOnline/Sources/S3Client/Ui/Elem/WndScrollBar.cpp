...
//	功能：窗口函数
//--------------------------------------------------------------------------
INT KWndScrollBar::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	if (IsDisable())
	...
	case WND_N_BUTTON_CLICK:
		if (m_Flag & WNDSCROLL_F_DRAGGING_SLIDE &&
			uParam == (UINT)(KWndWindow*)&m_SlideBtn)
		{
			m_Flag &= ~WNDSCROLL_F_DRAGGING_SLIDE;
			Wnd_ReleaseCapture();
		}
	...
	case WND_N_BUTTON_DOWN:
		if (uParam == (UINT)(KWndWindow*)&m_SlideBtn)
			OnSlideBtnPressed();
		break;
	default:
		return KWndImage::WndProc(uMsg, uParam, nParam);
...
void KWndScrollBar::SetScrollPos(INT nPosition)
{
	if (nPosition != m_nCurValue)
	{
		...
		if (nPosition != m_nCurValue)
		{
			...
			if (m_pParentWnd)
			{
				m_pParentWnd->WndProc(WND_N_SCORLLBAR_POS_CHANGED, (UINT)(KWndWindow*)this, m_nCurValue);
			}
		}
	}
}
