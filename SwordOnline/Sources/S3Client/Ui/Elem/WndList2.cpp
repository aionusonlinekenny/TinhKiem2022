...
UINT KWndList2::ms_uBtnTipTextColor = 0xffffffff;	//按钮提示名称文字的颜色
INT			 KWndList2::ms_nDisableBtnTip = false;		//是否静止按钮的提示文字
//设置按钮提示名称文字的颜色
void KWndList2::SetAllButtonTipTextColor(UINT uColor)
{
...
//	功能：窗口函数
//--------------------------------------------------------------------------
INT KWndList2::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
...
void KWndList2::OnLButtonDClick(INT x, INT y)
{
	...
	if (nSel != -1)
	{
		...
		m_nSelItemIndex = nSel;
		if (m_pParentWnd)
			m_pParentWnd->WndProc(WND_N_LIST_ITEM_D_CLICK, (UINT)(KWndWindow*)this, m_nSelItemIndex);
	}
}
...
//	功能：获得列表项数据中的一个整数
//--------------------------------------------------------------------------
INT	KWndList2::SetItemColor(INT nIndex, UINT Color)
{
	if (nIndex >= 0 && nIndex < m_nNumItem)
	{
...
//	功能：获得列表项数据中的一个整数
//--------------------------------------------------------------------------
UINT KWndList2::GetItemColor(INT nIndex)
{
	if (nIndex >= 0 && nIndex < m_nNumItem)
	{
...
//	功能：获得列表项的(显示)字串
//--------------------------------------------------------------------------
INT KWndList2::GetString(INT nIndex, char* pBuffer, UINT nSize)
{
	if (nIndex >= 0 && nIndex < m_nNumItem && pBuffer)
	{
		KWndList2Item* pItem = (KWndList2Item*)m_pContent[nIndex];
