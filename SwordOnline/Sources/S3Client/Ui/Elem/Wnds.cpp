...
//	功能：处理输入
//--------------------------------------------------------------------------
void Wnd_ProcessInput(UINT uMsg, UINT uParam, INT nParam)
{
				...
				if (s_WndStation.pMouseOverWnd)
				{
					...
					if (pOverParent)
					{
						pOverParent->WndProc(WND_N_MOUSE_LEAVE_CHILD,
							(UINT)s_WndStation.pMouseOverWnd, 0);
					}
				}
				if (pTopWnd)
				{
...
void Wnd_TransmitInputToGameSpace(UINT uMsg, UINT uParam, INT nParam)
{
	s_WndStation.pGameSpaceWnd->WndProc(uMsg, uParam, nParam);
}
...
void Wnd_GetCursorPos(INT x, INT y, INT* px, INT* py)
{
}
//--------------------------------------------------------------------------
//	功能：获取当前挂接的输入焦点窗口
...
void Wnd_SetFocusWnd(KWndWindow* pWnd)
{
	if (s_WndStation.pFocusWnd != pWnd)
	{
		if (s_WndStation.pFocusWnd)
			s_WndStation.pFocusWnd->WndProc(WND_M_KILL_FOCUS, (UINT)pWnd, 0);

		if (pWnd)
			pWnd->WndProc(WND_M_SET_FOCUS, (UINT)s_WndStation.pFocusWnd, 0);
