...
void KWndMessageListBox::SetItemActived(BOOL bItemActived)
{
	m_bItemActived = bItemActived;

	if (m_bItemActived)
	{
		KUiPlayerControlBar::ClearItemBtn();
		for (INT j = 0; j < m_nCapability; j++)
		{
			if (m_pMessages[j])
			{
...
// 功能	: 新添一条信息
// -------------------------------------------------------------------------
INT KWndMessageListBox::AddOneMessage(const char* pText, INT nLen, UINT uTextBKColor, INT nData/* = -1*/, KItemBtn* pIB/* = NULL*/)
{
	if (pText == NULL)
		return 0;
	if (nLen < 0)
	...
	if (m_nCapability > 0 && nLen)
	{
		...
		pMsg->nData = nData;
		if (pIB)
			memcpy(&pMsg->Item, pIB, sizeof(KItemBtn));
		else
			pMsg->Item.Release();
...
void KWndMessageListBox::PaintWindow()
{
	...
	while (nViewLines > 0 && nCurMsg < m_nNumMessage)
	{
		...
		BOOL bPaintItem = TRUE;
		if (Param.nNumLine > nViewLines)
		{
			Param.nNumLine = nViewLines;
			if (pCurMsg->Item.nLine > Param.nNumLine)
				bPaintItem = FALSE;
		}
		...
		if (bDraw)
		{
			...
			if (m_bItemActived)
			{
				if ((m_pMessages[nCurMsg]->Item.nLine) > Param.nSkipLine && bPaintItem)
					KUiPlayerControlBar::SetItemBtnPos(nCurMsg, Param.nX + (m_pMessages[nCurMsg]->Item.nLeft) * m_nFontSize / 2 + (m_pMessages[nCurMsg]->Item.nPic) * 2,
						Param.nY + ((m_pMessages[nCurMsg]->Item.nLine) - 1 - Param.nSkipLine) * m_nFontSize);
				else
	...
	if (nViewLines == 0 && nCurMsg < m_nNumMessage && m_bItemActived)
		...
		{
			if (m_pMessages[i])
				KUiPlayerControlBar::SetItemBtnPos(i, HIDE_POS);
		}
}
...
//	功能：窗口函数
//--------------------------------------------------------------------------
INT KWndMessageListBox::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WM_MOUSEMOVE:
	...
	case WM_MOUSEWHEEL:
	{
		...
		{
			INT zDelta = int16_t(HIWORD(uParam));
...
{
	if (m_nHideNumLine == m_nNumMaxShowLine)
		return -1;

	INT nSelLine = 0;

	nSelLine = (y - m_nAbsoluteTop - GetOffsetTextHeight()) / (m_nFontSize + 1 + m_nRowDis);
...
void KWndMessageListBox::OnLButtonDown(INT x, INT y)
{
	...
	if (nSel >= 0 && m_pParentWnd)
		m_pParentWnd->WndProc(WND_N_LIST_ITEM_ACTIVE, (UINT)(KWndWindow*)this, m_nSelMsgIndex);
	SetCurSel(nSel);
...
INT	KWndMessageListBox::SetCurSel(INT nIndex)
{
	...
	if (m_pParentWnd)
		m_pParentWnd->WndProc(WND_N_LIST_ITEM_SEL, (UINT)(KWndWindow*)this, m_nSelMsgIndex);
...
void KWndMessageListBox::OnMouseMove(INT x, INT y)
{
	...
	if (nSel != m_nHLMsgIndex)
	{
		...
		if (m_pParentWnd)
		{
			m_pParentWnd->WndProc(WND_N_LIST_ITEM_HIGHLIGHT, (UINT)(KWndWindow*)this, m_nHLMsgIndex);
		}
	}
}
...
INT KWndMessageListBox::GetOneMessageData(INT nIndex)
{
	if (nIndex < 0)
		nIndex = m_nNumMessage + nIndex;
	if (nIndex >= 0 && nIndex < m_nNumMessage)
	{
		return m_pMessages[nIndex]->nData;
	}
	return -1;
}
//--------------------------------------------------------------------------
...
//			否则会造成内存的泄漏。
//--------------------------------------------------------------------------
UINT KWndMessageListBox::SplitData()
{
...
//			否则会造成内存的泄漏。
//--------------------------------------------------------------------------
UINT KWndMessageListBox::BindData(UINT hData)
{
	if (hData == NULL)
		return SplitData();
...
//					BindData函数获得的返回值。
//--------------------------------------------------------------------------
void KWndMessageListBox::FreeData(UINT	hData)
{
	if (hData)
	{
...
UINT KWndMessageListBox::GetOffsetTextHeight()
{
	if (m_Style & MSGLISTBOX_S_TEXTBOTTOM)
	{
		if (m_nNumVisibleTextLine < m_nNumMaxShowLine)
...
//窗口函数
INT KScrollMessageListBox::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WND_N_SCORLLBAR_POS_CHANGED:
		if (uParam == (UINT)(KWndWindow*)&m_Scroll)
		{
	...
	case WND_N_LIST_ITEM_SEL:
		...
		if (m_pParentWnd)
			m_pParentWnd->WndProc(uMsg, (UINT)(KWndWindow*)this, nParam);
		break;
	...
	case WND_M_POPUPMENU:
		if (m_pParentWnd)
		{
			m_pParentWnd->WndProc(WND_M_POPUPMENU, (UINT)(KWndWindow*)this, nParam);
		}
		break;
	default:
		if (m_pParentWnd)
...
bool MsgListBox_LoadContent(KWndMessageListBox* pBox, KIniFile* pFile, const char* pszSection)
{
	if (pBox == NULL || pFile == NULL || pszSection == NULL)
		return false;
	pBox->Clear();
	char	szMsg[GOD_MAX_OBJ_DESC_LEN] = "", szIndex[8];
	INT		nIndex = 0;
	INT		nCapability = 0;
	while (true)
	{
		itoa(nIndex, szIndex, 10);
		if (!pFile->GetString(pszSection, szIndex, "", szMsg, sizeof(szMsg)))
			break;
		INT	nLen = strlen(szMsg);
		nLen = TEncodeText(szMsg, nLen);
		if (nLen <= 0)
		{
			szMsg[0] = ' ';
			szMsg[1] = 0;
