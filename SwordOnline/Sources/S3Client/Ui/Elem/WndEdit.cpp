...
UINT KWndEdit::ms_uBtnTipTextColor = 0xffffffff;	//按钮提示名称文字的颜色
...
//设置按钮提示名称文字的颜色
void KWndEdit::SetAllButtonTipTextColor(UINT uColor)
{
...
//	功能：窗口函数（处理消息）
//--------------------------------------------------------------------------
INT KWndEdit::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT	nRet = 0;
	switch (uMsg)
	{
	case WM_LBUTTONDOWN:
		...
		if (m_pParentWnd)
			m_pParentWnd->WndProc(WND_N_EDIT_LCLICK, (UINT)(KWndWindow*)this, nParam);
		break;
	case WM_RBUTTONDOWN:
		if (IsVisible() && Wnd_GetFocusWnd() != (KWndWindow*)this)
		{
			...
			nRet = KWndWindow::WndProc(uMsg, uParam, nParam);
		if (m_pParentWnd)
			m_pParentWnd->WndProc(WND_N_EDIT_RCLICK, (UINT)(KWndWindow*)this, nParam);
		break;
	case WM_MOUSEMOVE:
		if (m_bTipHover)
		{
	...
	case WND_M_KILL_FOCUS:
		...
		m_Flag &= ~WNDEDIT_F_HAVE_FOCUS;
		if (m_pParentWnd)
			m_pParentWnd->WndProc(WND_N_EDIT_KILL_FOCUS, (UINT)(KWndWindow*)this, 0);
		break;
	case WM_SYSKEYDOWN:
		if (m_pParentWnd)
...
void KWndEdit::SetColor(UINT uColor)
{
	m_TextColor = uColor;
}
...
INT KWndEdit::OnKeyDown(INT nKeyCode, INT nModifiers)
{
	...
	case VK_UP:
		if (MULTI_LINE && m_nCaretLineIndex)
		{
			...
			nRet = m_pParentWnd->WndProc(WND_N_EDIT_SPECIAL_KEY_DOWN, (UINT)(KWndWindow*)this, VK_UP);
		break;
	case VK_DOWN:
		if (MULTI_LINE)
		{
			...
			nRet = m_pParentWnd->WndProc(WND_N_EDIT_SPECIAL_KEY_DOWN, (UINT)(KWndWindow*)this, VK_DOWN);
		break;
	case VK_TAB:
	case VK_PRIOR:
	case VK_NEXT:
	case VK_ESCAPE:
		if (m_pParentWnd)
			nRet = m_pParentWnd->WndProc(WND_N_EDIT_SPECIAL_KEY_DOWN, (UINT)(KWndWindow*)this, nKeyCode);
		break;
	case VK_RETURN:
		if (m_pParentWnd)
		{
			if ((m_Flag & WNDEDIT_ES_MULTI_LINE) == 0)
				nRet = m_pParentWnd->WndProc(WND_N_EDIT_SPECIAL_KEY_DOWN, (UINT)(KWndWindow*)this, VK_RETURN);
...
void KWndEdit::UpdateData()
{
	...
	if (m_pParentWnd)
	{	//发送更新消息
		m_pParentWnd->WndProc(WND_N_EDIT_CHANGE, (UINT)(KWndWindow*)this, 0);
	}
}
