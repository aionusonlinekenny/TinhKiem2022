...
static UINT	s_uHoverObjDestTextColor = 0xffffffff;	//对象提示文字的颜色
...
void SetMouseHoverObjectDesc(void* pWnd, INT nObj, UINT uGenre,
	UINT uId, INT nContainer, INT x, INT y, bool LAlign)
{
	KGameObjDesc	Desc;
	...
	if (g_pCoreShell)
	{
		...
		g_pCoreShell->GetGameData(uIndex, (UINT)&Obj, (INT)&Desc);
		g_MouseOver.SetMouseHoverInfo(pWnd, nObj, x, y, false, false, LAlign);
		g_MouseOver.SetMouseHoverImage(g_pCoreShell->GetGameData(GDI_GAME_OBJ_DESC_INCLUDE_MOUSEHOVER, (UINT)&Obj, (INT)&g_MouseOver.m_HoverImage.szImage) == 1);
...
KMouseOver::KMouseOver()
{
	...
	m_uRectBordetShadowColor = 0;    //描述部分背景颜色
	m_uRectBordetColor = 0;      //描述部分文字颜色
	//////////////////
...
void KMouseOver::LoadScheme(const char* pScheme)
{
	...
	if (Ini.Load(Buff))
	{
		Ini.GetInteger("MouseOverWnd", "ImgWidth", 0, &m_nImgWidth);
		Ini.GetInteger("MouseOverWnd", "ImgHeight", 0, &m_nImgHeight);
...
void KMouseOver::PaintMouseHoverInfo()
{
	...
	INT nResolutionW, nResolutionH;
	UiGetScreenSize(nResolutionW, nResolutionH);
	...
	if (m_nLeft + m_nWndWidth <= nResolutionW)
	...
	if (m_bHeadTailImg)
		Shadow.oPosition.nY = m_nTop + m_nImgHeight - 5;
	else
	...
	if (m_nTitleLen > 0)
	{
		...
		while (true)
		{
			...
			else
			{
				...
				{
					if (m_LAlign)
						Param.nX = Shadow.oPosition.nX - m_nMaxWidthReduce + m_nIndent;
					else
