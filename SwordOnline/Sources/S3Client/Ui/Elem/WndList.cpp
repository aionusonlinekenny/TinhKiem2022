...
void KWndList::EnableMultiSel(INT bEnable)
{
	if (bEnable && IS_MULTI_SEL == 0)
	{
		...
		if (m_nNumItem > 32)
		{
			...
			m_puItemSelStatusList = (UINT*)malloc(sizeof(UINT) * m_uNumSelStatusList);
			if (m_puItemSelStatusList)
				memset(m_puItemSelStatusList, 0, sizeof(UINT) * m_uNumSelStatusList);
			else
			{
				m_uNumSelStatusList = 0;
				m_Style &= ~WNDLIST_ES_MULTI_SEL;
...
//	功能：窗口函数
//--------------------------------------------------------------------------
INT KWndList::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nRet = 0;
	switch(uMsg)
	{
	...
	case WM_MOUSEWHEEL:
		...
		break;
	case WM_RBUTTONDOWN:
		OnRButtonDown(LOWORD(nParam), HIWORD(nParam));
		break;
...
void KWndList::OnLButtonDown(INT x, INT y)
{
	...
	if (nSel != -1 && nSel != m_nSelItemIndex)
	{
		if (IS_MULTI_SEL == 0)
		{
			...
			if (m_pParentWnd)
			{
				m_pParentWnd->WndProc(WND_N_LIST_ITEM_SEL,
					(UINT)(KWndWindow*)this, m_nSelItemIndex);
			}
		}
		else if (nSel >= 0)
		{
			UINT uFlag = (1 << (nSel % 32));
			if (nSel < 32)
...
INT KWndList::SetCurSel(INT nSel)
{
	...
	else if (nSel != m_nSelItemIndex)
	{
		...
		UpdateData();
		if (m_pParentWnd)
			m_pParentWnd->WndProc(WND_N_LIST_ITEM_SEL, (UINT)(KWndWindow*)this, m_nSelItemIndex);
	}
	return nSel;
}
...
void KWndList::OnMouseMove(INT x, INT y)
{
	...
	if (nIndex != m_nHighLightItemIndex)
	{
		...
		if (m_pParentWnd)
		{
			m_pParentWnd->WndProc(WND_N_LIST_ITEM_HIGHLIGHT, (UINT)(KWndWindow*)this, m_nHighLightItemIndex);
		}
	}
}
