...
void SetSpaceOverObjectDesc(void* pWnd, INT nObj, UINT uGenre,
			UINT uId, INT nContainer, INT x, INT y, UINT uHoverTime, bool LAlign)
{
	...
	if (g_pCoreShell)
	{
		...
		g_pCoreShell->GetGameData(uIndex, (UINT)&Obj, (INT)&szTitle);
		g_SpaceOver.SetSpaceOverInfo(pWnd, nObj, x, y, uHoverTime, LAlign, false);
		g_SpaceOver.SetSpaceOverImage(g_pCoreShell->GetGameData(GDI_GAME_OBJ_DESC_INCLUDE_MOUSEHOVER, (UINT)&Obj, (INT)&g_SpaceOver.m_HoverImage.szImage) == 1);
...
*功能：设置鼠标提示窗口的基本信息
************************************************************************/
void KSpaceOver::SetSpaceOverInfo(void* pWnd, INT nObj, INT x, INT y, UINT uHoverTime, bool LAlign, bool bFollowCursor)
{
