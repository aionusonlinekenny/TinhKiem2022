...
void KWndWindow::GetPosition(INT* pLeft, INT* pTop)
{
	...
	//g_DebugLog("Top: %d - Left: %d", m_Top, m_Left);
}
...
INT KWndWindow::IsEnable()
{
	return (m_Style & WND_S_DISABLE);
}
...
INT KWndWindow::Init(KIniFile* pIniFile, const char* pSection)
{
	...
	{
#ifdef _DEBUG
		strncpy(m_Caption, pSection, sizeof(m_Caption));
#endif
		...
		pIniFile->GetInteger(pSection, "Value", 0, &m_nValue);
		if (nValue1)
			m_Style |= WND_S_DISABLE;
		else
			m_Style &= ~WND_S_DISABLE;
...
//	功能：窗口函数（处理消息）
//--------------------------------------------------------------------------
INT KWndWindow::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT	nRet = 0;
	switch (uMsg)
	{
	...
	case WM_LBUTTONUP:
		if (m_bMoving)
		{
			...
			if (m_pParentWnd)
			{
				m_pParentWnd->WndProc(WND_N_CHILD_MOVE, (UINT)(KWndWindow*)this, 0);
			}
		}
		nRet = 1;
		break;
...
void KWndWindow::OnMoveWnd()
{
	...
	if (x != m_nLastMouseHoldPosX || y != m_nLastMouseHoldPosY)
	{
		...
		if (x && m_pParentWnd)
		{
			m_pParentWnd->WndProc(WND_N_CHILD_MOVE, (UINT)(KWndWindow*)this, x);
		}
	}
}
...
{
	if (pszCaption)
		strncpy(m_Caption, pszCaption, sizeof(m_Caption));
}
#endif
