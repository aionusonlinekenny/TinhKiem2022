...
UINT	KPopupMenu::m_uCallerParam = 0;
int16_t			KPopupMenu::m_nImgWidth = 0;
int16_t			KPopupMenu::m_nImgHeight = 0;
int16_t			KPopupMenu::m_nIndent = 0;
int16_t			KPopupMenu::m_nFrame = 0;
INT				KPopupMenu::m_nMenuHeight = 0;
char			KPopupMenu::m_szImage[128] = "";
UINT	KPopupMenu::m_uDefBorderLineColor = 0xff000000;	//菜单的变框颜色，为0则无
UINT	KPopupMenu::m_uDefTextColor		 = 0xffffffff;	//菜单项的文字色
UINT	KPopupMenu::m_uDefTextBorderColor= 0xff000000;	//菜单项的文字边缘色
UINT	KPopupMenu::m_uDefSelTextColor	 = 0xffe0e0e0;	//被选中菜单项的文字色
UINT	KPopupMenu::m_uDefSelTextBorderColor = 0xff000000;	//被选中菜单项的文字边缘色
UINT	KPopupMenu::m_uDefItemBgColor	 = 0x0a404040;	//菜单项的背景色
UINT	KPopupMenu::m_uDefSelItemBgColor = 0x0a202020;	//被选中菜单项的背景色
...
void KPopupMenu::Popup(KPopupMenuData* pMenu, KWndWindow* pCaller, UINT uParam)
{
	Cancel();
...
//窗口函数
INT	KPopupMenu::HandleInput(UINT uMsg, UINT uParam, INT nParam)
{
	INT nRet = 0;
	if (m_pMenu)
	{
		...
		default:
			if ((uMsg >= WM_MOUSEFIRST && uMsg <= WM_MOUSELAST) || uMsg == WM_MOUSEHOVER)
			{
				if (uMsg == WM_MOUSEMOVE)
					SelectItem(LOWORD(nParam), HIWORD(nParam), false);
				else if (uMsg == WM_LBUTTONDOWN)
					SelectItem(LOWORD(nParam), HIWORD(nParam), true);
				else if (uMsg == WM_RBUTTONDOWN)
					SelectItem(LOWORD(nParam), HIWORD(nParam), true, true);
				nRet = true;
			}
			break;
		}
...
bool KPopupMenu::SelectItem(INT nIndex, bool bApply)
{
	...
	UINT nMID = WND_M_MENUITEM_SELECTED;
	...
	if (bApply)
	{
		...
		UINT uCallerParam = m_uCallerParam;
...
bool KPopupMenu::SelectItem(INT x, INT y, bool bApply, bool bRight)
{
	bool bRet = false;
	UINT nMID = WND_M_MENUITEM_SELECTED;
	m_pMenu->nSelectedItem = -1;
...
void KPopupMenu::InitMenuData(KPopupMenuData* pMenu, INT nNumItem)
{
