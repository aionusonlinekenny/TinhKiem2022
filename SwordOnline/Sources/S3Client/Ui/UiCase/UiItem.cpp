...
void KUiItem::UpdateData()
{
	...
	{
		g_pCoreShell->GetGameData(GDI_ITEM_TAKEN_WITH, (UINT)pObjs, nCount);//µ¥Ïß³ÌÖ´ÐÐ£¬nCountÖµ²»±ä
...
void KUiItem::LoadScheme(const char* pScheme)
{
	...
	if (m_pSelf && Ini.Load(Buff))
	{
		...
		m_pSelf->m_OpenStatusPadBtn.SetText("Trang bÞ");
		m_pSelf->m_CloseBtn.SetText("§ãng");
		m_pSelf->m_GetMoneyBtn.SetText("Göi tiÒn");
	}
}
...
// ¹¦ÄÜ	: ´°¿Úº¯Êý
// -------------------------------------------------------------------------
INT KUiItem::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	...
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_CloseBtn && g_UiBase.GetStatus() != UIS_S_TRADE_SETPRICE)
			CloseWindow(false);
		else if (uParam == (UINT)(KWndWindow*)&m_OpenStatusPadBtn)
			KShortcutKeyCentre::ExcuteScript(SCK_SHORTCUT_STATUS);
		else if (uParam == (UINT)(KWndWindow*)&m_GetMoneyBtn)
		{
			if (KUiStoreBox::GetIfVisible())
				KUiGetMoney::OpenWindow(0, m_nMoney, this, UIITEM_WAIT_GETMONEY, &m_Money);
		}
		else if (uParam == (UINT)(KWndWindow*)&m_MakeAdvBtn && m_MakeStallBtn.IsButtonChecked() == 0 && !g_UiBase.GetStatus())
			KUiGetString::OpenWindow(GSA_ADV, "", m_szAdvStr, this, UIITEM_WAIT_GETADV, 0, 4, 16);
		else if (uParam == (UINT)(KWndWindow*)&m_MarkPriceBtn && m_MakeStallBtn.IsButtonChecked() == 0)
		{
			if (g_UiBase.GetStatus() == UIS_S_IDLE)
			{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_MakeStallBtn && !g_UiBase.GetStatus())
		{
			if (g_pCoreShell)
			{
				if (m_szAdvStr[0])
				{
					if (!g_pCoreShell->OperationRequest(GOI_PLAYER_DOTRADE, (UINT)(&m_szAdvStr), 0))
						m_MakeStallBtn.CheckButton(false);
				}
				else
					KUiGetString::OpenWindow(GSA_ADV, "", m_szAdvStr, this, UIITEM_WAIT_GETADV, 0, 4, 16);
...
void KUiItem::OnBreakItem(KUiDraggedObject* pItem, bool bDoImmed)
{
	...
	if (bDoImmed)
		g_pCoreShell->BreakItem((UINT)(&Obj), 0, FALSE);
	else
	{
