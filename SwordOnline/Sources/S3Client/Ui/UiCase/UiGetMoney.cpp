...
#define	SCHEME_INI 	"UiGetMoney.ini"
...
KUiGetMoney* KUiGetMoney::OpenWindow(INT nMoney, INT nMaxMoney,
	KWndWindow* pRequester, UINT uParam, KWndWindow* pMoneyWnd)
{
	if (m_pSelf == NULL)
	{
...
//	功能：窗口消息函数
//--------------------------------------------------------------------------
INT KUiGetMoney::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nRet = 0;
	switch (uMsg)
	{
	...
	case WND_N_EDIT_SPECIAL_KEY_DOWN:
		if (nParam == VK_RETURN &&
			uParam == (UINT)(KWndWindow*)&m_Money)
		{
			OnOk();
			nRet = 1;
		}
...
{
	OnCheckInput();
	DWORD nMoney = m_Money.GetIntNumber();
	if (m_pRequester)
