...
//´°¿Úº¯Êý
INT	KUiShop::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
...
void KUiShop::OnBuyItem(KUiDraggedObject* pItem, bool bDoImmed)
{
	...
	if (bDoImmed == false)
	{
		...
		if (g_pCoreShell->GetGameData(GDI_TRADE_ITEM_PRICE, (UINT)(&Obj), (INT)(&Price)) && eStatus != UIS_S_TRADE_SALE && eStatus != UIS_S_TRADE_REPAIR)
		{
	...
	else
	{
		...
		{
			g_pCoreShell->OperationRequest(GOI_TRADE_NPC_BUY, (UINT)(&Obj), 0);
			return;
		}
		else
		{
			...
			{
				g_pCoreShell->OperationRequest(GOI_TRADE_NPC_BUY, (UINT)(&Obj), 1);
				return;
			}
			else
			{
...
void KUiShop::UpdateData()
{
	...
	{
		g_pCoreShell->GetGameData(GDI_TRADE_NPC_ITEM, (UINT)m_pObjsList, m_nObjCount);//µ¥Ïß³ÌÖ´ÐÐ£¬nCountÖµ²»±ä
