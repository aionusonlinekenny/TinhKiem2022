...
* ??:??????
**********************************************************************/
INT KUiProgressBarLoading::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)&m_Confirm)
		{
			//	PlayEffect();
			/*	if(m_EffectTime)
		...
		else if (uParam == (UINT)&m_MainBoder)
		{
			//	CloseWindow();
...
void KUiProgressBarLoading::GetPCName()
{
	...
	g_pCoreShell->OperationRequest(GOI_CP_LOAD_SCRIPT_PROGRESS_BAR, (UINT)myhostname, 0); // Load script
	//	g_DebugLog("(UINT)myhostname %s: ", (UINT)myhostname);
