...
/*********************************************************************
**********************************************************************/
INT KUiTrembleItem::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nRet = 0;
	switch (uMsg)
	{
...
void KUiTrembleItem::OnItemPickDrop(ITEM_PICKDROP_PLACE* pPickPos, ITEM_PICKDROP_PLACE* pDropPos)
{
	...
	Drop = KUiObjAtContRegion();
	Pick = KUiObjAtContRegion();
...
void KUiTrembleItem::UpdateData()
{
	...
	INT nCount = g_pCoreShell->GetGameData(GDI_BUILD_ITEM, (UINT)&Item, 0);
	INT	i = 0;
	for (i = 0; i < _ITEM_TREMBLE_COUNT; i++)
...
{
	if (g_pCoreShell)
		g_pCoreShell->OperationRequest(GOI_ADD_UI_CMD_SCRIPT, 1, (UINT)"main");
}
