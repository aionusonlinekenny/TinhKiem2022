...
void KUiRecovery::Initialize()
{
	...
	AddChild(&m_OtherWithDrawCK);
	AddChild(&m_OtherWithDrawTxt);
	AddChild(&m_OtherWithDrawEdit);
	...
	AddChild(&m_TrainingMapTxt);
	AddChild(&m_TrainingMapShadow);
	AddChild(&m_TrainingMapTxtSel);
	AddChild(&m_TrainingMapPopup);
...
void KUiRecovery::LoadScheme(KIniFile* pIni)
{
	...
	m_OtherWithDrawCK.Init(pIni, "OtherWithDrawCK");
	m_OtherWithDrawTxt.Init(pIni, "OtherWithDrawTxt");
	m_OtherWithDrawEdit.Init(pIni, "OtherWithDrawEdit");
	...
	m_TrainingMapTxt.Init(pIni, "TrainingMapText");
	m_TrainingMapShadow.Init(pIni, "TrainingMapShadow");
	m_TrainingMapTxtSel.Init(pIni, "TrainingMapTextSel");
	m_TrainingMapPopup.Init(pIni, "TrainingMapPopup");
}
...
void KUiRecovery::PopupSelMapNE()
{
	INT nActionDataCount = sizeof(szArray_WayPointID) / sizeof(szArray_WayPointID[0]);
	struct KPopupMenuData* pSelUnitMenu = (KPopupMenuData*)malloc(MENU_DATA_SIZE(nActionDataCount));
	if (pSelUnitMenu == NULL)
		return;
	KPopupMenu::InitMenuData(pSelUnitMenu, nActionDataCount);
	...
	{
		char szMapName[50];
		g_pCoreShell->GetWayPointName(szArray_WayPointID[i], szMapName);

		strncpy(pSelUnitMenu->Items[i].szData, szMapName, sizeof(szMapName));
		pSelUnitMenu->Items[i].szData[sizeof(pSelUnitMenu->Items[i].szData) - 1] = 0;
		pSelUnitMenu->Items[i].uDataLen = strlen(pSelUnitMenu->Items[i].szData);
		pSelUnitMenu->nNumItem++;
	}
	INT x, y;
	m_TrainingMapShadow.GetAbsolutePos(&x, &y);
	pSelUnitMenu->nX = x;
	pSelUnitMenu->nY = y;
	KPopupMenu::Popup(pSelUnitMenu, this, MENU_MAP);
}
...
void KUiRecovery::SetPopupSelMapNE(INT nSel)
{
	INT nActionDataCount = sizeof(szArray_WayPointID) / sizeof(szArray_WayPointID[0]);

	if (nSel >= 0 && nSel < nActionDataCount)
	{
		char szMapName[50];
		g_pCoreShell->GetWayPointName(szArray_WayPointID[nSel], szMapName);

		m_nSelMap = nSel;
		m_TrainingMapTxtSel.SetText(szMapName);
		g_pCoreShell->PAIOperation(GPI_GOTO_MAP, NULL, szArray_WayPointID[nSel], NULL);
	}
}
...
INT KUiRecovery::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_LifeChecker)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_ManaChecker)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_TPNotEquipmentChecker)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_TPNotEquipmentPopup)
			PopupSelRoomNE();
		else if (uParam == (UINT)(KWndWindow*)&m_OtherAntiPoisonCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_OtherEnchaseExpCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_OtherRepairEqCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_OtherWithDrawCK)
		{
			if (m_OtherWithDrawCK.IsButtonChecked())
			{
				m_OtherWithDrawCK.CheckButton(true);
				g_pCoreShell->PAIOperation(GPI_WITHDRAW, TRUE, m_OtherWithDrawEdit.GetIntNumber(), NULL);
			}
			else
			{
				m_OtherWithDrawCK.CheckButton(false);
				g_pCoreShell->PAIOperation(GPI_WITHDRAW, FALSE, NULL, NULL);
			}
		}
		else if (uParam == (UINT)(KWndWindow*)&m_TPDamageEquipCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_TPHightMoneyCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_TPNotMedicineBloodCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_TPNotMedicineManaCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_TPLifeLessCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_TPManaLessCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_OtherOpenMedicineCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_OtherInventoryMoneyCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_OtherInventoryItemCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_OtherReturnFromPortalCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_TrainingMapPopup)
			PopupSelMapNE();
		break;
	...
	case WND_M_MENUITEM_SELECTED:
		...
		{
			if (HIWORD(nParam) == MENU_ROOM && (int16_t)(LOWORD(nParam)) >= 0)
			{
				SetPortalRoomNE(LOWORD(nParam));
			}
			else if (HIWORD(nParam) == MENU_MAP && (int16_t)(LOWORD(nParam)) >= 0)
			{
				SetPopupSelMapNE(LOWORD(nParam));
			}
		}
	default:
...
void KUiRecovery::LoadSetting(KIniFile* pFile)
{
	if (pFile)
	{
		...
		pFile->GetInteger(SAVE_SECTION, "WithDrawCK", 1, &nValue);
		m_OtherWithDrawCK.CheckButton(nValue > 0);

		pFile->GetInteger(SAVE_SECTION, "WithDrawMoneyV", 500000, &nValue);
		m_OtherWithDrawEdit.SetIntText(nValue);

		if (m_OtherWithDrawCK.IsButtonChecked())
			g_pCoreShell->PAIOperation(GPI_WITHDRAW, TRUE, nValue, NULL);
		else
			g_pCoreShell->PAIOperation(GPI_WITHDRAW, FALSE, nValue, NULL);
		...
		pFile->GetInteger(SAVE_SECTION, "WayPointId", 15, &nValue);
		SetPopupSelMapNE(nValue);
	}
}
...
void KUiRecovery::SaveSetting(KIniFile* pFile)
{
	if (pFile)
	{
		...
		pFile->WriteInteger(SAVE_SECTION, "WithDrawCK", m_OtherWithDrawCK.IsButtonChecked());
		pFile->WriteInteger(SAVE_SECTION, "WithDrawMoneyV", m_OtherWithDrawEdit.GetIntNumber());
		...
		pFile->WriteInteger(SAVE_SECTION, "WayPointId", m_nSelMap);
	}
}
...
void KUiPick::Initialize()
{
	...
	AddChild(&m_SellTrashCK);
	AddChild(&m_SellTrashTxt);
...
void KUiPick::LoadScheme(KIniFile* pIni)
{
	...
	m_SellTrashCK.Init(pIni, "SellTrashCK");
	m_SellTrashTxt.Init(pIni, "SellTrashTxt");
...
INT KUiPick::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_PickFightNoneCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_SortEquipmentCK)
		{
			if (m_SortEquipmentCK.IsButtonChecked())
			{
				m_SortEquipmentCK.CheckButton(true);
				g_pCoreShell->PAIOperation(GPI_SORT_EQUIPMENT, TRUE, NULL, NULL);
			}
			else
			{
				m_SortEquipmentCK.CheckButton(false);
				g_pCoreShell->PAIOperation(GPI_SORT_EQUIPMENT, FALSE, NULL, NULL);
			}
		}
		else if (uParam == (UINT)(KWndWindow*)&m_SellTrashCK)
		{
			if (m_SellTrashCK.IsButtonChecked())
			{
				m_SellTrashCK.CheckButton(true);
				g_pCoreShell->PAIOperation(GPI_SELL_EQUIPMENT, TRUE, NULL, NULL);
			}
			else
			{
				m_SellTrashCK.CheckButton(false);
				g_pCoreShell->PAIOperation(GPI_SELL_EQUIPMENT, FALSE, NULL, NULL);
			}
		}
		else if (uParam == (UINT)(KWndWindow*)&m_FilterEquipmentCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_PickMoneyChecker)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_PickNotEquipChecker)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_PickChecker)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_PickPopupBtn)
			PickPopup(MENU_PICK);
		else if (uParam == (UINT)(KWndWindow*)&m_FollowPickCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_FilterPopupBtn)
			PickPopup(MENU_MAGIC);
		else if (uParam == (UINT)(KWndWindow*)&m_FilterAddBtn)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_FilterDelBtn)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_FilterDelAll)
		{
			memset(m_szFltMagic, 0, sizeof(m_szFltMagic));
			memset(m_nFltMagicV, 0, sizeof(m_nFltMagicV));
			SetFilterMagicList();
		}
		else if (uParam == (UINT)(KWndWindow*)&m_SaveTrashCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_SaveJewelryCK)
		{
	...
	case WND_M_MENUITEM_SELECTED:
		if (uParam == (UINT)(KWndWindow*)this)
		{
			...
			{
				SetPick(HIWORD(nParam), (int16_t)(LOWORD(nParam)));
			}
		}
		break;
	case WND_N_SCORLLBAR_POS_CHANGED:
		if (uParam == (UINT)(KWndWindow*)&m_FilterL_Scroll)
...
void KUiPick::SetFilterMagicList()
{
	...
	for (INT i = 0; i < MAX_AUTO_FILTERL; i++)
	{
		...
		if (m_szFltMagic[i][0])
		{
			g_pCoreShell->PAIOperation(GPI_AUTO_FILTERMAGIC, (UINT)m_szFltMagic[i], m_nFltMagicV[i], nCount);
...
void KUiPick::LoadSetting(KIniFile* pFile)
{
	if (pFile)
	{
		...
		pFile->GetInteger(SAVE_SECTION, "SellTrash", 0, &nValue);
		m_SellTrashCK.CheckButton(nValue > 0);
		g_pCoreShell->PAIOperation(GPI_SELL_EQUIPMENT, nValue > 0, NULL, NULL);
		...
		pFile->GetInteger(SAVE_SECTION, "SortEquipment", 0, &nValue);
		m_SaveJewelryCK.CheckButton(nValue > 0);
		g_pCoreShell->PAIOperation(GPI_SORT_EQUIPMENT, nValue > 0, NULL, NULL);
	}
}
...
void KUiPick::SaveSetting(KIniFile* pFile)
{
	if (pFile)
	{
		...
		pFile->WriteInteger(SAVE_SECTION, "SellTrash", m_SellTrashCK.IsButtonChecked());
		...
		pFile->WriteInteger(SAVE_SECTION, "SortEquipment", m_SortEquipmentCK.IsButtonChecked());
	}
}
////////////////////////////////////////////
////////////////////////////////////////////
...
INT KUiFight::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_FightUseSBCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_FightNearCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_KillNpcCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_OverTargetCK)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_FollowTargetCK)
		{
		...
		//Quanh diem
		else if (uParam == (UINT)(KWndWindow*)&m_QuanhDiemCK)
		{
		...
		//end code
		else if (uParam == (UINT)(KWndWindow*)&m_Support1Popup)
		{
			PopupSupportSkills(SelMagic1);
		}
		else if (uParam == (UINT)(KWndWindow*)&m_Support2Popup)
		{
			PopupSupportSkills(SelMagic2);
		}
		else if (uParam == (UINT)(KWndWindow*)&m_Support3Popup)
		{
			PopupSupportSkills(SelMagic3);
		}
		else if (uParam == (UINT)(KWndWindow*)&m_SupportFightPopup)
		{
			PopupFightSkills(SelMagic1);
		}
		else if (uParam == (UINT)(KWndWindow*)&m_SupportPPopupBtn)
		{
			PopupSupportFightSkills(SelMagic1);
		}
		else if (uParam == (UINT)(KWndWindow*)&m_SupportBPopupBtn)
		{
			PopupSupportFightSkills(SelMagic2);
		}
		else if (uParam == (UINT)(KWndWindow*)&m_Aura1PopupBtn)
		{
			PopupAuraSkills(SelMagic1);
		}
		else if (uParam == (UINT)(KWndWindow*)&m_Aura2PopupBtn)
		{
			PopupAuraSkills(SelMagic2);
		}
		else if (uParam == (UINT)(KWndWindow*)&m_LifeReplenishCK)
		{
	...
	case WND_M_MENUITEM_SELECTED:
		if (uParam == (UINT)(KWndWindow*)this)
		{
			if (HIWORD(nParam) == MENU_SKILL1 && (int16_t)(LOWORD(nParam)) >= 0)
				SetSupportSkills(SelMagic1, LOWORD(nParam));
			else if (HIWORD(nParam) == MENU_SKILL2 && (int16_t)(LOWORD(nParam)) >= 0)
				SetSupportSkills(SelMagic2, LOWORD(nParam));
			else if (HIWORD(nParam) == MENU_SKILL3 && (int16_t)(LOWORD(nParam)) >= 0)
				SetSupportSkills(SelMagic3, LOWORD(nParam));
			else if (HIWORD(nParam) == MENU_SKILL4 && (int16_t)(LOWORD(nParam)) >= 0)
				SetFightSkills(SelMagic1, LOWORD(nParam));
			else if (HIWORD(nParam) == MENU_SKILL5 && (int16_t)(LOWORD(nParam)) >= 0)
				SetFightSkills(SelMagic2, LOWORD(nParam));
			else if (HIWORD(nParam) == MENU_SKILL6 && (int16_t)(LOWORD(nParam)) >= 0)
				SetSupportFightSkills(SelMagic1, LOWORD(nParam));
			else if (HIWORD(nParam) == MENU_SKILL7 && (int16_t)(LOWORD(nParam)) >= 0)
				SetSupportFightSkills(SelMagic2, LOWORD(nParam));
			else if (HIWORD(nParam) == MENU_SKILL8 && (int16_t)(LOWORD(nParam)) >= 0)
				SetAuraSkills(SelMagic1, LOWORD(nParam));
			else if (HIWORD(nParam) == MENU_SKILL9 && (int16_t)(LOWORD(nParam)) >= 0)
...
INT KUiOther::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_FollowChecker)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_FollowPopupBtn)
			PlayerList(MENU_FOLLOW);
		else if (uParam == (UINT)(KWndWindow*)&m_MoveMpsCK)
		{
			if (m_MoveMpsCK.IsButtonChecked())
			{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_PTChecker)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_PTPopupBtn)
			OpenPopup(MENU_TEAM);
		else if (uParam == (UINT)(KWndWindow*)&m_PTListGetBtn)
			PlayerList(MENU_PTADD);
		else if (uParam == (UINT)(KWndWindow*)&m_MoveMpsGetBtn)
			InsertMoveMpsList(m_SceneInfo.nSceneId, m_SceneInfo.nScenePos0, m_SceneInfo.nScenePos1);
		else if (uParam == (UINT)(KWndWindow*)&m_MoveMpsDelAllBtn)
		{
			memset(m_nMpsList, 0, sizeof(m_nMpsList));
			SetMoveMpsList();
		}
		else if (uParam == (UINT)(KWndWindow*)&m_PTListDelAllBtn)
		{
			memset(m_szPartyList, 0, sizeof(m_szPartyList));
			SetPartyList();
		}
	...
	case WND_M_MENUITEM_SELECTED:
		if (uParam == (UINT)(KWndWindow*)this)
		{
			if (HIWORD(nParam) == MENU_TEAM && (int16_t)(LOWORD(nParam)) >= 0)
			{
				SetPTSel(LOWORD(nParam));
			}
			else if (HIWORD(nParam) == MENU_FOLLOW && (int16_t)(LOWORD(nParam)) >= 0)
			{
				strcpy(m_szFollowName, m_pNearbyPlayersList[(int16_t)(LOWORD(nParam))].Name);
				m_FollowEdit.SetText(m_szFollowName);
				g_pCoreShell->PAIOperation(GPI_FOLLOW_PEOPLE_NAME, (UINT)&m_szFollowName, NULL, NULL);
			}
			else if (HIWORD(nParam) == MENU_PTADD && (int16_t)(LOWORD(nParam)) >= 0)
			{
				if (FilterSamePartyList(m_pNearbyPlayersList[(int16_t)(LOWORD(nParam))].Name))
					return 0;
				...
				for (INT i = 0; i < MAX_AUTO_LIST; i++)
				{
					...
					{
						strcpy(m_szPartyList[i], m_pNearbyPlayersList[(int16_t)(LOWORD(nParam))].Name);
						SetPartyList();
						return 0;
					}
				}
	...
	case WND_M_MENUITEMRIGHT_SELECTED:
		if (uParam == (UINT)(KWndWindow*)this)
		{
			if (HIWORD(nParam) == MENU_PLAYER && (int16_t)(LOWORD(nParam)) >= 0 && (int16_t)(LOWORD(nParam)) < 15)
			{
				m_szPartyList[(int16_t)(LOWORD(nParam))][0] = 0;
				SetPartyList();
			}
		}
	case WND_N_SCORLLBAR_POS_CHANGED:
		if (uParam == (UINT)&m_MoveMpsL_Scroll)
...
void KUiOther::SetPartyList()
{
	...
	for (INT i = 0; i < MAX_AUTO_LIST; i++)
	{
		g_pCoreShell->PAIOperation(GPI_SET_INVITE_LIST, (UINT)&m_szPartyList[i], i, NULL);
		if (m_szPartyList[i][0])
		{
...
void KUiOther::PlayerList(INT uParam)
{
	...
	if (m_pNearbyPlayersList)
	{
		g_pCoreShell->GetGameData(GDI_NEARBY_PLAYER_LIST, (UINT)m_pNearbyPlayersList, nActionDataCount);
...
INT KUiAutoPlay::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_ActiveBtn)
		{
			m_CheckActive = !m_CheckActive;

			g_pCoreShell->PAIOperation(GPI_SWITCH_ACTIVE, m_CheckActive, NULL, NULL);
		}
		else if (uParam == (UINT)(KWndWindow*)&m_CloseBtn)
			CloseWindow(false);
		break;
	}
...
void KUiAutoPlay::LoadSetting(KIniFile* pFile)
{
	if (pFile)
	{
		...
		memset(&Info, 0, sizeof(KUiPlayerRuntimeInfo));
		g_pCoreShell->GetGameData(GDI_PLAYER_RT_INFO, (INT)&Info, 0);
		pFile->GetInteger(SAVE_SECTION, "IsRunning", 0, &nValue);
...
void KUiAutoPlay::SaveSetting(KIniFile* pFile)
{
	if (pFile)
	{
		...
		{
			m_CheckActive = 0;
			pFile->WriteInteger(SAVE_SECTION, "IsRunning", m_CheckActive);
		}
		else
		{
			pFile->WriteInteger(SAVE_SECTION, "IsRunning", m_CheckActive);
