...
#define	SCHEME_INI	"UiSelPlayerNearby.ini"
...
BOOL KUiSelPlayerNearby::DoesHaveVisibleWnd()
{
...
//窗口函数
INT KUiSelPlayerNearby::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nRet = 0;
	switch(uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_CancelBtn)
			CloseWindow(false);
		else if (uParam == (UINT)(KWndWindow*)&m_RefreshBtn)
			UpdateData();
		else
		{
			for (INT i = 0; i < m_nActionCount; i++)
			{
				if (uParam == (UINT)(KWndWindow*)&m_ActionBtns[i])
				{
					OnClickButton(i);
					break;
				}
	...
	case WND_N_SCORLLBAR_POS_CHANGED:
		if (uParam == (UINT)(KWndWindow*)&m_ListScroll)
			m_PlayerList.SetTopItemIndex(nParam);
		break;
	case WND_N_LIST_ITEM_SEL:
		if (uParam == (UINT)(KWndWindow*)&m_PlayerList)
		{
			char szCurText[256];
			szCurText[0] = 0;
...
void KUiSelPlayerNearby::OnClickButton(INT nAction)
{
	char	Buffer[64];
	KUiMsgParam	Param;
...
void KUiSelPlayerNearby::UpdateData()
{
	...
	if (nCount)
	{
		...
		{
			KUiPlayerItem Item;
			m_nPlayerCount = g_pCoreShell->GetGameData(GDI_NEARBY_PLAYER_LIST, (UINT)m_pDataList, nCount);
