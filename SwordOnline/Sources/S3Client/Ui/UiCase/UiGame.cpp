...
//--------------------------------------------------------------------------
INT KUiGameSpace::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	bool bDefault = true;
	INT nKind = -1;
	...
	case WND_M_MENUITEM_SELECTED:
		...
		{
			if (HIWORD(nParam) == SEL_ACTION_MENU && int16_t(LOWORD(nParam)) >= 0)
			{
				ProcessPeople(&m_SelectPlayer, LOWORD(nParam));
				bDefault = false;
			}
			if (HIWORD(nParam) == SEL_EMOTE_MENU && int16_t(LOWORD(nParam)) >= 0)
			{
...
void ProcessPeople(KUiPlayerItem* pDest, INT nAction)
{
	...
	case ACTION_MAKEFRIEND:	//ÒªÓÃName
		if (g_pCoreShell && !KUiChatCentre::IsMyFriend(pDest->Name))
			g_pCoreShell->OperationRequest(GOI_CHAT_FRIEND_ADD, (UINT)pDest, 0);
		break;
	case ACTION_INVITETEAM:			//ÒªÓÃuId
		if (g_pCoreShell && pDest->uId != 0)
		{
			...
			g_pCoreShell->TeamOperation(TEAM_OI_GD_INFO, (UINT)&TeamInfo, 0);
			if (TeamInfo.cNumMember == 0)
				g_pCoreShell->TeamOperation(TEAM_OI_CREATE, 0, 0);
			g_pCoreShell->TeamOperation(TEAM_OI_INVITE, (UINT)pDest, 0);
		}
		break;
	case ACTION_FOLLOW:	//ÒªÓÃnIndex
		if (g_pCoreShell && pDest->nIndex != -1)
			g_pCoreShell->OperationRequest(GOI_FOLLOW_SOMEONE, (UINT)pDest, 0);
		break;
	case ACTION_VIEWITEM:			//ÒªÓÃuId
		if (g_pCoreShell && pDest->uId != 0)
			g_pCoreShell->OperationRequest(GOI_VIEW_PLAYERITEM, (UINT)pDest->uId, 0);
		break;
	...
	case ACTION_TONG:
		if (g_pCoreShell && pDest->uId != 0)
		{
			if (g_pCoreShell->TongOperation(GTOI_TONG_FLAG, (UINT)pDest, 0))
	...
	case ACTION_REVENGE:			//ÒªÓÃuId
		if (g_pCoreShell && pDest->uId != 0)
			g_pCoreShell->OperationRequest(GOI_REVENGE_SOMEONE, (UINT)pDest, 0);
		break;
	}
}
...
void ProcessEmote(char* szDest, char* szDestChannel, INT nEmoteIndex)
{
	if (g_pCoreShell && szDest)
	{
		...
		if (nChannelIndex != -1)
		{
			...
			{
				//¿ªÊ¼»ñÈ¡ºÍ×ª»»×Ö´®
				g_pCoreShell->GetGameData(GDI_PLAYER_BASE_INFO, (UINT)&MyInfo, 0);
				if (szDest[0])
...
void PopUpContextPeopleMenu(const KUiPlayerItem& SelectPlayer, INT x, INT y)
{
	...
	BOOL bTongFlag = g_pCoreShell->TongOperation(GTOI_TONG_FLAG, (UINT)&SelectPlayer, 0);
	BYTE btFigure = g_pCoreShell->TongOperation(GTOI_TONG_FIGURE, (UINT)&SelectPlayer, 0);
