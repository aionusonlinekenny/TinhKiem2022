...
* 功能：窗口函数
**********************************************************************/
INT KUiTongAssignBox::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)&m_BtnConfirm)
		{
			OnConfirm();
		}
		...
		else if (uParam == (UINT)&m_BtnElder)
		{
			UpdateCheckButton(enumTONG_FIGURE_DIRECTOR);
		}
		else if (uParam == (UINT)&m_BtnCaptain)
		{
			UpdateCheckButton(enumTONG_FIGURE_MANAGER);
		}
		else if (uParam == (UINT)&m_BtnPilgarlic)
		{
	...
	case WND_M_OTHER_WORK_RESULT:
		if (uParam == RESULT_T_ASSIGN)
		{
			if (nParam && g_pCoreShell)
			{
				...
				g_pCoreShell->TongOperation(GTOI_TONG_ACTION, (UINT)&Param, (INT)&Member);
			}
			if (m_pMain)
			{
				m_pMain->Show();
...
* 功能：弹出确认窗口
**********************************************************************/
void KUiTongAssignBox::PopupConfirmWindow(const char* pszInfo, UINT uHandleID)
{
