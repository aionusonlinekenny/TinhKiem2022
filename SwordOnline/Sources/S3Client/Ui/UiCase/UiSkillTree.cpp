...
void KUiSkillTree::LoadConfig(KIniFile* pIni)
{
	...
	do
	{
		...
		INT nNum = g_pCoreShell->GetGameData(
			bLeft ? GDI_LEFT_ENABLE_SKILLS : GDI_RIGHT_ENABLE_SKILLS, (UINT)&Skills, 0);
		...
		pIni->GetStruct(SKILLTREE_SHORTCUT_SKILL_SAVE, bLeft ? "LeftSkill" : "RightSkill",
...
void KUiSkillTree::SaveConfig(KIniFile* pIni)
{
	...
	if (g_pCoreShell)
	{
		...
		g_pCoreShell->GetGameData(GDI_PLAYER_IMMED_ITEMSKILL, (UINT)&Skill, 0);

		if (Skill.IMmediaSkill[0].uGenre != CGOG_NOTHING)
			pIni->WriteStruct(SKILLTREE_SHORTCUT_SKILL_SAVE, "LeftSkill", &Skill.IMmediaSkill[0], sizeof(KUiGameObject));
		else
			pIni->EraseKey("Player", "LeftSkill");
		if (Skill.IMmediaSkill[1].uGenre != CGOG_NOTHING)
			pIni->WriteStruct(SKILLTREE_SHORTCUT_SKILL_SAVE, "RightSkill", &Skill.IMmediaSkill[1], sizeof(KUiGameObject));
		else
			pIni->EraseKey(SKILLTREE_SHORTCUT_SKILL_SAVE, "RightSkill");
	}
}
...
void KUiSkillTree::UpdateData()
{
	m_nNumSkills = g_pCoreShell->GetGameData(
		m_bLeft ? GDI_LEFT_ENABLE_SKILLS : GDI_RIGHT_ENABLE_SKILLS, (UINT)&m_Skills, 0);
...
{
	if (x < m_nAbsoluteLeft || y < m_nAbsoluteTop)
		return -1;
...
// 功能		: 窗口函数
// -------------------------------------------------------------------------
INT KUiSkillTree::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT x, y, nIndex;
	INT	nRet = 0;
	switch (uMsg)
...
void KUiSkillTree::PaintWindow()
{
	...
	for (INT i = 0; i < m_nNumSkills; i++)
	{
		...
		{
			if (m_bLeft == (UINT)ms_ShortcutSkills[j].IS_LEFT_SKILL &&
