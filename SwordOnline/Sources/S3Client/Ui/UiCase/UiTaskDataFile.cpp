...
INT										KTaskDataFile::ms_nSystemHeaderCount = 0;
	...
	while (File.Open(szFileName))
	{
		...
		for (INT i = 0; i < Header.nSystemRecordCount; i++)
		{
			...
			if (pNode)
			{
				...
				pNode->pNext = NULL;
				pNode->Record.bIsHeader = record.bIsHeader;
				pNode->Record.uHeaderID = record.uHeaderID;
				pNode->Record.nAct = record.nAct;
...
INT KTaskDataFile::GetSystemHeaderCount(INT nId/* = -1*/)
{
	INT nCount = 0;
	if (nId > -1)
	{
		for (INT j = 0; j < ms_nSystemRecordCount; j++)
		{
			const TASK_SYSTEM_RECORD* pRecord = GetSystemRecord(j);
			if (pRecord->uHeaderID == nId)
			{
				nCount++;
			}
		}
...
const TASK_SYSTEM_RECORD* KTaskDataFile::GetSystemRecord(INT nIndex)
{
	...
	{
		nIndex = ms_nSystemRecordCount - nIndex - 1;
	...
	if (pRecord)
	{
		if (pRecord->nContentLen > 0 && pRecord->pBuffer)
		{
			...
			if (pNode)
			{
				pNode->pNext = NULL;
				INT j = 0;
				for (j = 0; j < ms_nSystemRecordCount; j++)
				{
					const TASK_SYSTEM_RECORD* pRecordPrev = GetSystemRecord(j);
					if (pRecordPrev->uHeaderID == pRecord->uHeaderID)
					{
						pNode->Record.bIsHeader = false;
						break;
					}
				}
				if (j == ms_nSystemRecordCount)
				{
					pNode->Record.bIsHeader = true;
				}
				pNode->Record.uHeaderID = pRecord->uHeaderID;
				pNode->Record.nAct = pRecord->nAct;
...
void KTaskDataFile::AppendSystemRecord(KTaskSystemRecordNode* pNode)
{
	if (pNode)
	{
		...
		if (pNode->Record.bIsHeader)
			ms_nSystemHeaderCount++;
