...
//	¹¦ÄÜ£º´°¿ÚÏûÏ¢º¯Êý
//--------------------------------------------------------------------------
INT KUiBreakItem::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nRet = 0;
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_OkBtn)
		{
			...
			OnCancel();
		else if (uParam == (UINT)(KWndWindow*)&m_Increase)
			OnIncrease();
		else if (uParam == (UINT)(KWndWindow*)&m_Decrease)
			OnDecrease();
		else if (uParam == (UINT)(KWndWindow*)&m_CheckButton)
		{
...
void KUiBreakItem::OnOk()
{
	...
	{
		INT nNumber = m_Number.GetIntNumber();
		g_pCoreShell->BreakItem((UINT)(&m_ItemInfo), nNumber, m_CheckButton.IsButtonChecked());
	}
	CloseWindow(false);
}