...
#define	SCHEME_INI					"UiChatCentre.ini"
...
void KUiChatCentre::LoadScheme(class KIniFile* pIni)
{
	...
	m_Invisible.Init(pIni, "Invisible");
...
//	¹¦ÄÜ£º´°¿ÚÏûÏ¢º¯Êý
//--------------------------------------------------------------------------
INT KUiChatCentre::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nRet = 0;
	switch (uMsg)
	{
	...
	case WND_N_BUTTON_HOLD:
		if (uParam == (UINT)(KWndWindow*)&m_ScrollUpBtn)
			OnScrollList(true);
		else if (uParam == (UINT)(KWndWindow*)&m_ScrollDownBtn)
			OnScrollList(false);
		break;
	...
	case WND_M_MENUITEM_SELECTED:
		...
		{
			m_bPopupMenu = false;
			if (HIWORD(nParam) >= SEL_UNIT_MENU_SELECT_BASE && (int16_t)(LOWORD(nParam)) >= 0)
			{
				OnSelUnit(LOWORD(nParam));
			}
			else if (HIWORD(nParam) >= SEL_GROUP_MENU_SELECT_BASE && (int16_t)(LOWORD(nParam)) >= 0)
			{
				OnMoveFriendToGroup(HIWORD(nParam) - SEL_GROUP_MENU_SELECT_BASE, LOWORD(nParam));
				CancelMenu();
			}
			else if (HIWORD(nParam) >= FRIEND_MENU_SELECT_BASE)
				OnSelFriendMenu(HIWORD(nParam) - FRIEND_MENU_SELECT_BASE, LOWORD(nParam));
			else
				OnSelGroupMenu(HIWORD(nParam) - GROUP_MENU_SELECT_BASE, LOWORD(nParam));
		}
		break;
	...
	case WND_N_CHILD_MOVE:
		if (uParam == (UINT)(KWndWindow*)&m_TailImg)
			DragWndSize(nParam);
		break;
	default:
...
void KUiChatCentre::UpdateData(UICHATCENTRE_UPDATE eFlag, UINT uParam, INT nGroupIndex)
{
	if (m_pSelf)
	{
		switch (eFlag)
...
void KUiChatCentre::MoveGroupToGroup(INT nUnitIndex, const char* group, const char* group2, bool bDelgroup)
{
	if (m_pSelf)
	{
		...
		if (bDelgroup)
		{
			if (nSrcIndex != 0)	//0ÊÇÃ¿¸öUnitµÄÄ¬ÈÏ×é,²»ÄÜÉ¾³ý
			{
...
void KUiChatCentre::OnClickButton(KWndWindow* pWnd)
{
	...
	else if (pWnd == (KWndWindow*)&m_UnitBtnBlackList)
		OnSelUnit(BLACKLIST_INDEX);
...
void KUiChatCentre::PopupFriendMenu(INT nFriendIndex)
{
	...
	if (m_nNumValidGroup <= 1)
		pMenuData->nNumItem--;		//ÒÆ¶¯ºÃÓÑµ½ÆäËü×éÎÞÐ§
...
void KUiChatCentre::UpdateGroup(INT nGroupIndex)
{
	...
	if (nGroupIndex >= 0 && nGroupIndex < 10)
	{
...
void KUiChatCentre::UpdateInterface()
{
	...
	m_UnitBtnBlackList.CheckButton(0);
	...
	else if (m_nCurentFriendUnit == BLACKLIST_INDEX)
		m_UnitBtnBlackList.CheckButton(1);
	else
	...
	//¼ÆËãºÃÓÑÁÐ±íÏÔÊ¾µÄÐÐÊý£¬ÖÁÉÙÒªÏÔÊ¾Ò»ÐÐ
	...
	//µ÷ÕûÈ·ÈÏ±»ÏÔÊ¾µÄµÚÒ»ÐÐÄÚÈÝµÄÎ»ÖÃ
	...
	INT nTop = 0;	//nTopÓÃÓÚ±íÊ¾ÒÑÏÔÊ¾ÄÚÈÝµÄ¸ß¶È
	...
	//===´¦ÀíºÃÓÑÁÐ±íµÄÏÔÊ¾===
	INT nNumFriendSkip = 0;	//Í·Ç°ÒªÌø¹ýÏÔÊ¾µÄºÃÓÑµÄÊýÄ¿
	nHeight = 0;	//´Ó´Ë´¦¿ªÊ¼£¬nHeight±íÊ¾ºÃÓÑÁÐ±í´°¿ÚµÄÏÔÊ¾¸ß¶È
	if (nLimitLine == 0)
	{
