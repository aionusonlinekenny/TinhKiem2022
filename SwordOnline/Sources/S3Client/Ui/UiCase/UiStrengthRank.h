...
public:
	...
	static void   CloseWindow(bool bDestory = TRUE);//关闭窗口，同
	static void   LoadScheme(const char* pScheme);  //载入界面方案

	//新的排名数据来了
	static void   NewRankArrive(UINT uParam, KRankMessage* pRankData);
	//新的索引数据来了
	static void   NewIndexArrive(UINT uParam, struct KRankIndex* pIndex, bool command = false);
...
private:
	...
	static void   StoreNewDataToTempFile(UINT uParam,  //把新收到的排名资料存到临时文件
		KRankMessage* pInData);
	static void   StoreNewDataToTempFile(UINT uParam,  //把新收到的索引资料存到临时文件
		KRankIndex* pInData);
...
//	void   SendGetIndexToCore();                //功能同上，从Core那里获取索引数据
	...
	static void   AddRank(UINT uParam, KRankMessage* pRankData);
	static void   AddIndex(UINT uParam, struct KRankIndex* pIndex, bool command = false);
...
private:
	...
	uint16_t            m_usUpPic;            //上升的图片
	uint16_t            m_usSteadyPic;        //持平的图片
	uint16_t            m_usDownPic;          //下降的图片

	INT						  m_nLastIndex;
	UINT* m_pLastGettimes;
	UINT              m_uLastGetIndexTime;
	UINT             m_uLastGetRankTime;
