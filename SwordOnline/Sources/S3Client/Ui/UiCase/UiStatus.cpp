...
void KUiStatus::SwitchExpand(BOOL bShow)
{
	...
	/*for (INT i = UIEP_MASK; i <= UIEP_MANTLE; i++)
...
//	¹¦ÄÜ£º´°¿Úº¯Êý
//--------------------------------------------------------------------------
INT KUiStatus::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nRet = 0;
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_Close && !g_UiBase.GetStatus())
			CloseWindow(false);
		else if (uParam == (UINT)(KWndWindow*)&m_OpenItemPad)
			KShortcutKeyCentre::ExcuteScript(SCK_SHORTCUT_ITEMS);

		else if (uParam == (UINT)(KWndWindow*)&m_Avatar)
		{
			if (g_pCoreShell->GetGameData(GDI_PLAYER_IS_MALE, 0, 0))
			{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_BtnBind && g_pCoreShell->GetTradeState() == 0)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_BtnUnBind && g_pCoreShell->GetTradeState() == 0)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_EquipExpandBtn)
			SwitchExpand(m_EquipExpandBtn.IsButtonChecked());
		else if (uParam == (UINT)(KWndWindow*)&m_MaskFeature)
		...
		{
			if (uParam == (UINT)(KWndWindow*)&m_AddStrength)
			{
			...
			else if (uParam == (UINT)(KWndWindow*)&m_AddDexterity)
			{
				if (GetKeyState(VK_CONTROL) & 0x8000)
			...
			else if (uParam == (UINT)(KWndWindow*)&m_AddVitality)
			{
				if (GetKeyState(VK_CONTROL) & 0x8000)
			...
			else if (uParam == (UINT)(KWndWindow*)&m_AddEnergy)
			{
				if (GetKeyState(VK_CONTROL) & 0x8000)
...
//	¹¦ÄÜ£ºÉý¼¶Ä³ÏîÊôÐÔ
//--------------------------------------------------------------------------
void KUiStatus::UseRemainPoint(INT ntype, LONG nPoint)
{
	if (g_pCoreShell)
...
void KUiStatus::UpdateAllEquips()
{
	...
	INT nCount = g_pCoreShell->GetGameData(GDI_EQUIPMENT, (UINT)&Equips, 0);
	INT	i;
...
void KUiStatus::UpdateRuntimeAttribute(KUiPlayerAttribute* pInfo)
{
	if (pInfo && g_pCoreShell)
	{
		...
		m_AttackSpeed.Set6IntText((LONG)pInfo->nAttackSpeed, (LONG)pInfo->nCastSpeed);
...
void KUiStatus::OnEquiptChanged(ITEM_PICKDROP_PLACE* pPickPos, ITEM_PICKDROP_PLACE* pDropPos)
{
	...
	if (eStatus == UIS_S_TRADE_REPAIR)
	{
		...
		g_pCoreShell->OperationRequest(GOI_LOCKITEM, (UINT)(&Pick), 1);
	else if (eStatus == UIS_S_UNLOCK_ITEM)
		g_pCoreShell->OperationRequest(GOI_LOCKITEM, (UINT)(&Pick), 0);
	else
	{
