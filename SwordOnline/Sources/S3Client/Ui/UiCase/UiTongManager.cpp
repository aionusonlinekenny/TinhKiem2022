...
KUiTongManager::KUiTongManager()
{
	...
	m_szConfirm[0] = 0;
	m_szCancel[0] = 0;
	m_Relation = enumTONG_FIGURE_MEMBER;
}
...
void KUiTongManager::ArrangeComposition(char* pszPlayerName)
{
	if (g_pCoreShell)
	{
		...
		else
		{
			...
			g_pCoreShell->GetGameData(GDI_PLAYER_BASE_INFO, (UINT)&Me, 0);
		...
		if (m_nIndex >= 0)
		{
			...
			g_pCoreShell->TongOperation(GTOI_REQUEST_PLAYER_TONG, (UINT)&Player, (INT)TRUE);
		}
		else
		{
			CloseWindow();
...
void KUiTongManager::LoadScheme(const char* pScheme)
{
	if (ms_pSelf)
	{
		...
		if (Ini.Load(Buff))
		{
			...
			ms_pSelf->m_List.Init(&Ini, "ListRight");
...
* ¹¦ÄÜ£º´°¿Úº¯Êý
**********************************************************************/
INT KUiTongManager::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WND_N_LIST_ITEM_SEL:
		if (uParam == (UINT)&m_List)
		{
	...
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)&m_BtnPrev)
		{
			m_nCurrentPage--;
			UpdatePageofList();
		}
		else if (uParam == (UINT)&m_BtnNext)
		{
			m_nCurrentPage++;
			UpdatePageofList();
		}
		else if (uParam == (UINT)&m_BtnChangeCamp)
		{
		...
		else if (uParam == (UINT)&m_BtnDismiss)
		{
			PopupConfirmWindow(m_szDismiss, RESULT_T_DISMISS);
		}
		else if (uParam == (UINT)&m_BtnAssign)
		{
			KPilgarlicItem* pNode = GetSelectedPlayer();
		...
		else if (uParam == (UINT)&m_BtnDemise)
		{
			PopupConfirmWindow(m_szDemise, RESULT_T_DEMISE);
		}
		else if (uParam == (UINT)&m_BtnLeave)
		{
			PopupConfirmWindow(m_szLeave, RESULT_T_LEAVE);
		}
		else if (uParam == (UINT)&m_BtnRecruit)
		{
			if (m_bOtherTong)
		...
		else if (uParam == (UINT)&m_BtnApply)
		{
			OnAction(TONG_ACTION_APPLY);
		}
		else if (uParam == (UINT)&m_BtnCancel || uParam == (UINT)&m_BtnClose)
		{
			CloseWindow();
		}
		else if (uParam == (UINT)&m_BtnRefresh)
		{
			OnRefresh();
		}
		else if (uParam == (UINT)&m_BtnElderList)
		{
		...
		else if (uParam == (UINT)&m_BtnCaptainList)
		{
		...
		else if (uParam == (UINT)&m_BtnMemberList)
		{
	...
	case WND_M_OTHER_WORK_RESULT:
		...
		else if (uParam == RESULT_T_SAVEJIYU)
		{
			if (nParam == 0)
			{
				...
				g_pCoreShell->TongOperation(GTOI_TONG_CHANGE_CREDIT, (UINT)&Info, 0);
			}
		}
	...
	case WND_M_MENUITEM_SELECTED:
		if (uParam == (UINT)(KWndWindow*)this)
		{
			...
			int16_t j = (int16_t)(LOWORD(nParam));
			if (i >= 0 && i < MAX_TONGJIYU && j >= 0 && j < 8)
			{
...
void KUiTongManager::UpdatePageofList()
{
	...
	{
	case enumTONG_FIGURE_DIRECTOR:
		if (m_List.GetVisibleLineCount() > 0 && m_TongData.nDirectorCount / m_List.GetVisibleLineCount())
			nMax = m_TongData.nDirectorCount / m_List.GetVisibleLineCount();
		break;

	case enumTONG_FIGURE_MANAGER:
		if (m_List.GetVisibleLineCount() > 0 && m_TongData.nManagerCount / m_List.GetVisibleLineCount())
			nMax = m_TongData.nManagerCount / m_List.GetVisibleLineCount();
		break;

	case enumTONG_FIGURE_MEMBER:
		if (m_List.GetVisibleLineCount() > 0 && m_TongData.nMemberCount / m_List.GetVisibleLineCount())
...
* ¹¦ÄÜ£ºµ¯³öÈ·ÈÏ´°¿Ú
**********************************************************************/
void KUiTongManager::PopupConfirmWindow(const char* pszInfo, UINT uHandleID)
{
	...
	if (ParamInfo->uParam)
	{
		*pNum += ParamInfo->uParam;
		if (!strcmp(m_TongData.szName, ParamInfo->szName) && ms_pSelf)
		{
			...
			for (UINT i = 0; i < ParamInfo->uParam; i++)
			{
				if (!pIncome[i].Name[0])
				{
					continue;
...
INT KUiTongManager::IsPlayerExist(KPilgarlicItem* MeToCmp, KLinkStruct<KPilgarlicItem>* pData)
{
	...
	{
		if (pNode != NULL && pNode->nHashID == nID)
		{
...
void KUiTongManager::TongInfoArrive(KUiPlayerRelationWithOther* Relation, KTongInfo* pTongInfo)
{
	if (ms_pSelf)
	{
		if (Relation)
		{
			...
			{
				g_pCoreShell->TongOperation(GTOI_TONG_PARAM, (UINT)&m_TongParam, 0);
...
void KUiTongManager::RequestData(INT nType, INT nIndex)
{
	if (g_pCoreShell)
	{
		...
		g_pCoreShell->TongOperation(GTOI_REQUEST_TONG_DATA, (UINT)&ParamInfo, 0);
	}
}
...
void KUiTongManager::OnAction(INT nType)
{
	if (g_pCoreShell)
	{
		...
		g_pCoreShell->TongOperation(GTOI_TONG_ACTION, (UINT)&Param, (INT)&Player);
		memset(m_szData, 0, sizeof(m_szData));
	}
}
...
void KUiTongManager::OnMoney(INT ntype, INT nMoney, INT nSelect)
{
	if (g_pCoreShell)
	{
		...
		Param.nData = nMoney;
		Param.uId = 0;
		g_pCoreShell->TongOperation(GTOI_TONG_MONEY_ACTION, (UINT)&Param, ntype);
	}
}

ULONG StringToHash(const char* pString, BOOL bIsCaseSensitive)
{
