...
unsigned int m_uLastReconnectTime = 3000; // NEW: d? tránh reconnect quá nhanh
void KReconnectWnd::Breathe()
{
    ...
    if (m_bWaitToReconnect) {
        if (m_nChangeWorldReconnectTimes < 2) {
            StartReconnect();
            m_nChangeWorldReconnectTimes++;
            return;
        }

        unsigned int uRemain = IR_GetRemainTime(m_uToWaitTime, m_uWaitStartTime);
        if (uRemain) {
            if (m_nHideTimes == 0) {
        ...
        return; // ?? Ph?i return n?u dang ch? reconnect, tránh ch?y ti?p bên du?i
    }

    // === Ðây là ph?n b?n dán sai — nay dã dúng v? trí trong hàm ===
    ...
    case LL_S_IDLE:
    {
        ...
        if (eResult == LL_R_ACCOUNT_NOT_ENOUGH_POINT) {
            m_bStop = true;
            pReconnectMsg = "Kh«ng ®ñ thêi gian ®Ó ch¬i! H·y n¹p card ®Ó tiÕp tôc.";
        }
        else if (eResult == LL_R_ACCOUNT_FREEZE) {
            m_bStop = true;
            pReconnectMsg = "Tµi kho¶n nµy ®ang bÞ khãa.";
        }
        else {
            m_bWaitToReconnect = true;
            m_uWaitStartTime = IR_GetCurrentTime();
    ...
    case LL_S_ACCOUNT_CONFIRMING:
        pReconnectMsg = "§ang kiÓm tra tµi kho¶n vµ mËt khÈu.";
        break;
    ...
    if (m_bStop) {
        ...
        nInfoLen = sprintf(szInfo, MSG_RECONNECTING, m_nReconnectTimes + 1, pReconnectMsg);
        nInfoLen = TEncodeText(szInfo, nInfoLen);
...
int KReconnectWnd::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
    ...
    {
        if (nParam == 1)    // reconnect thành công
        {
            StartReconnect();
            Breathe();
        }
