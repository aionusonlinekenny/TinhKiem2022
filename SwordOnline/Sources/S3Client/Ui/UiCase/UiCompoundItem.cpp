...
INT KUiComItem::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)&m_Close)
		{
			CloseWindow();
		}
		else if (uParam == (UINT)&m_UpPropMineBtn && m_UpCryoliteBtn.IsVisible())
		{
			m_nNum = WINDOWS_COMP3;
			ShowWindow(0);
			m_CompoundPad.SetPosText(3);
		}
		else if (uParam == (UINT)&m_UpCryoliteBtn && m_UpCryoliteBtn.IsVisible())
		{
			m_nNum = WINDOWS_COMP2;
			ShowWindow(0);
			m_CompoundPad.SetPosText(2);
		}
		else if (uParam == (UINT)&m_CompoundPadBtn && m_UpCryoliteBtn.IsVisible())
		{
			m_nNum = WINDOWS_COMP;
			ShowWindow(0);
		...
		else if (uParam == (UINT)&m_DistillPadBtn)
		{
			m_nNum = WINDOWS_DISTill;
			ShowWindow(1);
		}
		else if (uParam == (UINT)&m_ForgePadBtn)
		{
			m_nNum = WINDOWS_FORG;
			ShowWindow(2);
		}
		else if (uParam == (UINT)&m_EnchasePadBtn)
		{
			m_nNum = WINDOWS_ENCHASE;
			ShowWindow(3);
		}
		else if (uParam == (UINT)&m_AtlasPadBtn)
		{
...
void KUiComItem::ComItem(UINT pItem, INT nWindowNum, INT nNum)
{
	switch (nWindowNum)
	{
	case WINDOWS_COMP:
...
INT KUiCompound::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WND_N_SCORLLBAR_POS_CHANGED:
		if (uParam == (UINT)&m_ListScroll)
		{
			m_Guide.SetFirstShowLine(nParam);
		}
		break;
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)&m_Cancle)
		{
			CleanItem();
		}
		else if (uParam == (UINT)&m_Compound)
		{
			...
			PlayEffect();
			pSelf->ComItem((UINT)(&pUP), m_nSelect, 3);
			CleanItem();
		}
		break;
...
INT KUiDistill::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WND_N_SCORLLBAR_POS_CHANGED:
...
INT KUiForge::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WND_N_SCORLLBAR_POS_CHANGED:
	...
	case WND_N_BUTTON_CLICK:
		...
		else if (uParam == (UINT)&m_ForgeBtn)
		{
			INT nNum = 0;
...
INT KUiEnchaseTim::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WND_N_SCORLLBAR_POS_CHANGED:
...
INT KUiAtlas::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	return 1;
}
