...
INT KUiTaskNote::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nResult = false;
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		...
		else if (uParam == (UINT)(KWndWindow*)&m_BtnGut)
		{
			UpdateData(0);
			nResult = true;
		}
		else if (uParam == (UINT)(KWndWindow*)&m_BtnTask)
		{
			UpdateData(1);
			nResult = true;
		}
		else if (uParam == (UINT)(KWndWindow*)&m_BtnEvent)
		{
			UpdateData(2);
			nResult = true;
		}
		else
		{
			for (INT i = 0; i < MAX_TASK_ON_PAGE; i++)
			{
				if (uParam == (UINT)(KWndWindow*)&m_TBtnTaskSel[i].btn)
				{
					UpdateList(i);
					break;
				}
	...
	if (nSel >= 0 && nSel < MAX_TASK_PAGE)
	{
		...
		if (nCnt > 0)
		{
			...
			for (i = 0; i < nCnt; i++)
			{
				...
				{
					if (pRecord->uHeaderID >= (nSel * MAX_TASK_ON_PAGE) && pRecord->uHeaderID < ((nSel + 1) * MAX_TASK_ON_PAGE) && pRecord->uHeaderID >= 0 && pRecord->uHeaderID < 36)
					{
...
INT KUiMissionNote::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nResult = false;
