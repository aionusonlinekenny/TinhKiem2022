...
void KUiStrengthRank::ArrangeDefaultData()
{
	...
	if (Ini.Load(INDEX_AND_RANK_MAPPING))
	{
		...
		m_pCurrentIndexNode = NULL;
		m_RankData.Clear();
		Ini.GetInteger("Main", "List", 0, &nCount);
		...
		if (pIndex)
		{
			...
			{
				sprintf(szSec, "%d", i);
				Ini.GetInteger(szSec, "ID", -1, &j);
				pIndex[i].usIndexId = (uint16_t)j;
...
void KUiStrengthRank::Initialize()
{
	...
	{
		m_pLastGettimes = new UINT[m_IndexData.GetCount()];
		if (m_pLastGettimes)
...
*功能：响应窗口消息的窗口函数
*****************************************************************************/
INT	KUiStrengthRank::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nRet = 0;
	switch (uMsg)
	{
	case WND_N_LIST_ITEM_SEL:
		if (uParam == (UINT)(KWndWindow*)&m_IndexList)
		{
			INT nIndex = nParam;
			if (nIndex < 0)
			{
	...
	case WND_N_SCORLLBAR_POS_CHANGED:
		if (uParam == (UINT)(KWndWindow*)&m_IndexScroll)
			m_IndexList.SetTopItemIndex(nParam);
		else if (uParam == (UINT)(KWndWindow*)&m_RankScroll)
			m_RankList.SetFirstShowLine(nParam);
		break;

	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_BtnClose)
			CloseWindow(false);
		break;
...
void KUiStrengthRank::SendGetRankToCore(INT nID)
{
	if (g_pCoreShell && m_pLastGettimes && IR_GetCurrentTime() - m_pLastGettimes[m_nLastIndex] > GET_RANK_TIME_OUT)
	{
		...
		g_pCoreShell->OperationRequest(GOI_QUERY_RANK_INFORMATION, (UINT)nID, 0);
	}
}
...
void KUiStrengthRank::ConstructIndexView()
{
	...
	for (i = 0; i < nCount; i++)
	{
		m_IndexList.AddString(i, pNode->szMsg, pNode->nNode > 0);
	...
	if (m_pCurrentIndexNode && (m_pCurrentIndexNode->bValueAppened || m_pCurrentIndexNode->bSortFlag))
	{
		...
		for (i = 0; i < nCount; i++)
		{
			...
			if (m_pCurrentIndexNode->bValueAppened)
			{
				...
				{
					INT nTranslifeCount = 0;

					if (pNode->nValueAppend > 200)
						nTranslifeCount = (INT)pNode->nValueAppend / MAX_LEVEL;

					if (m_pCurrentIndexNode->usIndexId == enumTopTenMasterHand || (m_pCurrentIndexNode->usIndexId >= enumFacTopTenMasterHand && m_pCurrentIndexNode->usIndexId < enumFacTopTenRicher))
						sprintf(szBuf, "TS %d C蕄 %d", nTranslifeCount, pNode->nValueAppend % MAX_LEVEL + (pNode->nValueAppend % MAX_LEVEL == 0 ? MAX_LEVEL : 0));
					else if (m_pCurrentIndexNode->usIndexId == enumTopTenRicher || (m_pCurrentIndexNode->usIndexId >= enumFacTopTenRicher))
					{
						if (pNode->nValueAppend > MONEY_FLOOR)
							sprintf(szBuf, "%d v筺 lng", pNode->nValueAppend / MONEY_FLOOR);
						else
							sprintf(szBuf, "%d lng", pNode->nValueAppend);
					}
					else
			...
			if (m_pCurrentIndexNode->bSortFlag)
			{
				...
				if (pNode->cSortFlag > 0)
				{
					*((int16_t*)(&pNode->szMsg[pNode->usMsgLen + 1])) = m_usUpPic;
				}
				else if (pNode->cSortFlag < 0)
				{
					*((int16_t*)(&pNode->szMsg[pNode->usMsgLen + 1])) = m_usDownPic;
				}
				else
				{
					*((int16_t*)(&pNode->szMsg[pNode->usMsgLen + 1])) = m_usSteadyPic;
				}
				pNode->usMsgLen += 3;
			}
			//展开
...
*功能：新的索引消息来了
*****************************************************************************/
void KUiStrengthRank::NewIndexArrive(UINT uParam, struct KRankIndex* pIndex, bool command)
{
	if (m_pSelf)
	{
		...
		if (uParam > 0)
		{
			m_pSelf->m_pLastGettimes = new UINT[uParam];
			if (m_pSelf->m_pLastGettimes)
			{
				memset(m_pSelf->m_pLastGettimes, 0, sizeof(UINT) * uParam);
			}
		}
	}
	AddIndex(uParam, pIndex, command);
...
*功能：新的排名信息来了
*****************************************************************************/
void KUiStrengthRank::NewRankArrive(UINT uParam, KRankMessage* pRankData)
{
...
*功能：添加索引信息
*****************************************************************************/
void KUiStrengthRank::AddIndex(UINT uParam, struct KRankIndex* pIndex, bool command)
{
	KRankIndex* pIncome;
	KRankIndexNode Node;
	char szSec[32];
	...
	for (i = 0; i < nCount; i++)
	{
		...
		if (command)
			sprintf(szSec, "%d", pIncome->usIndexId);
		else
			sprintf(szSec, "%d", i);

		m_IndexIDIni.GetString(szSec, "Name", "", Node.szMsg,
		...
		Node.usIndexId = nValue;
		if (command)
			Node.nNode = 1;
		else
			m_IndexIDIni.GetInteger(szSec, "Node", 0, &Node.nNode);
...
*功能：添加排名信息
*****************************************************************************/
void KUiStrengthRank::AddRank(UINT uParam, KRankMessage* pRankData)
{
...
*功能：把新收到的索引资料存到临时文件
*****************************************************************************/
void KUiStrengthRank::StoreNewDataToTempFile(UINT uParam, KRankIndex* pInData)
{
	KIniFile Ini;
...
*功能：把新收到的排名资料存到临时文件
*****************************************************************************/
void KUiStrengthRank::StoreNewDataToTempFile(UINT uParam, KRankMessage* pInData)
{
	KIniFile Ini;
	KRankMessage* pIncome;
