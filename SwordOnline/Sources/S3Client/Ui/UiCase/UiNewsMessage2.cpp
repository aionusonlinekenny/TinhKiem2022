...
#define NEWS_MESSAGE_INI "UiNewsMessage2.ini"
...
KUiNewsMessage2* KUiNewsMessage2::m_pSelf = NULL;
BOOL			KUiNewsMessage2::ms_bEnableIdleMsg = TRUE;

/*********************************************************************************
*功能：构造
**********************************************************************************/
KUiNewsMessage2::KUiNewsMessage2()
{
	m_pHead = NULL;
	m_pHandling = NULL;
...
void KUiNewsMessage2::EnableIdleMsg(BOOL bEnable)
{
...
*功能：增加一条消息在队列中
**********************************************************************************/
bool KUiNewsMessage2::AddMessage(KNewsMessage* pMsg, UINT uTime, UINT uTimeParam)
{
	KNewsMessageNode1* pNew = NULL;
	if (pMsg)
	{
		pNew = (KNewsMessageNode1*)malloc(sizeof(KNewsMessageNode1));
		if (pNew)
		{
...
**********************************************************************************/
KNewsMessageNode1* KUiNewsMessage2::SeparateMsg()
{
	KNewsMessageNode1* pNode = m_pHandling;
	if (m_pHandling)
	{
...
// 把pNode所指的消息加到链表末端！
void KUiNewsMessage2::AddToTail(KNewsMessageNode1* pNode)
{
	if (pNode)
	{
		if (m_pHead)
		{
			KNewsMessageNode1* pPrev = m_pHead;
			while (pPrev->pNext)
				pPrev = pPrev->pNext;
			pPrev->pNext = pNode;
		}
...
*功能：打开窗口
**********************************************************************************/
KUiNewsMessage2* KUiNewsMessage2::OpenWindow()
{
	if (m_pSelf == NULL)
	{
		m_pSelf = new KUiNewsMessage2;
		if (m_pSelf)
		{
			m_pSelf->Initialize();
		}
...
*功能：如果窗口正被显示，则返回实例指针
**********************************************************************************/
KUiNewsMessage2* KUiNewsMessage2::GetIfVisible()
{
...
*功能：关闭窗口
**********************************************************************************/
void KUiNewsMessage2::CloseWindow(BOOL bDestory)
{
	if (m_pSelf)
	{
		if (bDestory == TRUE)
...
*功能：初始化
**********************************************************************************/
void KUiNewsMessage2::Initialize()
{
	if (ms_bEnableIdleMsg)
...
*功能：载入界面方案
**********************************************************************************/
void KUiNewsMessage2::LoadScheme(const char* pszScheme)
{
...
*功能：两个字 - 穿透！！！
**********************************************************************************/
INT KUiNewsMessage2::PtInWindow(INT x, INT y)
{
	return 0;
}

//新闻消息真的来了
void KUiNewsMessage2::MessageArrival(KNewsMessage* pMsg, SYSTEMTIME* pTime)
{
	UINT uTime;
	if (m_pSelf && pMsg &&
...
**********************************************************************************/
void KUiNewsMessage2::PaintWindow()
{
...
**********************************************************************************/
bool KUiNewsMessage2::PickAMessage()
{
	m_pHandling = NULL;
	UINT	uCurrentTime = IR_GetCurrentTime();
	...
	{
		KNewsMessageNode1* pNode = m_pHead;
		while (pNode)
		{
			...
			{
				m_pHandling = pNode;
				if ((++pNode->uShowTimes) > pNode->uTime)
					bTobeFree = true;
			}
...
**********************************************************************************/
void KUiNewsMessage2::PickFromIni()
{
	INT         i, nValue;
	char        szNum[16];
	KNewsMessage	Msg;
...
void KUiNewsMessage2::ConvertMsg()
{
	//_ASSERT(m_pHandling);
	//填充当前显示消息的内容
...
bool KUiNewsMessage2::MakeCountingMsg()
{
	//_ASSERT(m_pHandling &&
	//	m_pHandling->nType == NEWSMESSAGE_COUNTING &&
	//	m_nInsertPlace != NOT_NEED_INSERT);
	UINT uCurrent = IR_GetCurrentTime();
...
bool KUiNewsMessage2::ScrollMsg()
{
