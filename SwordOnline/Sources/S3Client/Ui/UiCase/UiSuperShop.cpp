...
INT	KUiShoppingCart::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	...
	{
		if (uParam == (UINT)(KWndWindow*)&m_ScrollBar)
			return 0;
	}
...
INT		KWndSellItem::WndProc(UINT uMsg, UINT uParam, INT nParam)//´°¿Úº¯Êý
{
	if (uMsg == WND_N_BUTTON_CLICK)
	{
		if ((KWndWindow*)uParam == (KWndWindow*)&m_MarketGoods_Buy)
		{
			...
			if (g_pCoreShell->GetGameData(GDI_TRADE_ITEM_PRICE, (UINT)(&Obj), (INT)(&Price)))
			{
	...
	if (m_pSelf)
	{
		if (m_pSelf->m_pItemInfo.Obj.uGenre == CGOG_NPCSELLITEM)
		{
			g_pCoreShell->OperationRequest(GOI_TRADE_NPC_BUY,
				(UINT)(&m_pSelf->m_pItemInfo), nNumber);
			PutItem(NULL);
		}
	}
}
...
void KUiSuperShop::UpdateData()
{
	...
	{
		m_nPageCount = g_pCoreShell->GetDataSuperShop(m_nCurrentShopId, (UINT)m_pObjsList, m_nObjCount) + 1;
		SetPage(0);
	}
	else
		m_nObjCount = 0;
...
INT	KUiSuperShop::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	if (uMsg == WND_N_BUTTON_CLICK)
	{
		for (INT i = 0; i < MAX_SUPERSHOP_PAGETAB; i++)
		...
		else if ((KWndWindow*)uParam == (KWndWindow*)&m_ShoppingCartBtn)
		{
			...
			{
				KUiItemBuySelInfo	Price = { 0 };
				if (g_pCoreShell->GetGameData(GDI_TRADE_ITEM_PRICE, (UINT)(&m_pItemInfo), (INT)(&Price)))
				{
...
//#define SCHEME_INI_VN		"UiSuperShop.ini"
...
//´°¿Úº¯Êý
INT	KUiDynamicShop::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
...
void KUiDynamicShop::OnBuyItem(KUiDraggedObject* pItem, bool bDoImmed)
{
	...
	if (bDoImmed == false)
	{
		...
		if (g_pCoreShell->GetGameData(GDI_TRADE_ITEM_PRICE,
			(UINT)(&Obj), (INT)(&Price)) && eStatus != UIS_S_TRADE_SALE && eStatus != UIS_S_TRADE_REPAIR)
		{
...
void KUiDynamicShop::UpdateData()
{
	...
	if (m_pObjsList = (KUiObjAtContRegion*)malloc(sizeof(KUiObjAtContRegion) * m_nObjCount))
	{
		g_pCoreShell->GetDataDynamicShop(m_nCurrentShopId, (UINT)m_pObjsList, m_nObjCount);//µ¥Ïß³ÌÖ´ÐÐ£¬nCountÖµ²»±ä
