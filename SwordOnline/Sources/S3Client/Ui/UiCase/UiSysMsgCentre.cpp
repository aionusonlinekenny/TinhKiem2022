...
//	¹¦ÄÜ£º´°¿Úº¯Êý
//--------------------------------------------------------------------------
INT KUiSysMsgCentre::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT i, nRet = 0;
	switch (uMsg)
	{
	...
	case WND_N_BUTTON_CLICK:
		for (i = 0; i < SMT_WAR; i++)
		{
			if (uParam == (UINT)(KWndWindow*)&m_MsgIconBtn[i])
			{
...
void KUiSysMsgCentre::OnConfirmOperFinished(UINT uParam, INT nSelAction)
{
	...
	case SMCT_UI_TEAM_INVITE:	//´ðÓ¦»ò¾Ü¾ø¼ÓÈë¶ÓÎéµÄÑûÇë
		if (m_pHandlingMsg)
		{
			...
			g_pCoreShell->TeamOperation(TEAM_OI_INVITE_RESPONSE,
				(UINT)pPlayer, (nSelAction == 0));	//nSelAction=0£º´ðÓ¦, nSelAction=1£º¾Ü¾ø
		}
		break;
