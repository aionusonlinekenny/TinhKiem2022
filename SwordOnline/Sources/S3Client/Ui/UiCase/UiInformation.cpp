...
void UIMessageBox(const char* pMsg,
	...
	INT nLeftTime /*= -1*/)
{
	g_UiInformation.Show(pMsg, pszFirstBtnText, pszSecondBtnText, pCaller, uParam, -1, nLeftTime);
}
...
KUiInformation::KUiInformation()
{
	...
	m_nCentreBtnXPos = 0;
	m_uLastShowMessage = 0;
	m_nLeftTime = 0;
	m_nTipModeFlagFlag = 0;
	m_uCallerParam = 0;
}
...
//	功能：窗口函数
//--------------------------------------------------------------------------
INT KUiInformation::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nRet = 0;
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_FirstBtn)
			Hide(0);
		else if (uParam == (UINT)(KWndWindow*)&m_SecondBtn)
			Hide(1);
		break;
	case WM_KEYDOWN:
