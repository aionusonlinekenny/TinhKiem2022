...
#define	SCHEME_INI_SELSERV			"UiSelNativePlace.ini"
...
//	功能：窗口函数
//--------------------------------------------------------------------------
INT KUiSelNativePlace::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT	nRet = 0;
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_Ok)
			OnOk();
		else if (uParam == (UINT)(KWndWindow*)&m_Cancel)
			OnCancel();
		break;
	case WM_KEYDOWN:
		if (uParam == VK_RETURN)
	...
	case WND_N_LIST_ITEM_D_CLICK:
		if (nParam >= 0 && uParam == (UINT)(KWndWindow*)&m_List)
			OnOk();
		break;
	default:
