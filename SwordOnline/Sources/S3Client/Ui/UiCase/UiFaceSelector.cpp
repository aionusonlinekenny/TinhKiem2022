		...
		if (nCount > 0)
		{
			...
			if (ms_pFaceList)
			{
				...
				for (i = 0; i < nCount; i++)
				{
					...
					Ini.GetString(szSection, "Image", "", ms_pFaceList[i].szImage,
						sizeof(ms_pFaceList[i].szImage));
...
void KUiFaceSelector::OnSelFace()
{
	if (m_pCallerWnd && m_nCurrIndex >= 0 && m_nCurrIndex < ms_nNumFaces)
	{
		m_pCallerWnd->WndProc(WND_M_OTHER_WORK_RESULT,
			(UINT)m_pvCallerParam, (INT)ms_pFaceList[m_nCurrIndex].szFaceText);
	}
	CloseWindow(false);
}

INT KUiFaceSelector::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nResult = false;
	switch (uMsg)
	{
	...
	case WM_MOUSEMOVE:
	{
		INT x = (INT)(int16_t)(LOWORD(nParam));
		INT y = (INT)(int16_t)(HIWORD(nParam));
...
void KUiFaceSelector::PaintWindow()
{
	...
	// draw the face pic
	INT					nIndex = 0, nNumH = 0, nStartX = 0;
	char				cBuffer[8] = {};
