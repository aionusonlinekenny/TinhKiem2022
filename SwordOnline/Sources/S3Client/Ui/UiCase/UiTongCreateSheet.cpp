...
* 功能：为任命案的书写安排数据
**********************************************************************/
void KUiTongCreateSheet::ArrangeData(const char* szTongName, INT nCurCamp, UINT uHandleID)
{
	if (ms_pSelf)
	{
		switch (nCurCamp)
...
* 功能：窗口函数
**********************************************************************/
INT KUiTongCreateSheet::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)&m_BtnOrder)
		{
			AlignmentButtonCheck(camp_justice);
		}
		else if (uParam == (UINT)&m_BtnChaos)
		{
			AlignmentButtonCheck(camp_evil);
		}
		else if (uParam == (UINT)&m_BtnNatural)
		{
			AlignmentButtonCheck(camp_balance);
		}
		/*----------------------------------------------------------*/
		else if (uParam == (UINT)&m_BtnCancel)
		{
			if (m_pMain)
			{
				m_pMain->Show();
...
void KUiTongCreateSheet::OnDone()
{
	if (m_uHandleID == RESULT_T_CREATE)
	{
		...
		if (szName[0] && g_pCoreShell)
		{
			if (m_nSelectSide != -1)
			{
				...
				KTongParam	Param;
				g_pCoreShell->TongOperation(GTOI_TONG_PARAM, (UINT)&Param, 0);
				INT nRet = g_pCoreShell->TongOperation(GTOI_TONG_CREATE, (UINT)szName, (INT)m_nSelectSide);
				switch (nRet)
				{
	...
	else
	{
		if (m_nSelectSide != -1)
		{
			...
			{
				m_pMain->Show();
				m_pMain->WndProc(WND_M_OTHER_WORK_RESULT, m_uHandleID, (UINT)m_nSelectSide);
				m_pMain = NULL;
			}
			CloseWindow(true);
		}
