...
INT KUiChatItem::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nRet = 0;
	switch (uMsg)
	{
...
void KUiChatItem::UpdateData(INT nIdx)
{
	if (nIdx > 0)
	{
		...
		g_pCoreShell->GetGameData(GDI_ITEM_DESC, (UINT)&m_pItemDesc, nIdx);
		...
		INT nLeft = nResolutionW / 2 - m_Width / 2;
		INT nTop = nResolutionH / 2 - m_Height / 2;
