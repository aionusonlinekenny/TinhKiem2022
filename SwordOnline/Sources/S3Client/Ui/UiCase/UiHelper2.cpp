...
//窗口函数
INT KUiHelper2::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nRet = 0;
	switch (uMsg)
	{
	...
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_BtnClose)
			CloseWindow(false);
		else if (uParam == (UINT)(KWndWindow*)&m_BtnPic)
		{
			CloseWindow(false);
			KUiHelper::SelectInterfacePic();
			KUiHelper::OpenWindow(true);
		}
		else if (uParam == (UINT)(KWndWindow*)&m_BtnKeyboard)
		{
			CloseWindow(false);
			KUiHelper::SelectKeyboardPic();
			KUiHelper::OpenWindow(true);
		}
		else if (uParam == (UINT)(KWndWindow*)&m_BtnWuxing)
		{
			CloseWindow(false);
	...
	case WND_N_SCORLLBAR_POS_CHANGED:
		...
		else if (uParam == (UINT)(KWndWindow*)&m_MessageScroll)
	...
	case WM_KEYDOWN:
	{
		INT nCurIdx = 0;
		INT nTotal = 0;
		//			if (nParam == (UINT)(KWndWindow*)&m_IndexList)
		//			{
		...
		//			}
		//			else if (nParam == (UINT)(KWndWindow*)&m_MessageList)
		//			{
