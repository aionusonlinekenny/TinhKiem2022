...
public:
	virtual INT   WndProc(UINT uMsg, UINT uParam, INT nParam);
...
private:
	...
	void          PopupConfirmWindow(                //弹出确认窗口
		const char* pszInfo, UINT uHandleID);
...
private:
	...
	KWndText32    m_TitleTongInfo, m_TitleTongNotify, m_TongName, m_MasterName;          //帮会名字，老大名字(五月)
	KWndText32    m_Alignment;                       //阵营名字
	KWndText32    m_MemberCount, m_Money, m_TongLevel, m_TotalEff;            //人数，资金

	KWndList2     m_List;                            //中间的列表控件

#define MAX_TONGJIYU 5
	TONG_MEMBER_FIGURE		          m_Relation;                        //打开这个界面的人与目标帮会的关系
...
private:
	UINT  m_uLastRequestTongDataTime;        //上一次请求帮会资料的时间
	UINT  m_uLastRequestElderListTime;       //上一次请求长老列表的时间
	UINT  m_uLastRequestMemberListTime;      //上一次请求帮众列表的时间
	UINT  m_uLastRequestCaptainListTime;     //上一次请求队长列表的时间
	...
	char          m_szDismiss[64], m_szAssign[128];   //踢人和任命的确认框的字符串
	char          m_szDemise[64], m_szLeave[256], m_szSaveJiyu[128];     //传位和离开的确认框的字符串
	char          m_szJustice[16], m_szBalance[16];  //正和中的描述串
	char          m_szEvil[16];                      //邪的描述串
	char          m_szConfirm[16], m_szCancel[16];   //确认窗口上，确认和取消的文字

	char          m_szData[32];                  //MD5编码之后的密码串
	char          m_szTargetPlayerName[32];          //操作对象玩家的名字
	bool		  m_bOtherTong;
};

ULONG StringToHash(const char* pString, BOOL bIsCaseSensitive = TRUE);
