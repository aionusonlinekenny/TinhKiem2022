...
// ¹¦ÄÜ	: ´°¿Úº¯Êý
// -------------------------------------------------------------------------
INT	KUiTrade::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_Cancel)
			OnCancelTrade();
		else if (uParam == (UINT)(KWndWindow*)&m_Ok)
		{
			OnOk(nParam);
		}
		else if (uParam == (UINT)(KWndWindow*)&m_Trade)
			OnTrade();
		else if (uParam == (UINT)(KWndWindow*)&m_AddMoney)
		{
			OnAdjustMoney(true);
			m_nAdjustingMoney = 1;
		}
		else if (uParam == (UINT)(KWndWindow*)&m_ReduceMoney)
		{
			OnAdjustMoney(false);
			m_nAdjustingMoney = 0;
		}
	...
	case WND_N_BUTTON_HOLD:
		if (uParam == (UINT)(KWndWindow*)&m_AddMoney)
		{
			OnAdjustMoney(true);
			m_nAdjustingMoney = 1;
		}
