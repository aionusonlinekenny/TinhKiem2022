...
//--------------------------------------------------------------------------
//--------------------------------------------------------------------------
INT KUiGive::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nRet = 0;
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_Confirm)
		{
			if (g_pCoreShell)
	...
	case WND_N_SCORLLBAR_POS_CHANGED:
		if (uParam == (UINT)(KWndWindow*)&m_ContentScroll)
...
void KUiGive::UpdateData(const char* szName, char* szInitString)
{
	...
	{
		g_pCoreShell->GetGameData(GDI_ITEM_GIVE, (UINT)pObjs, nCount);//单线程执行，nCount值不变
