...
//	功能：窗口函数
//--------------------------------------------------------------------------
INT KUiMiniMap::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nRet = 0;
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_SwitchBtn)
			MapSetMode(s_eMapMode == MINIMAP_M_BRIEF_PIC ? MINIMAP_M_BRIEF_PIC_BROWSE : MINIMAP_M_BRIEF_PIC);
		else if (uParam == (UINT)(KWndWindow*)&m_WorldMapBtn)
			MapSetMode(MINIMAP_M_WORLD_MAP);
		else if (uParam == (UINT)(KWndWindow*)&m_CaveMapBtn)
			MapSetMode(MINIMAP_M_CAVELIST_MAP);
		else if (uParam == (UINT)(KWndWindow*)&m_BtnFlag)
		{
			BOOL bFlag = g_pCoreShell->GetFlagMode();
			g_pCoreShell->SetFlagMode(!bFlag);
			g_pCoreShell->SetPaintMode(!bFlag);
		}
		else if (uParam == (UINT)(KWndWindow*)&m_ScenePos)
		{
	...
	case WND_N_CHILD_MOVE:
		if (uParam == (UINT)(KWndWindow*)&m_SceneName ||
			uParam == (UINT)(KWndWindow*)&m_ScenePos ||
			uParam == (UINT)(KWndWindow*)&m_Shadow)
		{
			KWndWindow* pWnd = (KWndWindow*)uParam;
			INT nMoveOffsetX = (int16_t)(nParam & 0xffff);
			INT nMoveOffsetY = (int16_t)((nParam >> 16) & 0xffff);
...
void KUiMiniMap::UpdateSceneTimeInfo(KUiSceneTimeInfo* pInfo)
{
	if (ms_pSelf && pInfo)
	{
		...
		if (g_pCoreShell->GetFlagMode() && g_pCoreShell->GetPaintMode())
		{
			...
			g_pCoreShell->SceneMapOperation(GSMOI_SCENE_TIME_INFO_OFTEN, (UINT)&Spot, 0);
...
void KUiMiniMap::MapScroll(INT nbScrollScene)
{
	...
	if (g_pCoreShell && g_pCoreShell->SceneMapOperation(GSMOI_SCENE_MAP_INFO, (UINT)&MapInfo, 0))
	{
		...
		g_pCoreShell->SceneMapOperation(GSMOI_SCENE_MAP_FOCUS_OFFSET, (UINT)nX, nY);
	}
}
