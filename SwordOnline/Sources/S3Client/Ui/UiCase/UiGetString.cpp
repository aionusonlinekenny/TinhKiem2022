...
KUiGetString* KUiGetString::OpenWindow(GET_STRING_ACTION eAction, const char* pszTitle,
	...
	KWndWindow* pRequester, UINT uParam,
...
void KUiGetString::Show()
{
	...
	{
		Left = nResolutionW / 2 - m_Width / 2;
		Top = nResolutionH / 2 - m_Height / 2;
...
//	¹¦ÄÜ£º´°¿ÚÏûÏ¢º¯Êý
//--------------------------------------------------------------------------
INT KUiGetString::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nRet = 0;
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_OkBtn)
			OnSend();
		else if (uParam == (UINT)(KWndWindow*)&m_CancelBtn)
			OnCancel();
		break;
	case WM_KEYDOWN:
		if (uParam == VK_RETURN)
...
void KUiGetString::OnSend()
{
	...
	if (nLen >= m_nMinLen && nLen <= m_nMaxLen)
	{
		...
		case GSA_PW:
			if (g_pCoreShell)
				g_pCoreShell->OperationRequest(GOI_UNLOCK, (UINT)&lpszInfo, 0);
			break;
		case GSA_INPUT:
			if (g_pCoreShell)
				g_pCoreShell->OperationRequest(GOI_INPUT, (UINT)&lpszInfo, 0);
			break;
		default:
			if (m_pRequester)
