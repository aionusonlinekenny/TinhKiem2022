...
void KUiExpandItem::UpdateData()
{
	...
	{
		g_pCoreShell->GetGameData(GDI_ITEM_EXPANDITEM, (UINT)pObjs, nCount);//µ•œﬂ≥Ã÷¥––£¨nCount÷µ≤ª±‰
...
// π¶ƒ‹	: ¥∞ø⁄∫Ø ˝
// -------------------------------------------------------------------------
INT KUiExpandItem::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	...
	else
	{
		...
		else if (g_UiBase.IsOperationEnable(UIS_O_USE_ITEM))
		{
			g_pCoreShell->OperationRequest(GOI_USE_ITEM,
				(UINT)(&Obj), UOC_ITEM_TAKE_WITH_EX);
		}
	}
}
