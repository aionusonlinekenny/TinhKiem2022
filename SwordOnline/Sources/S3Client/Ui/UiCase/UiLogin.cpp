...
//	功能：窗口函数
//--------------------------------------------------------------------------
INT KUiLogin::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT	nRet = 0;
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_Login)
			OnLogin();
	...
	case WND_N_EDIT_SPECIAL_KEY_DOWN:
		if (nParam == VK_TAB)
		{
			...
			{
				// 如果没有按住Shift
				if (uParam == (UINT)(KWndWindow*)&m_Account)
				{
					INT x, y, cx;
				...
				else if (uParam == (UINT)(KWndWindow*)&m_PassWord)
				{
...
void KUiLogin::OnKeyDown(UINT uKey)
{
	if (uKey == VK_RETURN)
	{
