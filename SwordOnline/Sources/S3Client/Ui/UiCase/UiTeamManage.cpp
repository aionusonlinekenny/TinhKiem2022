...
//	功能：窗口函数
//--------------------------------------------------------------------------
INT KUiTeamManage::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_btnInvite)
			OnInvite();
		else if (uParam == (UINT)(KWndWindow*)&m_btnKick)
			OnKick();
		//		else if (uParam == (UINT)(KWndWindow*)&m_btnApplyJoin)
		//			OnApplyJoin();
		if (uParam == (UINT)(KWndWindow*)&m_btnAppoint)
			OnAppoint();
		else if (uParam == (UINT)(KWndWindow*)&m_btnLeave ||
			uParam == (UINT)(KWndWindow*)&m_btnDismiss)
			OnLeave();
		else if (uParam == (UINT)(KWndWindow*)&m_btnRefresh)
			OnRefresh();
		//		else if (uParam == (UINT)(KWndWindow*)&m_NearbyPlayerTab)
		//			SwitchNearbyMode(true);
		//		else if (uParam == (UINT)(KWndWindow*)&m_NearbyTeamTab)
		//			SwitchNearbyMode(false);
		else if (uParam == (UINT)(KWndWindow*)&m_btnCloseTeam)
			g_pCoreShell->TeamOperation(TEAM_OI_CLOSE, 0, nParam);
		else if (uParam == (UINT)(KWndWindow*)&m_btnCancel)
			CloseWindow();
		break;
	case WND_N_LIST_ITEM_SEL:
		if (uParam == (UINT)(KWndWindow*)&m_TeamMember)
			OnSelTeamMember(nParam);
		else if (uParam == (UINT)(KWndWindow*)&m_NearbyList)
			OnSelNearby(nParam);
		break;
	case WND_N_SCORLLBAR_POS_CHANGED:
		if (uParam == (UINT)(KWndWindow*)&m_MemberScroll)
			m_TeamMember.SetTopItemIndex(nParam);
		else if (uParam == (UINT)(KWndWindow*)&m_NearbyScroll)
...
void KUiTeamManage::UpdateData(KUiPlayerTeam* pInfo)
{
	...
	if (pInfo)
		m_Info = *pInfo;
	else if (!g_pCoreShell->TeamOperation(TEAM_OI_GD_INFO, (UINT)&m_Info, 0))
	{
	...
	if (m_Info.cNumMember > 0)
	{
		...
		if (m_pPlayersList)
		{
			INT nCount = g_pCoreShell->TeamOperation(TEAM_OI_GD_MEMBER_LIST,
				(UINT)m_pPlayersList, m_Info.cNumMember);
...
void KUiTeamManage::OnAppoint()
{
	...
	if (nCurSel >= 0)
		g_pCoreShell->TeamOperation(TEAM_OI_APPOINT, (UINT)&m_pPlayersList[nCurSel], 0);
}
...
void KUiTeamManage::OnKick()
{
	INT nCurSel = m_TeamMember.GetCurSel();
	if (nCurSel >= 0)
		g_pCoreShell->TeamOperation(TEAM_OI_KICK, (UINT)&m_pPlayersList[nCurSel], 0);
}
...
/*void KUiTeamManage::OnApplyJoin()
{
	...
	if (nCurSel >= 0)
	{
		g_pCoreShell->OperationRequest(GOI_TEAM_APPLY,
			(UINT)&m_pNearbyTeamsList[nCurSel], 0);
	}
}*/
...
void KUiTeamManage::OnInvite()
{
	...
	if (nCurSel >= 0)
	{
		...
		g_pCoreShell->TeamOperation(TEAM_OI_INVITE, (UINT)(&m_pNearbyPlayersList[nCurSel]), 0);
	}
}
...
void KUiTeamManage::OnRefresh()
{
	...
	//	if (m_NearbyPlayerTab.IsButtonChecked())
	{
		...
		if (nCount)
		{
			...
			if (m_pNearbyPlayersList)
			{
				g_pCoreShell->GetGameData(GDI_NEARBY_IDLE_PLAYER_LIST, (UINT)m_pNearbyPlayersList, nCount);
