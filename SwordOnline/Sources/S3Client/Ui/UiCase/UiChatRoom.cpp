...
//	π¶ƒ‹£∫¥∞ø⁄œ˚œ¢∫Ø ˝
//--------------------------------------------------------------------------
INT KUiChatRoom::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nRet = 0;
	switch (uMsg)
	{
	...
	case WND_N_LIST_ITEM_D_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_RoomList)
		{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_MemberList)
		{
			if (nParam >= 0)
			{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_BlackList)
		{
