...
void KUiStoreBox::UpdateData(INT nCurPage)
{
	...
	m_ItemBox.SetContainerId((INT)UOC_STORE_BOX);
	UINT uIndex = GDI_ITEM_IN_STORE_BOX;
	switch (m_nCurPage)
	{
	case REPOSITORY_ONE:
	...
	{
		g_pCoreShell->GetGameData(uIndex, (UINT)pObjs, nCount);//µ•œﬂ≥Ã÷¥––£¨nCount÷µ≤ª±‰
...
void KUiStoreBox::LoadScheme(const char* pScheme)
{
	...
	if (m_pSelf && Ini.Load(Buff))
	{
		...
		m_pSelf->m_CloseBtn.SetText("ß„ng");
		m_pSelf->m_GetMoneyBtn.SetText("L y ti“n");
		m_pSelf->m_UnlockBtn.SetText("MÎ kh„a");
	}
}
...
// π¶ƒ‹	: ¥∞ø⁄∫Ø ˝
// -------------------------------------------------------------------------
INT KUiStoreBox::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
	...
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_CloseBtn)
			CloseWindow();
		if (uParam == (UINT)(KWndWindow*)&m_UnlockBtn)
		{
			if (g_pCoreShell->GetLockState())
				KUiGetString::OpenWindow(GSA_PW, "NhÀp mÀt kh»u", "", this, 0, 0, 1, 16);
			else
		...
		else if (uParam == (UINT)(KWndWindow*)&m_PreBtn)
		{
			m_nCurPage--;
			if (m_nCurPage < -1)
			{
		...
		else if (uParam == (UINT)(KWndWindow*)&m_NextBtn)
		{
			m_nCurPage++;
