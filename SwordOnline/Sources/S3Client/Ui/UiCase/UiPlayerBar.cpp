...
{
	return sprintf(pString, "%02d:%02d %02d-%02d-%02d",
		ptm->tm_hour,
		ptm->tm_min,
		ptm->tm_mday,
		ptm->tm_mon + 1,
...
UINT s_CanFocusColor = 0;
UINT s_NotCanFocusColor = 0;
...
INT KUiPlayerBar::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT nRet = 0;
	switch (uMsg)
	{
	case WND_N_EDIT_RCLICK:
		if (uParam == (UINT)(KWndWindow*)(&m_InputEdit))
		{
			PopupPhraseMenu(LOWORD(nParam), HIWORD(nParam), true);
		}
		break;
	case WND_M_POPUPMENU:
		if (uParam == (UINT)(KWndWindow*)&m_ChannelSwitchBtn)
		{
			INT x, y;
			m_ChannelSwitchBtn.GetAbsolutePos(&x, &y);
			PopupChannelMenu(x, y);
		}
		else if (uParam == (UINT)(KWndWindow*)&m_ChannelOpenBtn)
		{
	...
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_Friend)
			KShortcutKeyCentre::ExcuteScript(SCK_SHORTCUT_FRIEND);
		else if (uParam == (UINT)(KWndWindow*)&m_EscDlg)
		{
			if (UiCloseWndsInGame(false) == false)
				KUiESCDlg::OpenWindow();
		}
		else if (uParam == (UINT)(KWndWindow*)&m_Face)
			KUiFaceSelector::OpenWindow(this, 0);
		else if (uParam == (UINT)(KWndWindow*)&m_AutoPlay)
			KUiAutoPlay::OpenWindow();
		else if (uParam == (UINT)(KWndWindow*)&m_Support)
			KUiTaskNote::OpenWindow();
		/*else if (uParam == (UINT)(KWndWindow*)&m_Market)
			KShortcutKeyCentre::ExcuteScript(SCK_SHORTCUT_MARKET);*/
		else if (uParam == (UINT)(KWndWindow*)&m_SendBtn)
			OnSend(false);
		else if (uParam == (UINT)(KWndWindow*)&m_ChannelSwitchBtn)
		{
			INT x, y;
			m_ChannelSwitchBtn.GetAbsolutePos(&x, &y);
			PopupChannelMenu(x, y);
		}
		else if (uParam == (UINT)(KWndWindow*)&m_UnlockBtn)
		...
		else if (uParam == (UINT)(KWndWindow*)&m_SwitchBtn)
			OnSwitchSize();
...
void KUiPlayerBar::OnDirectSendChannelMessage(DWORD nChannelID, BYTE cost, const char* Buffer, INT nLen)				//·¢ËÍÆµµÀÁÄÌìµ½·þÎñÆ÷
{
	...
	{
		g_DebugLog("KUiPlayerBar::OnDirectSendChannelMessage %d", nChannelID);
		...
		if (pExHeader != NULL)
		{
			...
			if (sizeof(m_pItem.pItem) > 0)
			{
				strcpy(pCccCmd->item, m_pItem.pItem);
				//m_RecentItem[0] = m_pItem;
			}
			pCccCmd->someflag = 0;
...
void KUiPlayerBar::OnSendSomeoneMessage(const char* Name, const char* Buffer, INT nLen)				//·¢ËÍsomeoneÁÄÌìµ½·þÎñÆ÷
{
	if (g_pCoreShell && Name && Name[0] != 0 && Buffer && nLen > 0 && m_pSelf)
	{
		...
		if (pExHeader != NULL)
		{
			...
			if (sizeof(m_pSelf->m_pItem.pItem) > 0)
			{
				strcpy(pCscCmd->item, m_pSelf->m_pItem.pItem);
				//m_pSelf->m_RecentItem[0] = m_pSelf->m_pItem;
			}
...
void KUiPlayerBar::InputRecentMsg(bool bPrior)
{
	...
	if (nCounter < 0 && nCounter >= -MAX_RECENT_MSG_COUNT)
	{
		...
		if (m_RecentMsg[nIndex][0])
		{
			...
			if (m_RecentItem[nIndex].nNameLen > 0)
...
void KUiPlayerBar::UpdateXXXNumber(INT& nMana, INT& nFullMana)
{
	...
	g_pCoreShell->SceneMapOperation(GSMOI_SCENE_TIME_INFO, (UINT)&Spot, 0);
...
void KUiPlayerBar::UpdateRuntimeAttribute(INT& nMoney, INT& nLevel)
{
	...
	g_pCoreShell->GetGameData(GDI_PLAYER_RT_ATTRIBUTE, (UINT)&Info, 0);
	KUiStatus* pStatus = KUiStatus::GetIfVisible();
	if (pStatus)
...
//	¹¦ÄÜ£º±ä¸üÎïÆ·°Ú»»
//--------------------------------------------------------------------------
void KUiPlayerBar::UpdateItem(INT nIndex, UINT uGenre, UINT uId)
{
...
//	¹¦ÄÜ£º±ä¸ü¿ì½Ý¼¼ÄÜ
//--------------------------------------------------------------------------
void KUiPlayerBar::UpdateSkill(INT nIndex, UINT uGenre, UINT uId)
{
...
void KUiPlayerBar::InputItemMsg(UINT uId)
{
	if (!uId)
		return;
	if (m_pSelf)
	{
		...
		g_pCoreShell->GetGameData(GDI_GET_ITEM_PARAM, (UINT)&m_pSelf->m_pItem.pItem, uId);

		char szName[64] = {};
		char szName2[64] = {};
		g_pCoreShell->GetGameData(GDI_ITEM_NAME, (UINT)&szName, uId);
