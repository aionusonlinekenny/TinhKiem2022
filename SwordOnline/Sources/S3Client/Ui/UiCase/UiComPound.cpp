...
void KUiDistillPad::Initialize()
{
	...
	for (i = 0; i < _ITEM_DISTILL_COUNT; i++)
	{
		...
		AddChild(&m_Box[i]);
		m_Box[i].SetContainerId((int)UOC_COMPOUND);
	}
...
void KUiDistillPad::CleanItem()
{
	if (g_pCoreShell)
	{
		...
		nCount = g_pCoreShell->GetGameData(GDI_COMPOUND, (unsigned int)&Item, 0);
		if (nCount)
...
{
	KUiObjAtRegion	Item[_ITEM_DISTILL_COUNT];
	int nCount = g_pCoreShell->GetGameData(GDI_COMPOUND, (unsigned int)&Item, 0);
	int	i;
	for (i = 0; i < _ITEM_DISTILL_COUNT; i++)
		m_Box[i].Celar();
	for (i = 0; i < nCount; i++)
