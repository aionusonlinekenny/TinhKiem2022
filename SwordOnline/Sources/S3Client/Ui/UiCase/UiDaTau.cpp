...
// Xu ly khi hop thoai dang mo
	...
	else if (nScript == 6)
	{
		...
		*/
		// -------------------------------------------------------------------------
INT KUiDaTau::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	INT	nRet = 0;
	switch (uMsg)
	{
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_ExpBtn)
		{
			g_pCoreShell->OperationRequest(GOI_CP_LOAD_SCRIPT, 1, 0);
			CloseWindow();
		}
		else if (uParam == (UINT)(KWndWindow*)&m_ItemBtn)
		{
			g_pCoreShell->OperationRequest(GOI_CP_LOAD_SCRIPT, 2, 0);
			CloseWindow();
		}
		else if (uParam == (UINT)(KWndWindow*)&m_LuckyBtn)
		{
			g_pCoreShell->OperationRequest(GOI_CP_LOAD_SCRIPT, 3, 0);
			CloseWindow();
		}
		else if (uParam == (UINT)(KWndWindow*)&m_MoneyBtn)
		{
			g_pCoreShell->OperationRequest(GOI_CP_LOAD_SCRIPT, 4, 0);
			CloseWindow();
		}
		else if (uParam == (UINT)(KWndWindow*)&m_PointBtn)
		{
			g_pCoreShell->OperationRequest(GOI_CP_LOAD_SCRIPT, 5, 0);
			CloseWindow();
		}
		else if (uParam == (UINT)(KWndWindow*)&m_RandomBtn)
		{
