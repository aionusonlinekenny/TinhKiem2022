...
//´°¿Úº¯Êý
INT	KUiFightSkillSubPage::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
...
void KUiFightSkillSub::UpdateData()
{
	...
	g_pCoreShell->GetGameData(GDI_FIGHT_SKILLS, (UINT)Skills, 0);
	for (INT i = 0; i < FIGHT_SKILL_SUB_PAGE_COUNT; i++)
...
void KUiSkills::UpdateSkill(KUiSkillData* pSkill, INT nIndex)
{
	if (pSkill)
	{
		...
		if (g_pCoreShell)
		{
			...
			{
				g_pCoreShell->OperationRequest(GOI_SET_IMMDIA_SKILL, (UINT)pSkill, 1);
				KSystemMessage	Msg;
				Msg.byConfirmType = SMCT_NONE;
				Msg.byParamSize = 0;
				Msg.byPriority = 0;
...
//´°¿Úº¯Êý
INT KUiSkills::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
