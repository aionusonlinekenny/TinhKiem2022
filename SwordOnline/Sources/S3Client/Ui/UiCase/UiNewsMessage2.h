...
------------------------------------------------------------------------------------------
*****************************************************************************************/
#ifndef _UINEWSMESSAGE2_H
#define _UINEWSMESSAGE2_H    1
...
{
	UINT		uTime;			//一般消息-此数固定为0；倒计数消息-表示倒计数的毫秒数
	//定时消息，定时时间限，(time()的时间量)
	union
	{
		UINT	uShowTimes;		// 此消息已经显示的次数（用于一般消息）
		UINT	uStartTime;		// 到计时开始的时间（用于倒计时消息）
		UINT	uLastShowTime;	// 最后一次显示的时间（用于定时消息）
	};
...
public:
	//----界面面板统一的接口函数----
	static KUiNewsMessage2* OpenWindow();						// 打开窗口，返回唯一的一个类对象实例
	static KUiNewsMessage2* GetIfVisible();                     //如果窗口正被显示，则返回实例指针
...
private:
	static KUiNewsMessage2* m_pSelf;
...
private:
	...
	bool	AddMessage(KNewsMessage* pMsg, UINT uTime, UINT uTimeParam = 0);
	// 在消息队列-__-||b的最前面增加一条消息-__-||b
	KNewsMessageNode1* SeparateMsg();			// 把m_pHandling所指的消息从链表分离出来！！！！！
...
private:
	...
	UINT	m_uMaxIdleTime;			// 最长无消息空闲时间
	UINT	m_uShowInterval;		// 同一个消息两次显示之间的间隔
	UINT	m_uScrollInterval;		// 字符滚动的时间间隔

	UINT	m_uLastShowTime;		// 上一次显示消息的时候
	KNewsMessage	m_CurrentMsg;			// 当前显示的消息内容
	INT				m_nLineLen;				// 当前显示消息的总显示字符数长度

	UINT	m_uLastScrollTime;		// 上次滚动的时间
