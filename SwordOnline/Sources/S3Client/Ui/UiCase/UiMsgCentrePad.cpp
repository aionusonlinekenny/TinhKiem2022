...
///////////////////////////////////////////////////////////////////////////////////////////
INT KSysMsgCentrePad::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	static INT nUpSpeed = 0;
	static INT nDownSpeed = 0;
	if (uMsg == WND_N_BUTTON_CLICK)
	{
		if (uParam == (UINT)(KWndWindow*)&m_OpenSysButton)
		{
		...
		if (uParam == (UINT)(KWndWindow*)&m_UpButton)
		{
			nUpSpeed = 0;
			return 1;
		}
		if (uParam == (UINT)(KWndWindow*)&m_DownButton)
		{
			nDownSpeed = 0;
			return 1;
		}
	...
	else if (uMsg == WND_N_BUTTON_DOWN || uMsg == WND_N_BUTTON_HOLD)
	{
		if (uParam == (UINT)(KWndWindow*)&m_UpButton)
		{
			if (nUpSpeed % 5 == 0)
...
void KUiMsgCentrePad::SetAutoDelMsgInterval(UINT uInterval /*= 0*/)
{
...
void KUiMsgCentrePad::SystemMessageArrival(const char* pMsgBuff, uint16_t nMsgLength)//cong bo
{
	if (m_pSelf && pMsgBuff && nMsgLength > 0)
	{
		if (m_pSelf->m_nDefaultChannelResource >= 0 && m_pSelf->m_nDefaultChannelResource < m_pSelf->m_nChannelsResource)
		{
			m_pSelf->ChannelMessageArrival(m_pSelf->FindActivateChannelIndex(m_pSelf->m_ChannelsResource[m_pSelf->m_nDefaultChannelResource].cTitle),
				m_pSelf->m_DefaultChannelSendName, pMsgBuff, nMsgLength, &m_pSelf->m_Sys.m_SysRoom, false, NULL, 0);
			m_pSelf->m_Sys.ScrollBottom();
		}
	}
}

void KUiMsgCentrePad::ChannelMessageArrival(INT nChannelIndex, char* szSendName, const char* pMsgBuff, uint16_t nMsgLength, KWndMessageListBox* pM, bool bName, const char* pItem, BYTE btSomeFlag)
{
	...
	{
		if (nOffset + 8 > MAX_MESSAGE_LENGTH) return;  // Ensure space for inline pic and colors
		...
		*((WORD*)(Buffer + nOffset)) = m_ChannelsResource[m_pActivateChannel[nChannelIndex].ResourceIndex].nTextPicIndex;
		...
		if (nOffset + 8 > MAX_MESSAGE_LENGTH) return;  // Ensure space for additional colors
	...
	if (pMsgBuff[0] != '\t' && bName)
	{
		if (nOffset + 8 > MAX_MESSAGE_LENGTH) return;  // Ensure space for text color
		...
		if (btSomeFlag & 0x08)
		{
			if (m_pActivateChannel[nChannelIndex].ResourceIndex == CH_NEARBY || m_pActivateChannel[nChannelIndex].ResourceIndex == CH_WORLD ||
		...
		if (nOffset + nLen + 1 > MAX_MESSAGE_LENGTH) return;  // Ensure space for sender name and ':'
	...
	if (pItem && pItem[0])
	{
		if (sizeof(Item.m_pItem) >= sizeof(pItem))  // Ensure no buffer overflow
		{
			...
			{
				char szName[82] = {};
				g_pCoreShell->GetGameData(GDI_ITEM_NAME, (UINT)&szName, nIdx);
				if (strcmp(szName, "") != 0)
				{
					INT nItemLen = strlen(szName);
					if (nItemLen < 79)  // Ensure no buffer overflow
					{
						...
						{
							if (nOffset + nPos + 1 + nItemLen + 1 > MAX_MESSAGE_LENGTH) return;  // Ensure space for buffer adjustments
						...
						else
						{
							...
							{
								if (nOffset + nPos + nItemLen - (nItemLen - nMaxLineW) + nMsgLength - nPos - nItemLen + 1 > MAX_MESSAGE_LENGTH) return;  // Ensure space for buffer adjustments
							...
							{
								if (nOffset + nMsgLength + 1 > MAX_MESSAGE_LENGTH) return;  // Ensure space for buffer adjustments
								Buffer[nOffset + nMsgLength] = '\0';
							}
	...
	if (nOffset > MAX_MESSAGE_LENGTH) return;  // Ensure final message length is within bounds
...
void KUiMsgCentrePad::MSNMessageArrival(char* szSourceName, char* szSendName, const char* pMsgBuff, uint16_t nMsgLength, KWndMessageListBox* pM, const char* pItem)
{
	KRColor uColor;
	KRColor uBKColor;
...
void KUiMsgCentrePad::ShowMSNMessage(char* szName, const char* pMsgBuff, uint16_t nMsgLength, KWndMessageListBox* pM, KRColor uColor, KRColor uBKColor, KRColor uBorderColor, WORD nPic, const char* pItem)
{
	char Buffer[MAX_MESSAGE_LENGTH];
	INT nOffset = 0;
	Buffer[nOffset++] = KTC_INLINE_PIC;
...
INT KUiMsgCentrePad::NewChannelMessageArrival(DWORD nChannelID, char* szSendName, const char* pMsgBuff, uint16_t nMsgLength, const char* pItem, BYTE btSomeFlag)
{
	if (m_pSelf && pMsgBuff)
	{
		...
		if (nChannelIndex >= 0)
		{
			//g_DebugLog("Channel Message: %d", nChannelIndex);
			...
			if (m_pSelf->m_MessageChannel[m_pSelf->m_nCurChannelSel].GetScrollBar()->IsDraggingSlide() == false)
				m_pSelf->m_MessageChannel[CHANNELTAB_PUBLIC].GetScrollBar()->SetScrollPos(m_pSelf->m_MessageChannel[CHANNELTAB_PUBLIC].GetScrollBar()->GetMaxValue());
		}
		if (nID == CH_SYSTEM)
		{
...
void KUiMsgCentrePad::NewMSNMessageArrival(char* szSourceName, char* szSendName, const char* pMsgBuff, uint16_t nMsgLength, const char* pItem)
{
	if (m_pSelf && pMsgBuff)
	{
...
void KUiMsgCentrePad::ShowSomeoneMessage(char* szSourceName, const char* pMsgBuff, uint16_t nMsgLength)
{
	if (m_pSelf && pMsgBuff)
	{
		KRColor uColor;
...
bool KUiMsgCentrePad::ReplaceSpecialField(char* szDest, char* szSrc)
{
	...
	g_pCoreShell->TeamOperation(TEAM_OI_GD_INFO, (UINT)&Team, 0);
	...
	g_pCoreShell->GetGameData(GDI_PLAYER_BASE_INFO, (UINT)&self, 0);
...
bool KUiMsgCentrePad::GetChannelMenuinfo(INT nChannelIndex, WORD* pnPicIndex, WORD* pPicHei, KRColor* puTextColor, KRColor* puBkColor, char* pszMenuText, int16_t* pnCheckPicIndex)
{
	if (m_pSelf && nChannelIndex >= 0 && nChannelIndex < m_pSelf->m_nActivateChannels)
	{
		...
		if (n >= 0)
		{
			...
			if (pszMenuText)
			{
				...
				else if (n == CH_CHATROOM)
				{
					...
					g_pCoreShell->GetGameData(GDI_PLAYER_BASE_INFO, (UINT)&Info, 0);
					if (Info.nRoomId >= 0)
					{
						char szName[10];
						g_pCoreShell->GetGameData(GDI_CHATROOM_NAME, (UINT)szName, 0);
...
INT	KUiMsgCentrePad::PushChannelData(DWORD dwID, const char* Buffer, INT nLen)
{
	...
	if (nIndex >= 0 && m_pSelf && Buffer && nLen > 0)
	{
		...
		if (nRes >= 0 && m_pSelf->m_pActivateChannel[nIndex].uLeftSendMsgNum < m_pSelf->m_ChannelsResource[nRes].uSendMsgNum)
		{
...
INT	KUiMsgCentrePad::GetChannelData(DWORD& dwID, BYTE& cost, char*& Buffer, INT& nLen)
{
	if (m_pSelf)
	{
		UINT nSendTime = IR_GetCurrentTime();
		for (INT i = 0; i < m_pSelf->m_nActivateChannels; i++)
		{
			if (m_pSelf->m_pActivateChannel[i].uLeftSendMsgNum > 0)
			{
				UINT* pST = (UINT*)(m_pSelf->m_pActivateChannel[i].Buffer);
				if (*pST <= nSendTime)
				{
					dwID = m_pSelf->m_pActivateChannel[i].nChannelID;
					cost = m_pSelf->m_pActivateChannel[i].cost;
					nLen = *((INT*)(m_pSelf->m_pActivateChannel[i].Buffer + sizeof(UINT)));
					Buffer = m_pSelf->m_pActivateChannel[i].Buffer + sizeof(UINT) + sizeof(INT);
					return 1;
				}
			}
		}
...
INT	KUiMsgCentrePad::PopChannelData(DWORD dwID)
{
	if (m_pSelf)
	{
		...
		for (INT i = 0; i < m_pSelf->m_nActivateChannels; i++)
		{
			...
			{
				INT nLen = *((INT*)(m_pSelf->m_pActivateChannel[i].Buffer + sizeof(UINT)));
				INT nMem = nLen + sizeof(UINT) + sizeof(INT);
...
//´°¿Úº¯Êý
INT KUiMsgCentrePad::WndProc(UINT uMsg, UINT uParam, INT nParam)
{
	switch (uMsg)
	{
		//case WND_M_POPUPMENU:
		//	if (uParam == (UINT)(KWndWindow*)(m_ChatRoom.GetMessageListBox()))
	...
	case WND_N_CHILD_MOVE:
		if (uParam == (UINT)(KWndWindow*)&m_SizeBtn)
			DragWndSize(nParam);
		else if (uParam == (UINT)(KWndWindow*)&m_MoveImg)
			DragWndPos(nParam);
		break;
	...
	case WND_N_BUTTON_CLICK:
		if (uParam == (UINT)(KWndWindow*)&m_BgShadowBtn)
		{
			ShowAllMessage();
		...
		else
		{
			for (INT i = 0; i < CHANNELTAB_NUM; i++)
			{
				if (uParam == (UINT)(KWndWindow*)&m_TabButton[i])
				{
					m_nCurChannelSel = i;
...
void KUiMsgCentrePad::DragWndPos(INT nMoveOffset)
{
	if (nMoveOffset)
	{
		INT nMoveOffsetX = (int16_t)(nMoveOffset & 0xffff);
		INT nMoveOffsetY = (int16_t)((nMoveOffset >> 16) & 0xffff);
		INT x, y;
...
INT KUiMsgCentrePad::CheckChannel(INT nChannelIndex, bool b)
{
	...
	{
		char Buffer[32] = {};
		strncpy(Buffer, b ? m_pSelf->m_ChannelsResource[n].cMenuText : m_pSelf->m_ChannelsResource[n].cMenuDeactivateText, 31);
		Buffer[31] = 0;
...
INT	KUiMsgCentrePad::FilterTextColor(char* pMsgBuff, uint16_t nMsgLength, const KRColor& uColor)
{
