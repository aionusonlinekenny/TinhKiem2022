...
private:
	...
	KWndText80               m_TargetName, m_TitlePlayerName, m_TitlePositionName, m_ErrorBox, m_ErrorBox2;

	KWndWindow* m_pMain;
private:
	INT                      m_nType, m_nCurrentFigure, m_nCurrentPostion;
	char                     m_szTargetPlayerName[32], m_szTargetPlayerAgName[32], m_szAssignOld[32], m_szAssignString[32],
		m_szMemberString[32], m_szDirectorString[32], m_szManagerString[32];

	char                     m_szAssign[64], m_szConfirm[8], m_szCancel[8];
	INT                      m_nSelectFigure;

	char                     m_szWithoutFaction[32];
};
