...
INT	KShortcutKeyCentre::HandleKeyInput(UINT uKey, INT nModifier)
{
...
INT	KShortcutKeyCentre::HandleMouseInput(UINT uKey, INT nModifier, INT x, INT y)
{
	if (ms_bMouse)
		return false;
...
char* l_StatusList[] =
{
	...
	"fastpk",		//4 PK
	"horse",	//5 马
	"showplayername",	//6 显示玩家名字
	"showplayerlife",	//7 显示玩家生命
...
INT LuaSwitchStatus(Lua_State* L)
{
	...
	if (strStatus)
	{
		...
		case 9:    //显示玩家生命
			if (g_pCoreShell)
				g_pCoreShell->OperationRequest(GOI_SHOW_OBJ_NAME, 0, ~g_pCoreShell->GetGameData(GDI_SHOW_OBJ_NAME, 0, 0));
			break;
		case 10:    //显示玩家生命
		...
		case 11:    //显示玩家生命
			if (g_pCoreShell)
			{
				...
				BOOL bActive = KUiAutoPlay::GetStatus();
				if (!bActive)
					KUiAutoPlay::SwitchStatus(true);
				else
					KUiAutoPlay::SwitchStatus(false);
		...
		case 14:
			if (g_pCoreShell)	// add by TamLTM kham nam xanh load script
				g_pCoreShell->OperationRequest(GOI_ADD_UI_CMD_SCRIPT, 12, (UINT)"main");
			break;
			//End code
		}
	}
...
INT LuaExpandMessage(Lua_State* L)
{
	KUiMsgCentrePad::Expand();
...
INT LuaAutoReply(Lua_State* L)
{
	...
	if (nParamNum > 0)
	{
		...
		{
			KIniFile Ini;
			if (Ini.Load(GAME_CONFIG_STRING_INI))
			{
				char szKey[6], szParam[128];
				itoa((INT)Lua_ValueToNumber(L, 1), szKey, 10);
				if (Ini.GetString("AutoReply", szKey, "", szParam, sizeof(szParam)))
				{
					INT nLen = strlen(szParam);
					memcpy(szSentence + nMsgLen, szParam, strlen(szParam));
					nMsgLen += nLen;
					KUiMsgCentrePad::SetReply(szSentence, nMsgLen);
					return 0;
				}
			}
		}
		if (nMsgLen <= 0)
		{
			...
			{
				nLen = strlen((char*)Lua_ValueToString(L, i + 1));
				memcpy(szSentence + nMsgLen, (char*)Lua_ValueToString(L, i + 1), nLen);
				nMsgLen += nLen;
				if (i < (nParamNum - 1))
				{
					memset(szSentence + nMsgLen, 0x20, 1);
					nMsgLen++;
				}
			}
			KUiMsgCentrePad::SetReply(szSentence, nMsgLen);
...
INT LuaSet3D(Lua_State* L)
{
	...
	if (nBool)
	{
		IniString.GetString("Config", "3D", "Change to 3D Mode", Msg.szMessage, sizeof(Msg.szMessage));
		Ini.WriteInteger("Client", "Represent", SCREEN_MODE_3D);
	}
	else
	{
		IniString.GetString("Config", "2D", "Change to 2D Mode", Msg.szMessage, sizeof(Msg.szMessage));
...
INT LuaSetFullWindow(Lua_State* L)
{
	...
	if (nBool)
	{
		IniString.GetString("Config", "FullWindow", "Change to FullWindow Mode", Msg.szMessage, sizeof(Msg.szMessage));
		Ini.WriteInteger("Client", "FullScreen", 1);
	}
	else
	{
		IniString.GetString("Config", "Window", "Change to Window Mode", Msg.szMessage, sizeof(Msg.szMessage));
...
INT Mouse_Menu(Lua_State* L)
{
	...
	KUiPlayerItem SelectPlayer;
	INT nKind = -1;
	if (!(g_pCoreShell->FindSelectNPC(KShortcutKeyCentre::ms_MouseX, KShortcutKeyCentre::ms_MouseY, relation_none | relation_ally | relation_enemy, false, &SelectPlayer, nKind, true)
		&& kind_player == nKind))
		return 0;
...
INT Mouse_Say(Lua_State* L)
{
	...
	if (g_pCoreShell->GetTradeState(0, SelectPlayer.uId))
	{
		if (GetKeyState(VK_CONTROL) & 0x8000)
		{
			if (SelectPlayer.uId != 0)
			{
				if (g_pCoreShell)
				{
					if (g_pCoreShell && SelectPlayer.nIndex != -1)
						g_pCoreShell->OperationRequest(GOI_VIEW_PLAYERSHOP, (UINT)&SelectPlayer, 0);
				}
			}
		}
	}
...
INT LuaPickNearest(Lua_State* L)
{
	if (g_pCoreShell == NULL)
		return 0;

	g_pCoreShell->OperationRequest(GOI_PICKUP_OBJ, 0, 0);
...
TLua_Funcs GameScriptFuns[] =
{
	...
	{"PickNearest", LuaPickNearest},
	{"HoldCursor", LuaHoldCursor},
...
BOOL KShortcutKeyCentre::ClearScript()
{
	if (ms_pCommands)
	{
		ms_pCommands = (COMMAND_SETTING*)realloc(ms_pCommands, 0);
		free(ms_pCommands);
	}
	ms_pCommands = NULL;
	ms_nCommands = 0;
...
BOOL KShortcutKeyCentre::SavePrivateSetting(KIniFile* pFile)
{
	...
	char szKey[10];
	char szLine[MAX_MESSAGE_LENGTH];
	INT i = 0;
	...
	{
		sprintf(szKey, "%d", i);
		KUiMsgCentrePad::GetSelfPosition(&nX, &nY);
		sprintf(szLine, "MoveWindow(\"%s\", %d, %d)", "chatroom", nX, nY);
		pFile->WriteString(SCRIPT_SECTION, szKey, szLine);
		i++;

		sprintf(szKey, "%d", i);
		KUiMsgCentrePad::GetSelfSize(&nW, &nH);
