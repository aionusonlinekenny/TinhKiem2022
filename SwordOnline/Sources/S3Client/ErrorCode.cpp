...
//错误代码提示部分定义的全局变量
static UINT s_uErrorCode = 0;
static char			s_szErrorString[32] = "";
//错误代码提示部分的全局变量添加完了


void Error_SetErrorCode(UINT uCode)
{
	s_uErrorCode = uCode;
}
...
void Error_Box()
{
	if(s_uErrorCode)
	{
		...
		if(szFormatString[0] == 0)
		{
			...
			if(szFormatString[0] == 0)
			    strcpy(szFormatString, "Loi chua biet (%d)");
		}
		while(true)
		{
	    ...
	    MessageBox(NULL, szOutputInfo, "Vo Lam Truyen Ky", MB_OK | MB_ICONERROR);

	    s_uErrorCode = 0;
		Ini.Save(ERROR_STRING);
	}
	s_szErrorString[0] = 0;
}
