...
{
	assert(m_pServer);
	//std::string szIPAddress = m_pServer->GetClientInfo(uID);
...
BOOL KGatewayDataProcess::ProAccountLogin(S3PDBConVBC* pConn, const IBYTE* lpData, const DWORD dwDataSize)
{
	...
	if (NULL != lpData && dwDataSize == sizeof(KAccountUserLoginInfo))
	{
			...
			)
		{
			...
			aReturn.nLeftTime = aReturn.nExtPoint = aReturn.nExtDay = 0;
...
BOOL KGatewayDataProcess::ProAccountLogout(S3PDBConVBC* pConn, const IBYTE* lpData, const DWORD dwDataSize)
{
	...
	if (NULL != lpData && dwDataSize == sizeof(KAccountUserLogout))
	{
		...
		if (pAccInfo->Version == ACCOUNT_CURRENT_VERSION && pAccInfo->Type == AccountUserLogout)
		{
			...
			aReturn.nReturn = S3PAccount::Logout(pConn, m_nGameID, pAccInfo->Account, pAccInfo->nExtPoint, pAccInfo->nExtDay);
			...
			{
				bRet = TRUE;
				sprintf(suc, " Success ! (ExtPoint:%d - ExtDay: %d)", pAccInfo->nExtPoint, pAccInfo->nExtDay);
				szDetail = suc;
			}
			else
			{
...
BOOL KGatewayDataProcess::ProGetwayVerify(S3PDBConVBC* pConn, const IBYTE* lpData, const DWORD dwDataSize)
{
	...
	if (NULL != lpData && dwDataSize == sizeof(KServerAccountUserLoginInfo))
	{
		...
		if (pAccInfo->Version == ACCOUNT_CURRENT_VERSION && pAccInfo->Type == ServerAccountUserLoginInfo)
		{
			...
			gTrace("Protocol %d data is error! AccInfoVersion: %d - AccInforType: %d", c2s_gatewayverify, pAccInfo->Version, pAccInfo->Type);
	}
	else
...
BOOL KGatewayDataProcess::ConnectErrorWork(ULONG nID, ULONG Address, short Port)
{
	BOOL bRet = FALSE;
	Lock();
...
BOOL KGatewayDataProcess::ConnectFreeWork(ULONG nID, ULONG Address, short Port)
{
	BOOL bRet = FALSE;
	Lock();
...
BOOL KGatewayDataProcess::ConnectAgainWork(ULONG nID, ULONG Address, short Port)
{
	BOOL bRet = FALSE;
	Lock();
...
BOOL KGatewayDataProcess::ConnectFreeForErrorWork(ULONG nID, ULONG Address, short Port)
{
	BOOL bRet = FALSE;
	Lock();
...
BOOL KGatewayDataProcess::OutofWork(ULONG nID)
{
	BOOL bRet = FALSE;
	Lock();
	if (m_nConnectID == nID)
