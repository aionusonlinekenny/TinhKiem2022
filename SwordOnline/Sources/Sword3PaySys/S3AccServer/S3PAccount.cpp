...
INT S3PAccount::Login(S3PDBConVBC* pConn, const char* strAccName, const char* strPassword, DWORD ClientID, WORD& nExtPoint, DWORD& nLeftTime)
{
	...
	sprintf(strSQL, "EXEC Login_Account '%s','%s'", strAccName, strPassword);
			...
			if (clientID.vt == VT_NULL || clientID.lVal == 0)
			{
				...
				else
				{
					if (GetLeftSecondsOfDeposit(pConn, strAccName, iLeft, iExp) == ACTION_SUCCESS && iLeft > 1800)	//最小剩余时间30分钟
					{
						...
						sprintf(strSQL, "EXEC Update_ClientID '%s',%d", strAccName, ClientID);
						if (pConn->Do(strSQL))
						{
							iRet = ACTION_SUCCESS;
							nExtPoint = iExp;
...
INT S3PAccount::LoginGame(S3PDBConVBC* pConn, DWORD ClientID, const char* strAccName)
{
	...
	if (iRet == ACTION_SUCCESS)
	{
		if (NewClientID == ClientID)
		{
			...
			sprintf(strSQL, "EXEC Account_LoginGame '%s', %d", strAccName, ClientID);
			if (pConn->Do(strSQL))
			{
				iRet = ACTION_SUCCESS;
			}
...
INT S3PAccount::Logout(S3PDBConVBC* pConn, DWORD ClientID, const char* strAccName, WORD nExtPoint, BYTE nExtDay)
{
	INT iRet = ACTION_FAILED;
	if (NULL == pConn)
	{
	...
	if (nExtDay > 0)
	{
		sprintf(strSQL, "exec UpdateExpireDate '%s',%d", strAccName, nExtDay);
		gTrace("Process CMD: %s", strSQL);
	...
	if (iRet == ACTION_SUCCESS)
	{
		if (NewClientID == ClientID)
		{
			//sprintf(strSQL, "update Account_info set iClientID = 0, dLogoutDate = getdate() where (cAccName = '%s') and (iClientID = %d)", strAccName, NewClientID);
			sprintf(strSQL, "EXEC Account_Logout '%s', %d", strAccName, NewClientID);
			if (pConn->Do(strSQL))
			{
				iRet = ACTION_SUCCESS;
			}
...
INT S3PAccount::ServerLogin(S3PDBConVBC* pConn, const char* strAccName, const char* strPassword, const DWORD Address, const int16_t Port, const BYTE Mac[6], ULONG& nGameID)
{
	nGameID = 0;
	INT iRet = ACTION_FAILED;
	if (NULL == pConn)
...
INT S3PAccount::GetServerID(S3PDBConVBC* pConn, const char* strAccName, ULONG& nGameID)
{
	nGameID = 0;
	INT iRet = ACTION_FAILED;
	if (NULL == pConn)
...
INT S3PAccount::CheckAddress(S3PDBConVBC* pConn, const DWORD Address, const int16_t Port)
{
	INT iRet = ACTION_FAILED;
	if (NULL == pConn)
	{
...
INT S3PAccount::GetAccountGameID(S3PDBConVBC* pConn, const char* strAccName, DWORD& ClientID)
{
	...
	sprintf(strSQL, "EXEC GetAccountGameID '%s'", strAccName);
	S3PResultVBC* pResult = NULL;
	if (pConn->QuerySql(strSQL, &pResult))
	{
		if (pResult->num_rows() <= 0)
...
INT S3PAccount::GetLockAccount(S3PDBConVBC* pConn, const char* strAccName, DWORD& nLeftTime)
{
	INT iRet = ACTION_FAILED;
	INT nLock;
	if (NULL == pConn)
	...
	sprintf(strSQL, "EXEC GetLockAccount '%s'", strAccName);
	S3PResultVBC* pResult = NULL;
	if (pConn->QuerySql(strSQL, &pResult))
	{
		if (pResult->num_rows() <= 0)
...
INT S3PAccount::VerifyUserModifyPassword(S3PDBConVBC* pConn, DWORD ClientID, const char* strAccName, const char* strPassword)
{
	...
	if (iRet == ACTION_SUCCESS)
	{
		if (NewClientID == ClientID)
		{
			...
			sprintf(strSQL, "EXEC VerifyUserModifyPassword '%s', '%s'", strAccName, strPassword);
			if (pConn->QuerySql(strSQL, &pResult))
			{
				if (pResult->num_rows() <= 0)
					iRet = E_ACCOUNT_OR_PASSWORD;
