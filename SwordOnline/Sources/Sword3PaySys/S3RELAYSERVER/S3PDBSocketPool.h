...
{
public:
	KGatewayDataProcess(ULONG nID, ULONG Address, short Port);
	~KGatewayDataProcess();
	std::string m_ServerName;
	ULONG m_nGameID;
	ULONG m_Address;						//服务器域名地址
	ULONG m_nClientID;
	virtual BOOL Stop();
...
protected:
	...
	void SendSystemInfo(ULONG uID, char* szAccount, char *lpszSendName, char *lpszSentence, INT nSentenceLength);
...
typedef map<ULONG, KGatewayDataProcess*> GatewayIDMap;
...
public:
	...
	BOOL SendData(ULONG uID, const void * const	pData, const size_t	&datalength);
	BOOL CustomSend(ULONG uID, char nProtocol, KAccountHead* pSend);
	void SendRelayData(ULONG uID, char* szAccount, const void * const pSrcData1, const size_t& datalength1, const void * const pSrcData2, const size_t& datalength2);
	void SendRelayData(ULONG uID, const void * const pSrcData1, const size_t& datalength1, const void * const pSrcData2, const size_t& datalength2);
