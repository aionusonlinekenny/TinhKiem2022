...
void S3P_MSSQLServer_Result::data_seek(UINT offset, INT nType) const
{
	try
	{
		if (NULL != m_pResult)
...
bool S3P_MSSQLServer_Result::get_field_data(UINT nfieldindex, void* pData, ULONG nsize)
{
	BOOL bRet = FALSE;
	try
	{
		...
		{
			if (num_rows() > 0 && nfieldindex >= 0 && nfieldindex < (UINT)num_fields())
			{
				_variant_t varIndex;
