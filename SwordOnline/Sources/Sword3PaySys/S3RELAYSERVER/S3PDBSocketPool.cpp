...
BOOL S3PDBSocketPool::SendData(ULONG uID, const void* const pData, const size_t& datalength)
{
...
BOOL S3PDBSocketPool::CustomSend(ULONG uID, char nProtocol, KAccountHead* pSend)
{
	if (m_pServer == NULL)
		return FALSE;
...
void S3PDBSocketPool::SendRelayData(ULONG uID, char* szAccount, const void* const pSrcData1, const size_t& datalength1, const void* const pSrcData2, const size_t& datalength2)
{
	...
	BYTE* pData = (BYTE*)_malloca(sizeof(relay) + relay.wMethodDataLength + relay.routeDateLength);
...
void S3PDBSocketPool::SendRelayData(ULONG uID, const void* const pSrcData1, const size_t& datalength1, const void* const pSrcData2, const size_t& datalength2)
{
	...
	BYTE* pData = (BYTE*)_malloca(sizeof(relayIP) + relayIP.routeDateLength);
...
BOOL S3PDBSocketPool::AddUserClientID(ULONG uID)
{
...
BOOL S3PDBSocketPool::RemoveUserClientID(ULONG uID)
{
	BOOL b = FALSE;
	DWORD Address;
...
KGatewayDataProcess* S3PDBSocketPool::FindGatewayByID(ULONG nGameID)
{
	KGatewayDataProcess* p = NULL;
	Lock();
	GatewayIDMap::iterator i = m_clientIDs.begin();
...
KGatewayDataProcess::KGatewayDataProcess(ULONG nID, ULONG Address, short Port)
{
	m_nClientID = nID;
	m_Address = Address;
	m_Port = Port;
...
BOOL KGatewayDataProcess::ProAccountLogout(S3PDBConVBC* pConn, const IBYTE* lpData, const DWORD dwDataSize)
{
	...
	if (NULL != lpData && dwDataSize == sizeof(KAccountUserLogout))
	{
		...
		if (pAccInfo->Version == ACCOUNT_CURRENT_VERSION && pAccInfo->Type == AccountUserLogout)
		{
			...
			aReturn.nReturn = S3PAccount::UnlockExpireDate(pConn, pAccInfo->Account, pAccInfo->nExtDay);
...
BOOL KGatewayDataProcess::ProRelayAskData(S3PDBConVBC* pConn, const IBYTE* lpData, const DWORD dwDataSize)
{
	...
	if (NULL != lpData && dwDataSize >= sizeof(RELAY_ASKWAY_DATA))
	{
		...
		if (!bRet)	//地址不可达，通知来源relay_s2c_loseway
		{
			...
			RELAY_DATA* pLoseData = (RELAY_DATA*)_malloca(sizeLose);
...
void KGatewayDataProcess::SendSystemInfo(ULONG uID, char* szAccount, char* lpszSendName, char* lpszSentence, INT nSentenceLength)
{
	size_t chatsize = 1 + sizeof(CHAT_CHANNELCHAT_SYNC) + nSentenceLength;

	BYTE* pFamily = (BYTE*)_malloca(chatsize);
	if (pFamily == NULL)
		return;
	*pFamily = pf_chat;
