...
public:
	//在指定的位置绘制字符串中指定的内容
	int32_t		DrawTextLine(iFont* pFont, int32_t nFontSize, KOutputTextParam* pParam);

	//判断某一个坐标，在指定的位置输出的字符串中，所占的偏移量
	//注意：如果超出字符所占的矩形则返回的偏移为离指定坐标最近的一个字符的偏移
	int32_t		TransXYPosToCharOffset(int32_t nX, int32_t nY, iFont* pFont, int32_t nFontSize, KOutputTextParam* pParam);

	//跳到指定行开始处
	int32_t		SeekToSpecialLine(int32_t nLineIndex, KTP_CTRL& Ctrl1, KTP_CTRL& Ctrl2,
		int32_t& nSkipedHalfLines, int32_t nFontSize, int32_t bPicPackInSingleLine);
	//设置要处理的文本串内容
	void	SetContent(const char* pBuffer, int32_t nCount, int32_t nLineLen);
	...
	void GetCurLineHeight(int32_t& nSpanLines, int32_t& nHeight, int32_t nFontSize, int32_t bPicPackInSingleLine);
	...
	//构造函数
	KTextProcess();
	KTextProcess(const char* pBuffer, int32_t nCount, int32_t nLineLen);

private:
	//获取字符串里单一性质同行的一个子串
	int32_t		GetSimplexText(int32_t& nSimplexStartPos, KTP_CTRL& HeadCtrl, KTP_CTRL& TailCtrl, int32_t nFontSize);

private:
	const char* m_pBuffer;	//文本缓冲区的指针
	int32_t			m_nCount;	//文本缓冲区数据的长度(以BYTE为单位)
	int32_t			m_nCurrentLineLen;
	float		m_fCurrentLineLen;
	int32_t			m_nMaxLineLen;	//每行可以容纳的（半角）字符的数目
	int32_t			m_nReadPos;	//当前度读取的位置

	//for backup
	int32_t			m_bBackuped;

	int32_t			m_nCountB;
	int32_t			m_nCurrentLineLenB;
	float		m_fCurrentLineLenB;
	int32_t			m_nMaxLineLenB;
	int32_t			m_nReadPosB;
};
