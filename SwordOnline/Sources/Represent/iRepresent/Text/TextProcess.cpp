...
//	功能：构造函数
//--------------------------------------------------------------------------
KTextProcess::KTextProcess(const char* pBuffer, int32_t nCount, int32_t nLineLen)
{
...
//	功能：设置要处理的文本串内容
//--------------------------------------------------------------------------
void KTextProcess::SetContent(const char* pBuffer, int32_t nCount, int32_t nLineLen)
{
	if (pBuffer && nCount > 0)
	{
		m_pBuffer = pBuffer;
...
//	次再调用又将重新从文本串起始处获取子串。
//--------------------------------------------------------------------------
int32_t KTextProcess::GetSimplexText(int32_t& nSimplexStartPos, KTP_CTRL& HeadCtrl, KTP_CTRL& TailCtrl, int32_t nFontSize)
{
	int32_t nBeforeLen = 0,
	...
	while (m_nReadPos < m_nCount)
	{
		...
		else if (cCode == KTC_INLINE_PIC)	//[wxb 2003-6-20]
		{
			...
			TailCtrl.wParam = *((uint16_t*)(m_pBuffer + m_nReadPos + 1));
			if (g_pIInlinePicSinkRP)
			{
				...
				if (SUCCEEDED(g_pIInlinePicSinkRP->GetPicSize(TailCtrl.wParam, cx, cy)))
				{
					...
					m_nCurrentLineLen = (int32_t)(m_fCurrentLineLen + 0.9999);
					if (m_nMaxLineLen == 0 || m_nCurrentLineLen < m_nMaxLineLen)
					{
						m_nReadPos += 1 + sizeof(uint16_t);
					}
		...
		else if (cCode == KTC_COLOR || cCode == KTC_BORDER_COLOR)//颜色控制
		{
			...
			{
				(*(int32_t*)(&TailCtrl)) = *(int32_t*)(m_pBuffer + m_nReadPos);
				m_nReadPos += 4;
				break;
			}
			else
			{
				(*(int32_t*)(&HeadCtrl)) = *(int32_t*)(m_pBuffer + m_nReadPos);
				m_nReadPos += 4;
				nSimplexStartPos = m_nReadPos;
			}
		}
...
//--------------------------------------------------------------------------
int32_t KTextProcess::SeekToSpecialLine(int32_t nLineIndex, KTP_CTRL& Ctrl1, KTP_CTRL& Ctrl2,
	int32_t& nSkipedHalfLines, int32_t nFontSize, int32_t bPicPackInSingleLine)
{
	m_nReadPos = 0;
	int32_t nSimplexTextPos;
	...
	int32_t nLinesSpan = 0, nLineHeight = 0;
...
//	返回：实际绘制的文字行数
//--------------------------------------------------------------------------
int32_t KTextProcess::DrawTextLine(iFont* pFont, int32_t nFontSize, KOutputTextParam* pParam)
{
	_ASSERT(pParam);
	...
	int32_t			nCount = 0, nPos = 0;
	int32_t			x = pParam->nX, y = pParam->nY, nLineDrawed = 0;

	int32_t			nHalfIndex = 0, nFontHalfWidth[2];
	int32_t			nSkipedHalfLines = 0;	//因为嵌入式图片造成的跨半行现象
...
#define	_SET_CURRENT_BORDER_COLOR(c)				\
	...
	int32_t nSpanLines = 0, nLineHeight = 0;
	int32_t bFirstLine = 1;	//true
	do
	{
				...
				if (SUCCEEDED(g_pIInlinePicSinkRP->GetPicSize(TailCtrl.wParam, cx, cy)))
				{
					...
					if (nSkipedHalfLines && cy > nFontSize)
					{
						...
						if (cy > (nSpanLines - nSkipedHalfLines) * nFontSize)
							...
							nPicY -= (int32_t)(((cy - (nSpanLines - nSkipedHalfLines) * nFontSize)) * 1.5);
					}
					else
...
//	[wxb 2003-7-21]
//--------------------------------------------------------------------------
int32_t KTextProcess::TransXYPosToCharOffset(int32_t nX, int32_t nY, iFont* pFont, int32_t nFontSize, KOutputTextParam* pParam)
{
	_ASSERT(pParam);
	if (pFont == NULL || pParam->nNumLine <= 0)
		return 0;
...
#define	_SET_CURRENT_BORDER_COLOR(c)				\
	...
	int32_t nSpanLines = 0, nLineHeight = 0;
	int32_t bFirstLine = 1;			//true
	int32_t nOffsetXThisLine = 0, nOldX = 0, nOldReadPos = 0;
	do
	{
		if (0 == nSpanLines || KTC_ENTER == TailCtrl.cCtrl)
		{
...
//[wxb 2003-6-20]
void KTextProcess::GetCurLineHeight(int32_t& nSpanLines, int32_t& nHeight, int32_t nFontSize, int32_t bPicPackInSingleLine/*=FALSE*/)
{
	uint16_t wIndex = 0;
	nHeight = nFontSize;
	...
	if (g_pIInlinePicSinkRP && !bPicPackInSingleLine)
	{
		...
		while (m_nReadPos < m_nCount)
		{
			...
			{
				wIndex = *((uint16_t*)(m_pBuffer + m_nReadPos + 1));
				m_nReadPos += 1 + sizeof(uint16_t);
				if (g_pIInlinePicSinkRP)
				{
					int32_t cx, cy;
