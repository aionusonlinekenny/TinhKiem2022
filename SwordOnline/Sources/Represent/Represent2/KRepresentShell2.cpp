...
bool KRepresentShell2::Create(int nWidth, int nHeight, bool bFullScreen)
{
	...
	if (m_DirectDraw.Init())
	{
		...
		//m_lpDDSCanvas = m_DirectDraw.CreateSurface(nWidth, nHeight);
		m_ImageStore.Init();
...
void KRepresentShell2::DrawPrimitives(int nPrimitiveCount, KRepresentUnit* pPrimitives, unsigned int uGenre, int bSinglePlaneCoord)
{
	...
	case RU_T_IMAGE:
	{
		for (i = 0; i < nPrimitiveCount; i++)
		{
			KRUImage* pTemp = &((KRUImage*)pPrimitives)[i];
			...
			{
				SPRFRAME* pFrame = 0;
				SPRHEAD* pSprHeader = (SPRHEAD*)m_ImageStore.GetImage(pTemp->szImage, pTemp->uImage, pTemp->nISPosition,
...
//				Check Current Draw Device??;
				...
				case IMAGE_RENDER_STYLE_ALPHA_NOT_BE_LIT:
					if (pSprHeader->Reserved[1] == 1)
					{
						...
						if (pBuffer)
						{
							m_Canvas.DrawSpriteAlphaMMX((BYTE)(pTemp->Color.Color_b.a), (DWORD)m_Canvas.m_nMask32, pBuffer, m_Canvas.GetWidth(), m_Canvas.GetHeight(),
	...
	case RU_T_IMAGE_4:
	{
		...
		for (i = 0; i < nPrimitiveCount; i++)
		{
			KRUImage4* pTemp = &((KRUImage4*)pPrimitives)[i];
			//_ASSERT(pTemp->nType == ISI_T_SPR);
			{
				...
				case IMAGE_RENDER_STYLE_ALPHA_NOT_BE_LIT:
					if (pSprHeader->Reserved[1] == 1)
					{
						...
						{
							m_Canvas.DrawSpriteAlphaMMX((BYTE)(pTemp->Color.Color_b.a), (DWORD)m_Canvas.m_nMask32, pBuffer, m_Canvas.GetWidth(), m_Canvas.GetHeight(),
								nPitch, nX, nY, pPalette, pFrame->Sprite, pFrame->Width, pFrame->Height, &rc);
							m_Canvas.UnlockCanvas();
						}
					}
					else
	...
	case RU_T_IMAGE_PART:
	{
		...
		for (i = 0; i < nPrimitiveCount; i++)
		{
			KRUImagePart* pTemp = &((KRUImagePart*)pPrimitives)[i];
			switch (pTemp->nType)
			{
			case ISI_T_SPR:
			{
