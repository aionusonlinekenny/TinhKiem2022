...
bool KScenePlaceRegionC::Load(const char* pszBaseFolderName)
{
	...
	{
		UINT uMaxElemFile = 0;
		Data.Read(&uMaxElemFile, sizeof(UINT));
		...
		UINT uOffsetAhead = sizeof(UINT) + sizeof(KCombinFileSection) * uMaxElemFile;
...
void KScenePlaceRegionC::SetNestRegion(KScenePlaceRegionC* pNest)
{
	...
	UINT  i = RWP_NEST_REGION_0 << cRelate;
	if (m_Flag & i)
		return;
	m_Flag |= i;
...
bool KScenePlaceRegionC::LoadGroundLayer(KPakFile* pDataFile, UINT uSize)
{
	//_ASSERT(pDataFile);
	KGroundFileHead	Head = { 0 };
	UINT uStartPos = pDataFile->Tell();
	...
	if (Head.uNumObject)
	{
		...
		if (m_GroundLayerData.pObjects)
		{
			...
			for (UINT i = 0; i < Head.uNumObject; i++)
...
void KScenePlaceRegionC::LoadObstacle(KPakFile* pDataFile, UINT uSize)
{
...
void KScenePlaceRegionC::LoadTrap(KPakFile* pDataFile, UINT uSize)
{
	//_ASSERT(pDataFile);
...
bool KScenePlaceRegionC::LoadAboveGroundObjects(KPakFile* pDataFile, UINT uSize)
{
	_ASSERT(pDataFile);
	...
	if (m_BiosData.Numbers.nNumBios)
	{
		...
		for (UINT i = m_BiosData.Numbers.nNumBios - m_BiosData.Numbers.nNumBiosAbove;
...
void KScenePlaceRegionC::PaintObstacle()
{
	...
	for (i = 0; i < 16; i++, nX += RWP_OBSTACLE_WIDTH)
	{
		...
		for (j = 0; j < 32; j++, nY += RWP_OBSTACLE_HEIGHT)
		{
			...
			if (nIdx >= 0)
			{
				INT nRef = SubWorld[0].m_Region[nIdx].GetNpcRef(i, j, obj_npc);
...
//## 获取高空对象最大的绘图次序编号。
UINT KScenePlaceRegionC::GetAboveHeadLayer(KBuildinObj*& pObjsAboveHead)
{
	pObjsAboveHead = NULL;
	UINT uNum = 0;
...
//获得内建光源的信息
UINT KScenePlaceRegionC::GetBuildinLights(KBuildInLightInfo*& pLights)
{
	UINT uNum = 0;
...
void KScenePlaceRegionC::GetBuildinObjs(
	KIpotBuildinObj*& pObjsPointList, UINT& nNumObjsPoint,
	KIpotBuildinObj*& pObjsLineList, UINT& nNumObjsLine,
	KIpotBuildinObj*& pObjsTreeList, UINT& nNumObjsTree)
{
	pObjsPointList = NULL;
	pObjsLineList = NULL;
	pObjsTreeList = NULL;
...
void KScenePlaceRegionC::EnterProcessArea(KRUImage* pImage)
{
	...
	UINT i;
	UINT nCount = m_BiosData.Numbers.nNumBios
...
void KScenePlaceRegionC::SetHightLightSpecialObject(UINT uBioIndex)
{
	if (m_BiosData.pBios && uBioIndex < m_BiosData.Numbers.nNumBios)
		m_BiosData.pBios[uBioIndex].Props |= SPBIO_F_HIGHT_LIGHT;
}
void KScenePlaceRegionC::UnsetHightLightSpecialObject(UINT uBioIndex)
{
	if (m_BiosData.pBios && uBioIndex < m_BiosData.Numbers.nNumBios)
		m_BiosData.pBios[uBioIndex].Props &= ~SPBIO_F_HIGHT_LIGHT;
}

BOOL KScenePlaceRegionC::AddObstacle(INT nX, INT nY, INT nObstacleKind)
{
	...
	nMapX = (nX - m_LeftTopCornerScenePos.x) / RWP_OBSTACLE_WIDTH;
	nMapY = (nY - m_LeftTopCornerScenePos.y) / RWP_OBSTACLE_HEIGHT;
	if (nMapX < 0 || nMapX >= RWP_NUM_GROUND_CELL_H ||
		nMapY < 0 || nMapY >= RWP_NUM_GROUND_CELL_V * 2)
		return FALSE;

	m_ObstacleInfo[nMapX][nMapY] = (Obstacle_Full << 4) + nObstacleKind;
	return TRUE;
}

BOOL KScenePlaceRegionC::ClearObstacle(INT nX, INT nY)
{
	INT	 nMapX, nMapY;
	...
	m_ObstacleInfo[nMapX][nMapY] = Obstacle_NULL;
	return TRUE;
}