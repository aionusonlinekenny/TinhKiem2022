...
KScenePlaceMapC::KScenePlaceMapC()
{
	...
	bSearch = FALSE;
	m_FirstCellSkipWidth.cx = m_FirstCellSkipWidth.cy = 0;
	m_LastCellSkipHeight.cx = m_LastCellSkipHeight.cy = 0;
	m_nMapId = 0;
	m_uSelfColor = 0;
	m_uTeammateColor = 0;
	m_uPlayerColor = 0;
	m_uFightNpcColor = 0;
	m_uNormalNpcColor = 0;
	m_uSelfPartnerColor = 0;
	m_uOtherPartnerColor = 0;
	m_uOtherPlayerColor = 0;
	m_uOtherNpcColor = 0;
	m_uSelfNpcColor = 0;
	m_MapPos.left = m_MapPos.top = m_MapPos.right = m_MapPos.bottom = 0;
	ZeroMemory(m_SymbolMap, sizeof(m_SymbolMap));
	m_uSelfPlayerColor = 0;
...
//设置场景地图的包含的元素
void KScenePlaceMapC::SetShowElemsFlag(UINT uShowElemsFlag)
{
	...
	if (m_bHavePicMap && g_pRepresent)
	{
		...
		if (nNpcIdx >= 0 && Npc[nNpcIdx].m_RegionIndex >= 0)
		{
			...
			{	
				//----draw auto pathing----
				if (nNpcX >= m_MapCoverArea.left && nNpcX < m_MapCoverArea.right &&
					nNpcY >= m_MapCoverArea.top  && nNpcY < m_MapCoverArea.bottom && m_bPaintLine)
				{
...
void KScenePlaceMapC::LoadSymbol(INT nSubWorldID)
{
	...
	if (g_MapTraffic.IsSectionExist(szSectName))
	{
		...
		for (INT j = 0; j < defMAX_NUM_SYMBOL; j++)
		{
			...
			g_MapTraffic.GetInteger(szSectName, szKeyName, -1, &m_SymbolMap[j + 1].Kind);
...
void KScenePlaceMapC::PaintFindPos(INT nX, INT nY, INT nDesX, INT nDesY)
{
	...
	if (nNpcIdx && Npc[nNpcIdx].m_RegionIndex >= 0)
	{
		...
		if (m_bPaintLine)
		{
			...
			Line.oPosition.nZ = Line.oEndPos.nZ = 0;
			Line.oEndPos.nX = nDesX - 2;
			Line.oEndPos.nY = nDesY - 2;
			// g_pRepresent->DrawPrimitives(1, &Line, RU_T_LINE, 1);
			char MapPos[80];
			DWORD dwColor;
			...
			/*target.x = m_DirectPos.x * 16;
			target.y = m_DirectPos.y * 32;
...
void KScenePlaceMapC::DirectFindPos(INT nX, INT nY, BOOL bSync, BOOL bPaintLine)
{
	...
	if(m_bPaintLine)// find way by kinnox;
   	{
   		AutoRunTo(nDesX, nDesY);   
		g_DebugLog("Check TOA DO: %d - %d", nDesX, nDesY);
   	}
