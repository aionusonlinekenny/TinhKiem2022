...
KWeatherRain::KWeatherRain()
{
	...
	m_nFlareStateDelay = g_Random(m_nFlareTime);
	m_nFlareStateStart = g_SubWorldSet.GetGameTime();
}
...
void KWeatherSnow::Breath()
{
	...
	// 要生成的粒子数目
	INT nCreateNum = (INT)m_nParticleNum / 20;
	// 生成粒子
	INT nZOff = (INT)m_nDownSpeed / 20;
	KParticle particle;
	for (INT j = 0; j < nCreateNum; j++)
	{
		particle.m_vPos.nX = m_nLeftTopX + g_Random(WEATHER_AREA_WIDTH);
		particle.m_vPos.nY = m_nLeftTopY + g_Random(WEATHER_AREA_HEIGHT);
		particle.m_vPos.nZ = (INT)(500.0f + (particle.m_vPos.nY - m_nFocusPosY) / 3 - g_Random(nZOff));
		particle.m_vSpeed.nX = (INT)m_nWindSpeed / 20;
		particle.m_vSpeed.nZ = (INT)m_nDownSpeed / 20;
