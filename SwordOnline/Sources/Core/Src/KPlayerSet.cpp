...
#ifdef _SERVER
	m_ulNextSaveTime = 0;
	m_ulMaxSaveTimePerPlayer = 60 * 20 * 30; //5 mins
...
INT KPlayerSet::Add(LPSTR szPlayerID, void* pGuid)
{
	...
	if (i > 0)
	{
		if (j > 0) // nUseIdx If there is a card number, remove the link and delete the linked list playeridx
		{
			if (Player[j].m_nNetConnectIdx >= 0)
			{ // This index is already linked to the server
				if (Player[j].m_byLixian == LIXIAN_DOING)
				{
					Player[j].m_byLixian = LIXIAN_IDLE;

					if (Player[j].Save())
						Player[j].m_uMustSave = SAVE_REQUEST;
					//return 0;
				}
...
INT	KPlayerSet::Broadcasting(char* pMessage, INT nLen)
{
	...
	//printf("Chat Message: %s", pMessage);
...
void KPlayerSet::PrepareRemove(INT nIndex)
{
	...
	//if (Player[nIndex].m_byLixian == LIXIAN_IDLE)
	{
	...
	if (Player[nIndex].IsWaitingRemove())
	{
		if (Player[nIndex].m_nIndex > 0)	// have npc
		{
			...
			if (nSubWorld >= 0 && nRegion >= 0)
			{
				...
				SubWorld[nSubWorld].m_Region[nRegion].DecNpcRef(Npc[Player[nIndex].m_nIndex].m_MapX, Npc[Player[nIndex].m_nIndex].m_MapY, obj_npc);
			}
...
INT		KMagicLevelExp::GetNextExp(INT nSkillId, INT nLevel)
{
	...
	if (nLevel > MAX_TRAIN_SKILLEXPLEVEL)
		nLevel = MAX_TRAIN_SKILLEXPLEVEL;
...
#ifdef _SERVER
INT		KPlayerSet::AttachPlayer(const ULONG lnID, GUID* pGuid)
{
	...
	while (nUseIdx)
	{
		if (Player[nUseIdx].m_nNetConnectIdx == -1)
		{
			if (0 == memcmp(&Player[nUseIdx].m_Guid, pGuid, sizeof(GUID)))
			{
				...
				//Player[nUseIdx].m_dwLoginTime = g_SubWorldSet.GetGameTime();

				std::string szIpAddress = "";
				char szDesMsg[200];
				szDesMsg[0] = 0;
				szIpAddress = g_pServer->GetClientInfo(Player[nUseIdx].m_nNetConnectIdx);
				szIpAddress = szIpAddress.substr(0, szIpAddress.find(":"));
				strcpy(Player[nUseIdx].m_szIpAddress, szIpAddress.c_str());
...
{
	ULONG uTime = g_SubWorldSet.GetGameTime();
