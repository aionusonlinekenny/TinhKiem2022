...
void KPlayerAI::Release()
{
	...
	m_bWithDrawMoney = TRUE;
	m_nWithDrawMoneyV = 500000;
	m_bMoveMps = FALSE;
	m_nMoveStep = 0;
	m_nMoveCount = 0;
...
void KPlayerAI::Active()
{
	...
	if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_Doing == do_death || Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_Doing == do_revive)
		return;
	...
	if (m_bIsActive)
	{
			...
			if (!m_nObject)
			{
				...
				if (m_nObject)
				{
					...
					else
					{
						...
						{
							if (FALSE == Player[CLIENT_PLAYER_INDEX].m_ItemList.SearchPosition(Object[m_nObject].m_nItemWidth, Object[m_nObject].m_nItemHeight, &sItemPos, true))
							{
								m_nObject = 0;
								m_nLifeOverLook = 0;
								m_nCount_Run_OverLook = 0;
		...
		else
		{
			...
			{
				RepairEquip();
				m_nTargetHP = Npc[m_nTargetIdx].m_CurrentLifeMax;
...
void KPlayerAI::MoveQuanhDiem()
{
	...
	if (Npc[nNpcIdx].m_Doing == do_attack || Npc[nNpcIdx].m_Doing == do_skill || Npc[nNpcIdx].m_Doing == do_magic || Npc[nNpcIdx].m_Doing == do_goattack)
		return;
	...
	else
	{
		...
		distance = (INT)sqrt((INT)(nDesX - newX) * (nDesX - newX) + (nDesY - newY) * (nDesY - newY));
		...
		else if (m_bAttackNpc == TRUE && m_bQuanhDiem == TRUE)
		{
			...
			else if (Npc[nNpcIdx].m_Doing == do_stand && m_bIsActive && distance >= 1000)
			{
		...
		else
		{
			...
			else
			{
					...
					Npc[nNpcIdx].m_bActivateAutoMoveBarrier4 == FALSE)
				{
					...
					distance = (INT)sqrt((INT)(newX - oldX1) * (newX - oldX1) + (newY - oldY1) * (newY - oldY1));
	...
	if (AddNpc2Array(nRelation))
	{
		...
		for (i = 0; i < MAX_AUTO_SIZE; i++)
		{
			if (m_nArrayNpcNeast[i] > 0)
			{
				...
				BYTE nBarrier = SubWorld[0].GetBarrier(nX, nY);
				if (nBarrier)
				{
					g_DebugLog("Muc tieu la vat can hoac co vat can! %d", nBarrier);
					continue;
				}
...
BOOL KPlayerAI::FollowAttack(INT i)
{
	...
	if (distance <= distanceskill && distance <= m_nRadiusAuto)
	{
		...
		if (pSkill)
		{
			...
			INT nNpcX, nNpcY;
			Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].GetMpsPos(&nNpcX, &nNpcY);

			Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].SendCommand(do_skill, Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_ActiveSkillID, -1, i);
			SendClientCmdSkill(Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_ActiveSkillID, -1, Npc[i].m_dwID, nNpcX, nNpcY);
	...
	else
	{
		...
		else
		{
			...
			if (m_IgnoreCornerLag >= 10)
			{
				...
				MoveTo(nDesX + g_Random(100), nDesY + g_Random(100));
...
BOOL KPlayerAI::SellItem()
{
	std::vector<PlayerItem> vItem;
	PlayerItem* pItem = Player[CLIENT_PLAYER_INDEX].m_ItemList.GetFirstItem();

	while (pItem)
	{
		if (pItem->nPlace == pos_equiproom && Item[pItem->nIdx].m_CommonAttrib.nItemGenre == item_equip)
		{
			BOOL bMatch = FALSE;
			if (Item[pItem->nIdx].GetLock()->IsLock() || Item[pItem->nIdx].GetLockSell()
				|| Item[pItem->nIdx].GetNature() == NATURE_GOLD || Item[pItem->nIdx].GetNature() == NATURE_PLATINA
				|| Item[pItem->nIdx].GetNature() == NATURE_VIOLET)
				bMatch = TRUE;
	...
	// Kiểm tra xem có ít nhất một mục để bán không
	if (!vItem.empty())
	{
		INT nCount = 0;
		// Lặp qua danh sách các mục để bán và gọi hàm bán
		do
		{
			const auto& item = vItem.back();
			SendClientCmdDirectSell(Item[item.nIdx].GetID(), 1);
			vItem.pop_back();
	...
	return TRUE; // Trả về FALSE nếu không có mục nào để bán
}
	...
	for (INT i = 0; i < MAX_AUTO_STATESKILL; i++)
	{
		if (m_nSupportSkills[i])
		{
				...
				Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].IsNpcSkillExist(m_nSupportSkills[i]))
			{
				...
				SendClientCmdSkill(m_nSupportSkills[i], -1, Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_dwID, nNpcX, nNpcY);
				return TRUE;
			}
		}
	}
	...
	if (m_bLifeReplenish)
	{
			...
			Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].Cost(pSkill->GetSkillCostType(), pSkill->GetSkillCost(this), TRUE, TRUE))
		{
			if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_CurrentLife * MAX_PERCENT / Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_CurrentLifeMax < m_nLifeReplenishP)
			{
				...
				SendClientCmdSkill(nSkillLifeReplenishID, -1, Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_dwID, nNpcX, nNpcY);
				return TRUE;
			}
		}
	}
...
void KPlayerAI::RepairEquip()
{
	if (m_bRepairEquip)
	{
		for (INT i = itempart_head; i < itempart_horse; i++)
		{
			...
			if (nIdx && Item[nIdx].CanBeRepaired())
			{
...
BOOL KPlayerAI::ReturnFromPortal()
{
	...
	case Step_InventoryItem:
		if (m_bInventoryItem)
		{
			if (InventoryItem())
			{
				...
				m_nReturnPortalStep = Step_SellTrash;
			}
		}
		else
		{
	...
	case Step_SellTrash:
		if (m_bSellEquipment)
		{
			...
			{
				m_nCurReturnPortalSec = KSG_GetCurSec() + m_nReturnPortalSec;
				m_nReturnPortalStep = Step_WithDraw;
			}
		}
		else
		{
	...
	case Step_WithDraw:
		if (m_bWithDrawMoney)
			Player[CLIENT_PLAYER_INDEX].m_ItemList.WithDrawaMoney(room_repository, room_equipment, m_nWithDrawMoneyV);
	...
	case Step_InventoryMoney:
	{
		...
		{
			INT nRoomMoney = Player[CLIENT_PLAYER_INDEX].m_ItemList.GetEquipmentMoney();

			if (m_bWithDrawMoney && nRoomMoney > m_nWithDrawMoneyV)
				Player[CLIENT_PLAYER_INDEX].m_ItemList.ExchangeMoney(room_equipment, room_repository, nRoomMoney - m_nWithDrawMoneyV);
			else
				Player[CLIENT_PLAYER_INDEX].m_ItemList.ExchangeMoney(room_equipment, room_repository, nRoomMoney);
		}
	...
	case Step_Return:
		if (m_bReturnPortal)
		{
			if (KSG_GetCurSec() >= m_nCurReturnPortalSec)
			{
				...
				sRP.m_lpBuf = 0;
				sRP.nParam = m_nWayPointId;
				sRP.m_wLength = sizeof(SHOW_MSG_SYNC) - 1;
				g_pClient->SendPackToServer((BYTE*)&sRP, sRP.m_wLength + 3);
				nRet = FALSE;
			}
		}
		break;
