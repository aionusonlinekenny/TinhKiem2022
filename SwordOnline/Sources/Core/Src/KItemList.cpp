...
BOOL KItemList::Equip(INT nIdx, INT nPlace /* = -1 */)
{
	...
	case itempart_head:
		...
		if (nCurIdx)
		{
			...
			else if (Item[nCurIdx].m_CommonAttrib.nItemNature == NATURE_PLATINA)
				Npc[nNpcIdx].m_HelmType = g_ItemChangeRes.GetPlatinaItemRes(Item[nCurIdx].GetDetailType(), Item[nCurIdx].GetRow());
			else
		...
		else
		{
			if (Item[nIdx].m_CommonAttrib.nItemNature == NATURE_GOLD)
				...
				Npc[nNpcIdx].m_HelmType = g_ItemChangeRes.GetPlatinaItemRes(Item[nIdx].GetDetailType(), Item[nIdx].GetRow());
			else
	...
	case itempart_body:
	{
		...
		if (nCurIdx)
		{
			if (Item[nCurIdx].m_CommonAttrib.nItemNature == NATURE_GOLD)
				...
				Npc[nNpcIdx].m_ArmorType = g_ItemChangeRes.GetPlatinaItemRes(Item[nCurIdx].GetDetailType(), Item[nCurIdx].GetRow());
			else
		...
		else
		{
			if (Item[nIdx].m_CommonAttrib.nItemNature == NATURE_GOLD)
				...
				Npc[nNpcIdx].m_ArmorType = g_ItemChangeRes.GetPlatinaItemRes(Item[nIdx].GetDetailType(), Item[nIdx].GetRow());
			else
	...
	case itempart_weapon:
		if (Item[nIdx].m_CommonAttrib.nItemNature == NATURE_GOLD)
			...
			Npc[nNpcIdx].m_WeaponType = g_ItemChangeRes.GetPlatinaItemRes(Item[nIdx].GetDetailType(), Item[nIdx].GetRow());
		else
		...
		if (!m_bMaskLock)
		{
			...
			else
			{
				if (Item[nIdx].m_CommonAttrib.nItemNature == NATURE_GOLD)
					...
					Npc[nNpcIdx].m_MaskType = g_ItemChangeRes.GetPlatinaItemRes(Item[nIdx].GetDetailType(), Item[nIdx].GetRow());
				else
	...
	case itempart_mantle:
		...
		if (Item[nIdx].m_CommonAttrib.nItemNature == NATURE_GOLD)
			...
			Npc[nNpcIdx].m_MantleType = g_ItemChangeRes.GetPlatinaItemRes(Item[nIdx].GetDetailType(), Item[nIdx].GetRow());
		else
	...
	case itempart_horse:
		if (Item[nIdx].m_CommonAttrib.nItemNature == NATURE_GOLD)
			...
			Npc[nNpcIdx].m_HorseType = g_ItemChangeRes.GetPlatinaItemRes(Item[nIdx].GetDetailType(), Item[nIdx].GetRow());
		else
...
void KItemList::ExchangeItem(ItemPos* SrcPos, ItemPos* DesPos)
{
	...
	case pos_equip:
	{
		...
		if (m_Hand)
		{
			if (DesPos->nX == 7)
			{
				INT checkRing1 = m_EquipItem[7];
				INT checkRing2 = m_EquipItem[8];
				if (checkRing1 > 0 && checkRing2 <= 0)
				{
					SrcPos->nX = 8;
					DesPos->nX = 8;
				}
...
BOOL KItemList::CanCombie(INT Source, INT Dest)
{
	...
	if (Player[m_PlayerIdx].GetLockMove()->bLock)
		return FALSE;
	if (Item[Source].GetLock()->IsLock() != Item[Dest].GetLock()->IsLock())
		return FALSE;
	...
	if (Item[Source].IsStack() && Item[Dest].IsStack() &&
		...
		Item[Source].GetLock()->IsLock() == Item[Dest].GetLock()->IsLock()
		)
		return TRUE;
...
void KItemList::SyncItem(INT nIdx, BOOL bIsNew, INT nPlace, INT nX, INT nY, INT nPlayerIndex)
{
	...
	//if (nPlayerIndex != m_PlayerIdx)
	//	printf("nPlayerIdx[%d] - nPlayerIndex[%d]", m_PlayerIdx, nPlayerIndex);
	...
	//g_DebugLog("[ITEM]Item Begin for Player [%d]", m_PlayerIdx);
	//while (nLoopIdx)
	//{
...
{
	INT p, x, y;
	if (CheckCanPlaceInEquipment(Item[nIdx].GetWidth(), Item[nIdx].GetHeight(), &p, &x, &y) && !Item[nIdx].GetLock()->IsLock())
	{
...
void KItemList::ExecuteScript(INT nIdx)
{
	...
	default:
		...
		if (szScriptFile[0])
			Player[m_PlayerIdx].ExecuteScript(szScriptFile, NORMAL_FUNCTION_NAME, nIdx, SCRIPT_ITEMINDEX);
		//if (Player[m_PlayerIdx].ExecuteScript(szScriptFile, NORMAL_FUNCTION_NAME, nIdx))
			//RemoveItem(nIdx, 1);
		break;
	}
}

void KItemList::MsgShow(INT MsgID) //47
{
	SHOW_MSG_SYNC	sMsg;
	sMsg.ProtocolType = s2c_msgshow;
	sMsg.m_wMsgID = MsgID;
	sMsg.m_wLength = sizeof(SHOW_MSG_SYNC) - 1;
	g_pServer->PackDataToClient(Player[m_PlayerIdx].m_nNetConnectIdx, (BYTE*)&sMsg, sMsg.m_wLength + 1);
}
