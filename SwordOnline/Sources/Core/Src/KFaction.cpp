...
#ifdef _SERVER
	printf("Size of Faction Ini file: %d\n", Ini.GetSectionCount());
#endif

	INT nMaxFaction = Ini.GetSectionCount() - 1;

	if (nMaxFaction < MAX_FACTION)
		nMaxFaction = MAX_FACTION;

	// 清空
	for (i = 0; i < nMaxFaction; i++)
	{
...
#ifdef _SERVER
		printf("Faction: Idx[%d] - %s - Series: %d - ShowName: %s - Camp: %d\n", m_sAttribute[nArrayPos].m_nIndex, m_sAttribute[nArrayPos].m_szName, m_sAttribute[nArrayPos].m_nSeries, m_sAttribute[nArrayPos].m_szShowName, m_sAttribute[nArrayPos].m_nCamp);
#endif
	}
