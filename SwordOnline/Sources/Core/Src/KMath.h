...
//inline INT	__cdecl g_InternalDirSinCos(INT pSinCosTable[], INT nDir, INT nMaxDir)
//{
...
inline INT __cdecl g_InternalIsAccrueConquer(INT pAccrueConquerTable[], INT nSrcSeries, INT nDesSeries)
{
...
const UINT sqrttable[SQRTTABLESIZE] =
{
...
// Returns the square of the distance (reducing the square root operation)
...
// ÏÂÃæµÄº¯Êý¼ÆËã·½ÏòÓÐÆ«²î£¬ÕâÀï×¨ÃÅÐ´Ò»¸öÌØÊâ°æ±¾Îª¸øÑ°Â·Ê¹ÓÃ
inline INT	g_GetDirIdxForFindPath(INT nX1, INT nY1, INT nX2, INT nY2)
{
	...
	for (INT i = 0; i < 32; i++)		// Ë³Ê±Õë·½Ïò ´Ó270¶Èµ½90¶È£¬sinÖµµÝ¼õ
	{
		if (nSin > g_nSin[i])
			break;
		nRet = i;
