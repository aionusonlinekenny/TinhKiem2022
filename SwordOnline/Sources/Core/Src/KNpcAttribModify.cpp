...
KNpcAttribModify::KNpcAttribModify()
{
	...
	ProcessFunc[magic_lightingdamage_p] = &KNpcAttribModify::AddLightingDamageP;
	...
	ProcessFunc[magic_deadlystrikeenhance_p] = &KNpcAttribModify::DeadlyStrikeEnhanceP;		//chi mang
	ProcessFunc[magic_fatallystrikeenhance_p] = &KNpcAttribModify::FatallyStrikeEnhanceP;	//chi tu
...
void KNpcAttribModify::AddLightingDamageP(KNpc* pNpc, void* pData)
{
	KMagicAttrib* pMagic = (KMagicAttrib*)pData;
	//pNpc->m_CurrentLightEnhance += pMagic->nValue[0];
	pNpc->m_CurrentLightDamage.nValue[0] += (LONG)(pNpc->m_Level / 2) * pMagic->nValue[0];
	pNpc->m_CurrentLightDamage.nValue[2] += (LONG)(pNpc->m_Level / 2) * pMagic->nValue[0];
}
...
void KNpcAttribModify::AddLightingMagicP(KNpc* pNpc, void* pData)
{
	KMagicAttrib* pMagic = (KMagicAttrib*)pData;
	//pNpc->m_CurrentLightEnhance += pMagic->nValue[0];
	pNpc->m_CurrentLightMagic.nValue[0] += (LONG)(pNpc->m_Level / 2) * pMagic->nValue[0];
	pNpc->m_CurrentLightMagic.nValue[2] += (LONG)(pNpc->m_Level / 2) * pMagic->nValue[0];	//pMagic->nValue[0];
}

void KNpcAttribModify::AddPhysicsDamageP(KNpc* pNpc, void* pData)
{
#define WEAPON_ALL			6	//((MAX_MELEE_WEAPON) + 0)
#define	WEAPON_RANGE_ALL	7	//((MAX_MELEE_WEAPON) + 1)
#define	WEAPON_MELEE_ALL	8	//((MAX_MELEE_WEAPON) + 2)
#define	WEAPON_NONE			9	//((MAX_MELEE_WEAPON) + 3)

	KMagicAttrib* pMagic = (KMagicAttrib*)pData;

	//pNpc->m_CurrentAddPhysicsDamageP += pMagic->nValue[0];//Current external percentage
	...
	{
		for (INT i = 0; i < MAX_MELEE_WEAPON * 2; i++)
		{
			pNpc->m_CurrentMeleeEnhance[i] += pMagic->nValue[0];
		}
	}
	else if ((nType >= 0 && nType < MAX_MELEE_WEAPON) || nType > WEAPON_NONE)
	{
		pNpc->m_CurrentMeleeEnhance[Player[pNpc->m_nPlayerIdx].m_ItemList.GetWeaponParticular()] += pMagic->nValue[0];
	}
}
...
{
	KMagicAttrib* pMagic = (KMagicAttrib*)pData;
	pNpc->m_CurrentWalkSpeed += ((INT)(pNpc->m_WalkSpeed * pMagic->nValue[0]) / 100);
	pNpc->m_CurrentRunSpeed += ((INT)(pNpc->m_RunSpeed * pMagic->nValue[0]) / 100);
}
...
{
	KMagicAttrib* pMagic = (KMagicAttrib*)pData;
	pNpc->m_CurrentLifeReplenish += (INT)pMagic->nValue[0];
}
...
void KNpcAttribModify::AutoCastSkill(KNpc* pNpc, void* pData)
{
	KMagicAttrib* pMagic = (KMagicAttrib*)pData;
	if (pMagic->nValue[0])
	{
		for (INT i = 0; i < MAX_AUTOSKILL; i++)
		{
			if (pNpc->m_CastSkill[i].nSkillId <= 0)
			{
				pNpc->m_CastSkill[i].nSkillId = pMagic->nValue[0] / 256;
				pNpc->m_CastSkill[i].nSkillLevel = pMagic->nValue[0] % 256;
				pNpc->m_CastSkill[i].dwNextCastTime = SubWorld[pNpc->m_SubWorldIndex].m_dwCurrentTime;
				pNpc->m_CastSkill[i].nWaitCastTime = pMagic->nValue[2] / 256;
				pNpc->m_CastSkill[i].nRate = pMagic->nValue[2] % 256;
				break;
			}
		}
	}
