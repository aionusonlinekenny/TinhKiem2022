...
KMissle::KMissle()
{
	...
	m_dwFollowNpcID = 0;
	m_dwLauncherId = 0;
	m_eFollowKind = MISSLE_MFK_None;
	m_eMissleStatus = MS_DoWait;
	m_eMoveKind = MISSLE_MMK_Stand;
...
BOOL	KMissle::CheckBeyondRegion(INT nDOffsetX, INT nDOffsetY)
{
	...
	// dungnd fix vuot kich thuoc
	if (abs(nNewXOffset) > CellWidth * 2) //2
		if (nNewXOffset < 0)
			nNewXOffset = -(CellWidth * 2);
		else
			nNewXOffset = CellWidth * 2;

	if (abs(nNewYOffset) > CellHeight * 2)
		if (nNewYOffset < 0)
			nNewYOffset = -(CellHeight * 2);
		else
			nNewYOffset = CellWidth * 2;
...
BOOL KMissle::ProcessDamage(INT nNpcId)
{
	...
	if (m_pMagicAttribsData)
	{
		KSkill* pSkill = (KSkill*)g_SkillManager.GetSkill(m_nSkillId, 1);

		if (m_nLauncher != nNpcId)
		{
			if (!Npc[nNpcId].IsPlayer() &&
				pSkill->IsAura() &&
				pSkill->IsTargetNoNpc())
				return TRUE;
		}
