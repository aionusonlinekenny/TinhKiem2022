...
KItem::KItem()
{
	...
	::memset(&m_CommonAttrib.LockItem, 0, sizeof(m_CommonAttrib.LockItem));
	m_nCurrentDur = -1;
	m_CommonAttrib.bLockTrade = FALSE;
	m_CommonAttrib.bLockSell = FALSE;
	m_CommonAttrib.bLockDrop = FALSE;
#ifndef _SERVER
...
void KItem::operator = (const KBASICPROP_QUEST& sData)
{
	...
	pCA->nPhysicVal = 0;
	pCA->nMagicVal = 0;
	pCA->bLockSell = sData.m_bLockSell;
	pCA->bLockTrade = sData.m_bLockTrade;
	pCA->bLockDrop = sData.m_bLockDrop;
	pCA->nParam = -1;
	pCA->nFortune = 0;
	pCA->nExpireTime = 0;
	pCA->LockItem.Clear();
...
void KItem::GetDesc(char* pszMsg, bool bShowPrice, bool bPriceScale, INT nActiveAttrib, INT nGoldActiveAttrib)
{
	...
	if (m_CommonAttrib.nItemGenre == item_equip)
	{
		...
		else if (m_CommonAttrib.nDetailType <= equip_shipin)
		{
	...
	if (m_CommonAttrib.szIntro[0])
	{
		...
		while (szIntro[offset] != 0)
		{
			if (szIntro[offset] == '<')
			{
				if (szIntro[offset + 1] == 'e' && szIntro[offset + 2] == 'n' && szIntro[offset + 3] == 't' && szIntro[offset + 4] == 'e' && szIntro[offset + 5] == 'r' && szIntro[offset + 6] == '>')
					nL = 0;
				if (szIntro[offset + 1] == 'c' && szIntro[offset + 2] == 'o' && szIntro[offset + 3] == 'l' && szIntro[offset + 4] == 'o' && szIntro[offset + 5] == 'r')
				{
					if (szIntro[offset + 6] == '>')
						nL -= 6;
					else
				...
				if (szIntro[offset + 1] == 'b' && szIntro[offset + 2] == 'c' && szIntro[offset + 3] == 'l' && szIntro[offset + 4] == 'r')
				{
					if (szIntro[offset + 5] == '>')
						nL -= 5;
					else
	...
	for (i = 0; i < MAX_ITEM_MAGICATTRIB; i++)
	{
		...
		if (m_aryMagicAttrib[i].nValue[0] == m_aryMagicAttrib[i].nMax)
			sprintf(pszMsg, "%s <color=255,85,0>[MAX]", pszMsg);
		else
			sprintf(pszMsg, "%s [%d-%d]", pszMsg, m_aryMagicAttrib[i].nMin, m_aryMagicAttrib[i].nMax);
		strcat(pszMsg, "\n");
		strcat(pszMsg, "<color=255,255,255>");
	}
	...
	if (/*!bActiveAllAttrib && */m_aryMagicAttrib[0].nAttribType && m_CommonAttrib.nItemGenre == item_equip && m_CommonAttrib.nDetailType < equip_horse)
	{
		if (m_pItems.nPlace == pos_equip)
		{
			...
			g_GameSetting.GetString("ActiveEquip", pszKeyName, "", szBuff, sizeof(pszKeyName));
			...
			g_GameSetting.GetString("ActiveEquip", pszKeyName, "", szBuffer, sizeof(pszKeyName));
	...
	if (Script.Load(m_CommonAttrib.szScript))
	{
		...
		if (Lua_IsString(pScript->m_LuaState, Lua_GetTopIndex(pScript->m_LuaState)) == 1)
		{
			...
			sprintf(pszMsg, "%s<color=255,255,255>\n<color=255,255,255>\n%s\n<color=255,255,255>", pszMsg, (char*)Lua_ValueToString(pScript->m_LuaState, Lua_GetTopIndex(pScript->m_LuaState)));
		}
	...
	if (m_CommonAttrib.nFortune)
	{
		...
		//strcat(pszMsg, pszTemp2);
		//strcat(pszMsg, "\n<color=255,255,255>");
		sprintf(pszMsg, "%s\n%s\n<color=255,255,255>", pszMsg, pszTemp2);
	}
	...
	{
		//strcat(pszMsg, "\n");
		//strcat(pszMsg, "\n<color=255,255,255>Gi¸ niªm yÕt: <color=255,255,0>");
		sprintf(pszMsg, "%s\n<color=255,255,255>Gi¸ niªm yÕt: <color=255,255,0>", pszMsg);
	...
	{
		char szTemp[256];
		sprintf(szTemp, "<color=green>ItemID: [%d]<color>\n", m_dwID);
		sprintf(szTemp, "%s<color=green>LockDrop: [%d]<color>\n", szTemp, m_CommonAttrib.bLockDrop);
		sprintf(szTemp, "%s<color=green>LockTrade: [%d]<color>\n", szTemp, m_CommonAttrib.bLockTrade);
		sprintf(szTemp, "%s<color=green>LockSell: [%d]<color>", szTemp, m_CommonAttrib.bLockSell);
		sprintf(pszMsg, "%s\n%s\n", pszMsg, szTemp);
		//strcat(pszMsg, "\n");
...
INT KItem::GetRepairPrice()
{
	...
	{
	case NATURE_GOLD:
		return m_CommonAttrib.nPrice * ItemSet.m_sRepairParam.nGoldScale / 300 * (nMaxDur - m_nCurrentDur); //100
	case NATURE_PLATINA:
		return m_CommonAttrib.nPrice * ItemSet.m_sRepairParam.nPlatinaScale / 300 * (nMaxDur - m_nCurrentDur); //100
	default:
