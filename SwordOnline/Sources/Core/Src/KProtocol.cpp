...
INT	g_nProtocolSize[MAX_PROTOCOL_NUM] =
{
	...
	sizeof(C2S_COMPOUND_ITEM),	//c2s_compound
	-1,
	sizeof(PLAYER_SELL_ITEM_COMMAND),			// c2s_directsellitem
...
void SendClientCmdSkill(INT nSkillID, INT nX, INT nY, INT nNpxPosX, INT nNpxPosY)
{
	try
	{
		...
		NetCommand.nMpsX = nX;
		NetCommand.nMpsY = nY;
		NetCommand.nPosX = nNpxPosX;
		NetCommand.nPosY = nNpxPosY;
		if (g_pClient)
...
void SendClientCmdDirectSell(INT nId, INT nNumber)
{
	if (Player[CLIENT_PLAYER_INDEX].m_ItemList.IsLockOperation())
		return;
	PLAYER_SELL_ITEM_COMMAND PlayerSell;
	PlayerSell.ProtocolType = c2s_directsale;
	PlayerSell.m_ID = nId;
	PlayerSell.m_Number = (BYTE)nNumber;
	if (g_pClient)
		g_pClient->SendPackToServer((BYTE*)&PlayerSell, sizeof(PLAYER_SELL_ITEM_COMMAND));
...
//Compound Item
void SendCompoundAction(INT nAction)
{
	C2S_COMPOUND_ITEM pInput;
	pInput.ProtocolType = c2s_compound;
	pInput.nAction = nAction;

	if (g_pClient)
		g_pClient->SendPackToServer((BYTE*)&pInput, sizeof(C2S_COMPOUND_ITEM));
}

void SendEnchaseAction(INT nAction)
{
	C2S_COMPOUND_ITEM pInput;
	pInput.ProtocolType = c2s_enchase;
	pInput.nAction = nAction;
