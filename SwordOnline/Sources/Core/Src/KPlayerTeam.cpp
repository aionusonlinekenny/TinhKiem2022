...
void	KPlayerTeam::UpdateInterface()
{
	...
	{
		CoreDataChanged(GDCNI_TEAM, (UINT)&sTeam, 0);
	}
}
#endif
...
{
	if (nTargetIdx < 0 || nTargetIdx > MAX_PLAYER || nSelfIdx < 0 || nSelfIdx > MAX_PLAYER)
		return;

	if (Player[nSelfIdx].m_bForbidParty || Player[nTargetIdx].m_bForbidParty)
		return;
...
{
	if (Player[nPlayerIndex].m_bForbidParty)
		return FALSE;
