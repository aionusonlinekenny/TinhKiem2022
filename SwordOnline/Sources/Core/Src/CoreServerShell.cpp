...
public:
	...
	bool IsPlayerOffline(INT nIndex);
	void PlayerOfflineGateway(INT nIndex);
	...
	bool PlayerDbLoading(INT nPlayerIndex, INT bSyncEnd, INT& nStep, UINT& nParam);
	...
	bool SetChangeRoleStatus(INT nIndex, INT uStatus, char* nRoleName);

	BOOL GroupChat(IClient* pClient, DWORD FromIP, ULONG FromRelayID, DWORD channid, BYTE tgtcls, DWORD tgtid, const void* pData, size_t size);
...
bool CoreServerShell::PlayerDbLoading(INT nPlayerIndex, INT bSyncEnd, INT& nStep, UINT& nParam)
{
...
//							GAMEDATA_INDEX的取值之一。
//		  UINT uParam  --> 依据uDataId的取值情况而定
//		  INT nParam --> 依据uDataId的取值情况而定
//	返回：依据uDataId的取值情况而定。
//--------------------------------------------------------------------------
INT	CoreServerShell::GetGameData(UINT uDataId, UINT uParam, INT nParam)
{
	INT nRet = 0;
	switch (uDataId)
	{
	...
	case SGDI_LOADEDMAP_ID:
		if (uParam)
		{
			...
			for (i = 0; i < nMax; i++)
			{
				...
				{
					((int16_t*)uParam)[i] = (int16_t)SubWorld[i].m_SubWorldID;
	...
	case SGDI_TONG_APPLY_CREATE:
		if (uParam)
		{
			...
			memcpy(szTongName, pApply->m_szTongName, sizeof(pApply->m_szTongName));
	...
	case SGDI_TONG_BE_CHANGED_EFF:
		...
		break;
	case SGDI_PLAYER_OFFLINE:
		if (uParam)
		{
			nRet = Player[nParam].IsOffLine();
		}
		break;
	case SGDI_PLAYER_LOGOUT_GATEWAY:
		if (uParam)
		{
			nRet = Player[nParam].IsLogoutGateway();
		}
		break;
	case SGDI_PLAYER_COUNT:
		if (uParam)
		{
			nRet = PlayerSet.GetOnlinePlayerCount();
			if (nRet == FALSE)
				((char*)uParam)[0] = 0;
		}
		break;
...
//		  UINT uParam  --> 依据uOperId的取值情况而定
//		  INT nParam --> 依据uOperId的取值情况而定
//	返回：如果成功发送操作请求，函数返回非0值，否则返回0值。
//--------------------------------------------------------------------------
INT	CoreServerShell::OperationRequest(UINT uOper, UINT uParam, INT nParam)
{
	INT nRet = 1;
	switch (uOper)
	{
...
INT CoreServerShell::OnSetData(LPVOID pServer)
{
	g_SetDataServer(pServer);
	return true;
...
INT CoreServerShell::Breathe()
{
	...
	else
	{
		...
		//printf("[ALERT] No timer script run!\n");
	}
...
INT CoreServerShell::AttachPlayer(const ULONG lnID, GUID* pGuid)
{
...
bool CoreServerShell::IsPlayerOffline(INT nIndex)
{
	if (nIndex <= 0 || nIndex >= MAX_PLAYER)
		return false;

	return Player[nIndex].IsOffLine() & Player[nIndex].IsExchangingServer();
}

void CoreServerShell::PlayerOfflineGateway(INT nIndex)
{
	if (nIndex <= 0 || nIndex >= MAX_PLAYER)
		return;

	Player[nIndex].LogoutGateway();
}
...
bool CoreServerShell::SetChangeRoleStatus(INT nIndex, INT uStatus, char* nRoleName)
{
	if (nIndex <= 0 || nIndex >= MAX_PLAYER)
		return FALSE;
	...
	if (uStatus == 1)
	{//存在了
		Player[nIndex].m_ItemList.MsgShow(enumMSG_ID_NPC_NAME_INVALID);
		return FALSE;
	}

	INT nLen = strlen(nRoleName);

	if (nLen > 31 || nLen < 8)
	{
		Player[nIndex].m_ItemList.MsgShow(enumMSG_ID_NPC_NAME_LENGHT_INVALID);
		return FALSE;
	}

	Player[nIndex].m_ItemList.MsgShow(enumMSG_ID_NPC_NAME_SUCCESS);

	if (Player[nIndex].m_nNetConnectIdx >= 0)
	{
		strcpy(Player[nIndex].m_szLastName, nRoleName);
		printf("GM Kick out changerolename player one.\n");
		...
		TChangeNameData sProcessData;
		sProcessData.nProtoId = c2s_rolechangename_result;	  //发往goddes存档
		sProcessData.ulIdentity = nIndex;
		sProcessData.nDataLen = 1;
		sprintf(sProcessData.nAccName, Player[nIndex].AccountName);
		sprintf(sProcessData.nSrcRoleName, pNpc->Name);
		sprintf(sProcessData.nTarRoleName, nRoleName);

		if (g_pDataClient)
		{
			g_pDataClient->SendPackToServer(&sProcessData, sizeof(TChangeNameData));
		}
...
BOOL CoreServerShell::GroupChat(IClient* pClient, DWORD FromIP, ULONG FromRelayID, DWORD channid, BYTE tgtcls, DWORD tgtid, const void* pData, size_t size)
{
	switch (tgtcls)
	{
	...
	case tgtcls_fac:
	{
		...
		if (pExHeader != NULL)
		{
			...
			while (nTargetIdx)
			{
				if (Player[nTargetIdx].m_cFaction.m_nCurFaction == tgtid && FromRelayID != Player[nTargetIdx].m_nNetConnectIdx)
					g_pServer->PackDataToClient(Player[nTargetIdx].m_nNetConnectIdx, pExHeader, pckgsize);
	...
	case tgtcls_tong:
	{
		...
		if (pExHeader != NULL)
		{
			...
			while (nTargetIdx)
			{
				if (Player[nTargetIdx].m_cTong.GetTongNameID() == tgtid && FromRelayID != Player[nTargetIdx].m_nNetConnectIdx)
					g_pServer->PackDataToClient(Player[nTargetIdx].m_nNetConnectIdx, pExHeader, pckgsize);
	...
	case tgtcls_msgr:
	{
		...
		if (pExHeader != NULL)
		{
			...
			while (nTargetIdx)
			{
				if (Npc[Player[nTargetIdx].m_nIndex].m_nMissionGroup == tgtid && FromRelayID != Player[nTargetIdx].m_nNetConnectIdx)
					g_pServer->PackDataToClient(Player[nTargetIdx].m_nNetConnectIdx, pExHeader, pckgsize);
				nTargetIdx = PlayerSet.GetNextPlayer();
			}
#ifndef WIN32
	...
	case tgtcls_cr:
	{
		...
		if (pExHeader != NULL)
		{
			...
			while (nTargetIdx)
			{
				if (Player[nTargetIdx].m_cRoom.m_nID == tgtid && FromRelayID != Player[nTargetIdx].m_nNetConnectIdx)
					g_pServer->PackDataToClient(Player[nTargetIdx].m_nNetConnectIdx, pExHeader, pckgsize);
				nTargetIdx = PlayerSet.GetNextPlayer();
			}
#ifndef WIN32
...
BOOL CoreServerShell::PayForSpeech(INT nIndex, INT nType)
{
	...
	if (nNpcIdx <= 0)
		return FALSE;
	if ((Player[nIndex].m_nForbiddenTm > (KSG_GetCurSec() & KPlayer::FF_CHAT)))	//被禁言
	{
		SHOW_MSG_SYNC	sMsg;
		sMsg.ProtocolType = s2c_msgshow;
