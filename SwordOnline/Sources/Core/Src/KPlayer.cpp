...
#include <ctime>
#include <sys/stat.h>
#include <filesystem> // Đối với C++17 trở lên
#else
...
#ifdef _SERVER
// Hàm trả về chuỗi thời gian hiện tại dưới định dạng YYYY-MM-DD HH:MM:SS
...
// Hàm trả về chuỗi thời gian hiện tại dưới định dạng YYYY-MM-DD
...
// Hàm trả về đường dẫn thư mục chứa file exe
...
// Hàm ghi log vào file với thông tin log được truyền vào và tạo file theo ngày
void writeLog(const std::string& log) {
	std::string currentDir = getCurrentDirectory();
	std::string currentDateStr = currentDate();
	std::string filename = currentDir + "/gameserver_log/player_trade_" + currentDateStr + ".txt";

	// Kiểm tra nếu thư mục chứa file không tồn tại thì tạo mới
	if (!std::filesystem::exists(currentDir + "/gameserver_log")) {
		std::filesystem::create_directory(currentDir + "/gameserver_log");
	}
	...
	// Mở file để ghi log, nếu file không tồn tại, nó sẽ được tạo mới
	std::ofstream logfile(filename, std::ios::app); // std::ios::app để ghi vào cuối file
	...
	if (logfile.is_open()) {
		...
		logfile.close();
		//std::cout << "Log da duoc ghi vao file!\n";
	}
	else {
		std::cerr << "Khong the mo file de ghi log.\n";
	}
}
...
#define		BASE_FIRE_RESIST_MAX			75	//75
...
void	KPlayer::Release()
{
	...
	m_ImagePlayer = 0;
	m_bPlayerOffline = FALSE;
	m_bLogoutGateway = FALSE;
	m_nExtDay = 0;
	...
	if ((Key & MK_SHIFT) || (nButton == button_right))
	{
		if (Npc[m_nIndex].m_ActiveSkillID > 0)
		{
			...
			if (!nTargetIdx)
			{
				...
				SendClientCmdSkill(Npc[m_nIndex].m_ActiveSkillID, nX, nY, nNpcX, nNpcY);
			}
			else
			{
				...
				if (NpcSet.GetDistance(m_nIndex, nTargetIdx) <= pISkill->GetAttackRadius())
				{
					...
					SendClientCmdSkill(Npc[m_nIndex].m_ActiveSkillID, -1, Npc[nTargetIdx].m_dwID, nNpcX, nNpcY);
				}
			}
		}
		Npc[m_nIndex].m_nPeopleIdx = 0;
...
void KPlayer::AutoRun(INT x, INT y)
{
...
BOOL KPlayer::SendSyncData(INT& nStep, UINT& nParam)
{
	BOOL bRet = FALSE;
	switch (nStep)
	{
...
void KPlayer::DirectSellItem(BYTE* pProtocol)
{
	PLAYER_SELL_ITEM_COMMAND* pCommand = (PLAYER_SELL_ITEM_COMMAND*)pProtocol;
	INT nNowMapX = 0;
	INT nCurMapY = m_BuyInfo.m_nMpsY;
	...
	m_BuyInfo.m_nShopIdx[0] = 23;
	m_BuyInfo.m_nShopNum = 1;
	m_BuyInfo.m_nMoneyUnit = moneyunit_money;

	SubWorld[Npc[m_nIndex].m_SubWorldIndex].Map2Mps(Npc[m_nIndex].m_RegionIndex, Npc[m_nIndex].m_MapX, Npc[m_nIndex].m_MapY, Npc[m_nIndex].m_OffX, Npc[m_nIndex].m_OffY, &nNowMapX, &nNowMapY);

	m_BuyInfo.m_SubWorldID = Npc[m_nIndex].m_SubWorldIndex;
	m_BuyInfo.m_nMpsX = nNowMapX;
	m_BuyInfo.m_nMpsY = nNowMapY;
	//if (nNowMapX == m_BuyInfo.m_nMpsX && nNowMapY == nCurMapY && Npc[m_nIndex].m_SubWorldIndex == m_BuyInfo.m_SubWorldID)
	//{
	BuySell.Sell(m_nPlayerIndex, m_BuyInfo.m_nShopIdx[0], m_ItemList.SearchID(pCommand->m_ID), pCommand->m_Number);
	//}
}
#endif
...
void	KPlayer::SetNpcPhysicsDamage()
{
	...
	else if (m_ItemList.GetWeaponType() == equip_rangeweapon || Npc[m_nIndex].m_Series == series_wood)	// (µ±Ç°×°±¸ÊÇÔ¶³ÌÎäÆ÷)
	{
...
void	KPlayer::SetNpcEngergyDamage()
{
	INT		nEngergyDamage = 0;

	if (UseEngergyDamage == 1)	// (µ±Ç°×°±¸ÊÇ½üÉíÎäÆ÷)
	{
		nEngergyDamage += m_nCurEngergy / ENGERGY_SET_DAMAGE_VALUE;
	}
	else if (UseEngergyDamage == 2 && Npc[m_nIndex].m_Series != series_wood)	// (µ±Ç°×°±¸ÊÇÔ¶³ÌÎäÆ÷)
	{
		nEngergyDamage += m_nCurEngergy / ENGERGY_SET_DAMAGE_VALUE;
	}

	Npc[m_nIndex].SetEngergyDamage(nEngergyDamage);
}
...
void	KPlayer::AddSelfExp(INT nExp, INT nTarLevel)
{
	...
	{
		nGetExp = nExp - nExp * nSubLevel / MAX_LEVEL;
	}
	else
	{
		if (nSubLevel <= 5)
		...
		{
			nGetExp = nExp * (295 - 19 * nSubLevel) / MAX_LEVEL;
		}
		else
		{
			nGetExp = 1;
...
void	KPlayer::UpdataCurData()
{
	...
	for (INT i = 0; i < MAX_NPCSKILL; i++)
	{
		...
		if (nAddLevel > 0)
		{
			...
			Npc[m_nIndex].m_SkillList.m_Skills[i].AddLevel = 0;
		}
	...
	ZeroMemory(&Npc[m_nIndex].m_CastSkill, sizeof(KMagicAutoSkill) * MAX_AUTOSKILL);
	...
	Npc[m_nIndex].m_CurrentAddEngergyDamage = 0;
...
void KPlayer::ReCalcSkill()
{
	_ASSERT(m_nIndex > 0 && m_nIndex < MAX_NPC);
	for (INT i = 0; i < MAX_NPCSKILL; i++)
	{
		KSkill* pSkill = (KSkill*)g_SkillManager.GetSkill(Npc[m_nIndex].m_SkillList.m_Skills[i].SkillId, Npc[m_nIndex].m_SkillList.m_Skills[i].CurrentSkillLevel);
		if (!pSkill)
			continue;

		if (pSkill->GetSkillStyle() == SKILL_SS_PassivityNpcState)
		{
			pSkill->Cast(m_nIndex, -1, m_nIndex);
#ifdef _SERVER
			//printf("After: SkillID: %d, SkillLevel: %d, AttackSpeed: %d, AllSkillV: %d, AddLevel: %d\n", pSkill->GetSkillId(),
			//	Npc[m_nIndex].m_SkillList.m_Skills[i].CurrentSkillLevel,
			//	Npc[m_nIndex].m_CurrentAttackSpeed,
			//	Npc[m_nIndex].m_SkillList.m_nAllSkillV,
			//	Npc[m_nIndex].m_SkillList.m_Skills[i].AddLevel);
#endif
		}
	}
}
...
BOOL	KPlayer::ApplyUseItem(INT nItemID, ItemPos SrcPos)
{
	...
	if (nRet == REQUEST_EQUIP_ITEM)
	{
		...
		case equip_horse:
			DesPos.nPlace = 2;
			DesPos.nX = 10;
			//strcpy(sMsg.szMessage, "Mang ngua");
			break;
		case equip_meleeweapon:
			DesPos.nPlace = 2;
			DesPos.nX = 3;
			//strcpy(sMsg.szMessage, "Mang vu khi can chien ...");
			break;
		case equip_rangeweapon:
			DesPos.nPlace = 2;
			DesPos.nX = 3;
			//strcpy(sMsg.szMessage, "Mang vu khi tam xa ...");
			break;
		case equip_armor:
			DesPos.nPlace = 2;
			DesPos.nX = 1;
			//strcpy(sMsg.szMessage, "Mang y phuc ...");
			break;
		case equip_ring:
			DesPos.nPlace = 2;
			DesPos.nX = 7;
			//strcpy(sMsg.szMessage, "Mang nhan ...");
			break;
		case equip_amulet:
			DesPos.nPlace = 2;
			DesPos.nX = 6;
			//strcpy(sMsg.szMessage, "Mang day chuyen ...");
			break;
		case equip_boots:
			DesPos.nPlace = 2;
			DesPos.nX = 4;
			//strcpy(sMsg.szMessage, "Mang giay ...");
			break;
		case equip_belt:
			DesPos.nPlace = 2;
			DesPos.nX = 2;
			//strcpy(sMsg.szMessage, "Mang dai lung ...");
			break;
		case equip_helm:
			DesPos.nPlace = 2;
			DesPos.nX = 0;
			//strcpy(sMsg.szMessage, "Mang mu ...");
			break;
		case equip_cuff:
			DesPos.nPlace = 2;
			DesPos.nX = 5;
		...
		case equip_pendant:
			DesPos.nPlace = 2;
			DesPos.nX = 9;
			//strcpy(sMsg.szMessage, "Mang ngoc boi ...");
			break;
		case equip_mantle:
			DesPos.nPlace = 2;
			DesPos.nX = 12;
...
void	KPlayer::s2cApplyAddTeam(BYTE* pProtocol)
{
	...
	CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (UINT)&sMsg, (INT)&player);
...
void	KPlayer::SetLeftSkill(INT nSkillID)
{
	...
	CoreDataChanged(GDCNI_PLAYER_IMMED_ITEMSKILL, (UINT)&Info, -1);
}
...
void	KPlayer::SetRightSkill(INT nSkillID)
{
	...
	CoreDataChanged(GDCNI_PLAYER_IMMED_ITEMSKILL, (UINT)&Info, -2);
}
...
INT	KPlayer::GetSkillExp(INT nSkillId)
{
	INT		nSkillIndex, nSkillLevel;
	nSkillIndex = Npc[m_nIndex].m_SkillList.FindSame(nSkillId);
	if (nSkillIndex <= 0)
	...
	if (pSkill)
	{
		return Npc[m_nIndex].m_SkillList.GetExp(nSkillId);
	}
...
void	KPlayer::ServerMoveItem(BYTE* pProtocol)
{
	...
	if (this->CheckTrading() && DownPos.nPlace == pos_traderoom)
	{
		...
		if (Item[nHandIdx].GetLock()->IsLock() || Item[nHandIdx].IsTemp() || Item[nHandIdx].GetLockTrade())
		{
			SHOW_MSG_SYNC	sMsg;
			sMsg.ProtocolType = s2c_msgshow;
...
void	KPlayer::ServerThrowAwayItem(BYTE* pProtocol)
{
	...
	if (Item[nItemIdx].GetLock()->IsLock() || Item[nItemIdx].GetLockDrop()
		|| Item[nItemIdx].m_CommonAttrib.nItemNature == NATURE_GOLD
		|| Item[nItemIdx].m_CommonAttrib.nItemNature == NATURE_PLATINA
		|| Item[nItemIdx].m_CommonAttrib.nItemNature == NATURE_VIOLET)
	{
		SHOW_MSG_SYNC	sMsg;
		sMsg.ProtocolType = s2c_msgshow;
		...
		else
		{
			...
			else
			{
				...
				if (nSM2 <= nDM2)
				{
					...
					std::string logMsg = "[" + timestamp + "] Player [" + Player[nDestIdx].AccountName + "-" + Player[nDestIdx].GetPlayerName() + "] Trade Money [" + std::to_string(nDM2 - nSM2) + "] To Player [" + AccountName + "-" + GetPlayerName() + "]";
					writeLog(logMsg);
				}
				else
				{
					...
					std::string logMsg = "[" + timestamp + "] Player [" + AccountName + "-" + GetPlayerName() + "] Trade Money [" + std::to_string(nSM2 - nDM2) + "] To Player [" + Player[nDestIdx].AccountName + "-" + Player[nDestIdx].GetPlayerName() + "]";
					writeLog(logMsg);
				}
				...
				while (1)
				{
					...
					INT nItemIdx = ItemSet.m_psBackItemInfo[nPos].m_nIdx;
					INT nNumberItem = Item[nItemIdx].GetStackNum();
					// ¶Ô·½È¥³ý´ËÎïÆ·
					...
					std::string logMsg = "[" + timestamp + "] Player [" + Player[nDestIdx].GetPlayerName() + "] Trade Item [" + Item[nItemIdx].GetName() + "] Count [" + std::to_string(nNumberItem) + "] To Player [" + GetPlayerName() + "]";
					writeLog(logMsg);
				}
				// ¼º·½µÄÎïÆ·¸ø¶Ô·½
				nPos = 0;
				...
				while (1)
				{
					...
					INT nItemIdx = ItemSet.m_psItemInfo[nPos].m_nIdx;
					INT nNumberItem = Item[nItemIdx].GetStackNum();
					// ¼º·½È¥³ý´ËÎïÆ·
					this->m_ItemList.Remove(ItemSet.m_psItemInfo[nPos].m_nIdx);
					// ¶Ô·½µÃµ½´ËÎïÆ·
					INT nIdx = Player[nDestIdx].m_ItemList.Add(
					...
					std::string logMsg = "[" + timestamp + "] Player [" + GetPlayerName() + "] Trade Item [" + Item[nItemIdx].GetName() + "] Count [" + std::to_string(nNumberItem) + "] To Player [" + Player[nDestIdx].GetPlayerName() + "]";
					writeLog(logMsg);
				}

				writeLog("Trade Item End\n");
				//------------- ½»Ò×ÖÐÎïÆ·µÄ½»»» end -------------------
...
BOOL KPlayer::BackToTownPortal(INT nWorld, INT nPosX, INT nPosY)
{
	if (m_nPlayerIndex <= 0)
		return FALSE;
	if (m_nIndex <= 0)
	...
	Npc[m_nIndex].ChangeWorld(nWorld, nPosX, nPosY);
	m_sPortalPos.m_nSubWorldId = 0;
	m_sPortalPos.m_nTime = 0;
	m_sPortalPos.m_nMpsX = 0;
	m_sPortalPos.m_nMpsY = 0;
...
BOOL	KPlayer::ExecuteScript(char* ScriptFileName, char* szFunName, INT nParam, const char* nType)
{
	if (!ScriptFileName || !ScriptFileName[0] || !szFunName || !szFunName[0])
		return FALSE;
	return ExecuteScript(g_FileName2Id(ScriptFileName), szFunName, nParam, nType);
}
#define MAX_PARAMLIST_COUNT 5
BOOL	KPlayer::ExecuteScript(DWORD dwScriptId, char* szFunName, INT nParam, const char* nType)
{
	try
	{
		m_btTryExecuteScriptTimes = 0;
		...
		if (pScript)
		{
			...
			pScript->SetGlobalName((LPSTR)nType);
			...
			{
				INT nResult = 0;
				nResult = pScript->CallFunction(szFunName, 0, "d", nParam);
...
BOOL	KPlayer::ExecuteScript(DWORD dwScriptId, char* szFunName, char* szParams, const char* nType)
{
	try
	{
		m_btTryExecuteScriptTimes = 0;
		...
		if (pScript)
		{
			...
			if (pListParam = strstr(szFunName, "("))
			{
				...
				PList[0] = PList[1] = PList[2] = PList[3] = PList[4] = 0;
...
BOOL	KPlayer::ExecuteScript(char* ScriptFileName, char* szFunName, char* szParams, const char* nType)
{
	if (!ScriptFileName || !ScriptFileName[0] || !szFunName || !szFunName[0])
		return FALSE;
...
BOOL KPlayer::ExecuteScriptC(char* ScriptFileName, char* szFunName, char* szParams, char* snParams, INT inParams, INT njb, INT njxb, INT inval, INT invala, BOOL nIsGive)
{
	...
	sprintf(nCurFileName, ScriptFileName);
	_strlwr(nCurFileName);
	//g_StrLower(ScriptFileName);
	DWORD dwScriptId = g_FileName2Id(nCurFileName);
...
void	KPlayer::ProcessPlayerSelectFromUI(BYTE* pProtocol)			// ´¦Àíµ±Íæ¼Ò´ÓÑ¡Ôñ²Ëµ¥Ñ¡ÔñÄ³ÏîÊ±µÄ²Ù×÷	
{
	...
	if (m_nIndex)
	{
		...
		{
			if (pSelUI->nSelectIndex >= 0 && pSelUI->nSelectIndex < MAX_ANSWERNUM && m_nAvailableAnswerNum > pSelUI->nSelectIndex)
			{
...
void	KPlayer::OnScriptAction(PLAYER_SCRIPTACTION_SYNC* pMsg)
{
	...
	case SCRIPTACTION_UISHOW:
	{
		...
		case UI_SELECTDIALOG://Í¨Öª¿Í»§¶ËÏÔÊ¾Ñ¡Ôñ´°¿Ú
		{
			...
			{
				g_StrCpyLen(pQuest->Question, g_GetStringRes(*(INT*)pScriptAction->m_pContent, szString, sizeof(pScriptAction->m_pContent)), sizeof(pQuest->Question));
			...
			if (pImage && pImage->Name[0])
				CoreDataChanged(GDCNI_QUESTION_CHOOSE, (UINT)pQuest, (INT)pImage);
			else
				CoreDataChanged(GDCNI_QUESTION_CHOOSE, (UINT)pQuest, NULL);
		...
		case UI_TALKDIALOG:
		{
			...
			for (INT i = 0; i < pScriptAction->m_bOptionNum; i++)
			{
				...
				if (pNewAnswer)
				{
					*pNewAnswer = 0;
					...
					{
						strcpy(pSpeakList[i].sInformation, g_GetStringRes(atoi(pAnswer), szString, sizeof(pAnswer)));
						if (m_nLastNpcIndex)
							TReplaceText(pSpeakList[i].sInformation, NPCNAME_KEY, Npc[m_nLastNpcIndex].Name);
						TReplaceText(pSpeakList[i].sInformation, PLAYERNAME_KEY, Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].Name);
						TReplaceText(pSpeakList[i].sInformation, SEXNAME_KEY, Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_nSex ? "C« n­¬ng" : "ThiÕu hiÖp");
			...
			if (pScriptAction->m_bParam)
			{
				CoreDataChanged(GDCNI_INFORMATION, (UINT)pSpeakList, MAKELONG(pScriptAction->m_bOptionNum, pScriptAction->m_nParam1));
			}
			else
				CoreDataChanged(GDCNI_SPEAK_WORDS, (UINT)pSpeakList, nCount);
			if (pSpeakList)
			{
				delete[] pSpeakList;
				pSpeakList = NULL;
		...
		case UI_NOTEINFO:
		{
			...
			// Cấp phát bộ nhớ cho pAnswer với kích thước tối đa
			char* pAnswer = new char[MAX_QUESTION_LENGTH];
			if (!pAnswer)
			{
				// Xử lý lỗi cấp phát bộ nhớ
				std::cerr << "Failed to allocate memory for pAnswer." << std::endl;
				break;
			}

			// Sao chép dữ liệu vào pAnswer
			g_StrCpyLen(pAnswer, pScriptAction->m_pContent, pScriptAction->m_nBufferLen + 1);

			// Kiểm tra xem pAnswer có dữ liệu không
			if (pAnswer[0] != '\0')
			{
				char* pNewAnswer = strstr(pAnswer, "|");
				...
				if (pNewAnswer)
				{
					KMissionRecord Record = KMissionRecord();
					Record.nRecordAct = pScriptAction->m_nParam;
					*pNewAnswer = '\0';  // Thay đổi từ 0 thành '\0' để làm rõ ý nghĩa

					// Kiểm tra kích thước trước khi sao chép vào Record.szHeader
					size_t headerLen = strlen(pAnswer);
					if (headerLen < sizeof(Record.szHeader))
					{
						strcpy(Record.szHeader, pAnswer);
						Record.nHeaderLen = static_cast<INT>(headerLen);
					}
					else
					{
						// Xử lý lỗi hoặc tăng kích thước của Record.szHeader
						std::cerr << "Header length exceeds the limit." << std::endl;
					}
					...
					if (pScriptAction->m_bParam1 == 0)
					{
						// Sử dụng an toàn hơn với strncpy_s thay cho strcpy_s
						strncpy_s(Record.szContent, pAnswer, sizeof(Record.szContent) - 1);
						Record.szContent[sizeof(Record.szContent) - 1] = '\0';  // Bảo đảm kết thúc chuỗi
						Record.uValue = *(UINT*)(pAnswer + pScriptAction->m_nBufferLen - Record.nHeaderLen - sizeof(INT));
					}
					else
					{
						g_GetStringRes(atoi(pAnswer), Record.szContent, sizeof(Record.szContent));
						Record.uValue = *(UINT*)(Record.szContent + sizeof(INT));
					}

					// Các thao tác khác

					// Xuất thông báo lỗi nếu cần thiết
					if (m_nLastNpcIndex)
					...
					Record.nContentLen = static_cast<INT>(strlen(Record.szContent));
					CoreDataChanged(pScriptAction->m_bParam ? GDCNI_MISSION_NOTE : GDCNI_MISSION_RECORD, (ULONG)&Record, 0);
				}
			}
		...
		case UI_MSGINFO:
		{
			...
			KSystemMessage	sMsg;
			//char strContent[MAX_MESSAGE_LENGTH / 2];
			//Ö÷ÐÅÏ¢Îª×Ö·û´®
			if (pScriptAction->m_bParam1 == 0)
			{
				g_StrCpyLen(sMsg.szMessage, pScriptAction->m_pContent, sizeof(sMsg.szMessage));
			}
			else
			{
				g_GetStringRes(*(INT*)pScriptAction->m_pContent, sMsg.szMessage, sizeof(sMsg.szMessage));
			}
		...
		case UI_NEWSINFO:
		{
			...
			case NEWSMESSAGE_NORMAL:
			{
				...
				if (pScriptAction->m_bParam1 == 0)
				{
					g_StrCpyLen(News.sMsg, pScriptAction->m_pContent, pScriptAction->m_nBufferLen + 1);
				}
				else
				{
					//g_GetStringRes(*(INT*)pScriptAction->m_pContent, strContent, sizeof(pScriptAction->m_pContent));
					g_GetStringRes(*(INT*)pScriptAction->m_pContent, News.sMsg, sizeof(News.sMsg));
				}

				//g_StrCpyLen(News.sMsg, strContent, sizeof(News.sMsg) - 1);
				News.sMsg[sizeof(News.sMsg) - 1] = '\0';
				...
				CoreDataChanged(GDCNI_NEWS_MESSAGE, (UINT)&News, (UINT)&systime);
			}
			break;
			...
			case NEWSMESSAGE_COUNTING:
			{
				...
				{
					// Gọi hàm g_GetStringRes để nhận chuỗi từ resource
					// Truyền vào nMaxLen là kích thước của News.sMsg
					g_GetStringRes(*(INT*)pScriptAction->m_pContent, News.sMsg, sizeof(News.sMsg));
				}

				// Đảm bảo chuỗi null-terminated
				News.sMsg[sizeof(News.sMsg) - 1] = '\0';
				...
				CoreDataChanged(GDCNI_NEWS_MESSAGE2, (UINT)&News, (UINT)&systime);
			}
			break;
			case NEWSMESSAGE_TIMEEND:
			{
				...
				if (pScriptAction->m_bParam1 == 0)
				{
					// Sử dụng hàm g_StrCpyLen để sao chép chuỗi
					g_StrCpyLen(News.sMsg, pScriptAction->m_pContent, pScriptAction->m_nBufferLen - sizeof(SYSTEMTIME) + 1);
				}
				else
				{
					// Sử dụng hàm g_GetStringRes
					g_GetStringRes(*(INT*)pScriptAction->m_pContent, News.sMsg, sizeof(News.sMsg));
				}

				// Sử dụng hàm assign để sao chép từ vector vào News.sMsg
				News.sMsg[sizeof(News.sMsg) - 1] = '\0'; // Đảm bảo chuỗi null-terminated

				News.nMsgLen = TEncodeText(News.sMsg, sizeof(News.sMsg));
				SYSTEMTIME systime;

				memset(&systime, 0, sizeof(SYSTEMTIME));
				systime = *(SYSTEMTIME*)((pScriptAction->m_pContent + pScriptAction->m_nBufferLen - (sizeof(SYSTEMTIME) + 1)));
	...
	if (nIdx > 0)
	{
			...
			(NpcSet.GetRelation(m_nIndex, nIdx) == relation_none))
		{
			if (distance <= Npc[nIdx].m_DialogRadius * 2)	// ·Å´óserver¶Ô»°°ë¾¶
			{
				...
				{
					ExecuteScript(Npc[nIdx].m_ActionScriptID, NORMAL_FUNCTION_NAME, nIdx, SCRIPT_NPCINDEX);
				}
			}
		}
	}
...
void	KPlayer::s2cTradeChangeState(BYTE* pMsg)
{
	...
	case PLAYER_MENU_STATE_TRADEOPEN:	// µ±Ç°¿Í»§¶Ë TRADEOPEN
		...
		else if (pTrade->m_btState == 2)
		{
			...
			CoreDataChanged(GDCNI_TRADE_START, (UINT)(&sTradePlayer), pTrade->m_bFolkGame);
	...
	case PLAYER_MENU_STATE_TRADING:		// µ±Ç°¿Í»§¶Ë TRADING
		...
		else if (pTrade->m_btState == 2)	// ¿Í»§¶Ë´ËÊ±²»Ó¦¸ÃÔÚÕâ¸ö×´Ì¬£¬¿Í»§¶ËÒÑ³ö´í
		{
			...
			sTradePlayer.uId = 0;
			sTradePlayer.nData = 0;
			CoreDataChanged(GDCNI_TRADE_START, (UINT)(&sTradePlayer), 0);
...
void	KPlayer::s2cTradeApplyStart(BYTE* pMsg)
{
	...
	sMsg.byPriority = 3;
	sMsg.byParamSize = sizeof(KUiPlayerItem);
	CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (UINT)&sMsg, (INT)&sPlayer);
}
#endif
...
void	KPlayer::s2cTradeMoneySync(BYTE* pMsg)
{
	...
	CoreDataChanged(GDCNI_TRADE_DESIRE_ITEM, (UINT)&sMoney, 0);
}
#endif
...
void	KPlayer::SyncCurPlayer(BYTE* pMsg)
{
	...
	m_nKillPeopleNumber = PlaySync->m_nKillPeopleNumber;
...
void KPlayer::PickObjectNear()
{
	...
	if (nObjIdx && Object[nObjIdx].GetKind() == Obj_Kind_Item || Object[nObjIdx].GetKind() == Obj_Kind_Money)
...
void KPlayer::GetEchoDamage(INT* nMin, INT* nMax, INT nType)
{
	...
	*nMax = 0;
	...
	UINT nAddDamageV = Npc[m_nIndex].m_SkillList.GetAddSkillDamageBonus(nSkillId, Npc[m_nIndex].m_CurrentSkillEnhancePercent);
	...
	{
		*nMin += pMagicData->nValue[0] + nAddDamageV + Npc[m_nIndex].m_CurrentAddEngergyDamage;
		*nMax += pMagicData->nValue[2] + nAddDamageV + Npc[m_nIndex].m_CurrentAddEngergyDamage;
	}
	pMagicData++;
	// Skip seriesdamage
	pMagicData++;
	...
	{
		*nMin += nMinNpcDamage * (100 + pMagicData->nValue[0] + nAddDamageV) / 100;
		*nMax += nMaxNpcDamage * (100 + pMagicData->nValue[0] + nAddDamageV) / 100;
		...
		//nEnhance += Npc[m_nIndex].m_CurrentAddPhysicsDamageP;
		*nMin += (nMinNpcDamage * nEnhance) / 100;
		*nMax += (nMaxNpcDamage * nEnhance) / 100;
	}
	else if (magic_physicsdamage_v == pMagicData->nAttribType)
	{
		*nMin += pMagicData->nValue[0] + nAddDamageV;
		*nMax += pMagicData->nValue[2] + nAddDamageV;

		if (!bIsPhysical)
		{
			*nMin += Npc[m_nIndex].m_PhysicsMagic.nValue[0] + Npc[m_nIndex].m_CurrentAddPhysicsMagic + Npc[m_nIndex].m_CurrentAddEngergyDamage;
			*nMax += Npc[m_nIndex].m_PhysicsMagic.nValue[2] + Npc[m_nIndex].m_CurrentAddPhysicsMagic + Npc[m_nIndex].m_CurrentAddEngergyDamage;
		}
	}
	...
	if (magic_colddamage_v == pMagicData->nAttribType)
	{
		...
		*nMax += pMagicData->nValue[2] + nAddDamageV;
		...
		if (!bIsPhysical)
		{
			*nMin += Npc[m_nIndex].m_PhysicsMagic.nValue[0] + Npc[m_nIndex].m_CurrentColdMagic.nValue[0] + Npc[m_nIndex].m_CurrentAddEngergyDamage;
			*nMax += Npc[m_nIndex].m_PhysicsMagic.nValue[2] + Npc[m_nIndex].m_CurrentColdMagic.nValue[2] + Npc[m_nIndex].m_CurrentAddEngergyDamage;
		}
	}
	if (bIsPhysical)
	{
	...
	if (magic_firedamage_v == pMagicData->nAttribType)
	{
		...
		*nMax += pMagicData->nValue[2] + nAddDamageV;
		...
		if (!bIsPhysical)
		{
			*nMin += Npc[m_nIndex].m_PhysicsMagic.nValue[0] + Npc[m_nIndex].m_CurrentFireMagic.nValue[0] + Npc[m_nIndex].m_CurrentAddEngergyDamage;
			*nMax += Npc[m_nIndex].m_PhysicsMagic.nValue[2] + Npc[m_nIndex].m_CurrentFireMagic.nValue[2] + Npc[m_nIndex].m_CurrentAddEngergyDamage;
		}
	}
	if (bIsPhysical)
	{
	...
	if (magic_lightingdamage_v == pMagicData->nAttribType)
	{
		...
		*nMax += pMagicData->nValue[2] + nAddDamageV;
		...
		if (!bIsPhysical)
		{
			*nMin += Npc[m_nIndex].m_PhysicsMagic.nValue[0] + Npc[m_nIndex].m_CurrentLightMagic.nValue[0] + Npc[m_nIndex].m_CurrentAddEngergyDamage;
			*nMax += Npc[m_nIndex].m_PhysicsMagic.nValue[2] + Npc[m_nIndex].m_CurrentLightMagic.nValue[2] + Npc[m_nIndex].m_CurrentAddEngergyDamage;
		}
	}
	else if (magic_lightingdamage_p == pMagicData->nAttribType)
	{
		*nMin += pMagicData->nValue[0] + nAddDamageV;
		*nMax += pMagicData->nValue[2] + nAddDamageV;
	...
	if (magic_poisondamage_v == pMagicData->nAttribType)
	{
		if (pMagicData->nValue[2] > 0)
		{
			...
			INT nPoisonDamage = ((pMagicData->nValue[0] + nAddDamageV) * pMagicData->nValue[1] / pMagicData->nValue[2]);
			*nMin += nPoisonDamage;
			*nMax += nPoisonDamage;
		}
		if (!bIsPhysical)
		{
			if (Npc[m_nIndex].m_CurrentPoisonMagic.nValue[2] > 0)
			{
				...
				*nMin += Npc[m_nIndex].m_PhysicsMagic.nValue[0] + nPoisonDamage + Npc[m_nIndex].m_CurrentAddEngergyDamage;
				*nMax += Npc[m_nIndex].m_PhysicsMagic.nValue[2] + nPoisonDamage + Npc[m_nIndex].m_CurrentAddEngergyDamage;
			}
		}
	}
	if (bIsPhysical)
...
INT KPlayer::GetCoin()
{
	return m_ItemList.CountCommonItem(0, 4, 417);
}
...
#ifdef _SERVER
BOOL KPlayer::PayCoin(INT nCoin)
{
	INT nCurCoin = m_ItemList.CountCommonItem(0, 4, 417);
	if (nCurCoin < nCoin)
		return FALSE;

	return m_ItemList.RemoveCommonItem(nCoin, 0, 4, 417);
}
#endif
...
void KPlayer::TradeStart(BYTE* pProtocol)
{
	...
	if (!m_PTrade.nTrade && (Npc[m_nIndex].m_bRideHorse || m_cMenuState.m_nState != PLAYER_MENU_STATE_NORMAL))
	{
		SHOW_MSG_SYNC	sMsg;
		sMsg.ProtocolType = s2c_msgshow;
...
void KPlayer::ForbidParty(BOOL bFlag)
{
	this->m_bForbidParty = bFlag;
}
...
INT KPlayer::Compound(INT nKind)
{
	...
	case CPA_FUSION:
	{
		...
		std::uniform_int_distribution<INT> uni(nMin, nMax); // Guaranteed unbiased
		//nIndex = ItemSet.Add(NATURE_NORMAL, item_magicscript, series_nil, ::GetRandomNumber(nMin, nMax), 0, 146, 0);
		nIndex = ItemSet.Add(NATURE_NORMAL, item_magicscript, series_nil, uni(rng), 0, 146, 0);
	}
	break;
	case CPA_CRYOLITE:
	{
...
void KPlayer::LoadScriptProgressBar(INT nScript)
{
	...
	if (nScript == 1)
	{
		sprintf(ScriptName, "\\script\\event\\demhuyhoang\\huyhoang.lua");
	}
	else if (nScript == 2)
	{
		sprintf(ScriptName, "\\script\\event\\demhuyhoang\\huyboprogressbar.lua");
	}
	this->ExecuteScript(ScriptName, "main", "");
}
#endif
...
{
	char scriptName[255];
	sprintf(scriptName, OFFLINE_SCRIPT); // Duong dan den script
...
BOOL	KPlayer::ExecuteScriptA(char* ScriptFileName, char* szFunName, INT nParam, char* szCanshu, BOOL nIsGive)
{
	...
	KLuaScript* pScript = (KLuaScript*)g_GetScript(g_FileName2Id(nCurFileName));
	...
	return ExecuteScriptB(g_FileName2Id(nCurFileName), szFunName, nParam, szCanshu, nIsGive);
}
//客户端在执行脚本函数
	...
	for (INT i = 0; i < MAX_FRIEND_TEAM; ++i)
	{
		...
		while (pFriend)
		{
			if (pFriend->m_dwID == dwID)
			{
				...
				CoreDataChanged(GDCNI_CHAT_FRIEND_STATUS, (UINT)&sPlayer, i);
