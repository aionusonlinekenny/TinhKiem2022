...
void	KNpcRes::_Init(char* lpszNpcName, KNpcResList* pNpcResList)
{
	...
	INT		nShadowFrame, nShadowDir, nShadowInterval, nShadowCgX, nShadowCgY;
	if (m_pcResNode->m_cShadowInfo.GetFile(m_nAction, &nShadowFrame, &nShadowDir, &nShadowInterval,
		&nShadowCgX, &nShadowCgY, szBuffer))
	{
		this->m_cNpcShadow.SetSprFile(szBuffer, nShadowFrame, nShadowDir, nShadowInterval);
		this->m_cNpcShadow.SetCenterPos(nShadowCgX, nShadowCgY);
	}
...
//Create an afterimage line
void KNpcRes::MakeBlurLine(INT nNpcIdx, INT x, INT y, INT z, INT nToX, INT nToY, INT nToZ)
{
	INT n, i, j;
	INT nDx = nToX - x;
	INT nDy = nToY - y;
	INT nDz = nToZ - z;
	INT nLength = nDx * nDx + nDy * nDy;
	const INT nMaxLen = 400;

	if (nLength > nMaxLen * nMaxLen)
	{
		FLOAT f = nMaxLen / sqrt((FLOAT)nLength);
		nDx = (INT)(nDx * f);
		nDy = (INT)(nDy * f);
		nDz = (INT)(nDz * f);
	}
	...
	m_cNpcBlur.m_dwInterval = 5;

	for (n = 0; n < MAX_BLUR_FRAME; n++)
	{
		x += nDx;
		y += nDy;
		z += nDz;
		...
		for (i = 0, j = 0; i < MAX_PART; i++)
		{
			...
			//for (INT nPartLayer = 0; nPartLayer < 2; nPartLayer++)
			{
				KSprControl& rcSprControl = m_cNpcImage[i];
				if (!rcSprControl.CheckExist())
					continue;
				m_cNpcBlur.SetFile(
					j++,
					rcSprControl.GetName(),
					rcSprControl.GetNameId(),
					rcSprControl.GetCurFrame(),
					//rcSprControl.GetChangeColor(),
					x,
					y,
					z,
					(rcSprControl.GetBlendStyle() == 1) ? IMAGE_RENDER_STYLE_SCREEN : IMAGE_RENDER_STYLE_ALPHA);
			}
		}

		m_cNpcBlur.SetMapPos(x, y, z, nNpcIdx);
		m_cNpcBlur.SetNextNo();
	}
}
