...
private:
	...
	UINT	m_uNextChangeNormalPK;
	UINT	m_uNextChangeFightPK;
public:
	KPlayerPK();							// 构造函数
	void	Init(INT nPlayerIdx);			// 初始化
	BOOL	CheckSwitchPK(BYTE bFlag, BOOL bCaptainSet);
	void	SetNormalPKState(BYTE bFlag, BOOL bCaptainSet = FALSE);	// 设定正常PK状态
	INT		GetNormalPKState();				// 获得正常PK状态
	void	SetLockPKState(BYTE bFlag, BYTE btState);
	BYTE	GetLockPKState() {return m_btLockPKState;};
	void	EnmityPKClose(BOOL bIsPKing = FALSE);				// 关闭仇杀PK
	BOOL	EnmityPKOpen(INT nAim, BOOL bSpar);			// 仇杀开始
...
public:
	...
	void	SetEnmityPKState(INT nState, INT nNpcID = 0, char *lpszName = NULL, BOOL bAim = 0, BOOL bSpar = FALSE);	// 设定仇杀PK状态
