...
INT KBuySell::Init()
{
	...
	for (INT k = 0; k < nHeight; k++)
	{
		...
		GoodsFile.GetInteger(k + 2, 7, series_nil, &ItemGenerator.nSeriesReq);
...
BOOL KBuySell::CanBuy(INT nPlayerIdx, INT nBuy, INT nBuyIdx, INT nBuyNumber)
{
	...
	else
	{
		...
		case moneyunit_money:
			...
			break;
		case moneyunit_coin:
			if (Player[nPlayerIdx].GetCoin() < nPrice)
			{
				bShowMsg = FALSE;
				sMsg.m_wMsgID = enumMSG_ID_SHOP_NO_COIN;
			}
			break;
...
BOOL KBuySell::Buy(INT nPlayerIdx, INT nBuy, INT nBuyIdx, BYTE nBuyNumber)
{
	...
	if (i)
	{
		...
		case moneyunit_extpoint:
			...
			break;
		case moneyunit_coin:
			Player[nPlayerIdx].PayCoin(nPrice * i);
			break;
		case moneyunit_fuyuan:
...
BOOL KBuySell::Sell(INT nPlayerIdx, INT nBuy, INT nIdx, INT nBuyNumber)
{
	...
	if (Item[nIdx].GetLock()->IsLock() || Item[nIdx].GetLockSell())
	{
		SHOW_MSG_SYNC	sMsg;
		sMsg.ProtocolType = s2c_msgshow;
...
void KBuySell::OpenSale(INT nSaleType, BuySellInfo* pInfo)
{
	...
	Player[CLIENT_PLAYER_INDEX].m_BuyInfo = *pInfo;
	Player[CLIENT_PLAYER_INDEX].m_BuyInfo.m_nCurShop = 0;
	CoreDataChanged(GDCNI_SUPERSHOP, (UINT)pInfo, nSaleType);
}
#endif
...
void KBuySell::OpenSale(INT nPlayerIdx, INT nSaleType, INT nMoneyUnit, INT nShopNum, INT* nShopId) //KTC
{
	if (nPlayerIdx <= 0 || nPlayerIdx > MAX_PLAYER)
		return;
	if (nSaleType < 0 || nSaleType > 1)
