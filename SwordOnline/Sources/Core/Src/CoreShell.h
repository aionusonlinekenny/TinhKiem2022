...
enum GAMEOPERATION_INDEX
{
	...
	//uParam = (UINT)bIn 为非0值时表示存入，否则表示取出
	//nParam = 钱的数额
	...
	GOI_COMPITEM_COM,
	GOI_COMPITEM_DISTILL,
	GOI_COMPITEM_FORGE,
	GOI_COMPITEM_ENCHASE,
	GOI_COMPITEM_ALTAL,
	GOI_AUTO_CASTSKILL,
	GOI_PLAYER_OFFLINE,
};
...
{
	virtual void CoreDataChanged(UINT uDataId, UINT uParam, INT nParam) = 0;
	virtual void ChannelMessageArrival(DWORD nChannelID, char* szSendName, const char* pMsgBuff, uint16_t nMsgLength = 0, const char* pItem = 0, BYTE btSomeFlag = 0, bool bSucc = false) = 0;
	virtual void MSNMessageArrival(char* szSourceName, char* szSendName, const char* pMsgBuff, uint16_t nMsgLength, const char* pItem = 0, bool bSucc = false) = 0;
...
struct _declspec (novtable) iCoreShell
{
	...
	//向游戏发送操作
	virtual INT	 OperationRequest(UINT uOper, UINT uParam = 0, INT nParam = 0) = 0;
	//发送输入设备的输入操作消息
	virtual void ProcessInput(UINT uMsg, UINT uParam, INT nParam) = 0;
	virtual void AutoRun(INT x, INT y) = 0;
	...
	virtual INT ChatSpecialPlayer(void* pPlayer, const char* pMsgBuff, uint16_t nMsgLength) = 0;
	...
	virtual INT	SceneMapOperation(UINT uOper, UINT uParam, INT nParam) = 0;
	//与帮会相关的操作, uOper的取值来自 GAME_TONG_OPERATION_INDEX
	virtual INT	TongOperation(UINT uOper, UINT uParam, INT nParam) = 0;
	//与组队相关的操作，uOper的取值来自 GAME_TEAM_OPERATION_INDEX
	virtual INT TeamOperation(UINT uOper, UINT uParam, INT nParam) = 0;

	virtual INT	PAIOperation(UINT uOper, UINT uParam, INT nParam, INT nParam1) = 0;
	//从游戏世界获取数据
	virtual INT	 GetGameData(UINT uDataId, UINT uParam, INT nParam) = 0;

	//绘制游戏对象
	virtual void DrawGameObj(UINT uObjGenre, UINT uId, INT x, INT y, INT Width, INT Height, INT nParam) = 0;
	//绘制游戏世界
	...
	virtual void SetScreenSize(INT nWidth, INT nHeight) = 0;
	//日常活动，core如果要寿终正寝则返回0，否则返回非0值
	virtual INT  Breathe(BOOL isDisconnected) = 0;
	//释放接口对象
	virtual void Release() = 0;
	virtual void SetClient(LPVOID pClient) = 0;

	virtual void SendNewDataToServer(void* pData, INT nLength) = 0;
	virtual void DirectFindPos(UINT uParam, INT nParam, BOOL bSync, BOOL bPaintLine) = 0;
	virtual void CheckHoverMouseMiniMap(BOOL hoverM) = 0;
	...
	virtual void GetSizeItem(UINT uItemId, INT* nW, INT* nH) = 0;
	virtual INT GetExtPoint() = 0;
	virtual INT GetNatureItem(UINT uItemId, UINT uGenre = 0) = 0;
	virtual INT GetGenreItem(UINT uItemId, UINT uGenre = 0) = 0;
	virtual BOOL IsDamage(UINT uItemId) = 0;
	virtual	BOOL GetLockState() = 0;
	virtual INT GetStackNum(UINT uItemId) = 0;
	virtual	BOOL GetSkillData(INT nSkillId, INT* nLevel) = 0;
	virtual	void GetSkillName(INT nSkillId, char* szSkillName) = 0;
	virtual	INT GetTradePrice(UINT uItemId) = 0;
	virtual	INT GetTradeState(INT nIndex = 0, INT uId = 0) = 0;
	virtual INT	GetFightState() = 0;
	virtual void BreakItem(UINT uParam, INT nParam, BOOL bIsBreakAll) = 0;
	virtual DWORD GetdwIDPlayer(UINT uId) = 0;
	virtual INT GetOwnValue(INT nMoneyUnit) = 0;
	virtual INT GetDataSuperShop(INT nSaleId, UINT uParam, INT nParam) = 0;
	virtual INT GetDataDynamicShop(INT nSaleId, UINT uParam, INT nParam) = 0;
	virtual INT GetNextSkill(INT nType, INT nIndex) = 0;
	virtual BOOL IsSkillAura(INT nSkillId) = 0;

	// TamLTM code kham nam xanh
	virtual INT GetKindItem(UINT uId) = 0;
	virtual INT GetDetailItem(UINT uId) = 0;
	virtual INT GetParticularItem(UINT uId) = 0;
	virtual INT GetLevelItem(UINT uId) = 0;
	virtual INT GetSeriesItem(UINT uId) = 0;
	virtual INT GetItemColor(UINT uItemId) = 0;
	// End code

	virtual void GetWayPointName(INT nMapID, char* szMapName) = 0;
	virtual void GetWayPointPos(INT nWayPointID, INT* nWorld, INT* nPosX, INT* nPosY) = 0;
};
...
//对外发送游戏世界数据发生改变的通知
void	CoreDataChanged(UINT uDataId, UINT uParam = 0, INT nParam = 0);
