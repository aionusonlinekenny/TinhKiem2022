...
KProtocolProcess::KProtocolProcess()
{
	...
	ProcessFunc[s2c_npcpos] = &KProtocolProcess::SyncNpcPos;
	ProcessFunc[s2c_exitgame] = &KProtocolProcess::ExitGame;	//ExitGame
	...
	ProcessFunc[c2s_compound] = &KProtocolProcess::CompoundItem;//CompoundItem
	ProcessFunc[c2s_enchase] = &KProtocolProcess::EnchaseItem;//EnchaseItem
	ProcessFunc[c2s_directsale] = &KProtocolProcess::DirectSell;
...
void KProtocolProcess::ProcessNetMsg(INT nIndex, BYTE* pMsg)
{
	...
	if (pMsg[0] == c2s_directsale)
		g_DebugLog("Direct Sale receive");
	...
	if (ProcessFunc[byProtocol])
	{
		if (nIndex > 0 && nIndex < MAX_PLAYER)
		{
			...
			else if (Player[nIndex].m_nPaceBarTime && (byProtocol >= c2s_npcwalk))
			{
				Player[nIndex].m_nPaceBarTime = 0;
				Player[nIndex].m_nPaceBarTimeMax = 0;
			}
			//else if (byProtocol == c2s_uicmdscript)
			//{
			//	UiCommandScript(nIndex, pMsg);
			//}
...
void KProtocolProcess::SyncGive(BYTE* pMsg)
{
	...
	CoreDataChanged(GDCNI_GIVE, (UINT)temp->m_szName, (INT)temp->m_szInitString);
}
...
void KProtocolProcess::NetCommandRemoveNpc(BYTE* pMsg)
{
	...
	if (Player[CLIENT_PLAYER_INDEX].ConformIdx(nIdx))
	{
		...
		{
			g_DebugLog("KProtocolProcess::NetCommandRemoveNpc [%d]-[%d]", dwNpcID, *(BOOL*)&pMsg[5]);
			if (*(BOOL*)&pMsg[5])
			{
...
void KProtocolProcess::NetCommandStand(BYTE* pMsg)
{
	...
	if (nIdx)
	{
		...
		{
			int nMpsX, nMpsY, dX, dY, dZ;
			Npc[nIdx].GetMpsPos(&nMpsX, &nMpsY);
			dX = nMpsX - pStandSync->MapX;
			dY = nMpsY - pStandSync->MapY;
			...
			if (dZ >= 400)
			{
				...
				if (nRegion > 0)
				{
					...
					SubWorld[0].m_Region[Npc[nIdx].m_RegionIndex].AddNpcRef(Npc[nIdx].m_MapX, Npc[nIdx].m_MapY, obj_npc);
				}
			}
		}
	}
...
void KProtocolProcess::s2cGetSkillLevel(BYTE* pMsg)
{
	...
	CoreDataChanged(GDCNI_SKILL_CHANGE, (UINT)&SkillData, nSkillPosition);
}
...
void KProtocolProcess::s2cTeamChangeCaptain(BYTE* pMsg)
{
	...
	if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_dwID == pChange->m_dwCaptainID)
	{
		...
		if (pChange->m_bMySelf)
			...
			sprintf(sMsg.szMessage, MSG_TEAM_AUTO_CAPTAIN_SELF);
		sMsg.eType = SMT_TEAM;
...
void KProtocolProcess::SyncNpc(BYTE* pMsg)
{
	...
	else
	{
		...
		Npc[nIdx].m_NpcSettingIdx = (int16_t)HIWORD(NpcSync->NpcSettingIdx);
...
void KProtocolProcess::SyncNpcPos(BYTE* pMsg)
{
	...
	//if (NpcSync->m_dwExParam)
	//	rcNpc.ProcNetCommand(nFrameDelta, do_stand);

	if (nIdx == Player[CLIENT_PLAYER_INDEX].m_nIndex && rcNpc.m_RegionIndex >= 0) // 主角
	{
		// 制造残影
		INT nSrcX = 0;
		INT nSrcY = 0;
		rcNpc.GetMpsPos(&nSrcX, &nSrcY);
		rcNpc.SetPos(NpcSync->MapX, NpcSync->MapY);
		rcNpc.m_Dir = g_GetDirIndex(nSrcX, nSrcY, NpcSync->MapX, NpcSync->MapY);
		rcNpc.m_DataRes.MakeBlurLine(rcNpc.m_Index, nSrcX, nSrcY, rcNpc.m_Height, NpcSync->MapX, NpcSync->MapY, 0);

		// 通知Ui主角被传送
		//::g_CoreEventNotify(emCOREEVENT_SYNC_TRANS_POSITION, "ddd", rcNpc.GetMapID(), nSrcX >> 5, nSrcY >> 5);
	}
	else
	{
		rcNpc.SetPos(NpcSync->MapX, NpcSync->MapY);
	...
	else
	{
		...
		if (Npc[nIdx].m_RegionIndex == -1 && nIdx != Player[CLIENT_PLAYER_INDEX].m_nIndex)
		{
			...
			else
			{
				...
				Npc[nIdx].m_dwRegionID = SubWorld[0].m_Region[nRegion].m_RegionID;
				SubWorld[0].m_Region[nRegion].AddNpc(nIdx);
				SubWorld[0].m_Region[nRegion].AddNpcRef(Npc[nIdx].m_MapX, Npc[nIdx].m_MapY, obj_npc);
			}
		...
		{
			LONG	nOldLife = Npc[nIdx].m_CurrentLife;
		...
		Npc[nIdx].m_CurrentRunSpeed = (INT)NpcSync->RunSpeed;
		Npc[nIdx].m_CurrentWalkSpeed = (INT)NpcSync->WalkSpeed;
		Npc[nIdx].m_CurrentAttackSpeed = (INT)NpcSync->AttackSpeed;
		Npc[nIdx].m_CurrentCastSpeed = (INT)NpcSync->CastSpeed;
...
void KProtocolProcess::SyncNpcMinPlayer(BYTE* pMsg)
{
	...
	//Npc[nNpcIdx].m_nSvrMpsPosX = pSync->MapX;
	//Npc[nNpcIdx].m_nSvrMpsPosY = pSync->MapY;

	//if (Npc[nNpcIdx].m_cAi.m_nObjectIdx > 0)
	//{
	//	INT nX1 = pSync->MapX, nY1 = pSync->MapY;
	//	INT nX2, nY2;
	//	Object[Npc[nNpcIdx].m_cAi.m_nObjectIdx].GetMpsPos(&nX2, &nY2);
	//	if ((nX1 - nX2) * (nX1 - nX2) + (nY1 - nY2) * (nY1 - nY2) < PLAYER_PICKUP_CLIENT_DISTANCE * PLAYER_PICKUP_CLIENT_DISTANCE)
	//		Player[CLIENT_PLAYER_INDEX].CheckObject(Npc[nNpcIdx].m_cAi.m_nObjectIdx);
	//}

	// The map has been switched (the protagonist's RegionIndex is -1) or the synchronization position exceeds the 9-screen range
	if (Npc[nNpcIdx].m_RegionIndex == -1 || nRegion < 0)
	{
		// The synchronization position exceeds the 9-screen range
		if (nRegion < 0)
		{
			// Keep the server and client acting consistent
			if (Npc[nNpcIdx].m_Doing == do_run)
				SendClientCmdRun(Npc[nNpcIdx].m_DesX, Npc[nNpcIdx].m_DesY);
			else if (Npc[nNpcIdx].m_Doing == do_walk)
				SendClientCmdWalk(Npc[nNpcIdx].m_DesX, Npc[nNpcIdx].m_DesY);
		}

		Npc[nNpcIdx].SetPos(pSync->MapX, pSync->MapY);
		Npc[nNpcIdx].m_SyncSignal = SubWorld[0].m_dwCurrentTime;
		ZeroStruct(Npc[nNpcIdx].m_sSyncPos);
		// Player[CLIENT_PLAYER_INDEX].ProcessAutoPath();
		return;
	}
}
...
void KProtocolProcess::s2cDirectlyCastSkill(BYTE* pMsg)
{
	...
	NPC_SKILL_SYNC* SkillCmd = (NPC_SKILL_SYNC*)pMsg;

	dwNpcID = SkillCmd->ID;
	nSkillID = SkillCmd->nSkillID;
	nSkillLevel = SkillCmd->nSkillLevel;
	MapX = SkillCmd->nMpsX;
	MapY = SkillCmd->nMpsY;

	INT nIdx = NpcSet.SearchID(dwNpcID);
	INT nTargetIdx = NpcSet.SearchID(MapY);

	KSkill* pOrdinSkill = (KSkill*)g_SkillManager.GetSkill(nSkillID, nSkillLevel);
	if (!pOrdinSkill)
		return;

	//When specifying a target (MapX == -1), MapY is the NpcdwID of the target and needs to be converted to the local NpcIndex.
	if (MapX == -1)
	{
		if (MapY < 0)
			return;
		...
		nX = Npc[MapY].m_MapX;
		nY = Npc[MapY].m_MapY;
		SubWorld[0].NewMap2Mps(Npc[MapY].m_RegionIndex, nX, nY, 0, 0, (INT*)&MapX, (INT*)&MapY);

		pOrdinSkill->Cast(nIdx, -1, nTargetIdx);
	}
	else
...
void KProtocolProcess::s2cShowMsg(BYTE* pMsg)
{
	...
	KSystemMessage	sMsg = KSystemMessage();
	...
	case enumMSG_ID_TONG_BE_KICK:
	{
		...
		sUi.uParam = 0;
		sUi.szString[0] = 0;
		CoreDataChanged(GDCNI_TONG_ACTION_RESULT, (UINT)&sUi, 1);
	...
	case enumMSG_ID_TONG_APPLY_ADD_ERROR4:
	{
		...
		sMsg.byPriority = 0;
		sMsg.byParamSize = 0;
		sprintf(sMsg.szMessage, MSG_TONG_APPLY_ADD_ERROR4, 86400 / 60);
		CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (UINT)&sMsg, 0);
	}
	break;
	...
	break;
	case enumMSG_ID_NPC_NAME_INVALID:
	{
		//KSystemMessage sMsg;
		sMsg.byConfirmType = SMCT_UI_RENASCENCE;
		sMsg.byParamSize = 0;
		sMsg.byPriority = 0;
		sMsg.eType = SMT_PLAYER;
		sprintf(sMsg.szMessage, MSG_PLAYER_CHANGE_NAME_INVALID);
		CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (UINT)&sMsg, 0);
	}
	break;
	case enumMSG_ID_NPC_NAME_LENGHT_INVALID:
	{
		//KSystemMessage sMsg;
		sMsg.byConfirmType = SMCT_UI_RENASCENCE;
		sMsg.byParamSize = 0;
		sMsg.byPriority = 0;
		sMsg.eType = SMT_PLAYER;
		sprintf(sMsg.szMessage, MSG_PLAYER_CHANGE_NAME_LENGHT_INVALID);
		CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (UINT)&sMsg, 0);
	}
	break;
	case enumMSG_ID_NPC_NAME_SUCCESS:
	{
		//KSystemMessage sMsg;
		sMsg.byConfirmType = SMCT_UI_RENASCENCE;
		sMsg.byParamSize = 0;
		sMsg.byPriority = 0;
		sMsg.eType = SMT_PLAYER;
		sprintf(sMsg.szMessage, MSG_PLAYER_CHANGE_NAME_SUCCESS);
		CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (UINT)&sMsg, 0);
	}
	break;
	default:
...
void KProtocolProcess::s2cExtendChat(BYTE* pMsg)
{
	...
	else if (protocol == chat_channelchat)
	{
		...
		l_pDataChangedNotifyFunc->ChannelMessageArrival(pCccSync->channelid, pCccSync->someone,
	...
	else if (protocol == chat_feedback)
	{
		...
		{//someone
			...
			l_pDataChangedNotifyFunc->MSNMessageArrival(Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].Name, pDstName,
		...
		{//channel
			...
			l_pDataChangedNotifyFunc->ChannelMessageArrival(*pChannelid, Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].Name,
...
void KProtocolProcess::s2cExtendTong(BYTE* pMsg)
{
	...
	case enumTONG_SYNC_ID_CREATE_FAIL:
	{
		...
		case enumTONG_CREATE_ERROR_ID7:		// µÈ¼¶ÎÊÌâ
			sprintf(sMsg.szMessage, MSG_TONG_CREATE_ERROR05, 60);
			CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (UINT)&sMsg, 0);
			break;
	...
	case enumTONG_SYNC_ID_HEAD_INFO:
	{
		...
		CoreDataChanged(GDCNI_TONG_INFO, (UINT)&sUi, (UINT)&sInfo);
		...
		CoreDataChanged(GDCNI_TONG_MEMBER_LIST, (UINT)&sObj, (UINT)sItem);
	}
	break;
	case enumTONG_SYNC_ID_SELF_INFO:
	{
		...
		sInfo.nTongJiyuParam = pInfo->m_nTongJiyuParam;
		strcpy(sInfo.szTongJiyuNotify, pInfo->m_szTongJiyuNotify);
		CoreDataChanged(GDCNI_TONG_INFO, NULL, (UINT)&sInfo);
	...
	case enumTONG_SYNC_ID_INSTATE:
	{
		...
		CoreDataChanged(GDCNI_TONG_ACTION_RESULT, (UINT)&sUi, (INT)pInstate->m_btSuccessFlag);
	}
	break;
	case enumTONG_SYNC_ID_KICK:
	{
		...
		sUi.szString[0] = 0;
		CoreDataChanged(GDCNI_TONG_ACTION_RESULT, (UINT)&sUi, pKick->m_btSuccessFlag);
	}
	break;
	...
	case enumTONG_SYNC_ID_CHANGE_CAMP_FAIL:
	{
		...
		case enumTONG_CHANGE_CAMP_ERROR_ID8:
			sprintf(sMsg.szMessage, MSG_TONG_CHANGE_CAMP_ERROR3, 100000);
			CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (UINT)&sMsg, 0);
			break;
		}
	}
...
void KProtocolProcess::LadderResult(BYTE* pMsg)
{
	...
	if (pLadderMessage)
	{
		...
		UINT uParam = 10 | (((WORD)pLadderData->dwLadderID) << 16);
...
void KProtocolProcess::s2cPlayerSync(BYTE* pMsg)
{
	...
	case enumS2C_PLAYERSYNC_ID_EXIT:
		...
		break;
	case enumS2C_PLAYERSYNC_ID_OFFLINE:
		CoreDataChanged(GDCNI_PLAYER_OFFLINE, NULL, NULL);
		break;
...
void KProtocolProcess::s2cRequestChatRoomList(BYTE* pMsg)
{
	...
	CoreDataChanged(GDCNI_REQUEST_CHATROOM_LIST, (UINT)&pRoom, NULL);
}
...
void KProtocolProcess::s2cRequestMemberList(BYTE* pMsg)
{
	...
	CoreDataChanged(GDCNI_REQUEST_MEMBER_LIST, (UINT)&pMem, NULL);
}
...
void KProtocolProcess::s2cRequestBlackList(BYTE* pMsg)
{
	...
	pMem.nMemIndex = pList->nMemIndex;
	pMem.bIsRoomHost = pList->bIsRoomHost;
	CoreDataChanged(GDCNI_REQUEST_BLACK_LIST, (UINT)&pMem, NULL);
}
...
void KProtocolProcess::ExitGame(BYTE* pMsg) {
	CoreDataChanged(GDCNI_EXIT_GAME, NULL, NULL);
}
...
void	KProtocolProcess::s2cUpdateNpcState(BYTE* pMsg)
{
	...
	{//客户端设置状态动画
		Npc[nIndex].SetNpcState((BYTE*)pInfo->m_btStateInfo);  // 设置NPC的状态
	}
}
...
void KProtocolProcess::NpcSkillCommand(INT nIndex, BYTE* pProtocol)
{
	...
	INT		nPosX = pNetCommand->nPosX;
	INT		nPosY = pNetCommand->nPosY;
	// ²ÎÊýºÏ·¨ÐÔ¼ì²é
	...
	if (ParamY < 0)
	{
		...
		if (nNpcIndex > 0)
		{
			INT nTargetMpsX, nTargetMpsY, nSrcMpsX, nSrcMpsY;
			Npc[nNpcIndex].GetMpsPos(&nTargetMpsX, &nTargetMpsY);
			//Npc[Player[nIndex].m_nIndex].GetMpsPos(&nSrcMpsX, &nSrcMpsY);
			KSkill* pSkill = (KSkill*)g_SkillManager.GetSkill(ParamX, Npc[Player[nIndex].m_nIndex].m_SkillList.GetLevel(ParamX));

			if ((NpcSet.GetDistance(Player[nIndex].m_nIndex, nNpcIndex) - pSkill->GetAttackRadius()) <= 0)
			{
				//Npc[nNpcIndex].SetPos(nPosX, nPosY);
			...
			else
			{
				...
				{
					Npc[nNpcIndex].m_CurrentRunSpeed = 30;
					Npc[nNpcIndex].SendCommand(do_run, nPosX, nPosY);
				}
	...
	//Npc[Player[nIndex].m_nIndex].NormalSync();
}
...
void KProtocolProcess::DirectSell(INT nIndex, BYTE* pProtocol)
{
	if (Player[nIndex].CheckTrading())
		return;
	Player[nIndex].DirectSellItem(pProtocol);
}
...
void KProtocolProcess::TradeBuy(INT nIndex, BYTE* pProtocol)
{
	...
	{
		if (Player[nPlayerIdx].m_ItemList.Remove(pPlayer->m_nIdx))
		{
			printf("Shopper Remove Item Successfully!\n");
		}
		...
		if (Player[nPlayerIdx].Earn(nPrice))
		{
			printf("Shopper Earn Successfully!\n");
		}
		...
		if (Player[nPlayerIdx].Save())
		{
			...
			printf("Shopper Save Successfully!\n");
		}
		Player[nIndex].Save();
	}
	else
...
void KProtocolProcess::PlayerCommand(INT nIndex, BYTE* pProtocol)
{
	...
	case enumC2S_PLAYERCOMMAND_ID_RETURN:
		if (!Player[nIndex].BackToTownPortal())
		{
			INT nWayPointId = static_cast<INT>(pCommand->nParam);
			...
			Player[nIndex].BackToTownPortal(nW, nX * 32, nY * 32);
		}
		break;
	default:
		break;
...
void KProtocolProcess::CompoundItem(INT nIndex, BYTE* pProtocol)
{
	C2S_COMPOUND_ITEM* pCompoundAction;

	pCompoundAction = (C2S_COMPOUND_ITEM*)pProtocol;
	Player[nIndex].Compound(pCompoundAction->nAction);
	//		g_DebugLog("pBarCmd->sScript: %s", pBarCmd->sScript);
	return;
}

void KProtocolProcess::EnchaseItem(INT nIndex, BYTE* pProtocol)
{
	C2S_COMPOUND_ITEM* pCompoundAction;

	pCompoundAction = (C2S_COMPOUND_ITEM*)pProtocol;
	Player[nIndex].Enchase(pCompoundAction->nAction, 0, 0, 0);
	//		g_DebugLog("pBarCmd->sScript: %s", pBarCmd->sScript);
	return;
}
