...
KSkillSpecial::KSkillSpecial()
{
	...
	m_nEndTime = 0;
	m_nCurDir = 0;
	m_nCurrentMapX = m_nCurrentMapY = 0;
	m_nCurrentMapZ = 0;
	m_nRegionId = -1;
...
void KSkillSpecial::Draw(INT nCurLifeFrame)
{
	...
	if (m_pMissleRes->nInterval > 0)
	{
		...
		m_RUImage.nFrame = (int16_t)(nCurLifeFrame - m_nBeginTime) / m_pMissleRes->nInterval + nImageDir * m_pMissleRes->nTotalFrame / m_pMissleRes->nDir;
#ifdef TOOLVERSION
...
void KSkillSpecial::SetAnimation(char* szFile, UINT uImage)
{
