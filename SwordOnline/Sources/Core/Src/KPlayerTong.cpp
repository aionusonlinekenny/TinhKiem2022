...
BOOL	KPlayerTong::ApplyAddTong(DWORD dwNpcID)
{
		...
		Player[CLIENT_PLAYER_INDEX].m_cTong.m_dwLeaveTime > KSG_GetCurSec())
	{
		...
		sMsg.byPriority = 0;
		sMsg.byParamSize = 0;
		sprintf(sMsg.szMessage, MSG_TONG_APPLY_ADD_ERROR4, (INT)Player[CLIENT_PLAYER_INDEX].m_cTong.m_dwLeaveTime - KSG_GetCurSec());
		CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (UINT)&sMsg, 0);
		return FALSE;
	}
...
void	KPlayerTong::Create(TONG_CREATE_SYNC* psCreate)
{
	...
	memcpy(m_szName, psCreate->m_szName, sizeof(m_szName));
...
void	KPlayerTong::SendRefuseMessage(INT nPlayerIdx, DWORD dwNameID)
{
	...
	memcpy(sMsg.m_lpBuf, &sMsg, sizeof(BYTE) * (sMsg.m_wLength + 1));
...
//	π¶ƒ‹£∫…Í«ÎªÒµ√∞Ôª·–≈œ¢
//-------------------------------------------------------------------------
BOOL	KPlayerTong::ApplyInfo(INT nInfoID, UINT nParam1, INT nParam2, INT nParam3, char* lpszName/*=NULL*/)
{
