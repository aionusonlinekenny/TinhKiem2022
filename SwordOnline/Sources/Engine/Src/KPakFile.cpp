...
//-------------------------------------------------------------------------
// File reading mode 0 = Read from disk first 1 = Read from file package first
#ifdef _SERVER
...
SPRHEAD* SprGetHeader(const char* pszFileName, SPROFFS*& pOffsetTable)
{
	...
	if (File.IsFileInPak())
	{
		...
		if (g_pPakList->FindElemFile(pszFileName, PakRef))
		{
			...
			if (pSpr)
				pSpr->Reserved[PAK_INDEX_STORE_IN_RESERVED] = (WORD)(int16_t)PakRef.nPackIndex;
		}
	}
	else
	{
		...
		{
			if (*(int32_t*)&Header.Comment[0] != SPR_COMMENT_FLAG || Header.Colors > 256)
				break;
			//---为输出缓冲区分配空间---
			uint32_t uEntireSize = File.Size();
...
SPRFRAME* SprGetFrame(SPRHEAD* pSprHeader, int32_t nFrame)
{
	SPRFRAME* pFrame = NULL;
	if (pSprHeader && g_pPakList)
	{
		int32_t nPakIndex = (int16_t)pSprHeader->Reserved[PAK_INDEX_STORE_IN_RESERVED];
		if (nPakIndex >= 0)
...
KSGImageContent* get_jpg_image(const char cszName[], uint32_t uRGBMask16)
{
	KPakFile	File;
	...
	nRetCode = jpeg_decode_info((PBYTE)pbyFileData, &JpegInfo);
	if (!nRetCode)
		goto Exit0;
