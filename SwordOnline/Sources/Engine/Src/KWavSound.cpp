...
BOOL KWavSound::Load(LPSTR FileName)
{
	...
	for (int32_t i = 1; i < BUFFER_COUNT; i++)
	{
...
void KWavSound::Free()
{
	if (g_pDirectSound->GetDirectSound() == NULL)
		return;
	for (int32_t i = BUFFER_COUNT - 1; i >= 0; i--)
	{
...
//---------------------------------------------------------------------------
int32_t KWavSound::GetFreeBuffer()
{
	DWORD	Status = 0;
	int32_t 	i;
...
// 返回:	Sound Buffer Index
//---------------------------------------------------------------------------
int32_t KWavSound::GetPlayBuffer()
{
	DWORD	Status = 0;
	int32_t 	i;
...
// 返回:	void
//---------------------------------------------------------------------------
void KWavSound::Play(int32_t nPan, int32_t nVolume, BOOL bLoop)
{
	int32_t i = GetFreeBuffer();
	if (i >= BUFFER_COUNT)
		return;
...
// 返回:	void
//---------------------------------------------------------------------------
void KWavSound::SetPan(int32_t nPan)
{
	int32_t i = GetPlayBuffer();
	if (i >= BUFFER_COUNT)
		return;
...
// 返回:	void
//---------------------------------------------------------------------------
void KWavSound::SetVolume(int32_t nVolume)
{
	int32_t i = GetPlayBuffer();
	if (i >= BUFFER_COUNT)
		return;
