...
public:
	...
	void SafeCallBegin(int32_t* pIndex);//记录当前堆栈栈顶位置，并保存到pIndex指向的数据中
	void SafeCallEnd(int32_t nIndex);//恢复至调用之前栈顶位置。
	...
	char m_szScriptName[260];
	int32_t	 m_UserTag;
	Lua_State* m_LuaState;
//private:
	BOOL CallFunction(LPSTR cFuncName, int32_t nResults, LPSTR cFormat, va_list vlist);
