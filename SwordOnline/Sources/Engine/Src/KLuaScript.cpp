...
{
	m_LuaState = Lua_Create((DWORD)512); //change from 256 -> 512 to fix memory error
	m_UserTag = 0;
	if (m_LuaState == NULL)
	{
...
// 返回:	BOOL 
//---------------------------------------------------------------------------
BOOL KLuaScript::CallFunction(LPSTR cFuncName, int32_t nResults, LPSTR cFormat, va_list vlist)
{
...
BOOL KLuaScript::GetValuesFromStack(char* cFormat, ...)
{
	...
	int32_t nTopIndex = 0;
	int32_t nIndex = 0;
	int32_t nValueNum = 0;//cFormat的字符长度，表示需要取的参数数量
...
BOOL KLuaScript::RegisterFunctions(TLua_Funcs Funcs[], int32_t n)
{
	...
	for (int32_t i = 0; i < n; i++)	Lua_Register(m_LuaState, Funcs[i].name, Funcs[i].func);
	return TRUE;
}
...
//																	Romandou
//---------------------------------------------------------------------------
void KLuaScript::SafeCallBegin(int32_t* pIndex)
{
	if (!m_LuaState)		return;
	pIndex = (int32_t*)Lua_SafeBegin(m_LuaState);
}
